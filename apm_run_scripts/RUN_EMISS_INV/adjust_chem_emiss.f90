...
program main
   ...
   integer                                     :: num_chemi_files,num_firechemi_files
   integer                                     :: i,j,k,isp,ifile
   integer                                     :: unit
   real                                        :: fac,facc,delt
   ...
   character(len=150),allocatable,dimension(:) :: wrfchemi_old,wrfchemi_new
   character(len=150),allocatable,dimension(:) :: wrffirechemi_old,wrffirechemi_new
   ...
   nfirechemi_emiss,num_chemi_files,num_firechemi_files
...
!
   allocate(wrfchemi_old(num_chemi_files))
   allocate(wrfchemi_new(num_chemi_files))
   allocate(wrffirechemi_old(num_firechemi_files))
   allocate(wrffirechemi_new(num_firechemi_files))
!
! Read data
   unit=20
...
!
   ...
   do ifile=1,num_chemi_files  
      print *, 'wrfchemi_old         ',trim(wrfchemi_old(ifile))
      print *, 'wrfchemi_new         ',trim(wrfchemi_new(ifile))
   enddo
   do ifile=1,num_firechemi_files  
      print *, 'wrffirechemi_old     ',trim(wrffirechemi_old(ifile))
      print *, 'wrffirechemi_new     ',trim(wrffirechemi_new(ifile))
   enddo
!
! Allocate arrays
...
!
! Adjust chemi emission   
   do ifile=1,num_chemi_files   
      do isp=1,nchemi_emiss
         print *, 'APM: Adjust chemi emissions file ',trim(wrfchemi_old(ifile)), &
         ' species ',trim(chemi_spcs(isp))
!
         chemi_emiss(isp)=chemi_spcs(isp)
         emiss_chemi_prior(:,:,:)=0.
         emiss_chemi_post(:,:,:)=0.
         emiss_chemi_old(:,:,:)=0.
         emiss_chemi_new(:,:,:)=0.
!         
         call get_WRFCHEM_emiss_data(trim(wrfchemi_prior),trim(chemi_emiss(isp)), &
         emiss_chemi_prior,nx,ny,nz_chemi)
         call get_WRFCHEM_emiss_data(trim(wrfchemi_post),trim(chemi_emiss(isp)), &
         emiss_chemi_post,nx,ny,nz_chemi)
         call get_WRFCHEM_emiss_data(trim(wrfchemi_old(ifile)),trim(chemi_emiss(isp)), &
...
!
         do i=1,nx
            ...
            enddo  
         enddo
         call put_WRFCHEM_emiss_data(trim(wrfchemi_new(ifile)),trim(chemi_emiss(isp)), &
         emiss_chemi_new,nx,ny,nz_chemi)
      enddo
   enddo
!
! Adjust firechemi emission   
   do ifile=1,num_firechemi_files   
      do isp=1,nfirechemi_emiss
         print *, 'APM: Adjust firechemi emissions file ',trim(wrffirechemi_old(ifile)), &
         ' species ',trim(firechemi_spcs(isp))
!
         firechemi_emiss(isp)=firechemi_spcs(isp)
         emiss_firechemi_prior(:,:,:)=0.
         emiss_firechemi_post(:,:,:)=0.
         emiss_firechemi_old(:,:,:)=0.
         emiss_firechemi_new(:,:,:)=0.
!
         call get_WRFCHEM_emiss_data(trim(wrffirechemi_prior),trim(firechemi_emiss(isp)), &
         emiss_firechemi_prior,nx,ny,nz_firechemi)
         call get_WRFCHEM_emiss_data(trim(wrffirechemi_post),trim(firechemi_emiss(isp)), &
         emiss_firechemi_post,nx,ny,nz_firechemi)
         call get_WRFCHEM_emiss_data(trim(wrffirechemi_old(ifile)),trim(firechemi_emiss(isp)), &
         emiss_firechemi_old,nx,ny,nz_firechemi)
!
         do i=1,nx
            do j=1,ny
               do k=1,nz_firechemi
                  if(emiss_firechemi_prior(i,j,k).gt.0. .and. emiss_firechemi_post(i,j,k).gt.0.) then
         ...
         enddo
!
         call put_WRFCHEM_emiss_data(trim(wrffirechemi_new(ifile)),trim(firechemi_emiss(isp)), &
