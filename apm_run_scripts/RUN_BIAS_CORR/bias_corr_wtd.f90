   ...
   if(airnow_co_cnt.gt.0) then
      do icnt=1,nobs
         if (trim(obs_list(icnt)).eq.'AIRNOW_CO') then
            print *, 'APM: AIRNOW CO Bias Correction (old) ',corrections_old(icnt)
            ...
            print *, 'APM: AIRNOW CO Bias Correction (new) ',corrections_old(icnt)
            exit
         endif
      enddo
   endif
      ...
      do jprs=1,mopitt_co_ndim
         ...
         do icnt=1,nobs
            if (trim(obs_list(icnt)).eq.'MOPITT_CO_1' .and. jprs.eq.1) then
               print *, 'APM:MOPITT CO 1 Bias Correction (old) ',corrections_old(icnt)
               print *, 'APM:MOPITT CO 1 Bias Correction (current) ',correction
               corrections_old(icnt)=corrections_old(icnt)+correction
               print *, 'APM:MOPITT CO 1 Bias Correction (new) ',corrections_old(icnt)
               exit
            elseif (trim(obs_list(icnt)).eq.'MOPITT_CO_2' .and. jprs.eq.2) then
               print *, 'APM:MOPITT CO 2 Bias Correction (old) ',corrections_old(icnt)
               print *, 'APM:MOPITT CO 2 Bias Correction (current) ',correction
               corrections_old(icnt)=corrections_old(icnt)+correction
               print *, 'APM:MOPITT CO 2 Bias Correction (new) ',corrections_old(icnt)
               exit
            elseif (trim(obs_list(icnt)).eq.'MOPITT_CO_3' .and. jprs.eq.3) then
               print *, 'APM:MOPITT CO 3 Bias Correction (old) ',corrections_old(icnt)
               print *, 'APM:MOPITT CO 3 Bias Correction (current) ',correction
               corrections_old(icnt)=corrections_old(icnt)+correction
               print *, 'APM:MOPITT CO 3 Bias Correction (new) ',corrections_old(icnt)
               exit
            elseif (trim(obs_list(icnt)).eq.'MOPITT_CO_4' .and. jprs.eq.4) then
               print *, 'APM:MOPITT CO 4 Bias Correction (old) ',corrections_old(icnt)
               print *, 'APM:MOPITT CO 4 Bias Correction (current) ',correction
               corrections_old(icnt)=corrections_old(icnt)+correction
               print *, 'APM:MOPITT CO 4 Bias Correction (new) ',corrections_old(icnt)
               exit
            elseif (trim(obs_list(icnt)).eq.'MOPITT_CO_5' .and. jprs.eq.5) then
               print *, 'APM:MOPITT CO 5 Bias Correction (old) ',corrections_old(icnt)
               print *, 'APM:MOPITT CO 5 Bias Correction (current) ',correction
               corrections_old(icnt)=corrections_old(icnt)+correction
               print *, 'APM:MOPITT CO 5 Bias Correction (new) ',corrections_old(icnt)
               exit
            elseif (trim(obs_list(icnt)).eq.'MOPITT_CO_6' .and. jprs.eq.6) then
               print *, 'APM:MOPITT CO 6 Bias Correction (old) ',corrections_old(icnt)
               print *, 'APM:MOPITT CO 6 Bias Correction (current) ',correction
               corrections_old(icnt)=corrections_old(icnt)+correction
               print *, 'APM:MOPITT CO 6 Bias Correction (new) ',corrections_old(icnt)
               exit
            elseif (trim(obs_list(icnt)).eq.'MOPITT_CO_7' .and. jprs.eq.7) then
               print *, 'APM:MOPITT CO 7 Bias Correction (old) ',corrections_old(icnt)
               print *, 'APM:MOPITT CO 7 Bias Correction (current) ',correction
               corrections_old(icnt)=corrections_old(icnt)+correction
               print *, 'APM:MOPITT CO 7 Bias Correction (new) ',corrections_old(icnt)
               exit
            elseif (trim(obs_list(icnt)).eq.'MOPITT_CO_8' .and. jprs.eq.8) then
               print *, 'APM:MOPITT CO 8 Bias Correction (old) ',corrections_old(icnt)
               print *, 'APM:MOPITT CO 8 Bias Correction (current) ',correction
               corrections_old(icnt)=corrections_old(icnt)+correction
               print *, 'APM:MOPITT CO 8 Bias Correction (new) ',corrections_old(icnt)
               exit
            elseif (trim(obs_list(icnt)).eq.'MOPITT_CO_9' .and. jprs.eq.9) then
               print *, 'APM:MOPITT CO 9 Bias Correction (old) ',corrections_old(icnt)
               print *, 'APM:MOPITT CO 9 Bias Correction (current) ',correction
               corrections_old(icnt)=corrections_old(icnt)+correction
               print *, 'APM:MOPITT CO 9 Bias Correction (new) ',corrections_old(icnt)
               exit
            elseif (trim(obs_list(icnt)).eq.'MOPITT_CO_10' .and. jprs.eq.10) then
               print *, 'APM:MOPITT CO 10 Bias Correction (old) ',corrections_old(icnt)
               print *, 'APM:MOPITT CO 10 Bias Correction (current) ',correction
               corrections_old(icnt)=corrections_old(icnt)+correction
               print *, 'APM:MOPITT CO 10 Bias Correction (new) ',corrections_old(icnt)
               exit
            endif
         enddo
      enddo
   ...
   if(tropomi_co_col_cnt.gt.0) then
      do icnt=1,nobs
         if (trim(obs_list(icnt)).eq.'TROPOMI_CO_COL') then
            ...
            print *, 'APM:TROPOMI CO Bias Correction (old) ',corrections_old(icnt)
            print *, 'APM:TROPOMI CO Bias Correction (current) ',correction
            corrections_old(icnt)=corrections_old(icnt)+correction
            print *, 'APM:TROPOMI CO Bias Correction (new) ',corrections_old(icnt)
            exit
         endif
      enddo
   endif
...
! read number of IASI CO levels
            ...
            allocate(iasi_avgk(iasi_npr_int))
            allocate(iasi_prs(iasi_npr_intp))
...
! read number of IASI O3 levels
            ...
            iasi_npr_intp=iasi_npr_int+1
            read(iunit,*) iasi_prior
            allocate(iasi_hgt(iasi_npr_intp))
            allocate(iasi_prs(iasi_npr_intp))
            allocate(iasi_avgk(iasi_npr_int))
            allocate(iasi_col(iasi_npr_int))
...
! read observation error variance
            read(iunit,*) err_var
            deallocate(iasi_hgt,iasi_prs,iasi_avgk,iasi_col)
            cycle
!
         case('OMI_O3_COLUMN')
!
! read number of OMI O3 levels
            ...
            allocate(omi_prs(omi_npr_intp))
            allocate(omi_avgk(omi_npr_int))
            allocate(omi_prior(omi_npr_int))
...
! save OMI data
            if(dart_qc.gt.qc_crit.or.(prior_mean.eq.missing.or. &
               post_mean.eq.missing)) then
               deallocate(omi_prs,omi_avgk,omi_prior)
               cycle
            endif
...
! read number of TROPOMI O3 levels
            ...
            allocate(tropomi_prs(tropomi_npr_intp))
            allocate(tropomi_avgk(tropomi_npr_int))
            allocate(tropomi_prior(tropomi_npr_int))
            read(iunit,*) tropomi_prs(1:tropomi_npr_intp)
            read(iunit,*) tropomi_avgk(1:tropomi_npr_int)
...
! save TROPOMI data
            if(dart_qc.gt.qc_crit.or.(prior_mean.eq.missing.or. &
               post_mean.eq.missing)) then
               deallocate(tropomi_prs,tropomi_avgk,tropomi_prior)
               cycle
            endif
...
! read number of TEMPO O3 levels
            ...
            allocate(tempo_prs(tempo_npr_intp))
            allocate(tempo_avgk(tempo_npr_int))
            allocate(tempo_prior(tempo_npr_int))
...
! save TEMPO data
            if(dart_qc.gt.qc_crit.or.(prior_mean.eq.missing.or. &
               post_mean.eq.missing)) then
               deallocate(tempo_prs,tempo_avgk,tempo_prior)
               cycle
            endif
...
! read number of TEMPO NO2 levels
            ...
            allocate(tempo_prs(tempo_npr_intp))
            allocate(tempo_scwt(tempo_npr_int))
            read(iunit,*) tempo_prs(1:tempo_npr_intp)
...
! save TEMPO data
            if(dart_qc.gt.qc_crit.or.(prior_mean.eq.missing.or. &
               post_mean.eq.missing)) then
               deallocate(tempo_prs,tempo_scwt)
               cycle
            endif
...
subroutine bias_correction(maxnobs,cnt,obs_val,err_var,prior_mean,prior_sprd,correction)
   ...
   real :: prior_var,sum,sum_wt,correction
   real,dimension(maxnobs) :: obs_val,err_var,prior_mean,prior_sprd
   sum_wt=0.
   sum=0.
   do icnt=1,cnt
      prior_var = prior_sprd(icnt)*prior_sprd(icnt)
      sum_wt = sum_wt + (1./prior_var + 1./err_var(icnt))
      sum = sum + (prior_mean(icnt) - obs_val(icnt)) * (1./prior_var + 1./err_var(icnt))
   enddo
   correction = sum / sum_wt
