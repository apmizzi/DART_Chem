...
!-----------------------------------------------------------------
      ...
      character(len=2)       :: numa
      character(len=256)     :: spc_map(specmax)

      namelist /control/ dir_moz, dir_wrf, init_cond_file_prefix, bdy_cond_file_prefix, fn_moz, &
                         do_bc, do_ic, spc_map, moz_var_prefix, moz_var_suffix, met_file_prefix, &
                         met_file_suffix, met_file_separator, wrf_ps_name, &
                         domain, def_missing_var, ps0, invert_p_moz, p_moz_fac, &
                         src_model, src_ps_name, dbg_species, press_a, press_b, dbg_i, dbg_j
...
!-----------------------------------------------------------------
      ...
      select case( trim(src_model) )
        case( 'MOZART', 'GEOS5', 'ECMWF' )
        case DEFAULT
          write(*,*) 'main_bc_wrfchem: source data model must be either MOZART or GEOS5'
          stop
      end select
...
!-----------------------------------------------------------------
      ...
      wrf_lons_are_nonneg = all( xlon(:,:) >= 0. )
...
!-----------------------------------------------------------------
      ...
      call init_mozart_lib( dir_moz, fn_moz, xlon, xlat, wrf_date(1), &
                            wrf_datesec(1), wrf_lons_are_nonneg, nx, ny, nspec )
