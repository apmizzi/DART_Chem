...
!------------------------------------------------------------------
      ...
      character(len=16) :: wrf_ps_name = 'PSFC'
...
file_loop1 : &
      do file = ic,bc
        ...
        status = nf_open( filespec(file), or(nf_write,nf_64bit_offset), ncids(file) )
...
!------------------------------------------------------------------
      ...
      if( status /= nf_noerr ) then
         ...
         write(*,*) 'init_wrfchem: MAP_PROJ = ',map_proj
      end if
...
!------------------------------------------------------------------
      ...
      if( len_trim( met_file_suffix ) > 0 ) then
         filenm = trim( wrf_dir ) // trim( met_file_prefix) // trim(met_file_separator) &
                  // domain_name // trim(met_file_separator) // tstring // trim(met_file_suffix)
      else
         filenm = trim( wrf_dir ) // trim( met_file_prefix) &
                  // trim( met_file_separator) // domain_name // trim(met_file_separator) // tstring
      end if
...
!     write IC values
!------------------------------------------------------------------
      write(*,*) 'ic_write4d; ncid_ic,varid = ',ncid_ic,varid
      write(*,*) 'ic_write4d; nstt = ',nstt
      write(*,*) 'ic_write4d; ncnt = ',ncnt
...
!------------------------------------------------------------------
      ...
      if( len_trim( met_file_suffix ) > 0 ) then
         filenm = trim( wrf_dir ) // trim( met_file_prefix) // trim(met_file_separator) &
                                  // domain_name // trim(met_file_separator) // tstring &
                                  // trim(met_file_suffix)
      else
         filenm = trim( wrf_dir ) // trim( met_file_prefix) // trim(met_file_separator)  &
                                  // domain_name // trim(met_file_separator) // tstring
      end if
      ...
      status = nf_inq_varid( ncid_bc, trim(wrf_ps_name), varid )
      if( status /= nf_noerr )  then
         write(*,*) 'wrf_ps: failed to find ',trim(wrf_ps_name)
         call handle_error( status )
      end if
      ...
      if( status /= nf_noerr ) then
         write(*,*) 'wrf_ps: failed to read ',trim(wrf_ps_name)
         call handle_error( status )
      end if
