...
export EXPERIMENT_DATA_DIR=${INPUT_DATA_DIR}/CMAQ_REAL_TIME_DATA
export RUN_DIR=${EXPERIMENT_DIR}/CMAQ_TESTCASE
...
export RUN_MOPITT_CO_PROFILE_OBS=false # (done, no thinner)
export RUN_MOPITT_CO_CPSR_OBS=false # (done, no thinner)
export RUN_IASI_CO_TOTAL_COL_OBS=false
export RUN_IASI_CO_PROFILE_OBS=false # (done, no thinner)
export RUN_IASI_CO_CPSR_OBS=false # (done, no thinner)
export RUN_IASI_O3_PROFILE_OBS=false
export RUN_IASI_O3_CPSR_OBS=false
export RUN_MODIS_AOD_TOTAL_COL_OBS=false # (done, no thinner)
export RUN_OMI_O3_TOTAL_COL_OBS=false
export RUN_OMI_O3_TROP_COL_OBS=false
export RUN_OMI_O3_PROFILE_OBS=false # (done, thinner)
export RUN_OMI_O3_CPSR_OBS=false # (works, thinner)
export RUN_OMI_NO2_TOTAL_COL_OBS=false
export RUN_OMI_NO2_TROP_COL_OBS=false # (done, thinner)
export RUN_OMI_NO2_DOMINO_TOTAL_COL_OBS=false
export RUN_OMI_NO2_DOMINO_TROP_COL_OBS=false # (works, thinner)
export RUN_OMI_SO2_TOTAL_COL_OBS=false
export RUN_OMI_SO2_PBL_COL_OBS=false # (works, thinner)
export RUN_OMI_HCHO_TOTAL_COL_OBS=false # (works, thinner)
...
export RUN_TES_CO_PROFILE_OBS=false # (works, thinner)
export RUN_TES_CO_CPSR_OBS=false # (works, thinner)
...
export RUN_TES_O3_PROFILE_OBS=false # (works, thinner)
export RUN_TES_O3_CPSR_OBS=false # (works, thinner)
...
export RUN_MLS_O3_PROFILE_OBS=false # (works, check, thinner)
export RUN_MLS_O3_CPSR_OBS=false # (works, vertical sum, thinner)
export RUN_MLS_HNO3_TOTAL_COL_OBS=false
export RUN_MLS_HNO3_PROFILE_OBS=false # (works, vertical sum, thinner)
export RUN_MLS_HNO3_CPSR_OBS=false # (works, vertical sum, thinner)
...
#
   ...
   source ${RS_SCRIPTS_DIR}/RS_Forecast_Time_Domain_Params_CMAQ.ksh > index_RS_Forecast_Time_Domain_Params_${DATE} 2>&1
   rm -rf index_RS_WRFChem_Namelists_${DATE}
   source ${RS_SCRIPTS_DIR}/RS_WRFChem_Namelists_CMAQ.ksh > index_RS_WRFChem_Namelists_${DATE} 2>&1
   ...
   source ${RS_SCRIPTS_DIR}/RS_DART_Namelists_CMAQ.ksh > index_RS_DART_Namelists_${DATE} 2>&1
...
#
   export CMAQ_DART_WORK_DIR=${DART_DIR}/models/wrf_cmaq/work
...
#
      ...
      if [[ ! -d preprocess_obs ]]; then
         mkdir -p preprocess_obs
         cd preprocess_obs
      else
         cd preprocess_obs
      fi
      cp ../combine_obs/obs_seq_comb_${DATE}.out obs_seq_comb_filtered_${DATE}.out
   fi
#
#########################################################################
#
...
#
   if ${RUN_DART_FILTER}; then
      ...
      source ${RS_SCRIPTS_DIR}/RS_DART_Filter_CMAQ.ksh > index_rs.html 2>&1 
   fi
