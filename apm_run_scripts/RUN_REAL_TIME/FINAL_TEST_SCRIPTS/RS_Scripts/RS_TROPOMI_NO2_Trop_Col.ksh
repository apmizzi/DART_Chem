...
# SET TROPOMI INPUT DATA FILE
      export INFILE=${EXPERIMENT_TROPOMI_NO2_DIR}/${YYYY}${MM}${DD}/${TROPOMI_FILE_PRE}${YYYY}${MM}${DD}T
#
# COPY EXECUTABLE
      ...
      ./run_tropomi_no2_trop_col_extract.sh ${MATLAB} ${INFILE} ${OUTFILE} ${TROPOMI_FILE_PRE} ${BIN_BEG_YY} ${BIN_BEG_MM} ${BIN_BEG_DD} ${BIN_BEG_HH} ${BIN_BEG_MN} ${BIN_BEG_SS} ${BIN_END_YY} ${BIN_END_MM} ${BIN_END_DD} ${BIN_END_HH} ${BIN_END_MN} ${BIN_END_SS} ${NL_PATH_MODEL} ${NL_FILE_MODEL} ${NL_NX_MODEL} ${NL_NY_MODEL} > index_mat1.html 2>&1
#
...
# END OF PREVIOUS DAY (hours 21 to 24 obs)
      if [[ ${FLG} -eq 1 ]]; then
         ...
         export BIN_END_MN=59
         export BIN_END_SS=59
	 export INFILE=\'${EXPERIMENT_TROPOMI_NO2_DIR}/${ASIM_MN_YYYY}${ASIM_MN_MM}${ASIM_MN_DD}/${TROPOMI_FILE_PRE}${ASIM_MN_YYYY}${ASIM_MN_MM}${ASIM_MN_DD}T\'
#
# COPY EXECUTABLE
         export FILE=tropomi_no2_trop_col_extract.m
         rm -rf ${FILE}
         ...
         ./run_tropomi_no2_trop_col_extract.sh ${MATLAB} ${INFILE} ${OUTFILE} ${TROPOMI_FILE_PRE} ${BIN_BEG_YY} ${BIN_BEG_MM} ${BIN_BEG_DD} ${BIN_BEG_HH} ${BIN_BEG_MN} ${BIN_BEG_SS} ${BIN_END_YY} ${BIN_END_MM} ${BIN_END_DD} ${BIN_END_HH} ${BIN_END_MN} ${BIN_END_SS} ${NL_NX_MODEL} ${NL_NY_MODEL} > index_mat2.html 2>&1
#
      fi
#
# CHECK IF OUTFILE EXISTS AND ATTACH TO ARCHIVE FILE
...
# Clean directory
      ...
      rm readme.* requiredMCRP* run_tropomi_no2_* unresolved* tropomi_no2_*
