...
# SET TEMPO PARAMETERS
      ...
      export TEMPO_FILE_PRE=TEMPO_NO2_L2_P01_
...
# SET TEMPO INPUT DATA DIR
      export INFILE=${EXPERIMENT_TEMPO_NO2_DIR}/${YYYY}${MM}${DD}/${TEMPO_FILE_PRE}${YYYY}${MM}${DD}T
#
# COPY EXECUTABLE
      ...
      ./run_tempo_no2_trop_col_extract.sh ${MATLAB} ${INFILE} ${OUTFILE} ${TEMPO_FILE_PRE} ${BIN_BEG_YY} ${BIN_BEG_MM} ${BIN_BEG_DD} ${BIN_BEG_HH} ${BIN_BEG_MN} ${BIN_BEG_SS} ${BIN_END_YY} ${BIN_END_MM} ${BIN_END_DD} ${BIN_END_HH} ${BIN_END_MN} ${BIN_END_SS} ${NL_PATH_MODEL} ${NL_FILE_MODEL} ${NL_NX_MODEL} ${NL_NY_MODEL} > index_mat1.html 2>&1
#
...
# END OF PREVIOUS DAY (hours 21 to 24 obs)
      if [[ ${FLG} -eq 1 ]]; then	 
         ...
         export BIN_END_MN=59
         export BIN_END_SS=59
         export INFILE=${EXPERIMENT_TEMPO_NO2_DIR}/${ASIM_MN_YYYY}${ASIM_MN_MM}${ASIM_MN_DD}/${TEMPO_FILE_PRE}${ASIM_MN_YYYY}${ASIM_MN_MM}${ASIM_MN_DD}T
#
# COPY EXECUTABLE
         export FILE=tempo_no2_trop_col_extract.m
         rm -rf ${FILE}
         cp ${DART_DIR}/observations/obs_converters/ATMOS_CHEM/TEMPO_NO2/native_to_ascii/${FILE} ./.
         mcc -m tempo_no2_trop_col_extract.m -o tempo_no2_trop_col_extract
         ./run_tempo_no2_trop_col_extract.sh ${MATLAB} ${INFILE} ${OUTFILE} ${TEMPO_FILE_PRE} ${BIN_BEG_YY} ${BIN_BEG_MM} ${BIN_BEG_DD} ${BIN_BEG_HH} ${BIN_BEG_MN} ${BIN_BEG_SS} ${BIN_END_YY} ${BIN_END_MM} ${BIN_END_DD} ${BIN_END_HH} ${BIN_END_MN} ${BIN_END_SS} ${NL_PATH_MODEL} ${NL_FILE_MODEL} ${NL_NX_MODEL} ${NL_NY_MODEL} > index_mat2.html 2>&1
#
      fi
#
# CHECK IF OUTFILE EXISTS AND ATTACH TO ARCHIVE FILE
...
# Clean directory
      ...
      rm readme.* requiredMCRP* run_tempo_no2_* unresolved* tempo_no2_*
