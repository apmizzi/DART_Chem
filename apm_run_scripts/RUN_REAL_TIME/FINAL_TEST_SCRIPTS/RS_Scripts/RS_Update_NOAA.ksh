...
# Get background forecasts
         ...
         export DA_PRIOR_FILE=${BACKGND_FCST_DIR}/run_${CMEM}/wrfout_d${CR_DOMAIN}_${FILE_DATE}
         ...
         cp ${BDYCDN_PRIOR} wrfbdy_d${CR_DOMAIN}_${FILE_DATE}_prior.${CMEM}
         cp ${BDYCDN_PRIOR} wrfbdy_d${CR_DOMAIN}_${FILE_DATE}_filt.${CMEM}
         export BDYCDN_POSTERIOR=wrfbdy_d${CR_DOMAIN}_${FILE_DATE}_filt.${CMEM}    
#
         export CYCLING=true
         export NL_LOW_BDY_ONLY=false
         export NL_UPDATE_LSM=false
         cp -f $DA_PRIOR_FILE real_output
         cp -f $DA_POSTERIOR_FILE wrfvar_output
         cp -f $BDYCDN_PRIOR wrfbdy_d01_input
         cp -f $BDYCDN_PRIOR wrfbdy_d01
#
         cat <<EOF > parame.in
&control_param
