...
#	 
      cd ${RUN_DIR}/${DATE}/wrfchem_cycle_cr
#
# Submit all WRFChem jobs in single script
      rm -rf job,ksh
      touch job.ksh
      RANDOM=$$
      export JOBRND=${RANDOM}_wrf_single
      let NPROC_SING=${WRFCHEM_SING_NODES}*${WRFCHEM_SING_TASKS}
      cat<<EOF > job.ksh
#!/bin/ksh -aeux
#PBS -W group_list=${ACCOUNT}
#PBS -N ${JOBRND}
#PBS -l walltime=${WRFCHEM_SING_LIMIT}
#PBS -q ${WRFCHEM_SING_CLASS}
#PBS -j oe
#PBS -l select=${WRFCHEM_SING_NODES}:ncpus=${WRFCHEM_SING_TASKS}:mpiprocs=${WRFCHEM_SING_TASKS}:model=has
...
while [[ \${IMEM} -le \${L_NUM_MEMBERS} ]]; do
   ...
   /u/scicon/tools/bin/several_tries mpiexec -np ${NPROC_SING} ./wrf.exe  > index.html 2>&1 
   let IMEM=\${IMEM}+1
done
EOF
#
