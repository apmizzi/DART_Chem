#!/bin/ksh -aux
#
      cd ${RUN_DIR}/${DATE}/dart_filter_TEST
#
...
#
   cp ${RUN_DIR}/${PAST_DATE}/dart_filter_TEST/\${NL_WRFCHEMI_PRIOR} \${NL_WRFCHEMI_PRIOR}
   cp ${RUN_DIR}/${PAST_DATE}/dart_filter_TEST/\${NL_WRFCHEMI} \${NL_WRFCHEMI_POST}
   cp ${RUN_DIR}/${PAST_DATE}/dart_filter_TEST/\${NL_WRFFIRECHEMI_PRIOR} \${NL_WRFFIRECHEMI_PRIOR}
   cp ${RUN_DIR}/${PAST_DATE}/dart_filter_TEST/\${NL_WRFFIRECHEMI} \${NL_WRFFIRECHEMI_POST}
#
   let ICNT=1
   export L_DATE=${DATE}
      ...
      if ${USE_DART_INFL}; then
         ...
         if [[ ${DATE} -ne ${FIRST_DART_INFLATE_DATE} ]]; then
            if [[ ${NL_INF_FLAVOR_PRIOR} != 0 ]]; then
               export INF_OUT_FILE_MN_PRIOR=${RUN_DIR}/${PAST_DATE}/dart_filter_TEST/output_priorinf_mean.nc
               export INF_OUT_FILE_SD_PRIOR=${RUN_DIR}/${PAST_DATE}/dart_filter_TEST/output_priorinf_sd.nc
               cp ${INF_OUT_FILE_MN_PRIOR} input_priorinf_mean.nc
               cp ${INF_OUT_FILE_SD_PRIOR} input_priorinf_sd.nc
            fi
            if [[ ${NL_INF_FLAVOR_POST} != 0 ]]; then
               export INF_OUT_FILE_MN_POST=${RUN_DIR}/${PAST_DATE}/dart_filter_TEST/output_postinf_mean.nc
               export INF_OUT_FILE_SD_POST=${RUN_DIR}/${PAST_DATE}/dart_filter_TEST/output_postinf_sd.nc
