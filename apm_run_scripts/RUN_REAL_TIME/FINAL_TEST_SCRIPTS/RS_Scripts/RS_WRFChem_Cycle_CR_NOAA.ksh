...
let ICNT=01
while [[ \${ICNT} -le ${NL_NUM_FIRECHEMI_FILES} ]]; do
...
#
# Loop through WRF-Chem run directories to save the log files      
      let IMEM=1
      while [[ ${IMEM} -le ${L_NUM_MEMBERS} ]]; do
         export MEM=${IMEM}
         if ${RUN_SPECIAL_FORECAST}; then
