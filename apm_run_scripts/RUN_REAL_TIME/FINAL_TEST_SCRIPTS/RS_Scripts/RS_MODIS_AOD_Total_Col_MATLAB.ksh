...
# COPY EXECUTABLE
         ...
         cp ${DART_DIR}/observations/obs_converters/ATMOS_CHEM/MODIS_AOD/native_to_ascii/${FILE} ./.
         mcc -m modis_aod_total_col_extract.m -o modis_aod_total_col_extract
         ./run_modis_aod_total_col_extract.sh ${MATLAB} ${INFILE} ${OUTFILE} ${MODIS_FILE_PRE} ${BIN_BEG_YY} ${BIN_BEG_MM} ${BIN_BEG_DD} ${BIN_BEG_HH} ${BIN_BEG_MN} ${BIN_BEG_SS} ${BIN_END_YY} ${BIN_END_MM} ${BIN_END_DD} ${BIN_END_HH} ${BIN_END_MN} ${BIN_END_SS} ${NL_PATH_MODEL} ${NL_FILE_MODEL} ${NL_NX_MODEL} ${NL_NY_MODEL} > index_mat2.html 2>&1
      fi   
#
# CHECK IF OUTFILE EXISTS AND ATTACH TO ARCHIVE FILE
      if [[ ! -e ${ARCHIVE_FILE} && -e ${OUTFILE_NQ} ]]; then
...
# GET EXECUTABLE
      ...
      ./modis_aod_total_col_ascii_to_obs_MATLAB > index_ascii.html 2>&1
#
