...
export RUN_SPECIAL_FORECAST=false #set to false clyu
...
#
# ROOT DIRECTORIES:
   export SCRATCH_DIR=/nobackupp12/clyu1/OUTPUT_DATA
   export WORK_DIR=/nobackupp12/clyu1
   export INPUT_DATA_DIR=/nobackupp11/amizzi/INPUT_DATA
#
# DEPENDENT INPUT DATA DIRECTORIES:
   export EXPERIMENT_DIR=${SCRATCH_DIR}
   export RUN_DIR=${EXPERIMENT_DIR}/real_FRAPPE_TEST_CASE_NASA_v4
   export TRUNK_DIR=${WORK_DIR}/TRUNK
   export WPS_DIR=/nobackupp11/amizzi/TRUNK/${WPS_VER}
   export WPS_GEOG_DIR=${INPUT_DATA_DIR}/${WPS_GEOG_VER}
   export WRFCHEM_DIR=/nobackupp11/amizzi/TRUNK/${WRFCHEM_VER}
   export WRFDA_DIR=/nobackupp11/amizzi/TRUNK/${WRFDA_VER}
   export DART_DIR=${TRUNK_DIR}/${DART_VER}
   export BUILD_DIR=${WRFDA_DIR}/var/da
   export WRF_DIR=/nobackupp11/amizzi/TRUNK/${WRF_VER}
...
#
# SELECT COMPONENT RUN OPTIONS: #for practice purpose set each option to false when it succeeds clyu
   if [[ ${RUN_SPECIAL_FORECAST} = "false" ]]; then
      export RUN_GEOGRID=false #success
      export RUN_UNGRIB=false #success
      export RUN_METGRID=false #success
      export RUN_REAL=false #success
      export RUN_PERT_WRFCHEM_MET_IC=false #success
      export RUN_PERT_WRFCHEM_MET_BC=false #success
      export RUN_EXO_COLDENS=false #success
      export RUN_SEASON_WES=false #success
      export RUN_WRFCHEM_BIO=false #success
      export RUN_WRFCHEM_FIRE=false #success
      export RUN_WRFCHEM_CHEMI=false #success
      export RUN_PERT_WRFCHEM_CHEM_ICBC=false #success
      export RUN_PERT_WRFCHEM_CHEM_EMISS=false #success
      ...
      export RUN_AIRNOW_O3_OBS=false #success
      export RUN_AIRNOW_CO_OBS=false #success
      export RUN_AIRNOW_NO2_OBS=false #success
      export RUN_AIRNOW_SO2_OBS=false #success
      export RUN_AIRNOW_PM10_OBS=false #success
      export RUN_AIRNOW_PM25_OBS=false #success
      export RUN_PANDA_CO_OBS=false
      export RUN_PANDA_O3_OBS=false
      export RUN_PANDA_PM25_OBS=false
      export RUN_MODIS_AOD_OBS=false
      export RUN_MET_OBS=false #success
      export RUN_COMBINE_OBS=false #success
#
      if [[ ${DATE} -eq ${INITIAL_DATE}  ]]; then
         export RUN_PREPROCESS_OBS=false
         export RUN_WRFCHEM_INITIAL=false #success
         export RUN_LOCALIZATION=false
         export RUN_DART_FILTER=false
	 export RUN_BIAS_CORRECTION=false
         export RUN_UPDATE_BC=false
         export RUN_ENSEMBLE_MEAN_INPUT=false #success
...
#            RANDOM=$$
            ...
            export ACCOUNT=s1395 #clyu added this line to debug
            #/nobackupp12/clyu1/TRUNK/DART_chem_upgrade/models/wrf_chem/hybrid_scripts/job_script_nasa.ksh: line 29: 8: parameter not set
...
EOF
#
      ./runICBC_parent_rt_CR.ksh #clyu: at one point it had the error:"module: command not found", but disappeared now
#
