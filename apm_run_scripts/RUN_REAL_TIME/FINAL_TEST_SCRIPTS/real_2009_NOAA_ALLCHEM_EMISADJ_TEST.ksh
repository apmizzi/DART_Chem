...
#
export RUN_DIR=${EXPERIMENT_DIR}/OUTPUT_2009_NOAA_EMISADJ_10MEMS
...
#
   if ${RUN_DART_FILTER}; then
      if [[ ! -d ${RUN_DIR}/${DATE}/dart_filter_TEST ]]; then
         mkdir -p ${RUN_DIR}/${DATE}/dart_filter_TEST
         cd ${RUN_DIR}/${DATE}/dart_filter_TEST
      else
         cd ${RUN_DIR}/${DATE}/dart_filter_TEST
      fi
      source ${RS_SCRIPTS_DIR}/RS_DART_Filter_NOAA_TEST.ksh > index_rs.html 2>&1 
   fi
