...
export RUN_DIR=${EXPERIMENT_DIR}/INPUT_DATA_TRACER_I
export RUN_INPUT_DIR=${EXPERIMENT_DIR}/INPUT_DATA_TRACER_I
export EXPERIMENT_INPUT_OBS=TRACER_I_OBS
...
#
# END CYCLE DATE-TIME:
export CYCLE_END_DATE=2014072712
#
...
# SELECT COMPONENT RUN OPTIONS:
   ...
   export RUN_TES_CO_PROFILE_OBS=true # (works)  TRACER I
   ...
   export RUN_TES_O3_PROFILE_OBS=true # (works)  TRACER I
   ...
   export RUN_GOME2A_NO2_TROP_COL_OBS=true # (works, vertical sum)  TRACER I
   export RUN_MLS_O3_TOTAL_COL_OBS=false
   export RUN_MLS_O3_PROFILE_OBS=true # (works, check)  TRACER I
...
#
   ...
   export NL_FAC_OBS_ERROR_OMI_O3=0.75          # good
...
#
   if [[ ${RUN_PERT_WRFCHEM_MET_IC} = "true" ]]; then 
      ...
      source ${RS_SCRIPTS_DIR}/RS_SINGLE_Pert_WRFChem_Met_IC.ksh > index_rc.html 2>&1
   fi
#
#########################################################################
#
...
#
   if [[ ${RUN_PERT_WRFCHEM_MET_BC} = "true" ]]; then 
      ...
      source ${RS_SCRIPTS_DIR}/RS_SINGLE_Pert_WRFChem_Met_BC.ksh > index_rc.html 2>&1
   fi
#
#########################################################################
#
...
#
   if ${RUN_PERT_WRFCHEM_CHEM_ICBC}; then
      ...
      source ${RS_SCRIPTS_DIR}/RS_SINGLE_Pert_WRFChem_Chem_ICBC.ksh > index_rc.html 2>&1
   fi
#
#########################################################################
#
...
#
   if ${RUN_PERT_WRFCHEM_CHEM_EMISS}; then
      ...
      source ${RS_SCRIPTS_DIR}/RS_SINGLE_Pert_WRFChem_Chem_Emiss.ksh > index_rc.html 2>&1
   fi
...
#      
      if ${RUN_MOPITT_V8_CO_PROFILE_OBS}; then 
         ...
         source ${RS_SCRIPTS_DIR}/RS_MOPITT_V8_CO_Profile_FRAPPE.ksh > index_rc.html 2>&1
      fi
   fi
#
########################################################################
...
#      source ${RS_SCRIPTS_DIR}/RS_MODIS_AOD_Total_Col.ksh > index_rc.html 2>&1
      source ${RS_SCRIPTS_DIR}/RS_MODIS_AOD_Total_Col_MATLAB.ksh > index_rc.html 2>&1
   fi
#
#########################################################################
#
