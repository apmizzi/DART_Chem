...
export RUN_DIR=${EXPERIMENT_DIR}/TRACER_I_ALLCHEM
...
export RUN_TES_CO_PROFILE_OBS=false # (works)  TRACER I
...
export RUN_TES_O3_PROFILE_OBS=false # (works)  TRACER I
...
#
   if ${RUN_DART_FILTER}; then
      ...
      source ${RS_SCRIPTS_DIR}/RS_SINGLE_DART_Filter.ksh > index_rs.html 2>&1 
   fi
...
#
   if ${RUN_WRFCHEM_INITIAL}; then
      ...
      source ${RS_SCRIPTS_DIR}/RS_SINGLE_WRFChem_Initial.ksh > index_rs.html 2>&1 
   fi
#
#########################################################################
#
...
#
   if ${RUN_WRFCHEM_CYCLE_CR}; then
      ...
      if [[ ${RUN_SPECIAL_FORECAST} == true ]]; then
         ...
         source ${RS_SCRIPTS_DIR}/RS_SINGLE_WRFChem_Cycle_CR_Single.ksh > index_rs.html 2>&1
      fi 
   fi
#
#########################################################################
...
#
   if ${RUN_WRFCHEM_CYCLE_FR}; then
      ...
      source ${RS_SCRIPTS_DIR}/RS_SINGLE_WRFChem_Cycle_FR.ksh > index_rs.html 2>&1 
   fi
#
#########################################################################
#
...
#
   if ${RUN_ENSMEAN_CYCLE_FR}; then
      ...
      source ${RS_SCRIPTS_DIR}/RS_SINGLE_Ensmean_Cycle_FR.ksh > index_rs.html 2>&1 
   fi
#
#########################################################################
#
