...
! version controlled file description for error handling, do not edit
   ...
   integer                                  :: unit,unita,unitb,stat
   integer                                  :: date,ngrid_corr
   integer                                  :: nx,ny,nz,nzp,nz_chem,nz_fire,nz_biog
   integer                                  :: nchem_spcs,nfire_spcs,nbiog_spcs,num_mem
...
! Anthropogenic emissions                
   if(sw_chem) then
      allocate(chem_fac_mem_old(nx,ny,nz_chem,nchem_spcs))
      allocate(chem_fac_mem_new(nx,ny,nz_chem,nchem_spcs))
      call perturb_fields(chem_fac_mem_old,chem_fac_mem_new, &
      lat,lon,A_chem,nx,ny,nz_chem,nchem_spcs,ngrid_corr,sw_corr_tm, &
      corr_lngth_hz)
      allocate(tmp_arry(nx*ny*nz_chem*nchem_spcs))
      if(sw_corr_tm) then
         call apm_pack(tmp_arry,chem_fac_mem_old,nx,ny,nz_chem,nchem_spcs)
         call mpi_send(tmp_arry,nx*ny*nz_chem*nchem_spcs,MPI_FLOAT,0,0*num_mem+rank, &
         MPI_COMM_WORLD,ierr)
         deallocate(chem_fac_mem_old)
      endif
      call apm_pack(tmp_arry,chem_fac_mem_new,nx,ny,nz_chem,nchem_spcs)
      call mpi_send(tmp_arry,nx*ny*nz_chem*nchem_spcs,MPI_FLOAT,0,1*num_mem+rank, &
      MPI_COMM_WORLD,ierr)
      deallocate(tmp_arry)
...
! Biomass burning emissions                
   if(sw_fire) then
      allocate(chem_fac_mem_old(nx,ny,nz_chem,nfire_spcs))
      allocate(chem_fac_mem_new(nx,ny,nz_chem,nfire_spcs))
      call perturb_fields(chem_fac_mem_old,chem_fac_mem_new, &
      lat,lon,A_fire,nx,ny,nz_fire,nfire_spcs,ngrid_corr,sw_corr_tm, &
      corr_lngth_hz)
      allocate(tmp_arry(nx*ny*nz_fire*nfire_spcs))
      if(sw_corr_tm) then
         call apm_pack(tmp_arry,chem_fac_mem_old,nx,ny,nz_fire,nfire_spcs)
         call mpi_send(tmp_arry,nx*ny*nz_fire*nfire_spcs,MPI_FLOAT,0,0*num_mem+rank, &
         MPI_COMM_WORLD,ierr)
         deallocate(chem_fac_mem_old)
      endif
      call apm_pack(tmp_arry,chem_fac_mem_new,nx,ny,nz_fire,nfire_spcs)
      call mpi_send(tmp_arry,nx*ny*nz_fire*nfire_spcs,MPI_FLOAT,0,1*num_mem+rank, &
      MPI_COMM_WORLD,ierr)
      deallocate(tmp_arry)
      deallocate(chem_fac_mem_new)
   endif
...
! Biogenic emissions                
   if(sw_biog) then
      allocate(chem_fac_mem_old(nx,ny,nz_chem,nbiog_spcs))
      allocate(chem_fac_mem_new(nx,ny,nz_chem,nbiog_spcs))
      call perturb_fields(chem_fac_mem_old,chem_fac_mem_new, &
      lat,lon,A_biog,nx,ny,nz_biog,nbiog_spcs,ngrid_corr,sw_corr_tm, &
      corr_lngth_hz)
      allocate(tmp_arry(nx*ny*nz_biog*nbiog_spcs))
      if(sw_corr_tm) then
         call apm_pack(tmp_arry,chem_fac_mem_old,nx,ny,nz_biog,nbiog_spcs)
         call mpi_send(tmp_arry,nx*ny*nz_biog*nbiog_spcs,MPI_FLOAT,0,0*num_mem+rank, &
         MPI_COMM_WORLD,ierr)
         deallocate(chem_fac_mem_old)
      endif
      call apm_pack(tmp_arry,chem_fac_mem_new,nx,ny,nz_biog,nbiog_spcs)
      call mpi_send(tmp_arry,nx*ny*nz_biog*nbiog_spcs,MPI_FLOAT,0,1*num_mem+rank, &
      MPI_COMM_WORLD,ierr)
      deallocate(tmp_arry)
      deallocate(chem_fac_mem_new)
   endif
...
! Root process
   if (rank.eq.0) then
      if(sw_chem) then 
         allocate(tmp_arry(nx*ny*nz_chem*nchem_spcs))
         if(sw_corr_tm) then
            allocate(chem_fac_old(nx,ny,nz_chem,nchem_spcs,num_mem))
            do imem=1,num_mem
               call mpi_recv(tmp_arry,nx*ny*nz_chem*nchem_spcs,MPI_FLOAT,imem,0*num_mem+imem, &
               MPI_COMM_WORLD,stat,ierr)
               call apm_unpack(tmp_arry,chem_fac_old(:,:,:,:,imem),nx,ny,nz_chem,nchem_spcs)
            enddo
         endif
         allocate(chem_fac_new(nx,ny,nz_chem,nchem_spcs,num_mem))
         do imem=1,num_mem
            call mpi_recv(tmp_arry,nx*ny*nz_chem*nchem_spcs,MPI_FLOAT,imem,1*num_mem+imem, &
            MPI_COMM_WORLD,stat,ierr)
            call apm_unpack(tmp_arry,chem_fac_new(:,:,:,:,imem),nx,ny,nz_chem,nchem_spcs)
         enddo
         deallocate(tmp_arry)
!
! Impose temporal correlations
         ...
         allocate(chem_fac_end(nx,ny,nz_chem,nchem_spcs,num_mem))
...
! Recenter the perturbation factors
         if(sw_corr_tm) then
            call recenter_factors(chem_fac_old,nx,ny,nz_chem,nchem_spcs, &
            num_mem,sprd_chem)
         endif
         call recenter_factors(chem_fac_new,nx,ny,nz_chem,nchem_spcs, &
         num_mem,sprd_chem)
         call recenter_factors(chem_fac_end,nx,ny,nz_chem,nchem_spcs, &
         num_mem,sprd_chem)
!
         ...
         do isp=1,nchem_spcs
            do imem=1,num_mem
               ...
               do i=1,nx
                  do j=1,ny
                     do k=1,nz_chem
                        if(chem_data3d(i,j,k)*(1.+chem_fac_end(i,j,k,isp,imem)) .le. 0.) then
                           chem_data3d(i,j,k)=fac_min*chem_data3d(i,j,k)
                        else
                           chem_data3d(i,j,k)=chem_data3d(i,j,k)*(1.+chem_fac_end(i,j,k,isp,imem))
                        endif
...
!            call put_WRFCHEM_emiss_data(wrfchem_file,ch_chem_spc(isp),chem_data3d_mean, &
!            nx,ny,nz_chem)
!            wrfchem_file=trim(wrfchemi)//'_sprd'
!            print *, 'put the spread ',trim(wrfchem_file)
!            call put_WRFCHEM_emiss_data(wrfchem_file,ch_chem_spc(isp),chem_data3d_sprd, &
!            nx,ny,nz_chem)
!            wrfchem_file=trim(wrfchemi)//'_frac'
!            print *, 'put the fraction ',trim(wrfchem_file)
!            call put_WRFCHEM_emiss_data(wrfchem_file,ch_chem_spc(isp),chem_data3d_frac, &
!            nx,ny,nz_chem)
...
!
      if(sw_fire) then 
         allocate(tmp_arry(nx*ny*nz_fire*nfire_spcs))
         if(sw_corr_tm) then
            allocate(chem_fac_old(nx,ny,nz_fire,nfire_spcs,num_mem))
            do imem=1,num_mem
               call mpi_recv(tmp_arry,nx*ny*nz_fire*nfire_spcs,MPI_FLOAT,imem,0*num_mem+imem, &
               MPI_COMM_WORLD,stat,ierr)
               call apm_unpack(tmp_arry,chem_fac_old(:,:,:,:,imem),nx,ny,nz_fire,nfire_spcs)
            enddo
         endif
         allocate(chem_fac_new(nx,ny,nz_fire,nfire_spcs,num_mem))
         do imem=1,num_mem
            call mpi_recv(tmp_arry,nx*ny*nz_fire*nfire_spcs,MPI_FLOAT,imem,1*num_mem+imem, &
            MPI_COMM_WORLD,stat,ierr)
            call apm_unpack(tmp_arry,chem_fac_new(:,:,:,:,imem),nx,ny,nz_fire,nfire_spcs)
         enddo
         deallocate(tmp_arry)
!
! Impose temporal correlations
         ...
         allocate(chem_fac_end(nx,ny,nz_fire,nfire_spcs,num_mem))
         wgt_end=exp(-1.0*corr_tm_delt/corr_lngth_tm)
         chem_fac_end(:,:,:,:,:)=wgt_end*chem_fac_old(:,:,:,:,:)+(1.-wgt_end)* &
         chem_fac_new(:,:,:,:,:) 
!
! Recenter the emissions factors            
         if(sw_corr_tm) then
            call recenter_factors(chem_fac_old,nx,ny,nz_fire,nfire_spcs, &
            num_mem,sprd_fire)
         endif
         call recenter_factors(chem_fac_new,nx,ny,nz_fire,nfire_spcs, &
         num_mem,sprd_fire)
         call recenter_factors(chem_fac_end,nx,ny,nz_fire,nfire_spcs, &
         num_mem,sprd_fire)
!
! Save emissions factors for the next cycle
         open(unit=unitb,file=trim(pert_path_po)//'/pert_chem_emiss', &
         form='unformatted',status='unknown')
         rewind(unitb)         
         write(unitb) chem_fac_end
...
! Perturb the species and members
         ...
         do isp=1,nfire_spcs
            do imem=1,num_mem
               ...
               if(imem.ge.100.and.imem.lt.1000) write(cmem,"('.e',i3)"),imem
               wrfchem_file=trim(wrfchemi)//trim(cmem)
               call get_WRFCHEM_emiss_data(wrfchem_file,ch_fire_spc(isp),chem_data3d, &
               nx,ny,nz_fire)
               do i=1,nx
                  do j=1,ny
                     do k=1,nz_fire
...
!            wrfchem_file=trim(wrfchemi)//'_mean'
!            print *, 'put the mean ',trim(wrfchem_file)
!            call put_WRFCHEM_emiss_data(wrfchem_file,ch_fire_spc(isp),chem_data3d_mean, &
!            nx,ny,nz_fire)
!            wrfchem_file=trim(wrfchemi)//'_sprd'
!            print *, 'put the spread ',trim(wrfchem_file)
!            call put_WRFCHEM_emiss_data(wrfchem_file,ch_fire_spc(isp),chem_data3d_sprd, &
!            nx,ny,nz_fire)
!            wrfchem_file=trim(wrfchemi)//'_frac'
!            print *, 'put the fraction ',trim(wrfchem_file)
!            call put_WRFCHEM_emiss_data(wrfchem_file,ch_fire_spc(isp),chem_data3d_frac, &
!            nx,ny,nz_fire)
!            print *, 'finished ',trim(wrfchem_file)
!            deallocate(chem_data3d_mean)
!            deallocate(chem_data3d_sprd)
...
!
      if(sw_biog) then 
         allocate(tmp_arry(nx*ny*nz_biog*nbiog_spcs))
         if(sw_corr_tm) then
            allocate(chem_fac_old(nx,ny,nz_biog,nbiog_spcs,num_mem))
            do imem=1,num_mem
               call mpi_recv(tmp_arry,nx*ny*nz_biog*nbiog_spcs,MPI_FLOAT,imem,0*num_mem+imem, &
               MPI_COMM_WORLD,stat,ierr)
               call apm_unpack(tmp_arry,chem_fac_old(:,:,:,:,imem),nx,ny,nz_biog,nbiog_spcs)
            enddo
         endif
         allocate(chem_fac_new(nx,ny,nz_biog,nbiog_spcs,num_mem))
         do imem=1,num_mem
            call mpi_recv(tmp_arry,nx*ny*nz_biog*nbiog_spcs,MPI_FLOAT,imem,1*num_mem+imem, &
            MPI_COMM_WORLD,stat,ierr)
            call apm_unpack(tmp_arry,chem_fac_new(:,:,:,:,imem),nx,ny,nz_biog,nbiog_spcs)
         enddo
         deallocate(tmp_arry)
!
! Impose temporal correlations
         ...
         allocate(chem_fac_end(nx,ny,nz_biog,nbiog_spcs,num_mem))
         wgt_end=exp(-1.0*corr_tm_delt/corr_lngth_tm)
         chem_fac_end(:,:,:,:,:)=wgt_end*chem_fac_old(:,:,:,:,:)+(1.-wgt_end)* &
         chem_fac_new(:,:,:,:,:) 
!
! Recenter the emissions factors            
         if(sw_corr_tm) then
            call recenter_factors(chem_fac_old,nx,ny,nz_biog,nbiog_spcs, &
            num_mem,sprd_biog)
         endif
         call recenter_factors(chem_fac_new,nx,ny,nz_biog,nbiog_spcs, &
         num_mem,sprd_biog)
         call recenter_factors(chem_fac_end,nx,ny,nz_biog,nbiog_spcs, &
         num_mem,sprd_biog)
!
! Save emissions factors for the next cycle
         open(unit=unitb,file=trim(pert_path_po)//'/pert_chem_emiss', &
...
! Perturb the species and members
         ...
         do isp=1,nbiog_spcs
            do imem=1,num_mem
               ...
               if(imem.ge.100.and.imem.lt.1000) write(cmem,"('.e',i3)"),imem
               wrfchem_file=trim(wrfchemi)//trim(cmem)
               call get_WRFCHEM_emiss_data(wrfchem_file,ch_biog_spc(isp),chem_data3d, &
               nx,ny,nz_biog)
               do i=1,nx
                  do j=1,ny
                     do k=1,nz_biog
...
!            wrfchem_file=trim(wrfchemi)//'_mean'
!            print *, 'put the mean ',trim(wrfchem_file)
!            call put_WRFCHEM_emiss_data(wrfchem_file,ch_biog_spc(isp),chem_data3d_mean, &
!            nx,ny,nz_biog)
!            wrfchem_file=trim(wrfchemi)//'_sprd'
!            print *, 'put the spread ',trim(wrfchem_file)
!            call put_WRFCHEM_emiss_data(wrfchem_file,ch_biog_spc(isp),chem_data3d_sprd, &
!            nx,ny,nz_biog)
!            wrfchem_file=trim(wrfchemi)//'_frac'
!            print *, 'put the fraction ',trim(wrfchem_file)
!            call put_WRFCHEM_emiss_data(wrfchem_file,ch_biog_spc(isp),chem_data3d_frac, &
!            nx,ny,nz_biog)
!            print *, 'finished ',trim(wrfchem_vfile)
!            deallocate(chem_data3d_mean)
!            deallocate(chem_data3d_sprd)
!            deallocate(chem_data3d_frac)
         enddo
