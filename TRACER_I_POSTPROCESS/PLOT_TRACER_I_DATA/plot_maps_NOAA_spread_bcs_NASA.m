function plot_maps_NOAA_spread_bcs_NASA
   clear all;
%
   ...
   zscal_bdy=20.;
   num_mem=30;
   rc=system('rm -rf NOAA_PRIOR_SPREAD_BCs.eps');
   save_file='NOAA_PRIOR_SPREAD_BCs.eps';
%
% set colormap
...
%
   SPRD_FAC_TRM='';
   SPRD_FAC_TRM='_SCL_4.5';
%
   SPRD_FAC_LBL='';
   SPRD_FAC_LBL='SCL 4.5';
%
...
%
      ...
      for imem=1:num_mem
         ...
         exp_pr=strcat(path_data,strcat('/',DATE_STR_PR(idate),'/',WRFCHEM_CHEM_ICBC_DIR,'/wrfbdy_d01_',file_date,run_dir))
%         exp_pr=strcat(path_data,strcat('/',DATE_STR_PR(idate),'/',WRFCHEM_CHEM_ICBC_DIR,'/wrfbdy_d01',run_dir))
%
% CO PRIOR BCs
         fac=1.e3;
         for ibdy=1:nbdy
   ...
   if (iplt==1)
      ...
      for i=1:nhalo
         for j=1:ny
            zlon(i,j)=i;
            zlat(i,j)=j;
            zfld(i,j)=co_bdy_rel_sprd_map(1,i,j);
	 end
      end
%      zfld
%      return
%      contourf(zlon,zlat,zfld);
      colorbar('eastoutside')
      title('CO Rel Sprd BC CO_XS','Fontsize',18,'FontWeight','bold');
      xlabel('Longitude (deg)','FontSize',16);
      ylabel('Latitude','FontSize',16);
      print(gcf,'-dpsc2','-append',save_file);
      clear zlat zlon zfld
   end
   iplt=1;
   if (iplt==1)
      figure1=figure;
      ...
      zlon=zeros(nhalo,ny);
      zfld=zeros(nhalo,ny);
      zlat(1:nhalo,1:ny)=co_lat_map(2,1:nhalo,1:ny);
      zlon(1:nhalo,1:ny)=co_lon_map(2,1:nhalo,1:ny);
      zfld(1:nhalo,1:ny)=co_bdy_rel_sprd_map(2,1:nhalo,1:ny);
      zfld
      return
      
      contourf(zlon,zlat,zfld);
      colorbar('eastoutside')
      title('CO Rel Sprd CO XE','Fontsize',18,'FontWeight','bold');
      xlabel('Longitude (deg)','FontSize',16);
      ylabel('Latitude','FontSize',16);
      print(gcf,'-dpsc2','-append',save_file);
   end
...
return
   ...
   if (iplt==1)
      ...
      cmin=0;
      cmax=.3;
      contourf(co_lon_map(3,1:nx,1:nhalo),co_lat_map(3,1:nx,1:nhalo),co_bdy_rel_sprd_map(3,1:nx,1:nhalo));
      colorbar('eastoutside')
      title('CO Rel Sprd BC CO_YS','Fontsize',18,'FontWeight','bold');
      xlabel('Longitude (deg)','FontSize',16);
      ylabel('Latitude','FontSize',16);
      print(gcf,'-dpsc2','-append',save_file);
   end
   ...
   if (iplt==1)
      ...
      cmin=0;
      cmax=.3;
      contourf(co_lon_map(4,1:nx,1:nhalo),co_lat_map(4,1:nx,1:nhalo),co_bdy_rel_sprd_map(4,1:nx,1:nhalo));
      colorbar('eastoutside')
      title('CO Rel Sprd CO XE','Fontsize',18,'FontWeight','bold');
      xlabel('Longitude (deg)','FontSize',16);
      ylabel('Latitude','FontSize',16);
