function plot_maps_NOAA_spread_emis_DART_NASA
   clear all;
%
   DATE_STR_PR=["2011040203","2011040206","2011040209","2011040212","2011040215"];
   DATE_STR_PO=["2011040206","2011040209","2011040212","2011040215","2011040218"];
   ndates=5;
   wrf_vert=[1,2,3];
   levl=1;
   nx=440;
   ...
   nz=20;
   num_mem=30;
   rc=system('rm -rf NOAA_PRIOR_SPREAD_EMIS_DART.eps');
   save_file='NOAA_PRIOR_SPREAD_EMIS_DART.eps';
%
% set colormap
...
%
      ...
      for imem=1:num_mem
         ...
         exp_pr=strcat(path_data,strcat('/',DATE_STR_PR(idate),'/dart_filter/wrfchemi_d01_',file_date,'_prior',mem_chr));
         exp_po=strcat(path_data,strcat('/',DATE_STR_PR(idate),'/dart_filter/wrfchemi_d01_',file_date,mem_chr));
%
% CO PRIOR and POST FIELDS
         ...
         co_exp_po(:,:,imem)=squeeze(fld_full(:,:,wrf_vert(levl)));
%
% NO2 PRIOR and POST FIELDS
         ...
         clear fld_full
         fld_full=double(ncread(exp_po,state_var));
         no2_exp_po(:,:,imem)=squeeze(fld_full(:,:,wrf_vert(levl)));
%
% SO2 PRIOR and POST FIELDS
         ...
         clear fld_full
         fld_full=double(ncread(exp_po,state_var));
         so2_exp_po(:,:,imem)=squeeze(fld_full(:,:,wrf_vert(levl)));
      end
%
