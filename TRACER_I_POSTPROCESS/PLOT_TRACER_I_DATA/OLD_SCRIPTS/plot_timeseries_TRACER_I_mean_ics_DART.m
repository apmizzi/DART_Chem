function plot_timeseries_TRACER_I_mean_ics_DART
   clear all;
%
...
%
   ...
   nz=50;
   num_mem=30;
   rc=system('rm -rf TRACER_I_MEAN_IC_DART_timeseries.eps');
   save_file='TRACER_I_MEAN_IC_DART_timeseries.eps';
%
% set colormap
...
% Loop through dates
   for idate=1:ndates
      ...
      hno3_exp_pr=zeros(nx,ny,num_mem);
      ...
      hno3_exp_po=zeros(nx,ny,num_mem);
      for imem=1:num_mem
         ...
         exp_pr=strcat(path_data,strcat('/',DATE_STR_PR(idate),'/wrfchem_cycle_cr/',run_dir,'/wrfout_d01_',file_date_po));
         exp_po=strcat(path_data,strcat('/',DATE_STR_PO(idate),'/dart_filter/wrfout_d01_',file_date_po,'_filt',mem_chr));
         if (idate==1)
            exp_pr=strcat(path_data,strcat('/',DATE_STR_PR(idate),'/wrfchem_initial/',run_dir,'/wrfout_d01_',file_date_po));
            exp_po=strcat(path_data,strcat('/',DATE_STR_PO(idate),'/dart_filter/wrfout_d01_',file_date_po,'_filt',mem_chr));
         end
%
...
%
% HNO3 PRIOR and POST FIELDS
         fac=1.e3;
         state_var='hno3'; 
         clear fld_full
         fld_full=double(ncread(exp_pr,state_var));
         hno3_exp_pr(:,:,imem)=squeeze(fld_full(:,:,wrf_vert(levl)))*fac;%ppbv
         clear fld_full
         fld_full=double(ncread(exp_po,state_var));
         hno3_exp_po(:,:,imem)=squeeze(fld_full(:,:,wrf_vert(levl)))*fac;%ppbv
      end
%
...
%
% HNO3 ENS MEAN and VARIACNE FIELDS
      hno3_exp_pr_mean(:,:)=zeros(nx,ny);
      hno3_exp_pr_vari(:,:)=zeros(nx,ny);
      hno3_exp_po_mean(:,:)=zeros(nx,ny);
      hno3_exp_po_vari(:,:)=zeros(nx,ny);
%
      ...
      for i=1:nx
         for j=1:ny
            for imem=1:num_mem  
               ...
               hno3_exp_pr_mean(i,j)=hno3_exp_pr_mean(i,j)+hno3_exp_pr(i,j,imem)/num_mem;
%
               ...
               hno3_exp_po_mean(i,j)=hno3_exp_po_mean(i,j)+hno3_exp_po(i,j,imem)/num_mem;
            end
         end
      end 
%
      ...
      for i=1:nx
         for j=1:ny
            for imem=1:num_mem
               ...
               hno3_exp_pr_vari(i,j)=hno3_exp_pr_vari(i,j)+((hno3_exp_pr(i,j,imem)- ...
               hno3_exp_pr_mean(i,j))^2.)/(num_mem-1);
%
               ...
               hno3_exp_po_vari(i,j)=hno3_exp_po_vari(i,j)+((hno3_exp_po(i,j,imem)- ...
               hno3_exp_po_mean(i,j))^2.)/(num_mem-1);
            end
         end
      end
%
...
%
      for i=1:nx
         for j=1:ny
            ...
            hcho_exp_pr_gr_mean(idate)=hcho_exp_pr_gr_mean(idate)+hcho_exp_pr_mean(i,j)/(nx*ny);
            hno3_exp_pr_gr_mean(idate)=hno3_exp_pr_gr_mean(idate)+hno3_exp_pr_mean(i,j)/(nx*ny);
            co_exp_po_gr_mean(idate)=co_exp_po_gr_mean(idate)+co_exp_po_mean(i,j)/(nx*ny);
            o3_exp_po_gr_mean(idate)=o3_exp_po_gr_mean(idate)+o3_exp_po_mean(i,j)/(nx*ny);
            ...
            hcho_exp_po_gr_mean(idate)=hcho_exp_po_gr_mean(idate)+hcho_exp_po_mean(i,j)/(nx*ny);
            hno3_exp_po_gr_mean(idate)=hno3_exp_po_gr_mean(idate)+hno3_exp_po_mean(i,j)/(nx*ny);
%
	    co_exp_pr_gr_vari(idate)=co_exp_pr_gr_vari(idate)+co_exp_pr_vari(i,j)/(nx*ny);
            o3_exp_pr_gr_vari(idate)=o3_exp_pr_gr_vari(idate)+o3_exp_pr_vari(i,j)/(nx*ny);
            ...
            hcho_exp_pr_gr_vari(idate)=hcho_exp_pr_gr_vari(idate)+hcho_exp_pr_vari(i,j)/(nx*ny);
            hno3_exp_pr_gr_vari(idate)=hno3_exp_pr_gr_vari(idate)+hno3_exp_pr_vari(i,j)/(nx*ny);
            co_exp_po_gr_vari(idate)=co_exp_po_gr_vari(idate)+co_exp_po_vari(i,j)/(nx*ny);
            o3_exp_po_gr_vari(idate)=o3_exp_po_gr_vari(idate)+o3_exp_po_vari(i,j)/(nx*ny);
            ...
            hcho_exp_po_gr_vari(idate)=hcho_exp_po_gr_vari(idate)+hcho_exp_po_vari(i,j)/(nx*ny);
            hno3_exp_po_gr_vari(idate)=hno3_exp_po_gr_vari(idate)+hno3_exp_po_vari(i,j)/(nx*ny);
         end
      end 
%
      co_exp_pr_gr_stdv(idate)=sqrt(co_exp_pr_gr_vari(idate));
      o3_exp_pr_gr_stdv(idate)=sqrt(o3_exp_pr_gr_vari(idate));
      ...
      hno3_exp_pr_gr_stdv(idate)=sqrt(hno3_exp_pr_gr_vari(idate));
      co_exp_po_gr_stdv(idate)=sqrt(co_exp_po_gr_vari(idate));
      o3_exp_po_gr_stdv(idate)=sqrt(o3_exp_po_gr_vari(idate));
      ...
      hno3_exp_po_gr_stdv(idate)=sqrt(hno3_exp_po_gr_vari(idate));
   end
%
...
% O3
   ...
   'PRIOR','POST',save_file,ndates,ymx,ymn,o3_exp_pr_gr_mean,o3_exp_po_gr_mean);
% NO2
...
% HNO3
   ...
   'PRIOR','POST',save_file,ndates,ymx,ymn,hno3_exp_pr_gr_mean,hno3_exp_po_gr_mean);
%
...
% O3
   ...
   'PRIOR','POST',save_file,ndates,ymx,ymn,o3_exp_pr_gr_stdv,o3_exp_po_gr_stdv);
% NO2
...
% HNO3
   ...
   'PRIOR','POST',save_file,ndates,ymx,ymn,hno3_exp_pr_gr_stdv,hno3_exp_po_gr_stdv);
end   
%
