function plot_maps_histo_TRACER_I_spread_ics
   clear all;
%
   ...
   nz=50;
   num_mem=30;
   rc=system('rm -rf TRACER_I_SPREAD_IC.eps');
   save_file='TRACER_I_SPREAD_IC.eps';
%
% set colormap
...
%
      ...
      for imem=1:num_mem
         ...
         exp_pr=strcat(path_data,strcat('/',DATE_STR_PR(idate),'/wrfchem_chem_icbc/wrfinput_d01_',file_date,run_dir));
%         exp_pr=strcat(path_data,strcat('/',DATE_STR_PR(idate),'/wrfchem_chem_icbc/wrfinput_d01_',file_date,'_new',run_dir));
%         exp_pr=strcat(path_data,strcat('/',DATE_STR_PR(idate),'/wrfchem_chem_icbc/wrfinput_d01',run_dir))
%
% CO PRIOR FIELDS
         fac=1.e3;
         state_var='co'; 
...
%
% HNO3 ENS MEAN and VARIANCE FIELDS
      hno3_exp_mean(:,:)=zeros(nx,ny);
      hno3_exp_vari(:,:)=zeros(nx,ny);
%
      ...
      for i=1:nx
         for j=1:ny
            for imem=1:num_mem  
               ...
               hno3_exp_mean(i,j)=hno3_exp_mean(i,j)+hno3_exp_pr(i,j,imem)/num_mem;
            end
         end
      end 
%
      ...
      for i=1:nx
         for j=1:ny
            for imem=1:num_mem
               ...
               hno3_exp_vari(i,j)=hno3_exp_vari(i,j)+((hno3_exp_pr(i,j,imem)- ...
               hno3_exp_mean(i,j))^2.)/(num_mem-1);
            end
            ...
            hno3_exp_stdv(i,j)=sqrt(hno3_exp_vari(i,j));
         end
      end
...
% LOOK AT RESULTS
      ...
      for i=1:nx
         for j=1:ny
            if(hno3_exp_mean(i,j)~=0)
              sum1=sum1+1;
              rel_sprd_hno3_map(i,j)=hno3_exp_stdv(i,j)/hno3_exp_mean(i,j)*100.;
              relative_sprd_hno3(sum1)=hno3_exp_stdv(i,j)/hno3_exp_mean(i,j)*100.;
            end
         end
      end
...
% SO2 HISTOGRAM
      if(iplt==1)
         ...
         box(axes1,'on');
         hold(axes1,'all');
         title(strcat('SO2 Relative Sprd',{' '},SPRD_FAC_LBL),'Fontsize',18,'FontWeight','bold');
         xlabel('Rel Sprd (%)','FontSize',16);
         xtickformat('%5.2f')
         ylabel('Probability','FontSize',16);
         ytickformat('%5.2f')
...
% HNO3 HISTOGRAM
      if(iplt==1)
         ...
         box(axes1,'on');
         hold(axes1,'all');
         title(strcat('HNO3 Relative Sprd',{' '},SPRD_FAC_LBL),'Fontsize',18,'FontWeight','bold');
         xlabel('Rel Sprd (%)','FontSize',16);
         xtickformat('%5.2f')
         ylabel('Probability','FontSize',16);
         ytickformat('%5.2f')
         histogram(relative_sprd_hno3,20,'Normalization','probability')
         print(gcf,'-dpsc2','-append',save_file);
      end
%
% CO MAPS
...
% SO2 MAPS
      ...
      if (iplt==1)
         ...
         colorbar('eastoutside')
         title(strcat('SO2 Rel Sprd',{' '},SPRD_FAC_LBL),'Fontsize',18,'FontWeight','bold');
         xlabel('Longitude (deg)','FontSize',16);
         ylabel('Latitude','FontSize',16);
         print(gcf,'-dpsc2','-append',save_file);
      end
...
% HNO3 MAPS
      ...
      if (iplt==1)
         ...
         cmin=0;
         cmax=.3;
         contourf(xlon,xlat,rel_sprd_hno3_map);
         colorbar('eastoutside')
         title(strcat('HNO3 Rel Sprd',{' '},SPRD_FAC_LBL),'Fontsize',18,'FontWeight','bold');
         xlabel('Longitude (deg)','FontSize',16);
         ylabel('Latitude','FontSize',16);
         print(gcf,'-dpsc2','-append',save_file);
      end
