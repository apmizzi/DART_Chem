  program main
      implicit none
      integer,parameter                    :: nvars=18
      integer                              :: nx,ny,nz_fire,ivar,unit
      integer                              :: i,j,k
...
! 
! File size wrfout (un-thinned and compressed):  876 K
! File size wrfout (thinned and compressed):     128 K 
!
      namelist/wrffirechemi_thinning_nml/path_input,path_output,file_input,file_output
!
      nx=440
      ny=284
      nz_fire=1
!
      arc_vars=['XLONG','XLAT','ebu_in_co','ebu_in_no','ebu_in_no2','ebu_in_so2','ebu_in_hcho','ebu_in_ch4','ebu_in_eth','ebu_in_hc3', &
      'ebu_in_hc5','ebu_in_hc8','ebu_in_ket','ebu_in_oli','ebu_in_olt','ebu_in_iso','ebu_in_pm25','ebu_in_pm10']
!
      arc_rank=[2,2,3,3,3,3,3,3,3,3,   3,3,3,3,3,3,3,3]
!
      ...
      'CH4 hourly emissions: mole km^-2 hr^-1',&
      ...
      'OLI hourly emissions: mole km^-2 hr^-1',&
      'OLT hourly emissions: mole km^-2 hr^-1',&
      'ISO hourly emissions: mole km^-2 hr^-1',&
      'PM25 hourly emissions: mole km^-2 hr^-1',&
      'PM10 hourly emissions: mole km^-2 hr^-1']
!      
      allocate(fld2d(nx,ny))
      allocate(fld3d(nx,ny,nz_fire))
!
! Read namelist data
      unit=20
      open(unit=unit,file="wrffirechemi_thinning_nml.nl",form="formatted", &
      status="old",action="read")
      rewind(unit)
      read(unit,wrffirechemi_thinning_nml)
      close(unit)
...
! Create thinned wrfchemi file
      ...
      call create_NETCDF_file(trim(write_file),nx,ny,nz_fire)
...
!
      do ivar=3,nvars
         ...
         elseif(arc_rank(ivar).eq.3) then
            call get_WRFCHEM_fld_real(trim(read_file),trim(arc_vars(ivar)),fld3d,nx,ny,nz_fire,1)
            call put_NETCDF_fld_real(trim(write_file),trim(arc_vars(ivar)),trim(arc_units_coms(ivar)), &
            fld3d,nx,ny,nz_fire)
            cycle     
         endif
...
!-------------------------------------------------------------------------------
   ...
   subroutine create_NETCDF_file(wrfchem_thin_file,nx,ny,nz_fire)
      use :: netcdf
      implicit none
!      
      integer                    :: nx,ny,nz_fire,rc,fid
      integer                    :: var_id,att_id,nx_dimid,ny_dimid,nz_fire_dimid
...
! Define dimensions
      ...
      rc=nf90_def_dim(fid,"bottom-top",nz_fire,nz_fire_dimid)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at nz_fire_dimid')
!
! Define variables
      ...
      rc=nf90_def_var(fid,"ebu_in_co",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_co')

      rc=nf90_def_var(fid,"ebu_in_no",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_no')

      rc=nf90_def_var(fid,"ebu_in_no2",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_no2')

      rc=nf90_def_var(fid,"ebu_in_so2",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_so2')

      rc=nf90_def_var(fid,"ebu_in_hcho",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_hcho')

      rc=nf90_def_var(fid,"ebu_in_ch4",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_ch4')

      rc=nf90_def_var(fid,"ebu_in_eth",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_eth')

      rc=nf90_def_var(fid,"ebu_in_hc3",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_hc3')

      rc=nf90_def_var(fid,"ebu_in_hc5",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_hc5')

      rc=nf90_def_var(fid,"ebu_in_hc8",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_hc8')

      rc=nf90_def_var(fid,"ebu_in_ket",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_ket')

      rc=nf90_def_var(fid,"ebu_in_oli",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_oli')

      rc=nf90_def_var(fid,"ebu_in_olt",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_olt')

      rc=nf90_def_var(fid,"ebu_in_iso",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_iso')

      rc=nf90_def_var(fid,"ebu_in_pm25",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_pm25')

      rc=nf90_def_var(fid,"ebu_in_pm10",NF90_REAL,(/nx_dimid,ny_dimid,nz_fire_dimid/),var_id)
      if(rc /= NF90_NOERR) call handle_err(rc,'APM: at ebu_in_pm10')
