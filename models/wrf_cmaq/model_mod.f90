...
! Assimilation interface for CMAQ model
!> \defgroup cmaq model_mod
!> Model mod
!> 
!> Model mod for CMAQ
!> @{
...
!
! APM +++
use      obs_kind_mod, only :   QTY_CO, QTY_O3, QTY_NO, QTY_NO2, QTY_SO2, QTY_SO4, &
                                QTY_N2O5, QTY_ACET, QTY_TOL, QTY_ISOPR, QTY_HCHO, &
                                QTY_HNO3, QTY_NH3, QTY_PAN, QTY_CH4
! APM ---
...
!
! APM +++
                                get_varid_from_kind, get_io_clamping_minval                              
! APM ---
...
! types
public :: cmaq_dom, cmaq_static_data_for_dart
...
character(len=*), parameter :: source   = 'wrf_cmaq/model_mod.f90'
...
namelist /model_nml/ num_moist_vars, &
                     ...
                     default_state_variables, cmaq_state_variables, &
                     cmaq_state_bounds, sfc_elev_max_diff, &
...
character(len = 20) :: cmaq_nml_file = 'namelist.input'
logical :: have_cmaq_nml_file = .false.
...
! Private definition of domain map projection use by CMAQ
...
!APM: map_latlon changed to the CMAQ mappings
integer, parameter :: map_latlon = 1, map_lambert = 2, map_polar_stereo = 4, map_mercator = 3
...
TYPE cmaq_static_data_for_dart

   integer  :: bt, bts, bt1, sn, sns, we, wes, sls
   ...
   !   section of a standard CMAQ "namelist.input" file), but for now we have included them
...
type cmaq_dom
   type(cmaq_static_data_for_dart), pointer :: dom(:)
...
if (adv_mod_command /= '') then
   msgstring2 = "Set the model advance command in the &dart_to_cmaq_nml namelist"
   call error_handler(E_MSG, 'static_init_model:', &
         "WARNING: adv_mod_command ignored in &model_mod namelist", &
          text2=msgstring2)
endif

allocate(cmaq%dom(num_domains))
allocate(domain_id(num_domains))

! get default state variable table if asked
! APM: currently no meteorology variables are constrained in CMAQ
if ( default_state_variables ) then
  cmaq_state_variables = 'NULL'
  call fill_default_state_table(cmaq_state_variables)
  msgstring2 = 'Set "default_state_variables" to .false. in the namelist'
  msgstring3 = 'to use the "cmaq_state_variables" list instead.'
  call error_handler(E_MSG, 'static_init_model:', &
...
!
! Consolidate all the input variable tables into one 'cmaq_state_variables' table.
...
!if ( debug ) then
!  print *,'CMAQ state vector table'
!  print *,'default_state_variables = ',default_state_variables
!  print *,'cmaq_state_variables ',cmaq_state_variables
!  print *,'conv_state_variables ',conv_state_variables
!  print *,'cmaq_state_bounds ',conv_state_variables
!endif
...
if (vert_localization_coord == VERTISLEVEL) then
   cmaq%dom(:)%localization_coord = VERTISLEVEL
elseif (vert_localization_coord == VERTISPRESSURE) then
   cmaq%dom(:)%localization_coord = VERTISPRESSURE
elseif (vert_localization_coord == VERTISHEIGHT) then
   cmaq%dom(:)%localization_coord = VERTISHEIGHT
elseif (vert_localization_coord == VERTISSCALEHEIGHT) then
   cmaq%dom(:)%localization_coord = VERTISSCALEHEIGHT
else
...
! timestep, which is part of the cmaq input netcdf file.
! call read_dt_from_wrf_nml()

dart_index = 1

CMAQDomains : do id=1,num_domains
...
! allocation for cmaq variable types 
   allocate(cmaq%dom(id)%var_type(cmaq%dom(id)%number_of_cmaq_variables))

! allocation for update/nocopyback/noupdate
   allocate(cmaq%dom(id)%var_update_list(cmaq%dom(id)%number_of_cmaq_variables))
   cmaq%dom(id)%var_update_list = var_update_list(1:cmaq%dom(id)%number_of_cmaq_variables)

! allocation for dart kinds
   allocate(cmaq%dom(id)%dart_kind(cmaq%dom(id)%number_of_cmaq_variables))

! allocation of var size 
   allocate(cmaq%dom(id)%var_size(3,cmaq%dom(id)%number_of_cmaq_variables))

! allocation for cmaq variable metadata
...
!      call error_handler(E_MSG, 'static_init_model: ', errstring)
      ...
      if ( debug ) then
         ...
         trim(cmaq_state_variables(1,my_index)), &
         ' are ',cmaq%dom(id)%lower_bound(ind), &
         cmaq%dom(id)%upper_bound(ind), &
         cmaq%dom(id)%clamp_or_fail(ind)
      endif
...
! start and stop of each variable in vector
   allocate(cmaq%dom(id)%var_index(2,cmaq%dom(id)%number_of_cmaq_variables))
...
! problems that it might cause.
   ...
   do ind = 1,cmaq%dom(id)%number_of_cmaq_variables
      ...
      if ( debug ) then
         write(*,*) 'Assigning dart vector indices for var_type ',cmaq%dom(id)%var_type(ind)
      ...
      do k=1,cmaq%dom(id)%var_size(3,ind)
         do j=1,cmaq%dom(id)%var_size(2,ind)
            do i=1,cmaq%dom(id)%var_size(1,ind)
               ! HK cmaq%dom(id)%dart_ind(i,j,k,ind) &
               !                               = dart_index
   ...
   cmaq%dom(id)%type_u      = get_type_ind_from_type_string(id,'U')
   cmaq%dom(id)%type_v      = get_type_ind_from_type_string(id,'V')
   cmaq%dom(id)%type_w      = get_type_ind_from_type_string(id,'W')
   cmaq%dom(id)%type_t      = get_type_ind_from_type_string(id,'T')
   cmaq%dom(id)%type_gz     = get_type_ind_from_type_string(id,'PH')
   cmaq%dom(id)%type_qv     = get_type_ind_from_type_string(id,'QVAPOR')
   cmaq%dom(id)%type_qr     = get_type_ind_from_type_string(id,'QRAIN')
   cmaq%dom(id)%type_qc     = get_type_ind_from_type_string(id,'QCLOUD')
   cmaq%dom(id)%type_qg     = get_type_ind_from_type_string(id,'QGRAUP')
   cmaq%dom(id)%type_qh     = get_type_ind_from_type_string(id,'QHAIL')
   cmaq%dom(id)%type_qi     = get_type_ind_from_type_string(id,'QICE')
   cmaq%dom(id)%type_qs     = get_type_ind_from_type_string(id,'QSNOW')
   cmaq%dom(id)%type_cldfra = get_type_ind_from_type_string(id,'CLDFRA')
   cmaq%dom(id)%type_qgvol  = get_type_ind_from_type_string(id,'QVGRAUPEL')
   cmaq%dom(id)%type_qhvol  = get_type_ind_from_type_string(id,'QVHAIL')
   cmaq%dom(id)%type_qnice  = get_type_ind_from_type_string(id,'QNICE')
   cmaq%dom(id)%type_qndrp  = get_type_ind_from_type_string(id,'QNDRP')
   cmaq%dom(id)%type_qnsnow = get_type_ind_from_type_string(id,'QNSNOW')
   cmaq%dom(id)%type_qnrain = get_type_ind_from_type_string(id,'QNRAIN')
   cmaq%dom(id)%type_qngraupel = get_type_ind_from_type_string(id,'QNGRAUPEL')
   cmaq%dom(id)%type_qnhail = get_type_ind_from_type_string(id,'QNHAIL')
   cmaq%dom(id)%type_u10    = get_type_ind_from_type_string(id,'U10')
   cmaq%dom(id)%type_v10    = get_type_ind_from_type_string(id,'V10')
   cmaq%dom(id)%type_t2     = get_type_ind_from_type_string(id,'T2')
   cmaq%dom(id)%type_th2    = get_type_ind_from_type_string(id,'TH2')
   cmaq%dom(id)%type_q2     = get_type_ind_from_type_string(id,'Q2')
   cmaq%dom(id)%type_ps     = get_type_ind_from_type_string(id,'PSFC')
   cmaq%dom(id)%type_mu     = get_type_ind_from_type_string(id,'MU')
   cmaq%dom(id)%type_tsk    = get_type_ind_from_type_string(id,'TSK')
   cmaq%dom(id)%type_2dflash = get_type_ind_from_type_string(id,'FLASH_RATE_2D')
   cmaq%dom(id)%type_tslb   = get_type_ind_from_type_string(id,'TSLB')
   cmaq%dom(id)%type_smois  = get_type_ind_from_type_string(id,'SMOIS')
   cmaq%dom(id)%type_sh2o   = get_type_ind_from_type_string(id,'SH2O')
   cmaq%dom(id)%type_refl   = get_type_ind_from_type_string(id,'REFL_10CM')
...
!
! APM +++
   cmaq%dom(id)%type_gz     = get_type_ind_from_type_string(id,'ZF')
   cmaq%dom(id)%type_t      = get_type_ind_from_type_string(id,'TA')
   cmaq%dom(id)%type_qv     = get_type_ind_from_type_string(id,'QV')
   cmaq%dom(id)%type_mu     = get_type_ind_from_type_string(id,'PRES')
   cmaq%dom(id)%type_co     = get_type_ind_from_type_string(id,'CO')
   cmaq%dom(id)%type_o3     = get_type_ind_from_type_string(id,'O3')
   cmaq%dom(id)%type_no     = get_type_ind_from_type_string(id,'NO')
...
! add domain - not doing anything with domain_id yet so just overwriting it
   domain_id(id) = add_domain( 'cmaqinput_d0'//idom, &
                           ...
                           kind_list   = cmaq%dom(id)%dart_kind, &
                           clamp_vals  = var_bounds_table(1:cmaq%dom(id)%number_of_cmaq_variables,:), &
                           update_list = var_update_list(1:cmaq%dom(id)%number_of_cmaq_variables) )
...
!#######################################################################
!> Convert from the state structure id to the cmaq domain number.
!> These are the same if there is only CMAQ involved in the assimilation
!> The state structure id may be different if CMAQ is coupled with another
!> model.
function get_cmaq_domain(state_id)

integer, intent(in) :: state_id
integer :: get_cmaq_domain
...
call error_handler(E_ERR, 'get_cmaq_domain', 'not a valid domain')

end function get_cmaq_domain
...
get_model_size = cmaq%model_size
...
function get_cmaq_static_data(dom_num)

integer, intent(in)  :: dom_num

type(cmaq_static_data_for_dart) :: get_cmaq_static_data

get_cmaq_static_data = cmaq%dom(dom_num)

return
end function get_cmaq_static_data

!#######################################################################

subroutine get_cmaq_state_variables(state_var)

character(len=129), intent(out) :: state_var(num_state_table_columns,max_state_variables)

state_var = cmaq_state_variables

end subroutine get_cmaq_state_variables
...
! any of the dart code, which can return the cmaq domain number.
...
var_type  = cmaq%dom(id)%var_type(var_id)
dart_type = cmaq%dom(id)%dart_kind(var_id)

! first obtain lat/lon from (ip,jp)
call get_cmaq_horizontal_location( ip, jp, var_type, id, lon, lat )

! return staggered levels for vertical types?  check this later.
if( (var_type == cmaq%dom(id)%type_w ) .or. (var_type == cmaq%dom(id)%type_gz) ) then
  lev = real(kp) - 0.5_r8
else
  lev = real(kp)
endif
...
! This is the main forward operator subroutine for CMAQ.
...
! modified 22 October 2007 to accomodate global CMAQ (3.0)
...
! JPH --- scm is only defined for d1
   ...
   if ( cmaq%dom(id)%polar .and. restrict_polar ) then
      if ( yloc < 1.0_r8 .or. yloc >= real(cmaq%dom(id)%sn,r8) ) then
   ...
   if(debug) then
      ...
      write(*,*) cmaq%dom(id)%latitude(i,j),cmaq%dom(id)%latitude(i+1,j),  &
           cmaq%dom(id)%latitude(i,j+1), &
           cmaq%dom(id)%latitude(i+1,j+1)
      write(*,*) ' corners of long '
      write(*,*) cmaq%dom(id)%longitude(i,j),cmaq%dom(id)%longitude(i+1,j),  &
           cmaq%dom(id)%longitude(i,j+1), &
           cmaq%dom(id)%longitude(i+1,j+1)
   endif
   ...
   allocate(v_h(0:cmaq%dom(id)%bt, ens_size), v_p(0:cmaq%dom(id)%bt, ens_size))
   ...
   elseif(is_vertical(location,"PRESSURE")) then
      ...
      call get_model_pressure_profile_distrib(i,j,dx,dy,dxm,dym,cmaq%dom(id)%bt,id,v_p,state_handle, ens_size)
...
!      return
      ...
      call pres_to_zk_distrib(xyz_loc(3), v_p, cmaq%dom(id)%bt, ens_size, zloc,is_lev0)
   ...
   elseif(is_vertical(location,"HEIGHT")) then
      ...
      call get_model_height_profile_distrib(i,j,dx,dy,dxm,dym,cmaq%dom(id)%bt,id,v_h, state_handle, ens_size)
      ! get height vertical co-ordinate
      do e = 1, ens_size ! HK should there be a height_to_zk_distrib?
         call height_to_zk(xyz_loc(3), v_h(:, e), cmaq%dom(id)%bt,zloc(e),is_lev0(e))
   ...
   elseif(is_vertical(location,"SURFACE") .or. obs_kind == QTY_SURFACE_ELEVATION) then
      ...
      ! Elevation check function drawn from Ryan's cmaq preprocessing code. 
...
!
      ! Check to make sure retrieved integer gridpoints are in valid range
         if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .and. &
              boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_t ) ) then

            call getCorners(i, j, id, cmaq%dom(id)%type_t, ll, ul, lr, ur, rc )
            if ( rc .ne. 0 ) &
            ...
            mod_sfc_elevation = dym*( dxm*cmaq%dom(id)%hgt(ll(1), ll(2)) + &
                                       dx*cmaq%dom(id)%hgt(lr(1), lr(2)) ) + &
                                 dy*( dxm*cmaq%dom(id)%hgt(ul(1), ul(2)) + &
                                       dx*cmaq%dom(id)%hgt(ur(1), ur(2)) )
         endif
   ...
   if (obs_kind == QTY_RAINWATER_MIXING_RATIO .or. & 
       ...
       call simple_interp_distrib(fld, cmaq, id, i, j, k, obs_kind, dxm, dx, dy, dym, uniquek, ens_size, state_handle)
   ...
   elseif( obs_kind == QTY_U_WIND_COMPONENT .or. obs_kind == QTY_V_WIND_COMPONENT) then   ! U, V
     ...
     ! This is for 3D wind fields -- surface winds later
      if(.not. surf_var) then
         ...
         if ( ( cmaq%dom(id)%type_u >= 0 ) .and. ( cmaq%dom(id)%type_v >= 0 ) ) then
            ...
            if ( cmaq%dom(id)%periodic_x .and. xloc_u > real(cmaq%dom(id)%wes,r8) ) &
                 xloc_u = xloc_u - real(cmaq%dom(id)%we,r8)
            ...
            do uk = 1, count ! for the different ks
               ...
               ! Check to make sure retrieved integer gridpoints are in valid range
               if ( boundsCheck( i_u, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_u) .and. &
                    boundsCheck( i,   cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_v) .and. &
                    boundsCheck( j,   cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_u) .and. &
                    boundsCheck( j_v, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_v) .and. &
                    boundsCheck( uniquek(uk),   .false.,                id, dim=3, type=cmaq%dom(id)%type_u) ) then
                  ...
                  call getCorners(i_u, j, id, cmaq%dom(id)%type_u, ll, ul, lr, ur, rc )
                  if ( rc .ne. 0 ) &
                       print*, 'model_mod.f90 :: model_interpolate :: getCorners U rc = ', rc
               
                  call getCorners(i, j_v, id, cmaq%dom(id)%type_v, ll_v, ul_v, lr_v, ur_v, rc )
                  if ( rc .ne. 0 ) &
                  ...
                  do k2 = 1, 2
                     ...
                     ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+k2-1, domain_id(id), cmaq%dom(id)%type_u)
                     iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+k2-1, domain_id(id), cmaq%dom(id)%type_u)
                     ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+k2-1, domain_id(id), cmaq%dom(id)%type_u)
                     iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+k2-1, domain_id(id), cmaq%dom(id)%type_u)
                     ...
                     ill = get_dart_vector_index(ll_v(1), ll_v(2), uniquek(uk)+k2-1, domain_id(id), cmaq%dom(id)%type_v)
                     iul = get_dart_vector_index(ul_v(1), ul_v(2), uniquek(uk)+k2-1, domain_id(id), cmaq%dom(id)%type_v)
                     ilr = get_dart_vector_index(lr_v(1), lr_v(2), uniquek(uk)+k2-1, domain_id(id), cmaq%dom(id)%type_v)
                     iur = get_dart_vector_index(ur_v(1), ur_v(2), uniquek(uk)+k2-1, domain_id(id), cmaq%dom(id)%type_v)
                     ...
                     vgrid = dym_v*( dxm*x_ill + dx*x_ilr ) + dy_v*( dxm*x_iul + dx*x_iur )
                        ...
                        do e = 1, ens_size
                           ...
                           if ( k(e) == uniquek(uk) ) then ! interpolate only if is the correct k
                              ...
                              call gridwind_to_truewind(xyz_loc(1), cmaq%dom(id)%proj, ugrid(e), vgrid(e), &
                                utrue(e), vtrue(e))
      ...
      !  only fld(1) will be filled.
         ...
         if ( ( cmaq%dom(id)%type_u10 >= 0 ) .and. ( cmaq%dom(id)%type_v10 >= 0 ) ) then

   ! JPH -- should test this for doubly periodic
   ! JPH -- does not pass for SCM config, so just do it below
            ! Check to make sure retrieved integer gridpoints are in valid range
            if ( ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .and. &
                   boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_t ) )     &
                   .or. cmaq%dom(id)%scm ) then
               ...
               ill = get_dart_vector_index(ll(1), ll(2), 1, domain_id(id), cmaq%dom(id)%type_u10)
               iul = get_dart_vector_index(ul(1), ul(2), 1, domain_id(id), cmaq%dom(id)%type_u10)
               ilr = get_dart_vector_index(lr(1), lr(2), 1, domain_id(id), cmaq%dom(id)%type_u10)
               iur = get_dart_vector_index(ur(1), ur(2), 1, domain_id(id), cmaq%dom(id)%type_u10)
               ...
               ill = get_dart_vector_index(ll(1), ll(2), 1, domain_id(id), cmaq%dom(id)%type_v10)
               iul = get_dart_vector_index(ul(1), ul(2), 1, domain_id(id), cmaq%dom(id)%type_v10)
               ilr = get_dart_vector_index(lr(1), lr(2), 1, domain_id(id), cmaq%dom(id)%type_v10)
               iur = get_dart_vector_index(ur(1), ur(2), 1, domain_id(id), cmaq%dom(id)%type_v10)
   ...
   elseif ( obs_kind == QTY_TEMPERATURE ) then
      ...
      if ( cmaq%dom(id)%type_t >= 0 ) then

         do uk = 1, count ! for the different ks

            ! Check to make sure retrieved integer gridpoints are in valid range
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .and. &
                 boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_t ) .and. &
                 boundsCheck( uniquek(uk), .false.,                id, dim=3, type=cmaq%dom(id)%type_t ) ) then
               ...
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id), cmaq%dom(id)%type_t)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id), cmaq%dom(id)%type_t)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id), cmaq%dom(id)%type_t)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id), cmaq%dom(id)%type_t)
               ...
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id), cmaq%dom(id)%type_t)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id), cmaq%dom(id)%type_t)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id), cmaq%dom(id)%type_t)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id), cmaq%dom(id)%type_t)
   ...
   elseif (obs_kind == QTY_2M_TEMPERATURE) then
      ...
      if ( cmaq%dom(id)%type_t2 >= 0 ) then ! HK is there a better way to do this?
         call surface_interp_distrib(fld, cmaq, id, i, j, obs_kind, cmaq%dom(id)%type_t2, dxm, dx, dy, dym, ens_size, state_handle)
         if (all(fld == missing_r8)) goto 200
      else
         fld = missing_r8
      endif
   ...
   elseif ( obs_kind == QTY_POTENTIAL_TEMPERATURE ) then
      ...
      ! This is for surface potential temperature (TH2)
         ...
         if ( cmaq%dom(id)%type_th2 >= 0 ) then

            call surface_interp_distrib(fld, cmaq, id, i, j, obs_kind, cmaq%dom(id)%type_th2, dxm, dx, dy, dym, ens_size, state_handle)
            if (all(fld == missing_r8)) goto 200
   ...
   elseif (obs_kind == QTY_DENSITY) then
      ...
      if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .and. &
         ...
         !   is not a reference to cmaq%dom(id)%dart_ind -- we'll have to go right from
         !   the corner indices
         ...
         if ( cmaq%dom(id)%type_qv >= 0 ) then
            ...
            UNIQUEK_LOOP: do uk = 1, count ! for the different ks
               ...
               if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .and. &
                  ...
                  call getCorners(i, j, id, cmaq%dom(id)%type_t, ll, ul, lr, ur, rc ) ! HK why is this type_t
                  if ( rc .ne. 0 ) &
                       print*, 'model_mod.f90 :: model_interpolate :: getCorners SH rc = ', rc

                  ! Interpolation for SH field at level k
                  ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id), cmaq%dom(id)%type_qv)
                  iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id), cmaq%dom(id)%type_qv)
                  ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id), cmaq%dom(id)%type_qv)
                  iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id), cmaq%dom(id)%type_qv)
                  ...
                  ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id), cmaq%dom(id)%type_qv)
                  iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id), cmaq%dom(id)%type_qv)
                  ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id), cmaq%dom(id)%type_qv)
                  iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id), cmaq%dom(id)%type_qv)
      ...
      ! This is for surface specific humidity (calculated from Q2)
         ...
         if ( cmaq%dom(id)%type_q2 >= 0 ) then
            ...
            if ( ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .and. &
               ...
               ill = get_dart_vector_index(ll(1), ll(2), 1, domain_id(id), cmaq%dom(id)%type_q2)
               iul = get_dart_vector_index(ul(1), ul(2), 1, domain_id(id), cmaq%dom(id)%type_q2)
               ilr = get_dart_vector_index(lr(1), lr(2), 1, domain_id(id), cmaq%dom(id)%type_q2)
               iur = get_dart_vector_index(ur(1), ur(2), 1, domain_id(id), cmaq%dom(id)%type_q2)
   ...
   ! 1.g Vapor Mixing Ratio (QV, Q2)  
      ...
      if(.not. surf_var) then
         ...
         if ( cmaq%dom(id)%type_q2 >= 0 ) then
            ...
            call surface_interp_distrib(fld, cmaq, id, i, j, obs_kind, cmaq%dom(id)%type_q2, dxm, dx, dy, dym, ens_size, state_handle)
            if (all(fld == missing_r8)) goto 200
         endif
      endif
      ...
      !  This is for surface pressure (PSFC)
         ...
         if ( cmaq%dom(id)%type_ps >= 0 ) then
            ...
            if ( ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .and. &
               ...
               ill = get_dart_vector_index(ll(1), ll(2), 1, domain_id(id), cmaq%dom(id)%type_ps)
               iul = get_dart_vector_index(ul(1), ul(2), 1, domain_id(id), cmaq%dom(id)%type_ps)
               ilr = get_dart_vector_index(lr(1), lr(2), 1, domain_id(id), cmaq%dom(id)%type_ps)
               iur = get_dart_vector_index(ur(1), ur(2), 1, domain_id(id), cmaq%dom(id)%type_ps)
               ...
               do e = 1, ens_size
                  ...
                  elseif ( cmaq%dom(id)%scm ) then !HK is this redunant? What happens if you fail both conditions.
                     fld(1, e) = x_ill(e)
                  endif
               enddo
            endif
   ...
   ! 1.u Vortex Center Stuff from Yongsheng
      ...
      if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .and. &
         ...
         center_search_half_size = nint(center_search_half_length/cmaq%dom(id)%dx)
         if ( cmaq%dom(id)%periodic_x ) then
            ...
            center_track_xmax = min(cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_mu),i+center_search_half_size)
         endif
         if ( cmaq%dom(id)%periodic_y ) then
            ...
            center_track_ymax = min(cmaq%dom(id)%var_size(2,cmaq%dom(id)%type_mu),j+center_search_half_size)
         endif
         if ( center_track_xmin<1 .or. center_track_xmax>cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_mu) .or. &
              center_track_ymin<1 .or. center_track_ymax>cmaq%dom(id)%var_size(2,cmaq%dom(id)%type_mu) .or. &
         ...
         if ( (obs_kind == QTY_VORTEX_LAT .or. obs_kind == QTY_VORTEX_LON) .and. (.not. use_old_vortex) ) then
            ...
            circ_half_size   = nint(circulation_radius/cmaq%dom(id)%dx)
            circ_half_length = circulation_radius / cmaq%dom(id)%dx
            ...
            if ( cmaq%dom(id)%periodic_x ) then
               ...
               circ_xmax = min(cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_mu),i+center_search_half_size+circ_half_size)
            endif
            ...
            if ( cmaq%dom(id)%periodic_y ) then
               ...
               circ_ymax = min(cmaq%dom(id)%var_size(2,cmaq%dom(id)%type_mu),j+center_search_half_size+circ_half_size)
            endif
            ...
            allocate(z1d(0:cmaq%dom(id)%bt, ens_size))

            do i1 = circ_xmin-1, circ_xmax+1

               ii1 = i1
               if ( cmaq%dom(id)%periodic_x ) then
                  if ( i1 > cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_mu) ) then
                     ii1 = i1 - cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_mu)
                  elseif ( i1 < 1 ) then
                     ii1 = i1 + cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_mu)
                  endif
               else
                  if ( i1 > cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_mu) ) then
                     ii1 = cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_mu)
                  elseif ( i1 < 1 ) then
                     ii1 = 1
                  endif
               endif
               ...
               do i2 = circ_ymin-1, circ_ymax+1
                  ...
                  ii2 = i2
                  if ( cmaq%dom(id)%periodic_y ) then
                     if ( i2 > cmaq%dom(id)%var_size(2,cmaq%dom(id)%type_mu) ) then
                        ii2 = i2 - cmaq%dom(id)%var_size(2,cmaq%dom(id)%type_mu)
                     elseif ( i2 < 1 ) then
                        ii2 = i2 + cmaq%dom(id)%var_size(2,cmaq%dom(id)%type_mu)
                     endif
                  else
                  ...
                  do k2 = 1, cmaq%dom(id)%var_size(3,cmaq%dom(id)%type_t)
                     !z1d(k2) = model_pressure_t(i1,i2,k2,id,x)
                     z1d(k2, :) = model_pressure_t_distrib(i1,i2,k2,id, state_handle, ens_size)
                  enddo
                  z1d(0, :) = z1d(1, :)
                  !call pres_to_zk(circulation_pres_level, z1d, cmaq%dom(id)%bt, zloc, is_lev0)
                  call pres_to_zk_distrib(circulation_pres_level, z1d, cmaq%dom(id)%bt, ens_size, zloc, is_lev0)
...
!                     ugrid = (dx  * (x(cmaq%dom(id)%dart_ind(ii1,  ii2,  k2,  cmaq%dom(id)%type_u))  + &
!                                     x(cmaq%dom(id)%dart_ind(ii1+1,ii2,  k2,  cmaq%dom(id)%type_u))) + &
!                              dxm * (x(cmaq%dom(id)%dart_ind(ii1,  ii2,  k2+1,cmaq%dom(id)%type_u))  + &
!                                     x(cmaq%dom(id)%dart_ind(ii1+1,ii2,  k2+1,cmaq%dom(id)%type_u)))) * 0.5_r8

                        ugrid_1 = get_dart_vector_index(ii1,  ii2,  k2,   domain_id(id), cmaq%dom(id)%type_u)
                        ugrid_2 = get_dart_vector_index(ii1+1,ii2,  k2,   domain_id(id), cmaq%dom(id)%type_u)
                        ugrid_3 = get_dart_vector_index(ii1,  ii2,  k2,   domain_id(id), cmaq%dom(id)%type_u)
                        ugrid_4 = get_dart_vector_index(ii1,  ii2,  k2+1, domain_id(id), cmaq%dom(id)%type_u )
                        ...
                        vgrid_1 = get_dart_vector_index(ii1,  ii2,  k2,   domain_id(id),cmaq%dom(id)%type_v)
                        vgrid_2 = get_dart_vector_index(ii1,  ii2+1,k2,   domain_id(id),cmaq%dom(id)%type_v)
                        vgrid_3 = get_dart_vector_index(ii1,  ii2,  k2+1, domain_id(id),cmaq%dom(id)%type_v)
                        vgrid_4 = get_dart_vector_index(ii1,  ii2+1,k2+1, domain_id(id),cmaq%dom(id)%type_v)
...
!                     vgrid = (dx  * (x(cmaq%dom(id)%dart_ind(ii1,  ii2,  k2,  cmaq%dom(id)%type_v))  + &
!                                     x(cmaq%dom(id)%dart_ind(ii1,  ii2+1,k2,  cmaq%dom(id)%type_v))) + &
!                              dxm * (x(cmaq%dom(id)%dart_ind(ii1,  ii2,  k2+1,cmaq%dom(id)%type_v))  + &
!                                     x(cmaq%dom(id)%dart_ind(ii1,  ii2+1,k2+1,cmaq%dom(id)%type_v)))) * 0.5_r8

                        vgrid = (dx  * (vgrid_1 + vgrid_2) + dxm * (vgrid_3 + vgrid_4)) * 0.5_r8

                     else  !  pressure level below ground.  Take model level 1 winds

!                     ugrid = (x(cmaq%dom(id)%dart_ind(ii1,  ii2,  1,cmaq%dom(id)%type_u)) + &
!                              x(cmaq%dom(id)%dart_ind(ii1+1,ii2,  1,cmaq%dom(id)%type_u))) * 0.5_r8

                        ugrid_1 = get_dart_vector_index(ii1,  ii2,  1, domain_id(id),cmaq%dom(id)%type_u)
                        ugrid_2 = get_dart_vector_index(ii1+1,ii2,  1, domain_id(id),cmaq%dom(id)%type_u)
...
!                     vgrid = (x(cmaq%dom(id)%dart_ind(ii1,  ii2,  1,cmaq%dom(id)%type_v)) + &
!                              x(cmaq%dom(id)%dart_ind(ii1,  ii2+1,1,cmaq%dom(id)%type_v))) * 0.5_r8

                        vgrid_1 = get_dart_vector_index(ii1,  ii2,  1, domain_id(id),cmaq%dom(id)%type_v)
                        vgrid_2 = get_dart_vector_index(ii1,  ii2+1,1, domain_id(id),cmaq%dom(id)%type_v)
            ...
            do i1 = circ_xmin, circ_xmax
               ...
               dgi1 = 2.0_r8
               if ( .not. cmaq%dom(id)%periodic_x ) then
                  if     ( i1 == 1 ) then
                    dgi1 = 1.0_r8
                  elseif ( i1 == cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_mu) ) then
                    dgi1 = 1.0_r8
                  endif
               endif
               ...
               do i2 = circ_ymin, circ_ymax
                  ...
                  if ( .not. cmaq%dom(id)%periodic_x ) then
                     if     ( i2 == 1 ) then
                       dgi2 = 1.0_r8
                     elseif ( i2 == cmaq%dom(id)%var_size(2,cmaq%dom(id)%type_mu) ) then
                       dgi2 = 1.0_r8
                     endif
                  endif
                  ...
                  vort(ii1,ii2, :) = (vwnd(ii1+2,ii2+1, :) - vwnd(ii1+1,ii2+1, :)) / (cmaq%dom(id)%dx * dgi2) + &
                                  (vwnd(ii1+1,ii2+1, :) - vwnd(ii1  ,ii2+1, :)) / (cmaq%dom(id)%dx * dgi2) - &
                                  (uwnd(ii1+1,ii2+2, :) - uwnd(ii1+1,ii2+1, :)) / (cmaq%dom(id)%dx * dgi1) - &
                                  (uwnd(ii1+1,ii2+1, :) - uwnd(ii1+1,ii2, :  )) / (cmaq%dom(id)%dx * dgi1)
            ...
            do i2 = center_track_ymin, center_track_ymax
               ...
               do ii2 = max(circ_ymin,i2-circ_half_size), min(circ_ymax,i2+circ_half_size)
                  ...
                  distgrid = sqrt(real(ii1-i1,r8) ** 2 + real(ii2-i2,r8) ** 2) * cmaq%dom(id)%dx
            ...
            call ij_to_latlon(cmaq%dom(id)%proj, cxloc, cyloc, clat, clon)
            ...
            allocate(p1d(cmaq%dom(id)%bt, ens_size),  t1d(cmaq%dom(id)%bt, ens_size))
            allocate(qv1d(cmaq%dom(id)%bt, ens_size), z1d(cmaq%dom(id)%bt, ens_size))
            allocate(z1d_1(cmaq%dom(id)%bt, ens_size), z1d_2(cmaq%dom(id)%bt, ens_size))
...
!                     t1d(k2) = x(cmaq%dom(id)%dart_ind(ii1,ii2,k2,cmaq%dom(id)%type_t)) + ts0
                     t1d_ind = get_dart_vector_index(ii1,ii2,k2, domain_id(id),cmaq%dom(id)%type_t)
                     t1d(k2, :) = get_state( t1d_ind, state_handle)
                     t1d(k2, :) = t1d(k2, :) + ts0
                     !print*, 't1d(k2, 1)', t1d(k2, 1)

!                     qv1d(k2)= x(cmaq%dom(id)%dart_ind(ii1,ii2,k2,cmaq%dom(id)%type_qv))
                     qv1d_ind = get_dart_vector_index(ii1,ii2,k2, domain_id(id),cmaq%dom(id)%type_qv)
                     qv1d(k2, :) = get_state(qv1d_ind, state_handle)
                     !print*, 'qv1d(k2, 1)', qv1d(k2, 1)

!                     z1d(k2) = (x(cmaq%dom(id)%dart_ind(ii1,ii2,k2,  cmaq%dom(id)%type_gz))+ &
!                                x(cmaq%dom(id)%dart_ind(ii1,ii2,k2+1,cmaq%dom(id)%type_gz))+ &
!                                cmaq%dom(id)%phb(ii1,ii2,k2)+cmaq%dom(id)%phb(ii1,ii2,k2+1))*0.5_r8/gravity
                     z1d_ind1 = get_dart_vector_index(ii1,ii2,k2,   domain_id(id),cmaq%dom(id)%type_gz)
                     z1d_ind2 = get_dart_vector_index(ii1,ii2,k2+1, domain_id(id),cmaq%dom(id)%type_gz)
                  ...
                  enddo
                  do e = 1, ens_size
                     call compute_seaprs(cmaq%dom(id)%bt, z1d(:,e), t1d(:,e), p1d(:,e), qv1d(:,e), &
            ...
            do i1 = center_track_xmin, center_track_xmax
               ...
               do i2 = center_track_ymin, center_track_ymax
                  ...
                  if ( ( cmaq%dom(id)%type_u10 >= 0 ) .and. ( cmaq%dom(id)%type_v10 >= 0 ) ) then
                     !ugrid = x(cmaq%dom(id)%dart_ind(ii1,ii2,1,cmaq%dom(id)%type_u10))
                     !vgrid = x(cmaq%dom(id)%dart_ind(ii1,ii2,1,cmaq%dom(id)%type_v10))

                     ugrid_1 = get_dart_vector_index(ii1,ii2,1, domain_id(id),cmaq%dom(id)%type_u10)
                     ugrid = get_state(ugrid_1, state_handle)
 
                     vgrid_1 = get_dart_vector_index(ii1,ii2,1, domain_id(id),cmaq%dom(id)%type_v10)
                     vgrid = get_state(vgrid_1, state_handle)

                  else

! Same code as above
!                     ugrid = 0.5_r8*(x(cmaq%dom(id)%dart_ind(ii1,  ii2,1,cmaq%dom(id)%type_u)) + &
!                                     x(cmaq%dom(id)%dart_ind(ii1+1,ii2,1,cmaq%dom(id)%type_u)))
!                     vgrid = 0.5_r8*(x(cmaq%dom(id)%dart_ind(ii1,ii2,  1,cmaq%dom(id)%type_v)) + &
!                                     x(cmaq%dom(id)%dart_ind(ii1,ii2+1,1,cmaq%dom(id)%type_v)))
                     ugrid_1 = get_dart_vector_index(ii1,  ii2,  1, domain_id(id),cmaq%dom(id)%type_u)
                     ugrid_2 = get_dart_vector_index(ii1+1,ii2,  1, domain_id(id),cmaq%dom(id)%type_u)
...
!*****************************************************************************
   ...
   !   (toGrid below should return dz ~ 0 and dzm ~ 1) 
      ...
      if ( cmaq%dom(id)%type_gz >= 0 ) then
         ...
         ! Check to make sure retrieved integer gridpoints are in valid range
         if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_gz ) .and. &
              boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_gz ) .and. &
              boundsCheck( k(1), .false.,                id, dim=3, type=cmaq%dom(id)%type_gz ) ) then
            
            call getCorners(i, j, id, cmaq%dom(id)%type_gz, ll, ul, lr, ur, rc )
            if ( rc .ne. 0 ) &
                 print*, 'model_mod.f90 :: model_interpolate :: getCorners GZ rc = ', rc
            
            ! Interpolation for GZ field at level k
            ill = get_dart_vector_index(ll(1), ll(2), k(1), domain_id(id), cmaq%dom(id)%type_gz)
            iul = get_dart_vector_index(ul(1), ul(2), k(1), domain_id(id), cmaq%dom(id)%type_gz)
            ilr = get_dart_vector_index(lr(1), lr(2), k(1), domain_id(id), cmaq%dom(id)%type_gz)
            iur = get_dart_vector_index(ur(1), ur(2), k(1), domain_id(id), cmaq%dom(id)%type_gz)
            ...
            fld(1,:) = ( dym*( dxm*x_ill + dx*x_ilr ) + dy*( dxm*x_iul + dx*x_iur ) + &
                       dym*( dxm*cmaq%dom(id)%phb(ll(1), ll(2), k)   + &
                             dx *cmaq%dom(id)%phb(lr(1), lr(2), k) ) + &
                       dy *( dxm*cmaq%dom(id)%phb(ul(1), ul(2), k)   + &
                             dx *cmaq%dom(id)%phb(ur(1), ur(2), k) ) )  / gravity
            
            ! Interpolation for GZ field at level k+1
            ill = get_dart_vector_index(ll(1), ll(2), k(1)+1, domain_id(id), cmaq%dom(id)%type_gz)
            iul = get_dart_vector_index(ul(1), ul(2), k(1)+1, domain_id(id), cmaq%dom(id)%type_gz)
            ilr = get_dart_vector_index(lr(1), lr(2), k(1)+1, domain_id(id), cmaq%dom(id)%type_gz)
            iur = get_dart_vector_index(ur(1), ur(2), k(1)+1, domain_id(id), cmaq%dom(id)%type_gz)
            ...
            fld(2, :) = ( dym*( dxm*x_ill + dx*x_ilr ) + dy*( dxm*x_iul + dx*x_iur ) + &
                       dym*( dxm*cmaq%dom(id)%phb(ll(1), ll(2), k(1)+1)   + &
                             dx *cmaq%dom(id)%phb(lr(1), lr(2), k(1)+1) ) + &
                       dy *( dxm*cmaq%dom(id)%phb(ul(1), ul(2), k(1)+1)   + &
                             dx *cmaq%dom(id)%phb(ur(1), ur(2), k(1)+1) ) )  / gravity
   ...
   !   HGT is not in the dart_ind vector, so get it from cmaq%dom(id)%hgt.
      ...
      if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .and. &
         ...
         ! Interpolation for the HGT field -- HGT is NOT part of state vector x, but rather
         !   in the associated domain meta data
         fld(1, :) = dym*( dxm*cmaq%dom(id)%hgt(ll(1), ll(2)) + &
                         dx*cmaq%dom(id)%hgt(lr(1), lr(2)) ) + &
                   dy*( dxm*cmaq%dom(id)%hgt(ul(1), ul(2)) + &
                         dx*cmaq%dom(id)%hgt(ur(1), ur(2)) )
   ...
   ! 1.y Surface Skin Temperature (TSK)
     ...
     ! make sure vector includes the needed field
     if ( cmaq%dom(id)%type_tsk >= 0 ) then
        call surface_interp_distrib(fld, cmaq, id, i, j, obs_kind, cmaq%dom(id)%type_tsk, dxm, dx, dy, dym, ens_size, state_handle)
        if (all(fld == missing_r8)) goto 200
     endif
   ...
   !   XLAND is not in the dart_ind vector, so get it from cmaq%dom(id)%land
      ...
      if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .and. &
         ...
         !   in the associated domain meta data
         fld(1, :) = dym*( dxm*real(cmaq%dom(id)%land(ll(1), ll(2))) + &
                         dx*real(cmaq%dom(id)%land(lr(1), lr(2))) ) + &
                   dy*( dxm*real(cmaq%dom(id)%land(ul(1), ul(2))) + &
                         dx*real(cmaq%dom(id)%land(ur(1), ur(2))) )
      ...
      if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .and. &
         ...
         fld(1, :) = dym*( dxm*real(cmaq%dom(id)%land(ll(1), ll(2))) + &
                         dx*real(cmaq%dom(id)%land(lr(1), lr(2))) ) + &
                   dy*( dxm*real(cmaq%dom(id)%land(ul(1), ul(2))) + &
                         dx*real(cmaq%dom(id)%land(ur(1), ur(2))) ) - 1

      endif
! 
! APM +++
!-----------------------------------------------------
! 1.za CARBON MONOXIDE (CO)
   else if( obs_kind == QTY_CO ) then
      if ( cmaq%dom(id)%type_co >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_co ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_co ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_co ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_co, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
...
! Interpolation for the CO field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_co)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_co)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_co)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_co)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
! Interpolation for the CO field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_co)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_co)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_co)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_co)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!
!-----------------------------------------------------
! 1.zb OZONE INTERPOLATION (O3)
   else if( obs_kind == QTY_O3 ) then
      if ( cmaq%dom(id)%type_o3 >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_o3 ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_o3 ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_o3 ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_o3, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
...
! Interpolation for the O3 field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_o3)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_o3)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_o3)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_o3)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
! Interpolation for the O3 field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_o3)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_o3)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_o3)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_o3)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!
!-----------------------------------------------------
! 1.zc NITROGEN OXIDE (NO)
   else if( obs_kind == QTY_NO ) then
      if ( cmaq%dom(id)%type_no >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_no ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_no ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_no ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_no, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
                  print*, 'model_mod.f90 :: model_interpolate :: getCorners NO rc = ', rc
                  call exit_all (-77)
               endif
!
! Interpolation for the NO field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_no)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_no)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_no)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_no)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
               ...
               count, dx, dy, dxm, dym, x_ill, x_iul, x_ilr, x_iur, domain_id(id))
!
! Interpolation for the NO field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_no)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_no)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_no)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_no)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!
!-----------------------------------------------------
! 1.zd NITROGEN DIOXIDE (NO2)
   else if( obs_kind == QTY_NO2 ) then
      if ( cmaq%dom(id)%type_no2 >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_no2 ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_no2 ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_no2 ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_no2, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
...
! Interpolation for the NO2 field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_no2)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_no2)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_no2)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_no2)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
! Interpolation for the NO2 field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_no2)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_no2)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_no2)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_no2)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!
!-----------------------------------------------------
!1.ze SULFUR DIOXIDE (SO2); SULFATE (SO4)
   elseif ( obs_kind == QTY_SO2 .or. obs_kind == QTY_SO4 ) then
!
! SO2 
      if( obs_kind == QTY_SO2) then               
         if ( cmaq%dom(id)%type_so2 >= 0 ) then
            do uk = 1, count
               if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_so2 ) .and. &
               boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_so2 ) .and. &
               boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_so2 ) ) then
                  call getCorners(i, j, id, cmaq%dom(id)%type_so2, ll, ul, lr, ur, rc )
                  if ( rc .ne. 0 ) then
...
! Interpolation for the SO2 field at level k
                  ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_so2)
                  iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_so2)
                  ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_so2)
                  iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_so2)
                  x_ill = get_state(ill, state_handle)
                  x_iul = get_state(iul, state_handle)
                  x_ilr = get_state(ilr, state_handle)
                  x_iur = get_state(iur, state_handle)
...
! Interpolation for the SO2 field at level k+1
                  ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_so2)
                  iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_so2)
                  ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_so2)
                  iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_so2)
                  x_ill = get_state(ill, state_handle)
                  x_iul = get_state(iul, state_handle)
                  x_ilr = get_state(ilr, state_handle)
                  x_iur = get_state(iur, state_handle)
...
! 1.zf SULFATE (SO4)
      if( obs_kind == QTY_SO4) then               
         if ( cmaq%dom(id)%type_so4 >= 0 ) then
            do uk = 1, count
               if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_so4 ) .and. &
               boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_so4 ) .and. &
               boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_so4 ) ) then
                  call getCorners(i, j, id, cmaq%dom(id)%type_so4, ll, ul, lr, ur, rc )
                  if ( rc .ne. 0 ) then
...
! Interpolation for the SO4 field at level k
                  ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_so4)
                  iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_so4)
                  ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_so4)
                  iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_so4)
                  x_ill = get_state(ill, state_handle)
                  x_iul = get_state(iul, state_handle)
                  x_ilr = get_state(ilr, state_handle)
                  x_iur = get_state(iur, state_handle)
...
! Interpolation for the SO4 field at level k+1
                  ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_so4)
                  iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_so4)
                  ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_so4)
                  iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_so4)
                  x_ill = get_state(ill, state_handle)
                  x_iul = get_state(iul, state_handle)
                  x_ilr = get_state(ilr, state_handle)
                  x_iur = get_state(iur, state_handle)
...
!
!-----------------------------------------------------
! 1.zg DINITROGEN PENTOXIDE (N2O5)
   else if( obs_kind == QTY_N2O5 ) then
      if ( cmaq%dom(id)%type_n2o5 >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_n2o5 ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_n2o5 ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_n2o5 ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_n2o5, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
                  print*, 'model_mod.f90 :: model_interpolate :: getCorners N2O5 rc = ', rc
                  call exit_all (-77)
               endif
!
! Interpolation for the N2O5 field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_n2o5)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_n2o5)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_n2o5)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_n2o5)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
               ...
               count, dx, dy, dxm, dym, x_ill, x_iul, x_ilr, x_iur, domain_id(id))
!
! Interpolation for the N2O5 field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_n2o5)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_n2o5)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_n2o5)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_n2o5)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!-----------------------------------------------------
! 1.zh ACET
   else if( obs_kind == QTY_ACET ) then
      if ( cmaq%dom(id)%type_acet >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_acet ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_acet ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_acet ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_acet, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
                  print*, 'model_mod.f90 :: model_interpolate :: getCorners ACET rc = ', rc
                  call exit_all(-77)
               endif
!
! Interpolation for the ACET field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_acet)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_acet)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_acet)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_acet)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
               ...
               count, dx, dy, dxm, dym, x_ill, x_iul, x_ilr, x_iur, domain_id(id))
!
! Interpolation for the ACET field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_acet)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_acet)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_acet)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_acet)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!-----------------------------------------------------
! 1.zi TOLUENE (TOL)
   else if( obs_kind == QTY_TOL ) then
      if ( cmaq%dom(id)%type_tol >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_tol ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_tol ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_tol ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_tol, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
                  print*, 'model_mod.f90 :: model_interpolate :: getCorners TOL rc = ', rc
                  call exit_all(-77)
               endif
!
! Interpolation for the TOL field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_tol)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_tol)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_tol)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_tol)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
               ...
               count, dx, dy, dxm, dym, x_ill, x_iul, x_ilr, x_iur, domain_id(id))
!
! Interpolation for the TOL field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_tol)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_tol)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_tol)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_tol)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!-----------------------------------------------------
! 1.zj ISOPR 
   else if( obs_kind == QTY_ISOPR ) then
      if ( cmaq%dom(id)%type_isopr >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_isopr ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_isopr ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_isopr ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_isopr, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
                  print*, 'model_mod.f90 :: model_interpolate :: getCorners ISOPR rc = ', rc
                  call exit_all(-77)
               endif
!
! Interpolation for the ISOPR field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_isopr)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_isopr)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_isopr)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_isopr)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
               ...
               count, dx, dy, dxm, dym, x_ill, x_iul, x_ilr, x_iur, domain_id(id))
!
! Interpolation for the ISOPR field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_isopr)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_isopr)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_isopr)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_isopr)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!
!-----------------------------------------------------
! 1.zk FORMALDEHYDE (HCHO)
   else if( obs_kind == QTY_HCHO ) then
      if ( cmaq%dom(id)%type_hcho >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_hcho ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_hcho ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_hcho ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_hcho, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
...
! Interpolation for the HCHO field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_hcho)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_hcho)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_hcho)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_hcho)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
! Interpolation for the HCHO field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_hcho)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_hcho)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_hcho)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_hcho)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!
!-----------------------------------------------------
! 1.zl NITRIC ACID (HNO3)
   else if( obs_kind == QTY_HNO3 ) then
      if ( cmaq%dom(id)%type_hno3 >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_hno3 ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_hno3 ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_hno3 ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_hno3, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
...
! Interpolation for the HNO3 field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_hno3)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_hno3)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_hno3)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_hno3)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
! Interpolation for the HNO3 field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_hno3)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_hno3)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_hno3)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_hno3)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!-----------------------------------------------------
! 1.zm AMMONIA (NH3)
   else if( obs_kind == QTY_NH3 ) then
      if ( cmaq%dom(id)%type_nh3 >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_nh3 ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_nh3 ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_nh3 ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_nh3, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
                  print*, 'model_mod.f90 :: model_interpolate :: getCorners NH3 rc = ', rc
                  call exit_all(-77)
               endif
!
! Interpolation for the NH3 field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_nh3)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_nh3)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_nh3)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_nh3)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
               ...
               count, dx, dy, dxm, dym, x_ill, x_iul, x_ilr, x_iur, domain_id(id))
!
! Interpolation for the NH3 field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_nh3)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_nh3)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_nh3)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_nh3)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!-----------------------------------------------------
! 1.zn PAN
   else if( obs_kind == QTY_PAN ) then
      if ( cmaq%dom(id)%type_pan >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_pan ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_pan ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_pan ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_pan, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
                  print*, 'model_mod.f90 :: model_interpolate :: getCorners PAN rc = ', rc
                  call exit_all(-77)
               endif
!
! Interpolation for the PAN field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_pan)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_pan)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_pan)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_pan)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
               ...
               count, dx, dy, dxm, dym, x_ill, x_iul, x_ilr, x_iur, domain_id(id))
!
! Interpolation for the PAN field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_pan)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_pan)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_pan)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_pan)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
!-----------------------------------------------------
! 1.zo METHANE (CH4)
   else if( obs_kind == QTY_CH4 ) then
      if ( cmaq%dom(id)%type_ch4 >= 0 ) then
         do uk = 1, count
            if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_ch4 ) .and. &
            boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_ch4 ) .and. &
            boundsCheck( uniquek(uk), .false.,      id, dim=3, type=cmaq%dom(id)%type_ch4 ) ) then
               call getCorners(i, j, id, cmaq%dom(id)%type_ch4, ll, ul, lr, ur, rc )
               if ( rc .ne. 0 ) then
                  print*, 'model_mod.f90 :: model_interpolate :: getCorners CH4 rc = ', rc
                  call exit_all(-77)
               endif
!
! Interpolation for the CH4 field at level k
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_ch4)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_ch4)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_ch4)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id),cmaq%dom(id)%type_ch4)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
               ...
               count, dx, dy, dxm, dym, x_ill, x_iul, x_ilr, x_iur, domain_id(id))
!
! Interpolation for the CH4 field at level k+1
               ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_ch4)
               iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_ch4)
               ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_ch4)
               iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id),cmaq%dom(id)%type_ch4)
               x_ill = get_state(ill, state_handle)
               x_iul = get_state(iul, state_handle)
               x_ilr = get_state(ilr, state_handle)
               x_iur = get_state(iur, state_handle)
...
do i=1, num
   ...
   if (cmaq%dom(id)%localization_coord == VERTISLEVEL) then
   ...
   elseif (cmaq%dom(id)%localization_coord == VERTISPRESSURE) then
      ! directly convert to pressure
      lev = model_pressure_distrib(ip, jp, kp, id, var_type, state_handle)
   elseif (cmaq%dom(id)%localization_coord == VERTISHEIGHT) then
      lev = model_height_distrib(ip, jp, kp, id, var_type, state_handle)
   elseif (cmaq%dom(id)%localization_coord == VERTISSCALEHEIGHT) then
   ...
   locs(i) = set_location(lon, lat, lev, cmaq%dom(id)%localization_coord)
...
if ( .not. boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_t ) .or. &
     .not. boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_t ) ) then
   location = set_location(xyz_loc(1),xyz_loc(2),missing_r8,ztypeout)
...
case (VERTISLEVEL)
   ...
   case (VERTISPRESSURE)
      ...
      if(.not. boundsCheck(k, .false., id, dim=3, type=cmaq%dom(id)%type_t)) goto 100
   ...
   case (VERTISHEIGHT) 
      ...
      ! Check that integer height index is in valid range.  if not, bail to end
      if(.not. boundsCheck(k, .false., id, dim=3, type=cmaq%dom(id)%type_gz)) then
...
case (VERTISPRESSURE)
   ...
   allocate(v_p(0:cmaq%dom(id)%bt)) 
   !HK This has already been called in model interpolate
   ! - not for observations that were not in the assimilate catagory
   call get_model_pressure_profile_distrib(i,j,dx,dy,dxm,dym,cmaq%dom(id)%bt,id,v_p, state_handle,1)

     !if (my_task_id() == 0) then
     !    write(10, *) v_p
     !endif

   call pres_to_zk(zin, v_p(:), cmaq%dom(id)%bt, zk, lev0)
   deallocate(v_p)
   ...
   case (VERTISHEIGHT)
      ...
      ! Check that integer height index is in valid range.  if not, bail to end
      if(.not. boundsCheck(k, .false., id, dim=3, type=cmaq%dom(id)%type_gz)) goto 100
...
case (VERTISHEIGHT)
   ...
   allocate(v_h(0:cmaq%dom(id)%bt))
   call get_model_height_profile_distrib(i,j,dx,dy,dxm,dym,cmaq%dom(id)%bt,id,v_h, state_handle,1)
   call height_to_zk(zin, v_h, cmaq%dom(id)%bt,zk,lev0)
   deallocate(v_h)
...
case (VERTISSCALEHEIGHT)
   ...
   allocate(v_p(0:cmaq%dom(id)%bt))
   call get_model_pressure_profile_distrib(i,j,dx,dy,dxm,dym,cmaq%dom(id)%bt,id,v_p, state_handle, 1)
   call pres_to_zk(exp(-zin)*psurf, v_p, cmaq%dom(id)%bt,zk,lev0)
   deallocate(v_p)
...
case(VERTISSURFACE)
   ...
   case (VERTISHEIGHT)
      ...
      !zout = dym*( dxm*cmaq%dom(id)%hgt(i,  j) + &
      !              dx*cmaq%dom(id)%hgt(i+1,j) ) + &
      !        dy*( dxm*cmaq%dom(id)%hgt(i,  j+1) + &
      !              dx*cmaq%dom(id)%hgt(i+1,j+1) )
...
subroutine get_cmaq_horizontal_location( i, j, var_type, id, long, lat )
...
elseif (var_type == cmaq%dom(id)%type_v) then
   long = cmaq%dom(id)%longitude_v(i,j)
   lat  = cmaq%dom(id)%latitude_v(i,j)
else
   long = cmaq%dom(id)%longitude(i,j)
   lat  = cmaq%dom(id)%latitude(i,j)
endif
...
end subroutine get_cmaq_horizontal_location
...
integer :: weDimID, weStagDimID, snDimID, snStagDimID, &
     btDimID, bt1DimID, btStagDimID, slSDimID
...
!print *,'APM: At start of subroutine nc_write_model_atts '
...
call nc_add_global_attribute(ncid, "model", "cmaq")
...
call nc_check(nf90_def_dim(ncid=ncid, name='COL', &
              len = cmaq%dom(id)%we,  dimid = weDimID), &
              'nc_write_model_atts','def_dim COL')
call nc_check(nf90_def_dim(ncid=ncid, name='ROW',      &
              len = cmaq%dom(id)%sn,  dimid = snDimID), &
              'nc_write_model_atts','def_dim ROW')
call nc_check(nf90_def_dim(ncid=ncid, name='LAY',       &
              len = cmaq%dom(id)%bt,  dimid = btDimID), &
              'nc_write_model_atts','def_dim LAY')
call nc_check(nf90_def_dim(ncid=ncid, name='LAY_2D',       &
              len = cmaq%dom(id)%bt,  dimid = bt1DimID), &
              'nc_write_model_atts','def_dim LAY_2D')
...
! Commented block is from cmaqinput
!-----------------------------------------------------------------
...
call nc_check(nf90_put_var(ncid,        DXVarID, cmaq%dom(id)%dx), &
              'nc_write_model_atts','put_var dx')
call nc_check(nf90_put_var(ncid,        DYVarID, cmaq%dom(id)%dy), &
              'nc_write_model_atts','put_var dy')
call nc_check(nf90_put_var(ncid,  TRUELAT1VarID, cmaq%dom(id)%proj%truelat1), &
              'nc_write_model_atts','put_var truelat1')
call nc_check(nf90_put_var(ncid,  TRUELAT2VarID, cmaq%dom(id)%proj%truelat2), &
              'nc_write_model_atts','put_var truelat2')
call nc_check(nf90_put_var(ncid, STAND_LONVarID, cmaq%dom(id)%proj%stdlon), &
              'nc_write_model_atts','put_var stdlon')
call nc_check(nf90_put_var(ncid,   CEN_LATVarID, cmaq%dom(id)%cen_lat), &
              'nc_write_model_atts','put_var cen_lat')
call nc_check(nf90_put_var(ncid,   CEN_LONVarID, cmaq%dom(id)%cen_lon), &
              'nc_write_model_atts','put_var cen_lon')
call nc_check(nf90_put_var(ncid,  MAP_PROJVarID, cmaq%dom(id)%map_proj), &
...
call nc_check(nf90_put_var(ncid,       DNVarID, cmaq%dom(id)%dn), &
              'nc_write_model_atts','put_var dn')
call nc_check(nf90_put_var(ncid,      ZNUVarID, cmaq%dom(id)%znu), &
              'nc_write_model_atts','put_var znu')
call nc_check(nf90_put_var(ncid,      ZNWVarID, cmaq%dom(id)%znw), &
              'nc_write_model_atts','put_var znw')
call nc_check(nf90_put_var(ncid,      DNWVarID, cmaq%dom(id)%dnw), &
              'nc_write_model_atts','put_var dnw')

! defining horizontal
call nc_check(nf90_put_var(ncid,      mubVarID, cmaq%dom(id)%mub), &
              'nc_write_model_atts','put_var mub')
call nc_check(nf90_put_var(ncid,      LonVarID, cmaq%dom(id)%longitude), &
              'nc_write_model_atts','put_var longitude')
call nc_check(nf90_put_var(ncid,      LonuVarID, cmaq%dom(id)%longitude_u), &
              'nc_write_model_atts','put_var longitude_u')
call nc_check(nf90_put_var(ncid,      LonvVarID, cmaq%dom(id)%longitude_v), &
              'nc_write_model_atts','put_var longitude_v')
call nc_check(nf90_put_var(ncid,      LatVarID, cmaq%dom(id)%latitude), &
              'nc_write_model_atts','put_var latitude')
call nc_check(nf90_put_var(ncid,      LatuVarID, cmaq%dom(id)%latitude_u), &
              'nc_write_model_atts','put_var latitude_u')
call nc_check(nf90_put_var(ncid,      LatvVarID, cmaq%dom(id)%latitude_v), &
              'nc_write_model_atts','put_var latitude_v')
call nc_check(nf90_put_var(ncid,     ilevVarID, (/ (i,i=1,cmaq%dom(id)%bt) /)), &
              'nc_write_model_atts','put_var bt')
call nc_check(nf90_put_var(ncid,    XlandVarID, cmaq%dom(id)%land), &
              'nc_write_model_atts','put_var land')
!   call nc_check(nf90_put_var(ncid,  MapFacMVarID(id), cmaq%dom(id)%mapfac_m), &
!             'nc_write_model_atts','put_var mapfac_m')
!   call nc_check(nf90_put_var(ncid,  MapFacUVarID(id), cmaq%dom(id)%mapfac_u), &
!             'nc_write_model_atts','put_var mapfac_u')
!   call nc_check(nf90_put_var(ncid,  MapFacVVarID(id), cmaq%dom(id)%mapfac_v), &
!             'nc_write_model_atts','put_var mapfac_v')
call nc_check(nf90_put_var(ncid,      phbVarID, cmaq%dom(id)%phb), &
              'nc_write_model_atts','put_var phb')
call nc_check(nf90_put_var(ncid,      hgtVarID, cmaq%dom(id)%hgt), &
...
!print *,'APM: At end of subroutine nc_write_model_atts '
...
!   print *, 'APM: In pres_to_zk_distrib'
...
!   print *, 'APM: In get_model_pressure_profile_distribution'
...
!   print *, 'APM: In model_pressure_distrib'
...
if( (var_type == cmaq%dom(id)%type_w) .or. (var_type == cmaq%dom(id)%type_gz) ) then
   ...
   if (var_type == cmaq%dom(id)%type_w) then
      type_x = cmaq%dom(id)%type_w
   else
      type_x = cmaq%dom(id)%type_gz
   endif
   ...
   elseif( k == cmaq%dom(id)%var_size(3,type_x) ) then
...
elseif( var_type == cmaq%dom(id)%type_u ) then

   if( i == cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_u) ) then
   ...
   elseif( i == 1 ) then
      ...
      if ( cmaq%dom(id)%periodic_x ) then
         ...
         ! We are at seam in longitude, take first and last M-grid points
         pres1 = scalar(model_pressure_t_distrib(i,             j,k,id, state_handle, ens_size))
         pres2 = scalar(model_pressure_t_distrib(cmaq%dom(id)%we,j,k,id, state_handle, ens_size))
         model_pressure_distrib = interp_pressure(pres1, pres2, vertical=.false.)
...
elseif( var_type == cmaq%dom(id)%type_v ) then
   ...
   if( j == cmaq%dom(id)%var_size(2,cmaq%dom(id)%type_v) ) then

      ! Check to see if periodic in latitude (polar)
      if ( cmaq%dom(id)%polar ) then

         ! The upper corner is 180 degrees of longitude away
         off = i + cmaq%dom(id)%we/2
         if ( off > cmaq%dom(id)%we ) off = off - cmaq%dom(id)%we
...
elseif( var_type == cmaq%dom(id)%type_mu    .or. var_type == cmaq%dom(id)%type_tslb .or. &
        var_type == cmaq%dom(id)%type_ps    .or. var_type == cmaq%dom(id)%type_u10  .or. &
        var_type == cmaq%dom(id)%type_v10   .or. var_type == cmaq%dom(id)%type_t2   .or. &
        var_type == cmaq%dom(id)%type_th2   .or.                                        &
        var_type == cmaq%dom(id)%type_q2    .or. var_type == cmaq%dom(id)%type_tsk  .or. &
        var_type == cmaq%dom(id)%type_smois .or. var_type == cmaq%dom(id)%type_sh2o) then
...
!   print *, 'APM: In model_surface_pressure_distrib'
...
if( var_type == cmaq%dom(id)%type_u ) then
   ...
   elseif( i == 1 ) then
      ...
      if ( cmaq%dom(id)%periodic_x ) then
         ...
         pres2 = model_pressure_s_distrib(cmaq%dom(id)%we,j,id, state_handle)
         model_surface_pressure_distrib = interp_pressure(pres1, pres2, vertical=.false.)
...
!   print *, 'APM: In model_pressure_t_distrib'

model_pressure_t_distrib = missing_r8

! Adapted the code from CMAQ module_big_step_utilities_em.F ----
!         subroutine calc_p_rho_phi      Y.-R. Guo (10/20/2004)

! Simplification: alb*mub = (phb(i,j,k+1) - phb(i,j,k))/dnw(k)

if (cmaq%dom(id)%type_qv < 0 .or. cmaq%dom(id)%type_t < 0) then
  call error_handler(E_ERR, 'model_pressure_t:', &
      'BOTH QVAPOR and T must be in state vector to compute total pressure', &
       source, revision, revdate)
endif

iqv = get_dart_vector_index(i,j,k, domain_id(id), cmaq%dom(id)%type_qv)
it  = get_dart_vector_index(i,j,k, domain_id(id), cmaq%dom(id)%type_t)
...
!   print *, 'APM: In model_pressure_s_distrib'

! make sure one of these is good.
if ( cmaq%dom(id)%type_mu < 0 .and. cmaq%dom(id)%type_ps < 0 ) then
...
if ( cmaq%dom(id)%type_ps >= 0 ) then
   ips = get_dart_vector_index(i,j,1, domain_id(id), cmaq%dom(id)%type_ps)
   x_ips = scalar(get_state(ips, state_handle))
   model_pressure_s_distrib = x_ips

else
   imu = get_dart_vector_index(i,j,1, domain_id(id), cmaq%dom(id)%type_mu)
   x_imu = minval(get_state(imu, state_handle))
   model_pressure_s_distrib = cmaq%dom(id)%p_top + cmaq%dom(id)%mub(i,j) + x_imu
...
!   print *, 'APM: In interp_4pressure_distrib'
...
!   print *, 'APM: In model_rho_t_distrib'
...
if (cmaq%dom(id)%type_mu < 0 .or. cmaq%dom(id)%type_gz < 0) then
...
imu   = get_dart_vector_index(i,j,1,   domain_id(id), cmaq%dom(id)%type_mu)
iph   = get_dart_vector_index(i,j,k,   domain_id(id), cmaq%dom(id)%type_gz)
iphp1 = get_dart_vector_index(i,j,k+1, domain_id(id), cmaq%dom(id)%type_gz)

x_imu = get_state(imu, state_handle)
x_iph = get_state(iph, state_handle)
x_iphp1 = get_state(iphp1, state_handle)

ph_e = ( (x_iphp1 + cmaq%dom(id)%phb(i,j,k+1)) &
       - (x_iph   + cmaq%dom(id)%phb(i,j,k  )) ) / cmaq%dom(id)%dnw(k)

! now calculate rho = - mu / dphi/deta

model_rho_t_distrib(:) = - (cmaq%dom(id)%mub(i,j)+x_imu) / ph_e
...
!   print *, 'APM: In get_model_height_profile'
...
if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq%dom(id)%type_gz ) .and. &
     boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq%dom(id)%type_gz ) ) then

   call getCorners(i, j, id, cmaq%dom(id)%type_gz, ll, ul, lr, ur, rc )
   if ( rc .ne. 0 ) &
        print*, 'model_mod.f90 :: get_model_height_profile :: getCorners rc = ', rc

   do k = 1, cmaq%dom(id)%var_size(3,cmaq%dom(id)%type_gz)

      ill = get_dart_vector_index(ll(1), ll(2), k, domain_id(id), cmaq%dom(id)%type_gz)
      iul = get_dart_vector_index(ul(1), ul(2), k, domain_id(id), cmaq%dom(id)%type_gz)
      ilr = get_dart_vector_index(lr(1), lr(2), k, domain_id(id), cmaq%dom(id)%type_gz)
      iur = get_dart_vector_index(ur(1), ur(2), k, domain_id(id), cmaq%dom(id)%type_gz)
      ...
      geop(:) = ( dym*( dxm*( cmaq%dom(id)%phb(ll(1),ll(2),k) + x_ill ) + &
                      dx*( cmaq%dom(id)%phb(lr(1),lr(2),k) + x_ilr ) ) + &
                dy*( dxm*( cmaq%dom(id)%phb(ul(1),ul(2),k) + x_iul ) + &
                      dx*( cmaq%dom(id)%phb(ur(1),ur(2),k) + x_iur ) ) )/gravity

      lat(:) = ( cmaq%dom(id)%latitude(ll(1),ll(2)) + &
              cmaq%dom(id)%latitude(lr(1),lr(2)) + &
              cmaq%dom(id)%latitude(ul(1),ul(2)) + &
              cmaq%dom(id)%latitude(ur(1),ur(2)) ) / 4.0_r8
   ...
   v_h(0, :) = dym*( dxm*cmaq%dom(id)%hgt(ll(1), ll(2)) + &
                   dx*cmaq%dom(id)%hgt(lr(1), lr(2)) ) + &
             dy*( dxm*cmaq%dom(id)%hgt(ul(1), ul(2)) + &
                   dx*cmaq%dom(id)%hgt(ur(1), ur(2)) )
...
!   fld as missing data
   ...
   print*,i,j,dx,dy,dxm,dym,n,id,cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_gz), &
        cmaq%dom(id)%var_size(2,cmaq%dom(id)%type_gz)
...
!   print *, 'APM: In model_height_distrib'
...
if( (var_type == cmaq%dom(id)%type_w) .or. (var_type == cmaq%dom(id)%type_gz) ) then
   ...
   i1 = get_dart_vector_index(i,j,k, domain_id(id),cmaq%dom(id)%type_gz)
   x_i1 = get_state(i1, state_handle)

   geop = minval((cmaq%dom(id)%phb(i,j,k)+x_i1)/gravity)
   model_height_distrib = compute_geometric_height(geop, cmaq%dom(id)%latitude(i, j))
...
elseif( var_type == cmaq%dom(id)%type_u ) then
   ...
   if( i == cmaq%dom(id)%var_size(1,cmaq%dom(id)%type_u) ) then
      ...
      if ( cmaq%dom(id)%periodic_x ) then
         ...
         i1 = get_dart_vector_index(i-1,j,k  , domain_id(id),cmaq%dom(id)%type_gz)
         i2 = get_dart_vector_index(i-1,j,k+1, domain_id(id),cmaq%dom(id)%type_gz)
         i3 = get_dart_vector_index(1,  j,k  , domain_id(id),cmaq%dom(id)%type_gz)
         i4 = get_dart_vector_index(1,  j,k+1, domain_id(id),cmaq%dom(id)%type_gz)
         ...
         geop = minval(( (cmaq%dom(id)%phb(i-1,j,k  ) + x_i1) &
                 +(cmaq%dom(id)%phb(i-1,j,k+1) + x_i2) &
                 +(cmaq%dom(id)%phb(1  ,j,k  ) + x_i3) &
                 +(cmaq%dom(id)%phb(1  ,j,k+1) + x_i4) )/(4.0_r8*gravity))
         
         lat = ( cmaq%dom(id)%latitude(i-1,j)  &
                +cmaq%dom(id)%latitude(i-1,j)  &
                +cmaq%dom(id)%latitude(1  ,j)  &
                +cmaq%dom(id)%latitude(1  ,j) ) / 4.0_r8
         ...
         geop = minval(( 3.0_r8*(cmaq%dom(id)%phb(i-1,j,k  )+x_i1) &
                 +3.0_r8*(cmaq%dom(id)%phb(i-1,j,k+1)+x_i2) &
                        -(cmaq%dom(id)%phb(i-2,j,k  )+x_i3) &
                        -(cmaq%dom(id)%phb(i-2,j,k+1)+x_i4) )/(4.0_r8*gravity))

         lat = ( 3.0_r8*cmaq%dom(id)%latitude(i-1,j)  &
                +3.0_r8*cmaq%dom(id)%latitude(i-1,j)  &
                       -cmaq%dom(id)%latitude(i-2,j)  &
                       -cmaq%dom(id)%latitude(i-2,j)) / 4.0_r8
   ...
   elseif( i == 1 ) then
      ...
      if ( cmaq%dom(id)%periodic_x ) then
         ...
         off = cmaq%dom(id)%we
         i1 = get_dart_vector_index(i  ,j,k  ,domain_id(id),cmaq%dom(id)%type_gz)
         i2 = get_dart_vector_index(i  ,j,k+1,domain_id(id),cmaq%dom(id)%type_gz)
         i3 = get_dart_vector_index(off,j,k  ,domain_id(id),cmaq%dom(id)%type_gz)
         i4 = get_dart_vector_index(off,j,k+1,domain_id(id),cmaq%dom(id)%type_gz)
         ...
         geop = minval(( (cmaq%dom(id)%phb(i  ,j,k  ) + x_i1) &
                 +(cmaq%dom(id)%phb(i  ,j,k+1) + x_i2) &
                 +(cmaq%dom(id)%phb(off,j,k  ) + x_i3) &
                 +(cmaq%dom(id)%phb(off,j,k+1) + x_i4) )/(4.0_r8*gravity))
         
         lat = ( cmaq%dom(id)%latitude(i  ,j)  &
                +cmaq%dom(id)%latitude(i  ,j)  &
                +cmaq%dom(id)%latitude(off,j)  &
                +cmaq%dom(id)%latitude(off,j)) / 4.0_r8
         ...
         geop = minval(( 3.0_r8*(cmaq%dom(id)%phb(i  ,j,k  )+x_i1)   &
                 +3.0_r8*(cmaq%dom(id)%phb(i  ,j,k+1)+x_i2)   &
                        -(cmaq%dom(id)%phb(i+1,j,k  )+x_i3) &
                        -(cmaq%dom(id)%phb(i+1,j,k+1)+x_i4) )/(4.0_r8*gravity))

         lat = ( 3.0_r8*cmaq%dom(id)%latitude(i  ,j)  &
                +3.0_r8*cmaq%dom(id)%latitude(i  ,j)  &
                       -cmaq%dom(id)%latitude(i+1,j)  &
                       -cmaq%dom(id)%latitude(i+1,j)) / 4.0_r8
...
elseif( var_type == cmaq%dom(id)%type_v ) then
   ...
   if( j == cmaq%dom(id)%var_size(2,cmaq%dom(id)%type_v) ) then
      ...
      if ( cmaq%dom(id)%polar ) then
         ...
         i1 = get_dart_vector_index(off,j-1,k  , domain_id(id),cmaq%dom(id)%type_gz)
         i2 = get_dart_vector_index(off,j-1,k+1, domain_id(id),cmaq%dom(id)%type_gz)
         i3 = get_dart_vector_index(i  ,j-1,k  , domain_id(id),cmaq%dom(id)%type_gz)
         i4 = get_dart_vector_index(i  ,j-1,k+1, domain_id(id),cmaq%dom(id)%type_gz)
         ...
         geop = minval(( (cmaq%dom(id)%phb(off,j-1,k  )+x_i1) &
                 +(cmaq%dom(id)%phb(off,j-1,k+1)+x_i2) &
                 +(cmaq%dom(id)%phb(i  ,j-1,k  )+x_i3) &
                 +(cmaq%dom(id)%phb(i  ,j-1,k+1)+x_i4) )/(4.0_r8*gravity))
         
         lat = ( cmaq%dom(id)%latitude(off,j-1)  &
                +cmaq%dom(id)%latitude(off,j-1)  &
                +cmaq%dom(id)%latitude(i  ,j-1)  &
                +cmaq%dom(id)%latitude(i  ,j-1)) / 4.0_r8
         ...
         ! If not periodic, then try extrapolating
         i1 = get_dart_vector_index(i,j-1,k ,  domain_id(id),cmaq%dom(id)%type_gz)
         i2 = get_dart_vector_index(i,j-1,k+1, domain_id(id),cmaq%dom(id)%type_gz)
         i3 = get_dart_vector_index(i,j-2,k  , domain_id(id),cmaq%dom(id)%type_gz)
         i4 = get_dart_vector_index(i,j-2,k+1, domain_id(id),cmaq%dom(id)%type_gz)
         ...
         geop = minval(( 3.0_r8*(cmaq%dom(id)%phb(i,j-1,k  )+x_i1) &
                 +3.0_r8*(cmaq%dom(id)%phb(i,j-1,k+1)+x_i2) &
                        -(cmaq%dom(id)%phb(i,j-2,k  )+x_i3) &
                        -(cmaq%dom(id)%phb(i,j-2,k+1)+x_i4) )/(4.0_r8*gravity))

         lat = ( 3.0_r8*cmaq%dom(id)%latitude(i,j-1)  &
                +3.0_r8*cmaq%dom(id)%latitude(i,j-1)  &
                       -cmaq%dom(id)%latitude(i,j-2)  &
                       -cmaq%dom(id)%latitude(i,j-2)) / 4.0_r8
   ...
   elseif( j == 1 ) then
      ...
      if ( cmaq%dom(id)%polar ) then
         ...
         i1 = get_dart_vector_index(off,j,k  , domain_id(id),cmaq%dom(id)%type_gz)
         i2 = get_dart_vector_index(off,j,k+1, domain_id(id),cmaq%dom(id)%type_gz)
         i3 = get_dart_vector_index(i  ,j,k  , domain_id(id),cmaq%dom(id)%type_gz)
         i4 = get_dart_vector_index(i  ,j,k+1, domain_id(id),cmaq%dom(id)%type_gz)
         ...
         geop = minval(( (cmaq%dom(id)%phb(off,j,k  )+x_i1) &
                 +(cmaq%dom(id)%phb(off,j,k+1)+x_i2) &
                 +(cmaq%dom(id)%phb(i  ,j,k  )+x_i3) &
                 +(cmaq%dom(id)%phb(i  ,j,k+1)+x_i4) )/(4.0_r8*gravity))
         
         lat = ( cmaq%dom(id)%latitude(off,j)  &
                +cmaq%dom(id)%latitude(off,j)  &
                +cmaq%dom(id)%latitude(i  ,j)  &
                +cmaq%dom(id)%latitude(i  ,j)) / 4.0_r8
...
elseif( var_type == cmaq%dom(id)%type_mu .or. &
        var_type == cmaq%dom(id)%type_ps .or. &
        var_type == cmaq%dom(id)%type_tsk) then

   model_height_distrib = cmaq%dom(id)%hgt(i,j)

elseif( var_type == cmaq%dom(id)%type_tslb  .or. &
        var_type == cmaq%dom(id)%type_smois .or. &
        var_type == cmaq%dom(id)%type_sh2o ) then

   model_height_distrib = cmaq%dom(id)%hgt(i,j) - cmaq%dom(id)%zs(k)

elseif( var_type == cmaq%dom(id)%type_u10 .or. &
        var_type == cmaq%dom(id)%type_v10 ) then

   model_height_distrib = cmaq%dom(id)%hgt(i,j) + 10.0_r8

elseif( var_type == cmaq%dom(id)%type_t2  .or. &
        var_type == cmaq%dom(id)%type_th2 .or. &
        var_type == cmaq%dom(id)%type_q2 ) then

   model_height_distrib = cmaq%dom(id)%hgt(i,j) + 2.0_r8
   ...
   geop = minval(( (cmaq%dom(id)%phb(i,j,k  )+x_i1) &
           +(cmaq%dom(id)%phb(i,j,k+1)+x_i2) )/(2.0_r8*gravity))
...
!   print *, 'APM: In model_height_w_distrib'
...
geop = (cmaq%dom(id)%phb(i,j,k) + x_i1)/gravity
model_height_w_distrib = compute_geometric_height(geop, cmaq%dom(id)%latitude(i, j))
...
!   print *, 'APM: In pert_model_copies'

   if (.not. allow_perturbed_ics) then
call error_handler(E_ERR,'pert_model_copies', &
                     'starting CMAQ model from a single vector requires additional steps', &
                  source, revision, revdate, &
                  text2='see comments in cmaq/model_mod.f90::pert_model_copies()')
endif
...
num_variables = 0
do id = 1, num_domains
  num_variables = num_variables + cmaq%dom(id)%number_of_cmaq_variables
enddo
...
!
! This routines has been taken "as is" from cmaq_user_fortran_util_0.f
!
...
do while (.not. dom_found)
   ...
   if( (cmaq%dom(id)%proj%hemi ==  1.0_r8 .and. obslat < -90.0_r8) .or. &
       (cmaq%dom(id)%proj%hemi == -1.0_r8 .and. obslat >  90.0_r8) .or. &
       (cmaq%dom(id)%proj%code == PROJ_MERC .and. abs(obslat) > 90.0_r8) ) then
      ...
      call latlon_to_ij(cmaq%dom(id)%proj,min(max(obslat,-89.9999999_r8),89.9999999_r8),obslon,iloc,jloc)

      ! Array bound checking depends on whether periodic or not -- these are
      !   real-valued indices here, so we cannot use boundsCheck  :( 

      if ( cmaq%dom(id)%periodic_x .and. .not. cmaq%dom(id)%periodic_y  ) then
         if ( cmaq%dom(id)%polar ) then        
            !   Periodic     X & M_grid ==> [1 we+1)    
            !   Periodic     Y & M_grid ==> [0.5 sn+0.5]
            if ( iloc >= 1.0_r8 .and. iloc <  real(cmaq%dom(id)%we,r8)+1.0_r8 .and. &
                 jloc >= 0.5_r8 .and. jloc <= real(cmaq%dom(id)%sn,r8)+0.5_r8 ) &
                 dom_found = .true.     
         else
            !   Periodic     X & M_grid ==> [1 we+1)    
            !   NOT Periodic Y & M_grid ==> [1 sn]
            if ( iloc >= 1.0_r8 .and. iloc <  real(cmaq%dom(id)%we,r8)+1.0_r8 .and. &
                 jloc >= 1.0_r8 .and. jloc <= real(cmaq%dom(id)%sn,r8) ) &
                 dom_found = .true.
         endif

      elseif ( cmaq%dom(id)%periodic_x .and. cmaq%dom(id)%periodic_y ) then
            !   Periodic     X & M_grid ==> [1 we+1)    
            !   Periodic     Y & M_grid ==> [1 sn+1]
            if ( iloc >= 1.0_r8 .and. iloc <  real(cmaq%dom(id)%we,r8)+1.0_r8 .and. &
                 jloc >= 1.0_r8 .and. jloc <= real(cmaq%dom(id)%sn,r8)+1.0_r8 ) &
                 dom_found = .true.
         ...
         if ( cmaq%dom(id)%polar ) then        
            ...
            !   Periodic     Y & M_grid ==> [0.5 sn+0.5]
            if ( iloc >= 1.0_r8 .and. iloc <= real(cmaq%dom(id)%we,r8) .and. &
                 jloc >= 0.5_r8 .and. jloc <= real(cmaq%dom(id)%sn,r8)+0.5_r8 ) &
                 dom_found = .true.     
         else
            !   NOT Periodic X & M_grid ==> [1 we]    
...
if (vertical_localization_on()) then
   if (base_which /= cmaq%dom(1)%localization_coord) then
      !print*, 'base_which ', base_which, 'loc coord ', cmaq%dom(1)%localization_coord
...
if (istatus1 == 0) then
   ...
   do k = 1, num_close
      ...
      if (vertical_localization_on()) then
         if (local_which /= cmaq%dom(1)%localization_coord) then
...
!  print *, 'APM: In boundsCheck'
  ...
  if ( dim == 1 ) then
     ...
     if ( periodic ) then
        ...
        if ( ind >= 1 .and. ind < cmaq%dom(id)%wes ) boundsCheck = .true.
        ...
        !   for dim == 1 stored in cmaq%dom(id)
        if ( cmaq%dom(id)%var_size(dim,type) == cmaq%dom(id)%wes ) then
           ! U-grid allows integer range of [1 wes)
           if ( ind >= 1 .and. ind < cmaq%dom(id)%wes ) boundsCheck = .true.
        else  
           ! M & V-grid allow [1 we)
           if ( ind >= 1 .and. ind < cmaq%dom(id)%we ) boundsCheck = .true.
        endif
...
!            be added in later.  
        ...
        ! Check staggering by comparing var_size(dim,type) to the staggered dimension 
        !   for dim == 2 stored in cmaq%dom(id)
        if ( cmaq%dom(id)%var_size(dim,type) == cmaq%dom(id)%sns ) then
           ! V-grid allows integer range [1 sns)
           if ( ind >= 1 .and. ind < cmaq%dom(id)%sns ) boundsCheck = .true.
        else  
           ...
           if ( restrict_polar ) then
              ...
              if ( ind >= 1 .and. ind < cmaq%dom(id)%sn ) boundsCheck = .true.
           else
              ! M & U-grid allow integer range [0 sns) in unrestricted circumstances
              if ( ind >= 0 .and. ind < cmaq%dom(id)%sns ) boundsCheck = .true.
           endif
        endif
  ...
  elseif ( dim == 3 ) then
     ...
     if ( cmaq%dom(id)%var_size(dim,type) == cmaq%dom(id)%bts ) then
        ! W vertical grid allows [1 bts)
        if ( ind >= 1 .and. ind < cmaq%dom(id)%bts ) boundsCheck = .true.
     else
        ! M vertical grid allows [1 bt)
        if ( ind >= 1 .and. ind < cmaq%dom(id)%bt ) boundsCheck = .true.
     endif
...
!     print*, 'model_mod.f90 :: getCorners :: Tried to do polar bc -- rc = ', rc
  ...
  if ( cmaq%dom(id)%periodic_x ) then
     ...
     if ( cmaq%dom(id)%var_size(1,type) == cmaq%dom(id)%wes ) then
...
!            be added in later.
  ...
  if ( cmaq%dom(id)%polar ) then
     ...
     if ( cmaq%dom(id)%var_size(2,type) == cmaq%dom(id)%sns ) then
        ...
        if ( restrict_polar ) then 
           ...
           if ( j == cmaq%dom(id)%sn ) then
              ! j > sn should be mapped to j = sn (ul is on other side of globe)
              ul(2) = cmaq%dom(id)%sn
     
              ! Need to map i index 180 degrees away
              ul(1) = ll(1) + cmaq%dom(id)%we/2
     
              ! Check validity of bounds & adjust by periodicity if necessary
              if ( ul(1) > cmaq%dom(id)%we ) ul(1) = ul(1) - cmaq%dom(id)%we
...
!              print*, 'model_mod.f90 :: getCorners :: Tried to do polar bc -- rc = ', rc
  ...
  elseif ( cmaq%dom(id)%periodic_y ) then
...
!            be added in later.
  ...
  if ( cmaq%dom(id)%polar .and. .not. restrict_polar ) then

     ! Only if j == 0 or j == sn
     if ( j == 0 .or. j ==  cmaq%dom(id)%sn) then
...
subroutine read_cmaq_dimensions(ncid,bt,bts,bt1,sn,we)

! ncid: input, file handl
! id:   input, domain id

integer, intent(in)            :: ncid
integer, intent(out)           :: bt,bts,bt1,sn,we
logical, parameter             :: debug = .false.
integer                        :: var_id 
character (len=NF90_MAX_NAME)  :: name

! get cmaq grid dimensions

!  print *, 'APM: In read_cmaq_dimensions'

  call nc_check( nf90_inq_dimid(ncid, "LAY", var_id), &
   ...
   bts=bt+1
   call nc_check( nf90_inq_dimid(ncid, "LAY_2D", var_id), &
                     'static_init_model','inq_dimid lay_2d')
   call nc_check( nf90_inquire_dimension(ncid, var_id, name, bt1), &
                     'static_init_model','inquire_dimension '//trim(name))
...
!                     'static_init_model','inquire_dimension '//trim(name))
   ...
   call nc_check( nf90_inq_dimid(ncid, "ROW", var_id), &
...
!                     'static_init_model','inquire_dimension '//trim(name))
   ...
   call nc_check( nf90_inq_dimid(ncid, "COL", var_id), &
...
end subroutine read_cmaq_dimensions

!--------------------------------------------------------------------
!--------------------------------------------------------------------

subroutine read_cmaq_file_attributes(ncid,id)
...
!   print *, 'APM: In read_cmaq_file_attributes'

   call nc_check( nf90_get_att(ncid, nf90_global, 'XCELL', cmaq%dom(id)%dx), &
                     'static_init_model', 'get_att XCELL')
   call nc_check( nf90_get_att(ncid, nf90_global, 'YCELL', cmaq%dom(id)%dy), &
                     'static_init_model', 'get_att YCELL')
   call nc_check( nf90_get_att(ncid, nf90_global, 'TSTEP', cmaq%dom(id)%dt), &
                     'static_init_model', 'get_att TSTEP')
   write(errstring, *) 'dt from cmaqinput_d0X file is: ', cmaq%dom(id)%dt
   call error_handler(E_MSG, ' ', errstring)
   if(debug) write(*,*) ' dx, dy, dt are ',cmaq%dom(id)%dx, &
        cmaq%dom(id)%dy, cmaq%dom(id)%dt

   call nc_check( nf90_get_att(ncid, nf90_global, 'GDTYP', cmaq%dom(id)%map_proj), &
                     'static_init_model', 'get_att GDTYP')
   if(debug) write(*,*) ' map_proj is ',cmaq%dom(id)%map_proj

   call nc_check( nf90_get_att(ncid, nf90_global, 'YCENT', cmaq%dom(id)%cen_lat), &
                     'static_init_model', 'get_att YCENT')
   if(debug) write(*,*) ' cen_lat is ',cmaq%dom(id)%cen_lat

   call nc_check( nf90_get_att(ncid, nf90_global, 'XCENT', cmaq%dom(id)%cen_lon), &
                     'static_init_model', 'get_att XCENT')
   if(debug) write(*,*) ' cen_lon is ',cmaq%dom(id)%cen_lon

   call nc_check( nf90_get_att(ncid, nf90_global, 'P_ALP', truelat1), &
                     'static_init_model', 'get_att P_ALP')
   if(debug) write(*,*) ' truelat1 is ',truelat1

   call nc_check( nf90_get_att(ncid, nf90_global, 'P_BET', truelat2), &
                     'static_init_model', 'get_att P_BET')
   if(debug) write(*,*) ' truelat2 is ',truelat2

   call nc_check( nf90_get_att(ncid, nf90_global, 'P_GAM', stdlon), &
                     'static_init_model', 'get_att P_GAM')
   if(debug) write(*,*) ' stdlon is ',stdlon

   RETURN

end subroutine read_cmaq_file_attributes
...
!    print *, 'APM: In assign_boundary_conditions'

   if ( id == 1 ) then
      cmaq%dom(id)%periodic_x = periodic_x
      cmaq%dom(id)%periodic_y = periodic_y
      cmaq%dom(id)%polar = polar
      cmaq%dom(id)%scm = scm
   else
      cmaq%dom(id)%periodic_x = .false.
      cmaq%dom(id)%periodic_y = .false.
      cmaq%dom(id)%polar = .false.      
      cmaq%dom(id)%scm = .false.      
   endif
   if(debug) write(*,*) ' periodic_x ',cmaq%dom(id)%periodic_x
   if(debug) write(*,*) ' periodic_y ',cmaq%dom(id)%periodic_y
   if(debug) write(*,*) ' polar ',cmaq%dom(id)%polar
   if(debug) write(*,*) ' scm   ',cmaq%dom(id)%scm   
...
!   print *, 'APM: In read_cmaq_static_data'

! VGTOP - pressure at full level grid top
!   write(*,*) 'APM: Read VGTOP '
   call nc_check( nf90_get_att(ncid, nf90_global, 'VGTOP', cmaq%dom(id)%p_top), &
                     'static_init_model', 'get_att VGTOP')

! PRSFC - pressure at full level grid bottom (surface pressure)
!   write(*,*) 'APM: Read PRSFVC '
   allocate(cmaq%dom(id)%prs_sfc(cmaq%dom(id)%we,cmaq%dom(id)%sn))
   call nc_check( nf90_inq_varid(ncid, "PRSFC", var_id), &
                     'read_cmaq_static_data','inq_varid PRSFC')
   call nc_check( nf90_get_var(ncid, var_id, cmaq%dom(id)%prs_sfc), &
...
!   write(*,*) 'APM: Read ZN '
   ...
   call nc_check( nf90_inq_varid(ncid, "ZH", var_id), &
                     'read_cmaq_static_data','inq_varid ZH')
   call nc_check( nf90_get_var(ncid, var_id, cmaq%dom(id)%zh), &
...
!   write(*,*) 'APM: Read ZF '
   ...
   call nc_check( nf90_inq_varid(ncid, "ZF", var_id), &
                     'read_cmaq_static_data','inq_varid ZF')
   call nc_check( nf90_get_var(ncid, var_id, cmaq%dom(id)%zf), &
...
!   write(*,*) 'APM: Read PRES '
   ...
   call nc_check( nf90_inq_varid(ncid, "PRES", var_id), &
                     'read_cmaq_static_data','inq_varid PRES')
   call nc_check( nf90_get_var(ncid, var_id, cmaq%dom(id)%prs), &
                     'read_cmaq_static_data','get_var PRES')
   if(debug) write(*,*) ' pres is ',cmaq%dom(id)%prs

! DENS - density at half (mass velocity) levels
!   write(*,*) 'APM: Read DENS '
   allocate(cmaq%dom(id)%dens(cmaq%dom(id)%we,cmaq%dom(id)%sn,cmaq%dom(id)%bt))
   call nc_check( nf90_inq_varid(ncid, "DENS", var_id), &
                     'read_cmaq_static_data','inq_varid DENS')
   call nc_check( nf90_get_var(ncid, var_id, cmaq%dom(id)%dens), &
                     'read_cmaq_static_data','get_var DENS')
   if(debug) write(*,*) ' dens is ',cmaq%dom(id)%dens

! LAT
!   write(*,*) 'APM: Read LAT '
   allocate(cmaq%dom(id)%latitude(cmaq%dom(id)%we,cmaq%dom(id)%sn))
   call nc_check( nf90_inq_varid(ncid, "LAT", var_id), &
...
!   write(*,*) 'APM: Read LON '
   ...
   call nc_check( nf90_inq_varid(ncid, "LON", var_id), &
...
!   write(*,*) 'APM: Read LWMASK '
   allocate(cmaq%dom(id)%land(cmaq%dom(id)%we,cmaq%dom(id)%sn))
   call nc_check( nf90_inq_varid(ncid, "LWMASK", var_id), &
...
!   write(*,*) 'APM: Read HT '
   ...
   call nc_check( nf90_inq_varid(ncid, "HT", var_id), &
...
!   print *, 'APM: In setup_map_projection'
   call map_init(cmaq%dom(id)%proj)

! Populate the map projection structure

!nc -- added in case structures for CASSINI and CYL
!nc -- global cmaqinput_d01 has truelat1 = 1.e20, so we need to change it where needed
...
!JPH --- this latinc/loninc calculations are only valid for global domains
   ...
   if ( cmaq%dom(id)%scm ) then
! JPH -- set to zero which should cause the map utils to return NaNs if called
      latinc = 0.0_r8 
      loninc = 0.0_r8 
   else
      latinc = 180.0_r8/cmaq%dom(id)%sn
      loninc = 360.0_r8/cmaq%dom(id)%we
   endif

   if(cmaq%dom(id)%map_proj == map_latlon) then
      truelat1 = latinc
      stdlon = loninc
      proj_code = PROJ_LATLON
   elseif(cmaq%dom(id)%map_proj == map_lambert) then
      proj_code = PROJ_LC
   elseif(cmaq%dom(id)%map_proj == map_polar_stereo) then
      proj_code = PROJ_PS
   elseif(cmaq%dom(id)%map_proj == map_mercator) then
      proj_code = PROJ_MERC
!   elseif(cmaq%dom(id)%map_proj == map_cyl) then
!      proj_code = PROJ_CYL
!   elseif(cmaq%dom(id)%map_proj == map_cassini) then
!      proj_code = PROJ_CASSINI
   else
      call error_handler(E_ERR,'static_init_model', &
        'Map projection not supported.', source, revision, revdate)
   endif
...
!
   call map_set( proj_code=proj_code, &
                 proj=cmaq%dom(id)%proj, &
                 lat1=cmaq%dom(id)%latitude(1,1), &
                 lon1=cmaq%dom(id)%longitude(1,1), &
                 lat0=90.0_r8, &
                 lon0=0.0_r8, &
                 knowni=1.0_r8, &
                 knownj=1.0_r8, &
...
!print *, 'APM: In fill_default_state_table'
default_table = 'NULL'
row = 0
...
!print *, 'APM: In concatenate_variable_tables'
irow = 0
...
character(len=*), intent(in) :: cmaq_state_variables(:, :)
logical, intent(inout)       :: in_state_vector(:)

integer :: row, i, nextkind

!print *, 'APM: In fill_dart_kinds_table'
in_state_vector = .false.

row = size(cmaq_state_variables, 2)
...
do i = 1, row
   ...
   ! end of the list?
   if (cmaq_state_variables(2, i) == 'NULL') exit
   ...
   case (-1, 0) 
      write(errstring, *) 'unrecognized KIND string: ' // trim(cmaq_state_variables(2, i))
   ...
   if ( cmaq_state_variables(1, i) == 'PSFC' ) then
...
! directly from the cmaqinput template file, and does not vary from
...
integer function get_number_of_cmaq_variables(id, state_table, var_element_list, var_update_list)
...
!print *, 'APM: In get_number_of_cmaq_variables'
...
if ( debug ) then
  print*,'In function get_number_of_cmaq_variables'
  print*,'Found ',num_vars,' state variables for domain id ',id
endif

get_number_of_cmaq_variables = num_vars 

return

end function get_number_of_cmaq_variables 
...
!   print *, 'APM: In set_variable_bound_defaults'
   lb(:) = missing_r8
   ub(:) = missing_r8
   instructions(:) = 'NULL'
...
subroutine get_variable_bounds(bounds_table,cmaq_var_name,lb,ub,instructions)

! matches CMAQ variable name in bounds table to input name, and assigns
! the bounds and instructions if they exist
   ...
   character(len=50)               :: cmaq_varname_trim, bounds_varname_trim
   character(len=50)               :: bound_trim
   integer :: ivar
   logical :: debug = .false.

!   print *, 'APM: In get_variable_bounds'
   cmaq_varname_trim = ''
   cmaq_varname_trim = trim(cmaq_var_name)
   ...
   do while ( trim(bounds_table(1,ivar)) /= 'NULL' ) 
     ...
     if ( bounds_varname_trim == cmaq_varname_trim ) then
        ...
        if ( instructions == 'NULL' ) then
          call error_handler(E_ERR,'get_variable_bounds','instructions for bounds on '//cmaq_varname_trim//' are required',&
                                    source,revision,revdate)
        endif
   ...
   if ( debug ) then
      write(*,*) 'In get_variable_bounds assigned ',cmaq_varname_trim
...
!print *, 'APM: In variable_is_on_domain'
...
subroutine get_variable_size_from_file(ncid,id,cmaq_var_name,bt,bt1,sn,we,stagger,var_size)
...
integer,           intent(in)   :: ncid,id
integer,           intent(in)   :: bt, bt1, sn, we
character(len=*),  intent(in)   :: cmaq_var_name
...
!   print *, 'APM: In get_variable_size_from_file'
   stagger = ''
...
! get number of dimensions and dimension IDs
   call nc_check( nf90_inquire_variable(ncid, var_id,ndims=ndims,  &
                     dimids=dimids),                               &
                     'get_variable_size_from_file',                &
                     'inquire_variable '//cmaq_var_name)
...
! get dimension length, ignoring first dimension (time)
   do idim = 1,ndims-1
      call nc_check( nf90_inquire_dimension(ncid, dimids(idim),  &
                     len=var_size(idim)),                               &
                     'get_variable_size_from_file',                &
                     'inquire_dimension '//cmaq_var_name)
   enddo
...
subroutine get_variable_metadata_from_file(ncid, &
...
!   print *, 'APM: In get_variable_metadata_from_file'
   call nc_check( nf90_inq_varid(ncid, trim(cmaq_var_name), var_id), &
...
integer function get_type_ind_from_type_string(id, cmaq_varname)

! simply loop through the state variable table to get the index of the
! type for this domain.  returns -1 if not found

   integer,          intent(in) :: id
   character(len=*), intent(in) :: cmaq_varname

   integer                      :: ivar, my_index
   logical                      :: debug = .false.
   character(len=50)            :: cmaq_varname_trim, cmaq_state_var_trim

!   print *, 'APM: In get_type_ind_from_type_string'
   get_type_ind_from_type_string = -1

   do ivar = 1,cmaq%dom(id)%number_of_cmaq_variables
      
      my_index =  cmaq%dom(id)%var_index_list(ivar)

      cmaq_state_var_trim = trim(cmaq_state_variables(1,my_index))
      cmaq_varname_trim   = trim(cmaq_varname)
      
      if ( cmaq_state_var_trim == cmaq_varname_trim ) then
   ...
   if ( debug ) write(*,*) 'get_type_from_ind ',trim(cmaq_varname),' ',get_type_ind_from_type_string
...
subroutine get_cmaq_date (tstring, year, month, day, hour, minute, second)

integer,           intent(out) :: year, month, day, hour, minute, second
character(len=19), intent(in)  :: tstring

!print *, 'APM: In get_cmaqq_date'
...
end subroutine get_cmaq_date

!----------------------------------------------------------------------
! Returns integers taken from tstring
! It is assumed that the tstring char array is as YYYY-MM-DD_hh:mm:ss

subroutine set_cmaq_date (tstring, year, month, day, hour, minute, second)
...
!print *, 'APM: In height_diff_check'
...
!   print *, 'APM: In compute_geometric_height'
...
subroutine simple_interp_distrib(fld, cmaq, id, i, j, k, obs_kind, dxm, dx, dy, dym, uniquek, ens_size, state_handle)

integer,             intent(in) :: ens_size
type(ensemble_type), intent(in) :: state_handle
type(cmaq_dom),       intent(in) :: cmaq
integer,             intent(in) :: id
...
!print *, 'APM: In simple_interp_distrib'
! Confirm that the obs kind is in the DART state vector and return the cmaq_type
!>@todo should boundsCheck always be temperatue type? This is what it is in the original code
call obs_kind_in_state_vector(in_state, cmaq_type, obs_kind, id)
...
if ( in_state ) then
   ...
   ! Check to make sure retrieved integer gridpoints are in valid range
   if ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq_type ) .and.&
      boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq_type ) .and. &
      boundsCheck( uniquek(uk), .false.,                id, dim=3, type=cmaq_type ) ) then
         
         call getCorners(i, j, id, cmaq_type, ll, ul, lr, ur, rc )
         if ( rc .ne. 0 ) &
         print*, 'model_mod.f90 :: model_interpolate :: getCorners QNSNOW rc = ', rc
               
         ! Interpolation for QNSNOW field at level k
         ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk), domain_id(id), cmaq_type)
         iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk), domain_id(id), cmaq_type)
         ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk), domain_id(id), cmaq_type)
         iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk), domain_id(id), cmaq_type)
         ...
         ill = get_dart_vector_index(ll(1), ll(2), uniquek(uk)+1, domain_id(id), cmaq_type)
         iul = get_dart_vector_index(ul(1), ul(2), uniquek(uk)+1, domain_id(id), cmaq_type)
         ilr = get_dart_vector_index(lr(1), lr(2), uniquek(uk)+1, domain_id(id), cmaq_type)
         iur = get_dart_vector_index(ur(1), ur(2), uniquek(uk)+1, domain_id(id), cmaq_type)
...
subroutine surface_interp_distrib(fld, cmaq, id, i, j, obs_kind, cmaq_surf_type, dxm, dx, dy, dym, ens_size, state_handle)
...
integer,             intent(in) :: id
integer,             intent(in) :: obs_kind
integer,             intent(in) :: cmaq_surf_type !< this has to be passed in is beccause obs_kind does not give a unique cmaq_type ( could check obs_kind then if surf var?)
integer,             intent(in) :: i,j
real(r8),            intent(in) :: dxm, dx, dy, dym
real(r8),           intent(out) :: fld(2, ens_size)
...
!print *, 'APM: In surface_interp_distrib'
fld(:,:) = missing_r8

! Find the cmaq_type from the obs kind
...
! Check to make sure retrieved integer gridpoints are in valid range
if ( ( boundsCheck( i, cmaq%dom(id)%periodic_x, id, dim=1, type=cmaq_type ) .and. &
       boundsCheck( j, cmaq%dom(id)%polar,      id, dim=2, type=cmaq_type ) ) &
       .or. cmaq%dom(id)%scm ) then
  ...
  if ( rc .ne. 0 ) &
     ...
     ill = get_dart_vector_index(ll(1), ll(2), 1, domain_id(id), cmaq_surf_type)
     iul = get_dart_vector_index(ul(1), ul(2), 1, domain_id(id), cmaq_surf_type)
     ilr = get_dart_vector_index(lr(1), lr(2), 1, domain_id(id), cmaq_surf_type)
     iur = get_dart_vector_index(ur(1), ur(2), 1, domain_id(id), cmaq_surf_type)
...
!--------------------------------------------------------------------------
!> test if an obs kind is in the state vector and set cmaq_type
subroutine obs_kind_in_state_vector(part_of_state_vector, cmaq_type, obs_kind, id)

logical, intent(out) :: part_of_state_vector
integer, intent(out) :: cmaq_type !< CMAQ
integer, intent(in)  :: obs_kind !< DART
integer, intent(in)  :: id


!print *, 'APM: In obs_kind_in_state_vector'
part_of_state_vector = .false. ! assume not in state vector


if    ( ( obs_kind == QTY_VERTICAL_VELOCITY)             .and. ( cmaq%dom(id)%type_w >= 0 ) )  then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_w
else if( ( obs_kind == QTY_RAINWATER_MIXING_RATIO )      .and. ( cmaq%dom(id)%type_qr >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_qr
else if( ( obs_kind == QTY_GRAUPEL_MIXING_RATIO )        .and. ( cmaq%dom(id)%type_qg >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_qg
else if( ( obs_kind == QTY_HAIL_MIXING_RATIO )           .and. ( cmaq%dom(id)%type_qh >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_qh
else if( ( obs_kind == QTY_SNOW_MIXING_RATIO )           .and. ( cmaq%dom(id)%type_qs >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_qs
else if( ( obs_kind == QTY_ICE_MIXING_RATIO )                   .and. ( cmaq%dom(id)%type_qi >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_qi
else if( ( obs_kind == QTY_CLOUDWATER_MIXING_RATIO )          .and. ( cmaq%dom(id)%type_qc >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_qc
else if ( ( obs_kind == QTY_CLOUD_FRACTION )             .and. ( cmaq%dom(id)%type_cldfra >= 0 ) )then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_cldfra
else if( ( obs_kind == QTY_DROPLET_NUMBER_CONCENTR )     .and. ( cmaq%dom(id)%type_qndrp >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_qndrp
else if( ( obs_kind == QTY_ICE_NUMBER_CONCENTRATION )    .and. ( cmaq%dom(id)%type_qnice >= 0 ) )then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_qnice
else if( ( obs_kind == QTY_SNOW_NUMBER_CONCENTR )        .and. ( cmaq%dom(id)%type_qnsnow >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_qnsnow
else if( ( obs_kind == QTY_RAIN_NUMBER_CONCENTR )        .and. ( cmaq%dom(id)%type_qnrain >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_qnrain
else if( ( obs_kind == QTY_GRAUPEL_NUMBER_CONCENTR )     .and. ( cmaq%dom(id)%type_qngraupel >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_qngraupel
else if( ( obs_kind == QTY_HAIL_NUMBER_CONCENTR )        .and. ( cmaq%dom(id)%type_qnhail >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_qnhail
else if( ( obs_kind == QTY_CONDENSATIONAL_HEATING )      .and. ( cmaq%dom(id)%type_hdiab >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_hdiab
else if( ( obs_kind == QTY_POWER_WEIGHTED_FALL_SPEED )   .and. ( cmaq%dom(id)%type_fall_spd >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_fall_spd
else if( ( obs_kind == QTY_RADAR_REFLECTIVITY )          .and. ( cmaq%dom(id)%type_refl >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_refl
else if( ( obs_kind == QTY_DIFFERENTIAL_REFLECTIVITY )   .and. ( cmaq%dom(id)%type_dref >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type =  cmaq%dom(id)%type_dref
else if( ( obs_kind == QTY_SPECIFIC_DIFFERENTIAL_PHASE ) .and. ( cmaq%dom(id)%type_spdp >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_spdp
else if ( ( obs_kind == QTY_VAPOR_MIXING_RATIO )         .and. ( cmaq%dom(id)%type_qv >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_qv
else if ( ( obs_kind == QTY_TEMPERATURE )                  .and. ( cmaq%dom(id)%type_t >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_t
else if ( ( obs_kind == QTY_POTENTIAL_TEMPERATURE )        .and. ( cmaq%dom(id)%type_t >= 0 ) ) then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_t
else if ( ( obs_kind == QTY_SKIN_TEMPERATURE )              .and. ( cmaq%dom(id)%type_tsk >= 0 ) )then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_tsk
else if ( ( obs_kind == QTY_GEOPOTENTIAL_HEIGHT )        .and. ( cmaq%dom(id)%type_gz >= 0 ) )then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_gz
else if ( ( obs_kind == QTY_2M_TEMPERATURE )        .and. ( cmaq%dom(id)%type_t2 >= 0 ) )then
   part_of_state_vector = .true.
   cmaq_type = cmaq%dom(id)%type_t2
else
...
!print *, 'APM: In query_vert_localization_coord'
...
call get_cmaq_date(timestring, year, month, day, hour, minute, second)
...
!print *, 'APM: In write_model_time'
call get_date(dart_time, year, month, day, hour, minute, second)
call set_cmaq_date(timestring, year, month, day, hour, minute, second)
...
!print *, 'APM: In static_data_sizes'
print*
print*, '******** cmaq_static_data_for_dart domain ',domain
print*, 'znu, dn, dnw, zs, znw ', size(cmaq%dom(domain)%znu), &
                                  size(cmaq%dom(domain)%dn ), &
                                  size(cmaq%dom(domain)%dnw), &
                                  size(cmaq%dom(domain)%zs ), &
                                  size(cmaq%dom(domain)%znw)

print*, 'mub, hgt ', size(cmaq%dom(domain)%mub), size(cmaq%dom(domain)%hgt)

print*, 'latitude, latitude_u, latitude_v ', size(cmaq%dom(domain)%latitude), &
                                             size(cmaq%dom(domain)%latitude_u), &
                                             size(cmaq%dom(domain)%latitude_v)

print*, 'longitude, longitude_u, longitude_v ', size(cmaq%dom(domain)%longitude), &
                                                size(cmaq%dom(domain)%longitude_u), &
                                                size(cmaq%dom(domain)%longitude_v)

print*, 'phb             ', size(cmaq%dom(domain)%phb)
print*, 'var_index       ', size(cmaq%dom(domain)%var_index)
print*, 'var_size        ', size(cmaq%dom(domain)%var_size)
print*, 'var_type        ', size(cmaq%dom(domain)%var_type)
print*, 'var_index_list  ', size(cmaq%dom(domain)%var_index_list)
print*, 'var_update_list ', size(cmaq%dom(domain)%var_update_list)
print*, 'dart_kind       ', size(cmaq%dom(domain)%dart_kind)
print*, 'land            ', size(cmaq%dom(domain)%land)

print*, 'lower_bound,upper_bound ', size(cmaq%dom(domain)%lower_bound), &
                                    size(cmaq%dom(domain)%upper_bound)
