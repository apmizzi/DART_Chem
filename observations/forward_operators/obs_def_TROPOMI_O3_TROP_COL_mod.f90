...
! TROPOMI_O3_TROP_COL, QTY_O3
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_tropomi_o3_trop_col_mod, only : get_expected_tropomi_o3_trop_col, &
!                                  read_tropomi_o3_trop_col, &
!                                  write_tropomi_o3_trop_col, &
!                                  interactive_tropomi_o3_trop_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(TROPOMI_O3_TROP_COL)                                                           
!         call get_expected_tropomi_o3_trop_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(TROPOMI_O3_TROP_COL)
!         call read_tropomi_o3_trop_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(TROPOMI_O3_TROP_COL)
!         call write_tropomi_o3_trop_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(TROPOMI_O3_TROP_COL)
!         call interactive_tropomi_o3_trop_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_tropomi_o3_trop_col_mod
...
! Record the namelist values
   ...
   nlayer_tropomi=nlayer_tropomi_o3_trop_col
...
! Check for valid values
   ...
   allocate(    trop_top_prs(max_tropomi_o3_obs))
   allocate(  pressure(max_tropomi_o3_obs,nlayer_tropomi+1))
   allocate(avg_kernel(max_tropomi_o3_obs,nlayer_tropomi))
   allocate(     prior(max_tropomi_o3_obs,nlayer_tropomi))

end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_tropomi_o3_trop_col(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   trop_top_prs_1 = read_r8_scalar( ifile, fileformat, 'trop_top_prs_1')
   ...
   if(counts1 > max_tropomi_o3_obs) then
      ...
      call error_handler(E_ERR,'read_tropomi_o3_trop_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_tropomi_o3_trop_col(key, pressure_1, avg_kernel_1,prior_1, trop_top_prs_1, nlayer_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)

end subroutine read_tropomi_o3_trop_col

!-------------------------------------------------------------------------------

subroutine write_tropomi_o3_trop_col(key, ifile, fform)
...
! you can come extend the context strings to include the key if desired.
   ...
   call write_r8_scalar(ifile,                trop_top_prs(key), fileformat,'trop_top_prs')
...
end subroutine write_tropomi_o3_trop_col

!-------------------------------------------------------------------------------

subroutine interactive_tropomi_o3_trop_col(key)
...
! STOP because routine is not finished.
   ...
   call error_handler(E_ERR, 'interactive_tropomi_o3_trop_col', string1, source)
   
   if(num_tropomi_o3_obs >= max_tropomi_o3_obs) then
      write(string1, *)'Not enough space for an tropomi o3 obs.'
      write(string2, *)'Can only have max_tropomi_o3_obs (currently ',max_tropomi_o3_obs,')'
      call error_handler(E_ERR, 'interactive_tropomi_o3_trop_col', string1, &
...
end subroutine interactive_tropomi_o3_trop_col

!-------------------------------------------------------------------------------

subroutine get_expected_tropomi_o3_trop_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_tropomi_o3_trop_col'
...
! Use large scale ozone data above the regional model top
   do imem=1,ens_size
      ...
      enddo
      ncnt=level_tropomi-kstart+1
      allocate(prs_tropomi_top(ncnt))
      allocate(o3_prf_mdl(ncnt),tmp_prf_mdl(ncnt),qmr_prf_mdl(ncnt))
      do k=kstart,level_tropomi
         prs_tropomi_top(k-kstart+1)=prs_tropomi(k)
      enddo
...
!
      ...
      call get_upper_bdy_fld(fld,model,data_file,25,21,88,57,lon_obs,lon_obs,lat_obs,prs_tropomi_top,ncnt, &
...
!
!   do k=1,level_tropomi
!      print *, k,o3_val(1,k),o3_val(int(ens_size/2),k),o3_val(ens_size,k)
!   enddo
!
! Find index for TROPOMI's top of troposphere   
    write(string1, *) 'APM: TROPOMI pressure at top of troposphere  ',trop_top_prs(key)
    call error_handler(E_MSG, routine, string1, source)
    do k=1,level_tropomi
       write(string1, *) 'APM: TROPOMI pressure  ',prs_tropomi(k)
       call error_handler(E_MSG, routine, string1, source)
    enddo
!
   istatus=0
...
end subroutine get_expected_tropomi_o3_trop_col

!-------------------------------------------------------------------------------

subroutine set_obs_def_tropomi_o3_trop_col(key, o3_pressure, o3_avg_kernel, o3_prior, o3_trop_top_prs, o3_nlayer)

   integer,                           intent(in)   :: key, o3_nlayer
   real(r8),                          intent(in)   :: o3_trop_top_prs
   ...
   if(num_tropomi_o3_obs >= max_tropomi_o3_obs) then
      write(string1, *)'Not enough space for tropomi o3 obs.'
      write(string2, *)'Can only have max_tropomi_o3_obs (currently ',max_tropomi_o3_obs,')'
      call error_handler(E_ERR,'set_obs_def_tropomi_o3_trop_col',string1,source,revision, &
      revdate,text2=string2)
   endif
   
   nlayer(key) = o3_nlayer
   trop_top_prs(key) = o3_trop_top_prs
...
end subroutine set_obs_def_tropomi_o3_trop_col

end module obs_def_tropomi_o3_trop_col_mod
