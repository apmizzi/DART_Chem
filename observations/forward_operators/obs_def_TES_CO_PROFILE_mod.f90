...
subroutine get_expected_tes_co_profile(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   real(r8) :: co_val_conv, co_term,VMR_conv
...
! TES vertical is from bottom to top in Pa
!
   do imem=1,ens_size
!!      do k=1,layer_tes
...
!!         prs_tes_top(k-kstart+1)=prs_tes(k)/100.
!!      enddo
!!!
...
!!      do k=kstart,layer_tes
!!         co_val(imem,k)=co_prf_mdl(k-kstart+1)*co_val(imem,kstart-1)/ &
...
!
!            if(prs_tes(k).le.prs_mdl_1(imem) .and. prs_tes(k).ge.prs_mdl_2(imem)) then
            if(prs_tes(k).le.prs_mdl_1(imem) .and. prs_tes(k).ge.(prs_mdl_2(imem)-20000.)) then
...
!            elseif(prs_tes(k).le.prs_mdl_nm(imem) .and. prs_tes(k).ge.prs_mdl_n(imem)) then
...
!               if(qmr_val(imem,k).lt.0.) qmr_val(imem,k)=qmr_mdl_n(imem)
               ...
               write(string1, *) &
               'APM: CO VAL ',(co_val(imem,kk),kk=1,layer_tes)
               call error_handler(E_ALLMSG, routine, string1, source)
               write(string1, *) &
               'APM: TM VAL ',(tmp_val(imem,kk),kk=1,layer_tes)
               call error_handler(E_ALLMSG, routine, string1, source)
               write(string1, *) &
               'APM: QV VAL ',(qmr_val(imem,kk),kk=1,layer_tes)
               call error_handler(E_ALLMSG, routine, string1, source)
               write(string1, *) &
               'APM: PR VAL ',(prs_tes(kk),kk=1,layer_tes)
               call error_handler(E_ALLMSG, routine, string1, source)
               write(string1, *) &
   ...
   do imem=1,ens_size
      do k=1,layer_tes
         if(prior(key,k).lt.0.) then
            write(string1, *) &
            'APM: TES CO Prior is negative. Level may be below surface. Key,Layer: ',key,k
            call error_handler(E_ALLMSG, routine, string1, source)
            cycle
         endif
!
! Get expected observation
        co_term=co_val(imem,k)
        if(prs_tes(k).le.tcr2_top) co_term=3.0e-7_r8
!        
         prior_term=-1.*avg_kernel(key,k)
         if(k.eq.klev_tes) prior_term=(1.0_r8 - avg_kernel(key,k)) 
!
         expct_val(imem) = expct_val(imem) + log(co_term) * &
...
!
      if(expct_val(imem).lt.0) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE:TES_CO expected value is negative '
         call error_handler(E_ALLMSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
