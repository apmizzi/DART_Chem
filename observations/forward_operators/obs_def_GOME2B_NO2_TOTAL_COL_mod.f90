...
! GOME2B_NO2_TOTAL_COL, QTY_NO2
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_gome2b_no2_total_col_mod, only : get_expected_gome2b_no2_total_col, &
!                                  read_gome2b_no2_total_col, &
!                                  write_gome2b_no2_total_col, &
!                                  interactive_gome2b_no2_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(GOME2B_NO2_TOTAL_COL)                                                           
!         call get_expected_gome2b_no2_total_col(state_handle, ens_size, location, obs_def%key, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(GOME2B_NO2_TOTAL_COL)
!         call read_gome2b_no2_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(GOME2B_NO2_TOTAL_COL)
!         call write_gome2b_no2_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(GOME2B_NO2_TOTAL_COL)
!         call interactive_gome2b_no2_total_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_gome2b_no2_total_col_mod
...
public :: write_gome2b_no2_total_col, &
          read_gome2b_no2_total_col, &
          interactive_gome2b_no2_total_col, &
          get_expected_gome2b_no2_total_col, &
          set_obs_def_gome2b_no2_total_col

! Storage for the special information required for observations of this type
integer, parameter    :: max_gome2b_no2_obs = 10000000
integer               :: num_gome2b_no2_obs = 0
integer,  allocatable :: nlayer(:)
integer,  allocatable :: total_indx(:)
real(r8), allocatable :: amf_total(:)
real(r8), allocatable :: pressure(:,:)
real(r8), allocatable :: scat_wts(:,:)

! version controlled file description for error handling, do not edit
character(len=*), parameter :: source   = 'obs_def_gome2b_no2_total_col_mod.f90'
...
integer :: nlayer_gome2b = -9999
integer :: nlayer_gome2b_no2_total_col = -9999
integer :: nlayer_gome2b_no2_trop_col = -9999

namelist /obs_def_GOME2B_NO2_nml/ upper_data_file, use_log_no2, nlayer_model, &
nlayer_gome2b_no2_total_col, nlayer_gome2b_no2_trop_col, &
...
call find_namelist_in_file("input.nml", "obs_def_GOME2B_NO2_nml", iunit)
read(iunit, nml = obs_def_GOME2B_NO2_nml, iostat = rc)
call check_namelist_read(iunit, rc, "obs_def_GOME2B_NO2_nml")

! Record the namelist values
if (do_nml_file()) write(nmlfileunit, nml=obs_def_GOME2B_NO2_nml)
if (do_nml_term()) write(     *     , nml=obs_def_GOME2B_NO2_nml)
nlayer_gome2b=nlayer_gome2b_no2_total_col

! Check for valid values

if (nlayer_model < 1) then
   write(string1,*)'obs_def_GOME2B_NO2_nml:nlayer_model must be > 0, it is ',nlayer_model
   call error_handler(E_ERR,'initialize_module',string1,source)
endif

if (nlayer_gome2b < 1) then
   write(string1,*)'obs_def_GOME2B_NO2_nml:nlayer_gome2b must be > 0, it is ',nlayer_gome2b
   call error_handler(E_ERR,'initialize_module',string1,source)
endif

allocate(    nlayer(max_gome2b_no2_obs))
allocate(    total_indx(max_gome2b_no2_obs))
allocate(    amf_total(max_gome2b_no2_obs))
allocate(  pressure(max_gome2b_no2_obs,nlayer_gome2b+1))
allocate(scat_wts(max_gome2b_no2_obs,nlayer_gome2b))

end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_gome2b_no2_total_col(key, ifile, fform)

integer,          intent(out)          :: key
integer,          intent(in)           :: ifile
character(len=*), intent(in), optional :: fform

! gome2brary arrays to hold buffer till we decide if we have enough room

integer               :: keyin
integer               :: nlayer_1
integer               :: total_indx_1
real(r8)              :: amf_total_1
...
total_indx_1 = read_int_scalar( ifile, fileformat, 'total_indx_1')
amf_total_1 = read_r8_scalar( ifile, fileformat, 'amf_total_1')
...
if(counts1 > max_gome2b_no2_obs) then
   write(string1, *)'Not enough space for gome2b no2 total col obs.'
   write(string2, *)'Can only have max_gome2b_no2_obs (currently ',max_gome2b_no2_obs,')'
   call error_handler(E_ERR,'read_gome2b_no2_total_col',string1,source,text2=string2)
endif

call set_obs_def_gome2b_no2_total_col(key, pressure_1, scat_wts_1, amf_total_1, total_indx_1, nlayer_1)

deallocate(pressure_1, scat_wts_1)

end subroutine read_gome2b_no2_total_col

!-------------------------------------------------------------------------------

subroutine write_gome2b_no2_total_col(key, ifile, fform)
...
! nlayer, amf_total, total_indx, pressure, and scatering weights are all scoped in this module
...
call write_int_scalar(ifile,                     total_indx(key), fileformat,'total_indx')
call write_r8_scalar( ifile,                     amf_total(key), fileformat,'amf_total')
...
end subroutine write_gome2b_no2_total_col

!-------------------------------------------------------------------------------

subroutine interactive_gome2b_no2_total_col(key)
...
write(string1,*)'interactive_gome2b_no2_total_col not yet working.'
call error_handler(E_ERR, 'interactive_gome2b_no2_total_col', string1, source)

if(num_gome2b_no2_obs >= max_gome2b_no2_obs) then
   write(string1, *)'Not enough space for an gome2b no2 total col obs.'
   write(string2, *)'Can only have max_gome2b_no2_obs (currently ',max_gome2b_no2_obs,')'
   call error_handler(E_ERR, 'interactive_gome2b_no2_total_col', string1, &
              source, text2=string2)
endif

! Increment the index
num_gome2b_no2_obs = num_gome2b_no2_obs + 1
key            = num_gome2b_no2_obs

! Otherwise, prompt for input for the three required beasts

write(*, *) 'Creating an interactive_gome2b_no2_total_col observation'
write(*, *) 'This featue is not setup '

end subroutine interactive_gome2b_no2_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_gome2b_no2_total_col(state_handle, ens_size, location, key, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_gome2b_no2_total_col'
   type(location_type) :: loc2

   integer :: layer_gome2b,level_gome2b
   integer :: layer_mdl,level_mdl
   integer :: k,imem,kend_gome2b
   integer :: interp_new
   ...
   real(r8), allocatable, dimension(:)   :: thick, prs_gome2b, prs_gome2b_mem
...
! Assign vertical grid information
   ...
   layer_gome2b = nlayer(key)
   level_gome2b = nlayer(key)+1
   kend_gome2b  = total_indx(key)
   layer_mdl = nlayer_model
   level_mdl = nlayer_model+1

   allocate(prs_gome2b(level_gome2b))
   allocate(prs_gome2b_mem(level_gome2b))
   prs_gome2b(1:level_gome2b)=pressure(key,1:level_gome2b)
...
! Get profiles at GOME2B pressure levels

   allocate(no2_val(ens_size,level_gome2b))
   allocate(tmp_val(ens_size,level_gome2b))
   allocate(qmr_val(ens_size,level_gome2b))

   do k=1,level_gome2b
      zstatus(:)=0
      loc2 = set_location(mloc(1), mloc(2), prs_gome2b(k), VERTISPRESSURE)
...
! Correcting for expected failures near the surface
      do imem=1,ens_size
         if (prs_gome2b(k).ge.prs_mdl_1(imem)) then
...
! Correcting for expected failures near the top
         if (prs_gome2b(k).le.prs_mdl_n(imem)) then
...
! Convert units for no2 from ppmv
   ...
   zstatus(:)=0.
   expct_val(:)=0.0
   allocate(thick(layer_gome2b))
!
   do imem=1,ens_size
! Adjust the GOME2B pressure for WRF-Chem lower/upper boudary pressure
! (GOME2B NO2 vertical grid is bottom to top)
      prs_gome2b_mem(:)=prs_gome2b(:)
      if (prs_sfc(imem).gt.prs_gome2b_mem(1)) then
         prs_gome2b_mem(1)=prs_sfc(imem)
      endif   

! Calculate the thicknesses

      do k=1,kend_gome2b
         lnpr_mid=(log(prs_gome2b_mem(k+1))+log(prs_gome2b_mem(k)))/2.
         up_wt=log(prs_gome2b_mem(k))-lnpr_mid
         dw_wt=lnpr_mid-log(prs_gome2b_mem(k+1))
         tl_wt=up_wt+dw_wt
         ...
         thick(k)   = Rd*(dw_wt*tmp_vir_k + up_wt*tmp_vir_kp)/tl_wt/grav* &
                   log(prs_gome2b_mem(k)/prs_gome2b_mem(k+1))
      enddo
...
! Convert from VMR to molar density (mol/m^3)
         if(use_log_no2) then
            ...
            (dw_wt*prs_gome2b_mem(k)+up_wt*prs_gome2b_mem(k+1)) / &
...
!         call error_handler(E_MSG, routine, string1, source)
      ...
      if(expct_val(imem).lt.0) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: TOTALOMI NO2 expected value is negative '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
! Clean up and return
   ...
   deallocate(thick)
   deallocate(prs_gome2b, prs_gome2b_mem)

end subroutine get_expected_gome2b_no2_total_col

!-------------------------------------------------------------------------------

subroutine set_obs_def_gome2b_no2_total_col(key, no2_pressure, no2_scat_wts, no2_amf_total, no2_total_indx, no2_nlayer)

integer,                           intent(in)   :: key, no2_nlayer
integer,                           intent(in)   :: no2_total_indx
real(r8),                          intent(in)   :: no2_amf_total
...
if(num_gome2b_no2_obs >= max_gome2b_no2_obs) then
   write(string1, *)'Not enough space for gome2b no2 total col obs.'
   write(string2, *)'Can only have max_gome2b_no2_obs (currently ',max_gome2b_no2_obs,')'
   call error_handler(E_ERR,'set_obs_def_gome2b_no2_total_col',string1,source,revision, &
   revdate,text2=string2)
endif

nlayer(key) = no2_nlayer
total_indx(key) = no2_total_indx
amf_total(key) = no2_amf_total
pressure(key,1:no2_nlayer+1) = no2_pressure(1:no2_nlayer+1)
scat_wts(key,1:no2_nlayer) = no2_scat_wts(1:no2_nlayer)

end subroutine set_obs_def_gome2b_no2_total_col

end module obs_def_gome2b_no2_total_col_mod
