...
! TEMPO_NO2_TOTAL_COL, QTY_NO2
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_tempo_no2_total_col_mod, only : get_expected_tempo_no2_total_col, &
!                                  read_tempo_no2_total_col, &
!                                  write_tempo_no2_total_col, &
!                                  interactive_tempo_no2_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(TEMPO_NO2_TOTAL_COL)                                                           
!         call get_expected_tempo_no2_total_col(state_handle, ens_size, location, obs_def%key, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(TEMPO_NO2_TOTAL_COL)
!         call read_tempo_no2_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(TEMPO_NO2_TOTAL_COL)
!         call write_tempo_no2_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(TEMPO_NO2_TOTAL_COL)
!         call interactive_tempo_no2_total_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_tempo_no2_total_col_mod
...
public :: write_tempo_no2_total_col, &
          read_tempo_no2_total_col, &
          interactive_tempo_no2_total_col, &
          get_expected_tempo_no2_total_col, &
          set_obs_def_tempo_no2_total_col
...
character(len=*), parameter :: source   = 'obs_def_tempo_no2_total_col_mod.f90'
...
integer :: nlayer_tempo_no2_total_col = -9999
integer :: nlayer_tempo_no2_trop_col = -9999

namelist /obs_def_TEMPO_NO2_nml/ upper_data_file, use_log_no2, &
nlayer_model, nlayer_tempo_no2_total_col, nlayer_tempo_no2_trop_col, &
...
nlayer_tempo=nlayer_tempo_no2_total_col

! Check for valid values

if (nlayer_model < 1) then
   write(string1,*)'obs_def_TEMPO_NO2_nml:nlayer_model must be > 0, it is ',nlayer_model
   call error_handler(E_ERR,'initialize_module',string1,source)
endif

if (nlayer_tempo < 1) then
   write(string1,*)'obs_def_TEMPO_NO2_nml:nlayer_tempo must be > 0, it is ',nlayer_tempo
   call error_handler(E_ERR,'initialize_module',string1,source)
endif

allocate(    nlayer(max_tempo_no2_obs))
allocate(    trop_indx(max_tempo_no2_obs))
allocate(    amf_trop(max_tempo_no2_obs))
allocate(  pressure(max_tempo_no2_obs,nlayer_tempo+1))
allocate(scat_wts(max_tempo_no2_obs,nlayer_tempo))

end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_tempo_no2_total_col(key, ifile, fform)
...
if(counts1 > max_tempo_no2_obs) then
   write(string1, *)'Not enough space for tempo no2 total col obs.'
   write(string2, *)'Can only have max_tempo_no2_obs (currently ',max_tempo_no2_obs,')'
   call error_handler(E_ERR,'read_tempo_no2_total_col',string1,source,text2=string2)
endif

call set_obs_def_tempo_no2_total_col(key, pressure_1, scat_wts_1, amf_trop_1, trop_indx_1, nlayer_1)

deallocate(pressure_1, scat_wts_1)

end subroutine read_tempo_no2_total_col

!-------------------------------------------------------------------------------

subroutine write_tempo_no2_total_col(key, ifile, fform)
...
end subroutine write_tempo_no2_total_col

!-------------------------------------------------------------------------------

subroutine interactive_tempo_no2_total_col(key)
...
write(string1,*)'interactive_tempo_no2_total_col not yet working.'
call error_handler(E_ERR, 'interactive_tempo_no2_total_col', string1, source)

if(num_tempo_no2_obs >= max_tempo_no2_obs) then
   write(string1, *)'Not enough space for an tempo no2 total col obs.'
   write(string2, *)'Can only have max_tempo_no2_obs (currently ',max_tempo_no2_obs,')'
   call error_handler(E_ERR, 'interactive_tempo_no2_total_col', string1, &
...
write(*, *) 'Creating an interactive_tempo_no2_total_col observation'
write(*, *) 'This featue is not setup '

end subroutine interactive_tempo_no2_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_tempo_no2_total_col(state_handle, ens_size, location, key, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_tempo_no2_total_col'
   type(location_type) :: loc2

   integer :: layer_tempo,level_tempo
   integer :: layer_mdl,level_mdl
   integer :: k,imem,kend_tempo
   integer :: interp_new
...
! Get profiles at TEMPO pressure levels
   ...
   allocate(no2_val(ens_size,level_tempo))
...
! Calculate the thicknesses
      ...
      do k=1,kend_tempo
         ...
         dw_wt=lnpr_mid-log(prs_tempo_mem(k+1))
         tl_wt=up_wt+dw_wt
...
end subroutine get_expected_tempo_no2_total_col

!-------------------------------------------------------------------------------

subroutine set_obs_def_tempo_no2_total_col(key, no2_pressure, no2_scat_wts, no2_amf_trop, no2_trop_indx, no2_nlayer)
...
if(num_tempo_no2_obs >= max_tempo_no2_obs) then
   write(string1, *)'Not enough space for tempo no2 total col obs.'
   write(string2, *)'Can only have max_tempo_no2_obs (currently ',max_tempo_no2_obs,')'
   call error_handler(E_ERR,'set_obs_def_tempo_no2_total_col',string1,source,revision, &
...
end subroutine set_obs_def_tempo_no2_total_col

end module obs_def_tempo_no2_total_col_mod
