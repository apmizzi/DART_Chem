...
! CRIS_CO_TOTAL_COL, QTY_CO
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_cris_co_total_col_mod, only : get_expected_cris_co_total_col, &
!                                  read_cris_co_total_col, &
!                                  write_cris_co_total_col, &
!                                  interactive_cris_co_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(CRIS_CO_TOTAL_COL)                                                           
!         call get_expected_cris_co_total_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(CRIS_CO_TOTAL_COL)
!         call read_cris_co_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(CRIS_CO_TOTAL_COL)
!         call write_cris_co_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(CRIS_CO_TOTAL_COL)
!         call interactive_cris_co_total_col(obs_def%key)
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_cris_co_total_col, &
             read_cris_co_total_col, &
          interactive_cris_co_total_col, &
          get_expected_cris_co_total_col, &
          set_obs_def_cris_co_total_col

! Storage for the special information required for observations of this type
   integer, parameter    :: max_cris_co_obs = 10000000
   integer               :: num_cris_co_obs = 0
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_cris_co_total_col_mod.f90'
...
! Namelist with default values
   ...
   integer :: nlayer_cris_co_total_col = -9999
   integer :: nlayer_cris_co_trop_col = -9999
   integer :: nlayer_cris_co_profile = -9999
   
   namelist /obs_def_CRIS_CO_nml/ use_log_co, nlayer_model, &
...
! Read namelist values
   call find_namelist_in_file("input.nml", "obs_def_CRIS_CO_nml", iunit)
   read(iunit, nml = obs_def_CRIS_CO_nml, iostat = rc)
   call check_namelist_read(iunit, rc, "obs_def_CRIS_CO_nml")

! Record the namelist values
   if (do_nml_file()) write(nmlfileunit, nml=obs_def_CRIS_CO_nml)
   if (do_nml_term()) write(     *     , nml=obs_def_CRIS_CO_nml)

! Check for valid values
   nlayer_cris=nlayer_cris_co_total_col

   if (nlayer_model < 1) then
      write(string1,*)'obs_def_CRIS_CO_nml:nlayer_model must be > 0, it is ',nlayer_model
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   if (nlayer_cris < 1) then
      write(string1,*)'obs_def_CRIS_CO_nml:nlayer_cris must be > 0, it is ',nlayer_cris
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   allocate(    nlayer(max_cris_co_obs))
   allocate(    klev(max_cris_co_obs))
   allocate(    kend(max_cris_co_obs))
   allocate(  pressure(max_cris_co_obs,nlayer_cris+1))
   allocate(avg_kernel(max_cris_co_obs,nlayer_cris))
   allocate(     prior(max_cris_co_obs,nlayer_cris))
   
end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_cris_co_total_col(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_cris_co_obs) then
      write(string1, *)'Not enough space for cris co obs.'
      write(string2, *)'Can only have max_cris_co_obs (currently ',max_cris_co_obs,')'
      call error_handler(E_ERR,'read_cris_co_total_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_cris_co_total_col(key, pressure_1(1:nlayer_1+1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_cris_co_total_col

!-------------------------------------------------------------------------------

subroutine write_cris_co_total_col(key, ifile, fform)
...
end subroutine write_cris_co_total_col

!-------------------------------------------------------------------------------

subroutine interactive_cris_co_total_col(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_cris_co_total_col not yet working.'
   call error_handler(E_ERR, 'interactive_cris_co_total_col', string1, source)
   
   if(num_cris_co_obs >= max_cris_co_obs) then
      write(string1, *)'Not enough space for an cris co obs.'
      write(string2, *)'Can only have max_cris_co_obs (currently ',max_cris_co_obs,')'
      call error_handler(E_ERR, 'interactive_cris_co_total_col', string1, &
                 source, text2=string2)
   endif
   
! Increment the index
   num_cris_co_obs = num_cris_co_obs + 1
   key            = num_cris_co_obs

! Otherwise, prompt for input for the three required beasts

   write(*, *) 'Creating an interactive_cris_co_total_col observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_cris_co_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_cris_co_total_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_cris_co_total_col'
   ...
   real(r8), dimension(ens_size) :: co_mdl_tmp, tmp_mdl_tmp, qmr_mdl_tmp, prs_mdl_tmp
   real(r8), dimension(ens_size) :: prs_sfc,rec_co_val,rec_tmp_val,rec_qmr_val
...
! Assign vertical grid information (CRIS CO grid is top to bottom)
...
! pressure at model surface (Pa)
   ...
   do k=1,layer_mdl
      ...
      call interpolate(state_handle, ens_size, loc2, QTY_CO, co_mdl_tmp, zstatus) ! ppmv 
      zstatus(:)=0
...
!
      interp_new=0
      do imem=1,ens_size
         if(co_mdl_tmp(imem).eq.missing_r8 .or. tmp_mdl_tmp(imem).eq.missing_r8 .or. &
         qmr_mdl_tmp(imem).eq.missing_r8 .or. prs_mdl_tmp(imem).eq.missing_r8) then
            interp_new=1
         else
            kbnd_1(imem)=k
            co_mdl_1(imem)=co_mdl_tmp(imem)
...
! pressure at model top (Pa)
   ...
   do k=layer_mdl,1,-1
      ...
      zstatus(:)=0
      loc2 = set_location(mloc(1), mloc(2), level, VERTISLEVEL)
      call interpolate(state_handle, ens_size, loc2, QTY_CO, co_mdl_tmp, zstatus) 
      zstatus(:)=0
...
! Get total_cols at CRIS pressure levels
   ...
   allocate(co_val(ens_size,level_cris))
...
!      write(string1, *)'APM: co ',key,k,co_val(1,k)
!      call error_handler(E_MSG, routine, string1, source)
...
! Check data for missing values      
      do imem=1,ens_size
         ...
         qmr_val(imem,k).eq.missing_r8) then
            ...
            do imemm=1,ens_size
               write(string1, *) &
               'APM: Model total_col values: co,tmp,qmr',key,imem,k,co_val(imemm,k), &
...
!
         call get_upper_bdy_co(lon_obs,lat_obs,prs_cris_top,ncnt, &
...
! Impose ensemble perturbations from level kstart(imem)+1      
         ...
         do k=1,kstart(imem) 
            co_val(imem,k)=co_prf_mdl(k)*co_val(imem,kstart(imem)+1)/ &
            (sum(co_val(:,kstart(imem)+1))/real(ens_size))
      ...
      do k=1,level_cris   
         ...
         qmr_val(imem,k).lt.0.) then
            ...
            if(k.le.kstart(imem)) then
               do kk=1,level_cris
                  write(string1, *) &
                  'APM: prs, co, tmp, qmr',key,imem,prs_cris(kk),co_val(imem,kk), &
...
! (CRIS CO vertical grid is top to bottom)
...
!         'avgk*thick*co_conv ', avg_kernel(key,k)*thick(k)*co_val_conv, &
...
!      
      if(isnan(expct_val(imem))) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: CRIS CO expected value is NaN '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
!
      if(expct_val(imem).lt.0) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: CRIS CO expected value is negative '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
end subroutine get_expected_cris_co_total_col

!-------------------------------------------------------------------------------

subroutine get_upper_bdy_co(lon_obs,lat_obs,prs_obs,nprs_obs, &
co_prf_mdl,tmp_prf_mdl,qmr_prf_mdl,date_obs,datesec_obs)
...
! mozart
   ...
   real(r8),dimension(nprs_obs),      intent(out)   :: co_prf_mdl,tmp_prf_mdl,qmr_prf_mdl
   ...
   real,dimension(nz)                               :: prs_glb,ztrp_co,ztrp_tmp,ztrp_qmr
   real,dimension(nz)                               :: co_glb_xmym,co_glb_xpym,co_glb_xmyp,co_glb_xpyp
   ...
   real,dimension(nx,ny,nz,ntim)                    :: co_glb,tmp_glb,qmr_glb
   character(len=120)                               :: data_file
   character(len=*), parameter                      :: routine = 'get_upper_bdy_co'
!
...
! mozart
!   call get_MOZART_REAL_DATA(data_file,'CO_VMR_inst',nx,ny,nz,ntim,co_glb)
! waccm
   call get_MOZART_REAL_DATA(data_file,'CO',nx,ny,nz,ntim,co_glb)
...
! Temporal
   do k=1,nz
      co_glb_xmym(k)=(wt_bck*co_glb(indx,jndx,k,itim_sav) + &
      wt_fwd*co_glb(indx,jndx,k,itim_sav+1))/(wt_bck+wt_fwd)
      co_glb_xpym(k)=(wt_bck*co_glb(indx+1,jndx,k,itim_sav) + &
      wt_fwd*co_glb(indx+1,jndx,k,itim_sav+1))/(wt_bck+wt_fwd)
      co_glb_xmyp(k)=(wt_bck*co_glb(indx,jndx+1,k,itim_sav) + &
      wt_fwd*co_glb(indx,jndx+1,k,itim_sav+1))/(wt_bck+wt_fwd)
      co_glb_xpyp(k)=(wt_bck*co_glb(indx+1,jndx+1,k,itim_sav) + &
      wt_fwd*co_glb(indx+1,jndx+1,k,itim_sav+1))/(wt_bck+wt_fwd)
!
...
! Horizontal   
   do k=1,nz
      ztrp_jbck=(bck_xwt*co_glb_xmym(k) + fwd_xwt*co_glb_xpym(k))/twtx
      ztrp_jfwd=(bck_xwt*co_glb_xmyp(k) + fwd_xwt*co_glb_xpyp(k))/twtx
      ztrp_co(k)=(bck_ywt*ztrp_jbck + fwd_ywt*ztrp_jfwd)/twty
!      
...
! Vertical   
   do k=1,nprs_obs
      ...
      co_prf_mdl(k)=(zwt_up*ztrp_co(kndx) + zwt_dw*ztrp_co(kndx+1))/twt
   ...
   enddo
 end subroutine get_upper_bdy_co
...
subroutine set_obs_def_cris_co_total_col(key, co_pressure, co_avg_kernel, co_prior, &
co_nlayer, co_klev, co_kend)

   integer,                           intent(in)   :: key, co_nlayer, co_klev, co_kend
   ...
   if(num_cris_co_obs >= max_cris_co_obs) then
      write(string1, *)'Not enough space for cris co obs.'
      write(string2, *)'Can only have max_cris_co_obs (currently ',max_cris_co_obs,')'
      call error_handler(E_ERR,'set_obs_def_cris_co_total_col',string1,source,revision, &
...
end subroutine set_obs_def_cris_co_total_col

end module obs_def_cris_co_total_col_mod
