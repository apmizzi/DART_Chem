...
!   use obs_def_tempo_no2_mod, only : get_expected_tempo_no2, &
!                                  read_tempo_no2, &
!                                  write_tempo_no2, &
...
if (nlayer_tempo_no2 < 1) then
   write(string1,*)'obs_def_TEMPO_NO2_nml:nlayer_tempo_no2 must be > 0, it is ',nlayer_tempo_no2
   call error_handler(E_ERR,'initialize_module',string1,source)
endif
...
allocate(  pressure(max_tempo_no2_obs,nlayer_tempo_no2+1))
allocate(scat_wts(max_tempo_no2_obs,nlayer_tempo_no2))
...
if(counts1 > max_tempo_no2_obs) then
   ...
   call error_handler(E_ERR,'read_tempo_no2',string1,source,text2=string2)
endif

call set_obs_def_tempo_no2(key, pressure_1, scat_wts_1, amf_trop_1, trop_indx_1, nlayer_1)
...
write(string1,*)'interactive_tempo_no2 not yet working.'
call error_handler(E_ERR, 'interactive_tempo_no2', string1, source)

if(num_tempo_no2_obs >= max_tempo_no2_obs) then
   write(string1, *)'Not enough space for an tempo no2 obs.'
   write(string2, *)'Can only have max_tempo_no2_obs (currently ',max_tempo_no2_obs,')'
   call error_handler(E_ERR, 'interactive_tempo_no2', string1, &
...
subroutine get_expected_tempo_no2(state_handle, ens_size, location, key, expct_val, istatus)
...
character(len=*), parameter :: routine = 'get_expected_tempo_no2'
...
if(any(no2_istatus /= 0)) then
   write(string1, *)'APM NOTICE: MDL no2_mdl_1 is bad ',key
   call error_handler(E_MSG, routine, string1, source)
endif
call track_status(ens_size, no2_istatus, no2_mdl_1, istatus, return_now)
if(return_now) return
!write(string1, *) 'APM: no2_mdl_1 ',key,no2_mdl_1(1)
!call error_handler(E_MSG, routine, string1, source)
...
if(any(no2_istatus /= 0)) then
   write(string1, *)'APM NOTICE: MDL no2_mdl_n is bad ',key
   call error_handler(E_MSG, routine, string1, source)
endif
call track_status(ens_size, no2_istatus, no2_mdl_n, istatus, return_now)
if(return_now) return
!write(string1, *) 'APM: no2_mdl_n ',key,no2_mdl_n(1)
!call error_handler(E_MSG, routine, string1, source)
...
do k=1,level_tempo
   ...
   where(prs_tempo(k).ge.prs_mdl_1)
      no2_istatus = 0
      tmp_istatus = 0
      qmr_istatus = 0
      no2_temp    = no2_mdl_1
   ...
   where(prs_tempo(k).le.prs_mdl_n) 
      no2_istatus = 0
      tmp_istatus = 0
      qmr_istatus = 0
      no2_temp    = no2_mdl_n
   ...
   no2_return_now=.false.
   if(any(no2_istatus /= 0)) then
      write(string1,*) &
      'APM NOTICE: model NO2 obs value on TEMPO grid has a bad value '
      call error_handler(E_MSG, routine, string1, source)
   ...
   if(no2_return_now .or. tmp_return_now .or. qmr_return_now) return
...
enddo
!write(string1, *) 'APM: no2_val mem=1 ',key,no2_val(1,:)
!call error_handler(E_MSG, routine, string1, source)
...
subroutine set_obs_def_tempo_no2(key, no2_pressure, no2_scat_wts, no2_amf_trop, no2_trop_indx, no2_nlayer)
...
if(num_tempo_no2_obs >= max_tempo_no2_obs) then
   write(string1, *)'Not enough space for tempo no2 obs.'
   write(string2, *)'Can only have max_tempo_no2_obs (currently ',max_tempo_no2_obs,')'
   call error_handler(E_ERR,'set_obs_def_tempo_no2',string1,source,revision, &
