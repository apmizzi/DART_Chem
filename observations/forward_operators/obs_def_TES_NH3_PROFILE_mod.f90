...
! TES_NH3_PROFILE, QTY_NH3
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_tes_nh3_profile_mod, only : get_expected_tes_nh3_profile, &
!                                  read_tes_nh3_profile, &
!                                  write_tes_nh3_profile, &
!                                  interactive_tes_nh3_profile
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(TES_NH3_PROFILE)                                                           
!         call get_expected_tes_nh3_profile(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(TES_NH3_PROFILE)
!         call read_tes_nh3_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(TES_NH3_PROFILE)
!         call write_tes_nh3_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(TES_NH3_PROFILE)
!         call interactive_tes_nh3_profile(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_tes_nh3_profile_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_tes_nh3_profile, &
             read_tes_nh3_profile, &
          interactive_tes_nh3_profile, &
          get_expected_tes_nh3_profile, &
          set_obs_def_tes_nh3_profile

! Storage for the special information required for observations of this type
   integer, parameter    :: max_tes_nh3_obs = 10000000
   integer               :: num_tes_nh3_obs = 0
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_tes_nh3_profile_mod.f90'
...
! Namelist with default values
   ...
   integer :: nlayer_tes_nh3_total_col = -9999
...
! Read namelist values
   call find_namelist_in_file("input.nml", "obs_def_TES_NH3_nml", iunit)
   read(iunit, nml = obs_def_TES_NH3_nml, iostat = rc)
   call check_namelist_read(iunit, rc, "obs_def_TES_NH3_nml")

! Record the namelist values
   if (do_nml_file()) write(nmlfileunit, nml=obs_def_TES_NH3_nml)
   if (do_nml_term()) write(     *     , nml=obs_def_TES_NH3_nml)

! Check for valid values
   nlayer_tes=nlayer_tes_nh3_profile

   if (nlayer_model < 1) then
      write(string1,*)'obs_def_TES_NH3_nml:nlayer_model must be > 0, it is ',nlayer_model
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   if (nlayer_tes < 1) then
      write(string1,*)'obs_def_TES_NH3_nml:nlayer_tes must be > 0, it is ',nlayer_tes
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   allocate(    nlayer(max_tes_nh3_obs))
   allocate(    klev(max_tes_nh3_obs))
   allocate(    kend(max_tes_nh3_obs))
   allocate(  pressure(max_tes_nh3_obs,nlayer_tes))
   allocate(avg_kernel(max_tes_nh3_obs,nlayer_tes))
   allocate(     prior(max_tes_nh3_obs,nlayer_tes))
   
end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_tes_nh3_profile(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_tes_nh3_obs) then
      write(string1, *)'Not enough space for tes nh3 obs.'
      write(string2, *)'Can only have max_tes_nh3_obs (currently ',max_tes_nh3_obs,')'
      call error_handler(E_ERR,'read_tes_nh3_profile',string1,source,text2=string2)
   endif
   
   call set_obs_def_tes_nh3_profile(key, pressure_1(1:nlayer_1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_tes_nh3_profile

!-------------------------------------------------------------------------------

subroutine write_tes_nh3_profile(key, ifile, fform)
...
end subroutine write_tes_nh3_profile

!-------------------------------------------------------------------------------

subroutine interactive_tes_nh3_profile(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_tes_nh3_profile not yet working.'
   call error_handler(E_ERR, 'interactive_tes_nh3_profile', string1, source)
   
   if(num_tes_nh3_obs >= max_tes_nh3_obs) then
      write(string1, *)'Not enough space for an tes nh3 obs.'
      write(string2, *)'Can only have max_tes_nh3_obs (currently ',max_tes_nh3_obs,')'
      call error_handler(E_ERR, 'interactive_tes_nh3_profile', string1, &
                 source, text2=string2)
   endif
   
! Increment the index
   num_tes_nh3_obs = num_tes_nh3_obs + 1
   key            = num_tes_nh3_obs

! Otherwise, prompt for input for the three required beasts

   write(*, *) 'Creating an interactive_tes_nh3_profile observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_tes_nh3_profile

!-------------------------------------------------------------------------------

subroutine get_expected_tes_nh3_profile(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   integer :: interp_new
   integer :: icnt,ncnt,klev_tes,kstart
   integer :: kstart_fld,kstart_tmp,kstart_qmr,kstart_prs
   ...
   real(r8), dimension(ens_size) :: prsn_fld, prsn_tmp, prsn_qmr, prsn_prs
   real(r8), dimension(ens_size) :: fld_mdl_1, tmp_mdl_1, qmr_mdl_1, prs_mdl_1
   real(r8), dimension(ens_size) :: fld_mdl_n, tmp_mdl_n, qmr_mdl_n, prs_mdl_n
   real(r8), dimension(ens_size) :: prs_sfc
   
   real(r8), allocatable, dimension(:)   :: thick, prs_tes, prs_tes_mem
   real(r8), allocatable, dimension(:,:) :: fld_val, tmp_val, qmr_val
   logical  :: return_now,nh3_return_now,tmp_return_now,qmr_return_now
!
! Upper BC variables
   real(r8), allocatable, dimension(:)   :: fld_prf_mdl,tmp_prf_mdl,qmr_prf_mdl
...
! Assign vertical grid information (TES NH3 grid is top to bottom)
...
! lower bounds
   fld_mdl_1(:)=missing_r8
   do k=1,layer_mdl
      level=real(k)
      zstatus(:)=0
      loc2 = set_location(mloc(1), mloc(2), level, VERTISLEVEL)
      call interpolate(state_handle, ens_size, loc2, QTY_NH3, fld_mdl_1, zstatus) ! ppmv
      interp_new=0
      do imem=1,ens_size
         if(fld_mdl_1(imem).eq.missing_r8) then
            interp_new=1
            exit
         endif
      enddo
   ...
   do k=1,layer_mdl
      ...
      interp_new=0
      do imem=1,ens_size
         if(tmp_mdl_1(imem).eq.missing_r8) then
            interp_new=1
            exit
         endif
      enddo
   ...
   do k=1,layer_mdl
      ...
      interp_new=0
      do imem=1,ens_size
         if(qmr_mdl_1(imem).eq.missing_r8) then
            interp_new=1
            exit
         endif
      enddo
   ...
   do k=1,layer_mdl
      ...
      interp_new=0
      do imem=1,ens_size
         if(prs_mdl_1(imem).eq.missing_r8) then
            interp_new=1
            exit
         endif
      enddo
...
!   write(string1, *)'APM: fld lower bound 1 ',fld_mdl_1
...
! upper bounds
   kstart_fld=layer_mdl-1
   fld_mdl_n(:)=missing_r8
   do k=layer_mdl-1,1,-1
      zstatus(:)=0
      level=real(k)
      loc2 = set_location(mloc(1), mloc(2), level, VERTISLEVEL)
      call interpolate(state_handle, ens_size, loc2, QTY_NH3, fld_mdl_n, zstatus) ! ppmv
      interp_new=0
      if(any(fld_mdl_n(:).eq.missing_r8)) then 
         interp_new=1
         kstart_fld=k
         if(k.eq.1) then
            zstatus(:)=0
            kstart_fld=1
            level = real(kstart_fld)
            loc2 = set_location(mloc(1), mloc(2), level, VERTISLEVEL)
            call interpolate(state_handle, ens_size, loc2, QTY_PRESSURE, prsn_fld, zstatus) ! ppmv
         endif
         fld_mdl_n(:)=missing_r8
      endif
      if(interp_new.eq.0) then
   ...
   do k=layer_mdl-1,1,-1
      ...
      interp_new=0
      if(any(tmp_mdl_n(:).eq.missing_r8)) then
         interp_new=1
         kstart_tmp=k
         if(k.eq.1) then
            zstatus(:)=0
            kstart_tmp=1
            level = real(kstart_tmp)
            loc2 = set_location(mloc(1), mloc(2), level, VERTISLEVEL)
            call interpolate(state_handle, ens_size, loc2, QTY_PRESSURE, prsn_tmp, zstatus) ! ppmv
         endif
         tmp_mdl_n(:)=missing_r8
      endif
      if(interp_new.eq.0) then
   ...
   do k=layer_mdl-1,1,-1
      ...
      interp_new=0
      if(any(qmr_mdl_n(:).eq.missing_r8)) then
         interp_new=1
         kstart_qmr=k
         if(k.eq.1) then
            zstatus(:)=0
            kstart_qmr=1
            level = real(kstart_qmr)
            loc2 = set_location(mloc(1), mloc(2), level, VERTISLEVEL)
            call interpolate(state_handle, ens_size, loc2, QTY_PRESSURE, prsn_qmr, zstatus) ! ppmv
         endif
         qmr_mdl_n(:)=missing_r8
      endif
      if(interp_new.eq.0) then
   ...
   do k=layer_mdl-1,1,-1
      ...
      interp_new=0
      if(any(prs_mdl_n(:).eq.missing_r8)) then
         interp_new=1
         kstart_prs=k
         if(k.eq.1) then
            zstatus(:)=0
            kstart_prs=1
            level = real(kstart_prs)
            loc2 = set_location(mloc(1), mloc(2), level, VERTISLEVEL)
            call interpolate(state_handle, ens_size, loc2, QTY_PRESSURE, prsn_prs, zstatus) ! ppmv
         endif
         prs_mdl_n(:)=missing_r8
      endif
      if(interp_new.eq.0) then
   ...
   write(string1, *)'APM: kstart_fld ',kstart_fld, layer_mdl
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'APM: fld_mdl_n ',fld_mdl_n
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'APM: prsn_fld ',prsn_fld(:)
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'  '
   call error_handler(E_MSG, routine, string1, source)

   write(string1, *)'APM: kstart_tmp ',kstart_tmp,layer_mdl
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'APM: tmp_mdl_n ',tmp_mdl_n
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'APM: prsn_tmp ',prsn_tmp(:)
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'  '
   call error_handler(E_MSG, routine, string1, source)

   write(string1, *)'APM: kstart_qmr ',kstart_qmr,layer_mdl
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'APM: qmr_mdl_n ',qmr_mdl_n
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'APM: prsn_qmr ',prsn_qmr(:)
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'  '
   call error_handler(E_MSG, routine, string1, source)

   write(string1, *)'APM: kstart_prs ',kstart_prs,layer_mdl
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'APM: prs_mdl_n ',prs_mdl_n
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'APM: prsn_prs ',prsn_prs(:)
   call error_handler(E_MSG, routine, string1, source)
   write(string1, *)'  '
   call error_handler(E_MSG, routine, string1, source)
...
!   write(string1, *)'APM: fld upper bound n ',fld_mdl_n
!   call error_handler(E_MSG, routine, string1, source)
!   write(string1, *)'APM: tmp upper bound n ',tmp_mdl_n
!   call error_handler(E_MSG, routine, string1, source)
!   write(string1, *)'APM: qmr upper bound n ',qmr_mdl_n
!   call error_handler(E_MSG, routine, string1, source)
!   write(string1, *)'APM: prs upper bound n ',prs_mdl_n
!   call error_handler(E_MSG, routine, string1, source)

! Get profiles at TES pressure levels

   allocate(fld_val(ens_size,layer_tes))
   ...
   do k=1,layer_tes
      ...
      call interpolate(state_handle, ens_size, loc2, QTY_NH3, fld_val(:,k), zstatus)  
      zstatus=0
...
! Convert units for nh3 from ppmv
      fld_val(:,k) = fld_val(:,k) * 1.e-6_r8
   enddo
   fld_mdl_1(:)=fld_mdl_1(:) * 1.e-6_r8
   fld_mdl_n(:)=fld_mdl_n(:) * 1.e-6_r8
!
! Use large scale data above the regional model top
! TES vertical is from bottom to top   
   ...
   do imem=1,ens_size
      if(prs_tes(layer_tes).lt.prsn_fld(imem)) then
         do k=1,layer_tes
            if(prs_tes(k).lt.prsn_fld(imem)) then
               kstart=k
               exit
            endif
         enddo
...
! Impose ensemble perturbations from level kstart-1      
         if(kstart.gt.1) then 
            do k=kstart,layer_tes
                kk=k-kstart+1
                fld_val(imem,k)=fld_prf_mdl(kk)*fld_val(imem,kstart-1)/ &
                (sum(fld_val(:,kstart-1))/real(ens_size))
            enddo
         else
            do k=kstart,layer_tes
                kk=k-kstart+1
                fld_val(imem,k)=fld_prf_mdl(kk)*abs(1.-0.3*random_stdnormal(1.))
            enddo
         endif
         deallocate(prs_tes_top)
         deallocate(fld_prf_mdl)
      endif
      print *, 'mem, nh3 profile ',fld_val(imem,:)
      print *, ' '
   enddo
...
!
   do imem=1,ens_size
      ncnt=layer_tes-kstart_tmp
      if(ncnt.gt.0) then
         allocate(prs_tes_top(ncnt))
         allocate(tmp_prf_mdl(ncnt))
         do k=kstart_tmp,layer_tes
            prs_tes_top(k-kstart_tmp+1)=prs_tes(k)
         enddo
         prs_tes_top(:)=prs_tes_top(:)/100.
!
         fld = 'T'         
...
! Impose ensemble perturbations from level kstart(imem)-1      
         if(kstart_tmp.gt.1) then 
            do k=kstart_tmp,layer_tes
               kk=k-kstart_tmp+1
               tmp_val(imem,k)=tmp_prf_mdl(kk)*tmp_val(imem,kstart_tmp-1)/ &
               (sum(tmp_val(:,kstart_tmp-1))/real(ens_size))
            enddo
         else
            do k=kstart_tmp,layer_tes
               kk=k-kstart_tmp+1
               tmp_val(imem,k)=tmp_prf_mdl(kk)*abs(1.-0.3*random_stdnormal(1.))
            enddo
         endif
         deallocate(prs_tes_top)
         deallocate(tmp_prf_mdl)
      endif
!
      ncnt=layer_tes-kstart_qmr
      if(ncnt.gt.0) then
         allocate(prs_tes_top(ncnt))
         allocate(qmr_prf_mdl(ncnt))
         do k=kstart_qmr,layer_tes
            prs_tes_top(k-kstart_qmr+1)=prs_tes(k)
         enddo
         prs_tes_top(:)=prs_tes_top(:)/100.
!
         fld = 'Q'         
...
! Impose ensemble perturbations from level kstart(imem)-1      
         if(kstart_qmr.gt.1) then 
            do k=kstart_qmr,layer_tes
               kk=k-kstart_qmr+1
               tmp_val(imem,k)=tmp_prf_mdl(kk)*tmp_val(imem,kstart_qmr-1)/ &
               (sum(tmp_val(:,kstart_qmr-1))/real(ens_size))
            enddo
         else
            do k=kstart_qmr,layer_tes
               kk=k-kstart_qmr+1
...
!      do k=1,layer_tes
!         write(string1, *) &
!         'APM: prs, nk3, tmp, qmr ',k,prs_tes(k),fld_val(imem,k), &
...
!
   do imem=1,ens_size
      flg=0
      do k=1,layer_tes   
         if(fld_val(imem,k).lt.0. .or. tmp_val(imem,k).lt.0. .or. &
...
! Get expected observation
         ...
         expct_val(imem) = expct_val(imem) + log(fld_val(imem,k)) * &
         avg_kernel(key,k) + prior_term * log(prior(key,k))

!         write(string1, *) 'APM: exp_val, nh3, avgk, prior_trm, prior',imem,k, &
!         expct_val(imem),fld_val(imem,k),avg_kernel(key,k),prior_term,prior(key,k)
!         call error_handler(E_MSG, routine, string1, source)
      enddo
...
!      call error_handler(E_MSG, routine, string1, source)
      ...
      if(isnan(expct_val(imem))) then
         ...
         expct_val(:)=missing_r8
!         write(string1, *) &
!         'APM NOTICE: TES NH3 expected value is NaN'
!         call error_handler(E_ALLMSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
! Clean up and return
   deallocate(fld_val, tmp_val, qmr_val)
   deallocate(prs_tes)

end subroutine get_expected_tes_nh3_profile

!-------------------------------------------------------------------------------

subroutine set_obs_def_tes_nh3_profile(key, nh3_pressure, nh3_avg_kernel, nh3_prior, &
nh3_nlayer, nh3_klev, nh3_kend)

   integer,                           intent(in)   :: key, nh3_nlayer, nh3_klev, nh3_kend
   real(r8), dimension(nh3_nlayer),    intent(in)   :: nh3_pressure
   ...
   if(num_tes_nh3_obs >= max_tes_nh3_obs) then
      write(string1, *)'Not enough space for tes nh3 obs.'
      write(string2, *)'Can only have max_tes_nh3_obs (currently ',max_tes_nh3_obs,')'
      call error_handler(E_ERR,'set_obs_def_tes_nh3_profile',string1,source,revision, &
   ...
   klev(key) = nh3_klev
   kend(key) = nh3_kend
   pressure(key,1:nh3_nlayer)   = nh3_pressure(1:nh3_nlayer)
   avg_kernel(key,1:nh3_nlayer) = nh3_avg_kernel(1:nh3_nlayer)
   prior(key,1:nh3_nlayer)      = nh3_prior(1:nh3_nlayer)
   
end subroutine set_obs_def_tes_nh3_profile

real function random_stdnormal(x)
   implicit none
   ...
   random_stdnormal = sqrt(-2.*log(u1))*cos(2.*pi*u2)
end function random_stdnormal

end module obs_def_tes_nh3_profile_mod
