...
!   use obs_def_tes_o3_cpsr_mod, only : get_expected_tes_o3_cpsr, &
!                                  read_tes_o3_cpsr, &
!                                  write_tes_o3_cpsr, &
!                                  interactive_tes_o3_cpsr
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(TES_O3_CPSR)                                                           
!         call get_expected_tes_o3_cpsr(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(TES_O3_CPSR)
!         call read_tes_o3_cpsr(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(TES_O3_CPSR)
!         call write_tes_o3_cpsr(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(TES_O3_CPSR)
!         call interactive_tes_o3_cpsr(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_tes_o3_cpsr_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_tes_o3_cpsr, &
             read_tes_o3_cpsr, &
          interactive_tes_o3_cpsr, &
          get_expected_tes_o3_cpsr, &
          set_obs_def_tes_o3_cpsr
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_tes_o3_cpsr_mod.f90'
...
! Check for valid values
   ...
   allocate(    kmodes(max_tes_o3_obs))
...
subroutine read_tes_o3_cpsr(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_tes_o3_obs) then
      ...
      call error_handler(E_ERR,'read_tes_o3_cpsr',string1,source,text2=string2)
   endif
   
   call set_obs_def_tes_o3_cpsr(key, pressure_1(1:nlayer_1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, kmodes_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_tes_o3_cpsr

!-------------------------------------------------------------------------------

subroutine write_tes_o3_cpsr(key, ifile, fform)
...
end subroutine write_tes_o3_cpsr

!-------------------------------------------------------------------------------

subroutine interactive_tes_o3_cpsr(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_tes_o3_cpsr not yet working.'
   call error_handler(E_ERR, 'interactive_tes_o3_cpsr', string1, source)
   
   if(num_tes_o3_obs >= max_tes_o3_obs) then
      write(string1, *)'Not enough space for an tes o3 obs.'
      write(string2, *)'Can only have max_tes_o3_obs (currently ',max_tes_o3_obs,')'
      call error_handler(E_ERR, 'interactive_tes_o3_cpsr', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_tes_o3_cpsr observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_tes_o3_cpsr

!-------------------------------------------------------------------------------

subroutine get_expected_tes_o3_cpsr(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_tes_o3_cpsr'
...
! (TES O3 grid is bottom to top) prs is in Pa
      prs_tes_mem(:)=prs_tes(:)      
...
! o3
      delta=(o3_val(imem,layer_tes)-o3_val(imem,layer_tes-1))/ &
      (prs_tes(layer_tes)-prs_tes(layer_tes-1))
      o3_top=o3_val(imem,layer_tes) + delta*(prs_top-prs_tes(layer_tes))
      if(o3_top.le.0.) then
         if(delta.le.0.) o3_top=bdy_coef*o3_val(imem,layer_tes)
         if(delta.gt.0.) o3_top=(2.-bdy_coef)*o3_val(imem,layer_tes)
      endif
! tmp
...
! expected retrieval sum
         ...
         if(key.eq.1 .and. imem.eq.1) then
            ...
            avg_kernel(key,k), o3_val_conv, prior(key,k)
            call error_handler(E_MSG, routine, string1, source)
         endif
!
! k=layer_tes term
...
! expected retrieval
      ...
      if(expct_val(imem).lt.0) then
         zstatus(imem)=20
         expct_val(:)=missing_r8
         write(string1, *) 'APM NOTICE: TES O3 expected value is negative '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
end subroutine get_expected_tes_o3_cpsr
...
subroutine set_obs_def_tes_o3_cpsr(key, o3_pressure, o3_avg_kernel, o3_prior, &
o3_nlayer, o3_kmodes, o3_kend)

   integer,                           intent(in)   :: key, o3_nlayer, o3_kmodes, o3_kend
   ...
   if(num_tes_o3_obs >= max_tes_o3_obs) then
      write(string1, *)'Not enough space for tes o3 obs.'
      write(string2, *)'Can only have max_tes_o3_obs (currently ',max_tes_o3_obs,')'
      call error_handler(E_ERR,'set_obs_def_tes_o3_cpsr',string1,source,revision, &
      revdate,text2=string2)
   endif
   
   nlayer(key) = o3_nlayer
   kmodes(key) = o3_kmodes
   kend(key) = o3_kend
...
end subroutine set_obs_def_tes_o3_cpsr
...
end module obs_def_tes_o3_cpsr_mod
