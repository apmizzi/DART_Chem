...
! TES_O3_TOTAL_COL, QTY_O3
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_tes_o3_total_col_mod, only : get_expected_tes_o3_total_col, &
!                                  read_tes_o3_total_col, &
!                                  write_tes_o3_total_col, &
!                                  interactive_tes_o3_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(TES_O3_TOTAL_COL)                                                           
!         call get_expected_tes_o3_total_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(TES_O3_TOTAL_COL)
!         call read_tes_o3_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(TES_O3_TOTAL_COL)
!         call write_tes_o3_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(TES_O3_TOTAL_COL)
!         call interactive_tes_o3_total_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_tes_o3_total_col_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_tes_o3_total_col, &
             read_tes_o3_total_col, &
          interactive_tes_o3_total_col, &
          get_expected_tes_o3_total_col, &
          set_obs_def_tes_o3_total_col

! Storage for the special information required for observations of this type
   integer, parameter    :: max_tes_o3_obs = 10000000
   integer               :: num_tes_o3_obs = 0
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_tes_o3_total_col_mod.f90'
...
! Namelist with default values
   ...
   integer :: nlayer_tes_o3_total_col = -9999
   integer :: nlayer_tes_o3_trop_col = -9999
   integer :: nlayer_tes_o3_profile = -9999
   
   namelist /obs_def_TES_O3_nml/ use_log_o3, nlayer_model, &
   nlayer_tes_o3_total_col, nlayer_tes_o3_trop_col, nlayer_tes_o3_profile
...
! Read namelist values
   call find_namelist_in_file("input.nml", "obs_def_TES_O3_nml", iunit)
   read(iunit, nml = obs_def_TES_O3_nml, iostat = rc)
   call check_namelist_read(iunit, rc, "obs_def_TES_O3_nml")

! Record the namelist values
   if (do_nml_file()) write(nmlfileunit, nml=obs_def_TES_O3_nml)
   if (do_nml_term()) write(     *     , nml=obs_def_TES_O3_nml)

! Check for valid values
   nlayer_tes=nlayer_tes_o3_total_col

   if (nlayer_model < 1) then
      write(string1,*)'obs_def_TES_O3_nml:nlayer_model must be > 0, it is ',nlayer_model
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   if (nlayer_tes < 1) then
      write(string1,*)'obs_def_TES_O3_nml:nlayer_tes must be > 0, it is ',nlayer_tes
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   allocate(    nlayer(max_tes_o3_obs))
   allocate(    klev(max_tes_o3_obs))
   allocate(    kend(max_tes_o3_obs))
   allocate(  pressure(max_tes_o3_obs,nlayer_tes+1))
   allocate(avg_kernel(max_tes_o3_obs,nlayer_tes))
   allocate(     prior(max_tes_o3_obs,nlayer_tes))
   
end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_tes_o3_total_col(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_tes_o3_obs) then
      write(string1, *)'Not enough space for tes o3 obs.'
      write(string2, *)'Can only have max_tes_o3_obs (currently ',max_tes_o3_obs,')'
      call error_handler(E_ERR,'read_tes_o3_total_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_tes_o3_total_col(key, pressure_1(1:nlayer_1+1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_tes_o3_total_col

!-------------------------------------------------------------------------------

subroutine write_tes_o3_total_col(key, ifile, fform)
...
end subroutine write_tes_o3_total_col

!-------------------------------------------------------------------------------

subroutine interactive_tes_o3_total_col(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_tes_o3_total_col not yet working.'
   call error_handler(E_ERR, 'interactive_tes_o3_total_col', string1, source)
   
   if(num_tes_o3_obs >= max_tes_o3_obs) then
      write(string1, *)'Not enough space for an tes o3 obs.'
      write(string2, *)'Can only have max_tes_o3_obs (currently ',max_tes_o3_obs,')'
      call error_handler(E_ERR, 'interactive_tes_o3_total_col', string1, &
                 source, text2=string2)
   endif
   
! Increment the index
   num_tes_o3_obs = num_tes_o3_obs + 1
   key            = num_tes_o3_obs

! Otherwise, prompt for input for the three required beasts

   write(*, *) 'Creating an interactive_tes_o3_total_col observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_tes_o3_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_tes_o3_total_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_tes_o3_total_col'
...
! Assign vertical grid information (TES O3 grid is top to bottom)
...
! Get total_cols at TES pressure levels
   ...
   allocate(o3_val(ens_size,level_tes))
...
!
         call get_upper_bdy_o3(lon_obs,lat_obs,prs_tes_top,ncnt, &
      ...
      do k=1,level_tes   
         ...
         qmr_val(imem,k).lt.0.) then
            ...
            if(k.le.kstart(imem)) then
               do kk=1,level_tes
                  write(string1, *) &
                  'APM: prs, o3, tmp, qmr',key,imem,prs_tes(kk),o3_val(imem,kk), &
...
! (TES O3 vertical grid is top to bottom)
      prs_tes_mem(:)=prs_tes(:)
...
!
      if(expct_val(imem).lt.0) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: TES O3 expected value is negative '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
end subroutine get_expected_tes_o3_total_col
...
subroutine set_obs_def_tes_o3_total_col(key, o3_pressure, o3_avg_kernel, o3_prior, &
o3_nlayer, o3_klev, o3_kend)
   ...
   if(num_tes_o3_obs >= max_tes_o3_obs) then
      write(string1, *)'Not enough space for tes o3 obs.'
      write(string2, *)'Can only have max_tes_o3_obs (currently ',max_tes_o3_obs,')'
      call error_handler(E_ERR,'set_obs_def_tes_o3_total_col',string1,source,revision, &
...
end subroutine set_obs_def_tes_o3_total_col
...
end module obs_def_tes_o3_total_col_mod
