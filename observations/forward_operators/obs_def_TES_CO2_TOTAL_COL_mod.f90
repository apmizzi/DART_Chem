...
! TES_CO2_TOTAL_COL, QTY_CO2
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_tes_co2_total_col_mod, only : get_expected_tes_co2_total_col, &
!                                  read_tes_co2_total_col, &
!                                  write_tes_co2_total_col, &
!                                  interactive_tes_co2_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(TES_CO2_TOTAL_COL)                                                           
!         call get_expected_tes_co2_total_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(TES_CO2_TOTAL_COL)
!         call read_tes_co2_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(TES_CO2_TOTAL_COL)
!         call write_tes_co2_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(TES_CO2_TOTAL_COL)
!         call interactive_tes_co2_total_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_tes_co2_total_col_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_tes_co2_total_col, &
             read_tes_co2_total_col, &
          interactive_tes_co2_total_col, &
          get_expected_tes_co2_total_col, &
          set_obs_def_tes_co2_total_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_tes_co2_total_col_mod.f90'
...
! Namelist with default values
   ...
   namelist /obs_def_TES_CO2_nml/ use_log_co2, nlayer_model, &
...
! Check for valid values
   nlayer_tes=nlayer_tes_co2_total_col
...
subroutine read_tes_co2_total_col(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_tes_co2_obs) then
      ...
      call error_handler(E_ERR,'read_tes_co2_total_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_tes_co2_total_col(key, pressure_1(1:nlayer_1+1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_tes_co2_total_col

!-------------------------------------------------------------------------------

subroutine write_tes_co2_total_col(key, ifile, fform)
...
end subroutine write_tes_co2_total_col

!-------------------------------------------------------------------------------

subroutine interactive_tes_co2_total_col(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_tes_co2_total_col not yet working.'
   call error_handler(E_ERR, 'interactive_tes_co2_total_col', string1, source)
   
   if(num_tes_co2_obs >= max_tes_co2_obs) then
      write(string1, *)'Not enough space for an tes co2 obs.'
      write(string2, *)'Can only have max_tes_co2_obs (currently ',max_tes_co2_obs,')'
      call error_handler(E_ERR, 'interactive_tes_co2_total_col', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_tes_co2_total_col observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_tes_co2_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_tes_co2_total_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_tes_co2_total_col'
...
! pressure at model surface (Pa)
   ...
   do k=1,layer_mdl
      ...
      call interpolate(state_handle, ens_size, loc2, QTY_CO2, co2_mdl_tmp, zstatus) ! ppmv 
      zstatus(:)=0
...
!
      interp_new=0
      do imem=1,ens_size
         if(co2_mdl_tmp(imem).eq.missing_r8 .or. tmp_mdl_tmp(imem).eq.missing_r8 .or. &
         qmr_mdl_tmp(imem).eq.missing_r8 .or. prs_mdl_tmp(imem).eq.missing_r8) then
            interp_new=1
         else
            kbnd_1(imem)=k
            co2_mdl_1(imem)=co2_mdl_tmp(imem)
...
!   write(string1, *) 'APM: co2 lower bound ',key,co2_mdl_1
!   call error_handler(E_MSG, routine, string1, source)
...
! pressure at model top (Pa)
   ...
   do k=layer_mdl,1,-1
      ...
      zstatus(:)=0
      loc2 = set_location(mloc(1), mloc(2), level, VERTISLEVEL)
      call interpolate(state_handle, ens_size, loc2, QTY_CO2, co2_mdl_tmp, zstatus) 
      zstatus(:)=0
...
!
      ...
      do imem=1,ens_size
         ...
         qmr_mdl_tmp(imem).eq.missing_r8 .or. prs_mdl_tmp(imem).eq.missing_r8) then
            ...
            kbnd_n(imem)=k
            co2_mdl_n(imem)=co2_mdl_tmp(imem)
...
!   write(string1, *) 'APM: co2 upper bound ',key,co2_mdl_n
!   call error_handler(E_MSG, routine, string1, source)
...
! Check data for missing values      
      do imem=1,ens_size
         ...
         qmr_val(imem,k).eq.missing_r8) then
            ...
            do imemm=1,ens_size
               write(string1, *) &
               'APM: Model total_col values: co2,tmp,qmr',key,imem,k,co2_val(imemm,k), &
...
!
         call get_upper_bdy_co2(lon_obs,lat_obs,prs_tes_top,ncnt, &
...
! Impose ensemble perturbations from level kstart(imem)+1      
         ...
         do k=1,kstart(imem) 
            co2_val(imem,k)=co2_prf_mdl(k)*co2_val(imem,kstart(imem)+1)/ &
            (sum(co2_val(:,kstart(imem)+1))/real(ens_size))
...
end subroutine get_expected_tes_co2_total_col
...
subroutine set_obs_def_tes_co2_total_col(key, co2_pressure, co2_avg_kernel, co2_prior, &
co2_nlayer, co2_klev, co2_kend)

   integer,                           intent(in)   :: key, co2_nlayer, co2_klev, co2_kend
   real(r8), dimension(co2_nlayer+1),  intent(in)   :: co2_pressure
   ...
   if(num_tes_co2_obs >= max_tes_co2_obs) then
      write(string1, *)'Not enough space for tes co2 obs.'
      write(string2, *)'Can only have max_tes_co2_obs (currently ',max_tes_co2_obs,')'
      call error_handler(E_ERR,'set_obs_def_tes_co2_total_col',string1,source,revision, &
...
end module obs_def_tes_co2_total_col_mod
