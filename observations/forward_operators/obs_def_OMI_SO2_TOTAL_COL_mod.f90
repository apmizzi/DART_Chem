...
!                                   read_omi_so2_total_col, &
!                                   write_omi_so2_total_col, &
!                                   interactive_omi_so2_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(OMI_SO2_TOTAL_COL)                                                           
!         call get_expected_omi_so2_total_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(OMI_SO2_TOTAL_COL)
!         call read_omi_so2_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(OMI_SO2_TOTAL_COL)
!         call write_omi_so2_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(OMI_SO2_TOTAL_COL)
!         call interactive_omi_so2_total_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_omi_so2_total_col_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_omi_so2_total_col, &
          read_omi_so2_total_col, &
          interactive_omi_so2_total_col, &
          get_expected_omi_so2_total_col, &
          set_obs_def_omi_so2_total_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_omi_so2_total_col_mod.f90'
...
! Namelist with default values
   ...
   namelist /obs_def_OMI_SO2_nml/ use_log_so2, nlayer_model, &
   nlayer_omi_so2_total_col, nlayer_omi_so2_pbl_col
...
! Record the namelist values
   ...
   nlayer_omi=nlayer_omi_so2_total_col
...
subroutine read_omi_so2_total_col(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_omi_so2_obs) then
      ...
      call error_handler(E_ERR,'read_omi_so2_total_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_omi_so2_total_col(key, pressure_1, scat_wt_1, nlayer_1, kend_1)
   
   deallocate(pressure_1, scat_wt_1)

end subroutine read_omi_so2_total_col

!-------------------------------------------------------------------------------

subroutine write_omi_so2_total_col(key, ifile, fform)
...
end subroutine write_omi_so2_total_col

!-------------------------------------------------------------------------------

subroutine interactive_omi_so2_total_col(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_omi_so2_total_col not yet working.'
   call error_handler(E_ERR, 'interactive_omi_so2_total_col', string1, source)
   
   if(num_omi_so2_obs >= max_omi_so2_obs) then
      write(string1, *)'Not enough space for an omi so2 obs.'
      write(string2, *)'Can only have max_omi_so2_obs (currently ',max_omi_so2_obs,')'
      call error_handler(E_ERR, 'interactive_omi_so2_total_col', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_omi_so2_total_col observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_omi_so2_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_omi_so2_total_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_omi_so2_total_col'
...
!
         call get_upper_bdy_so2(lon_obs,lat_obs,prs_omi_top,ncnt, &
         so2_prf_mdl,tmp_prf_mdl,qmr_prf_mdl,date_obs,datesec_obs)
!
...
end subroutine get_expected_omi_so2_total_col

!-------------------------------------------------------------------------------

subroutine get_upper_bdy_so2(lon_obs,lat_obs,prs_obs,nprs_obs, &
so2_prf_mdl,tmp_prf_mdl,qmr_prf_mdl,date_obs,datesec_obs)
...
!   integer,parameter                                :: ntim=69
   ...
   real(r8),dimension(nprs_obs),      intent(out)   :: so2_prf_mdl,tmp_prf_mdl,qmr_prf_mdl
   ...
   real,dimension(nz)                               :: prs_glb,ztrp_so2,ztrp_tmp,ztrp_qmr
   real,dimension(nz)                               :: so2_glb_xmym,so2_glb_xpym,so2_glb_xmyp,so2_glb_xpyp
   ...
   real,dimension(nx,ny,nz,ntim)                    :: so2_glb,tmp_glb,qmr_glb
   character(len=120)                               :: data_file
   character(len=*), parameter                      :: routine = 'get_upper_bdy_so2'
!
...
! mozart
   call get_MOZART_REAL_DATA(data_file,'SO2_VMR_inst',nx,ny,nz,ntim,so2_glb)
! waccm
!   call get_MOZART_REAL_DATA(data_file,'SO2',nx,ny,nz,ntim,so2_glb)
...
! Temporal
   do k=1,nz
      so2_glb_xmym(k)=(wt_bck*so2_glb(indx,jndx,k,itim_sav) + &
      wt_fwd*so2_glb(indx,jndx,k,itim_sav+1))/(wt_bck+wt_fwd)
      so2_glb_xpym(k)=(wt_bck*so2_glb(indx+1,jndx,k,itim_sav) + &
      wt_fwd*so2_glb(indx+1,jndx,k,itim_sav+1))/(wt_bck+wt_fwd)
      so2_glb_xmyp(k)=(wt_bck*so2_glb(indx,jndx+1,k,itim_sav) + &
      wt_fwd*so2_glb(indx,jndx+1,k,itim_sav+1))/(wt_bck+wt_fwd)
      so2_glb_xpyp(k)=(wt_bck*so2_glb(indx+1,jndx+1,k,itim_sav) + &
      wt_fwd*so2_glb(indx+1,jndx+1,k,itim_sav+1))/(wt_bck+wt_fwd)
!
...
! Horizontal   
   do k=1,nz
      ztrp_jbck=(bck_xwt*so2_glb_xmym(k) + fwd_xwt*so2_glb_xpym(k))/twtx
      ztrp_jfwd=(bck_xwt*so2_glb_xmyp(k) + fwd_xwt*so2_glb_xpyp(k))/twtx
      ztrp_so2(k)=(bck_ywt*ztrp_jbck + fwd_ywt*ztrp_jfwd)/twty
!      
...
! Vertical   
   do k=1,nprs_obs
      ...
      so2_prf_mdl(k)=(zwt_up*ztrp_so2(kndx) + zwt_dw*ztrp_so2(kndx+1))/twt
   ...
   enddo
 end subroutine get_upper_bdy_so2
...
subroutine set_obs_def_omi_so2_total_col(key, so2_pressure, so2_scat_wt, so2_nlayer, so2_kend)
   ...
   if(num_omi_so2_obs >= max_omi_so2_obs) then
      write(string1, *)'Not enough space for omi so2 obs.'
      write(string2, *)'Can only have max_omi_so2_obs (currently ',max_omi_so2_obs,')'
      call error_handler(E_ERR,'set_obs_def_omi_so2_total_col',string1,source,revision, &
...
end subroutine set_obs_def_omi_so2_total_col

end module obs_def_omi_so2_total_col_mod
