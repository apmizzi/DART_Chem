...
! OMI_O3_PROFILE, QTY_O3
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_omi_o3_profile_mod, only : get_expected_omi_o3_profile, &
!                                  read_omi_o3_profile, &
!                                  write_omi_o3_profile, &
!                                  interactive_omi_o3_profile
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(OMI_O3_PROFILE)                                                           
!         call get_expected_omi_o3_profile(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(OMI_O3_PROFILE)
!         call read_omi_o3_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(OMI_O3_PROFILE)
!         call write_omi_o3_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(OMI_O3_PROFILE)
!         call interactive_omi_o3_profile(obs_def%key)
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_omi_o3_profile, &
          read_omi_o3_profile, &
          interactive_omi_o3_profile, &
          get_expected_omi_o3_profile, &
          set_obs_def_omi_o3_profile
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_omi_o3_profile_mod.f90'
...
! Namelist with default values
   ...
   integer :: nlayer_omi_o3_total_col = -9999
   integer :: nlayer_omi_o3_trop_col  = -9999
   integer :: nlayer_omi_o3_profile   = -9999

   namelist /obs_def_OMI_O3_nml/ use_log_o3, nlayer_model, &
   nlayer_omi_o3_total_col, nlayer_omi_o3_trop_col, nlayer_omi_o3_profile
...
! Check for valid values
   nlayer_omi=nlayer_omi_o3_profile
   ...
   if (nlayer_omi < 1) then
      write(string1,*) 'obs_def_OMI_O3_nml:nlayer_omi must be > 0, it is ',nlayer_omi
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif

   allocate(    nlayer(max_omi_o3_obs))
   allocate(    klev(max_omi_o3_obs))
...
subroutine read_omi_o3_profile(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_omi_o3_obs) then
      write(string1, *) 'Not enough space for omi o3 profile obs.'
      write(string2, *) 'Can only have max_omi_o3_obs (currently ',max_omi_o3_obs,')'
      call error_handler(E_ERR,'read_omi_o3_profile',string1,source,text2=string2)
   endif
...
subroutine write_omi_o3_profile(key, ifile, fform)
...
end subroutine write_omi_o3_profile

!-------------------------------------------------------------------------------

subroutine interactive_omi_o3_profile(key)
...
! STOP because routine is not finished.
   write(string1,*) 'interactive_omi_o3_profile not yet working.'
   call error_handler(E_ERR, 'interactive_omi_o3_profile', string1, source)
   
   if(num_omi_o3_obs >= max_omi_o3_obs) then
      write(string1, *) 'Not enough space for an omi o3 obs.'
      write(string2, *) 'Can only have max_omi_o3_obs (currently ',max_omi_o3_obs,')'
      call error_handler(E_ERR, 'interactive_omi_o3_profile', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_omi_o3_profile observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_omi_o3_profile

!-------------------------------------------------------------------------------

subroutine get_expected_omi_o3_profile(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
...
! 
      if(isnan(expct_val(imem))) then
         ...
         expct_val(:)=missing_r8
!         write(string1, *) &
!         'APM NOTICE: OMI O3 expected value is NaN for obs ',key
!         call error_handler(E_ALLMSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
!
      if(expct_val(imem).lt.0) then
         ...
         expct_val(:)=missing_r8
!         write(string1, *) &
!         'APM NOTICE: OMI O3 expected value is negative for obs ',key
!         call error_handler(E_ALLMSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
end subroutine get_expected_omi_o3_profile
...
subroutine set_obs_def_omi_o3_profile(key, o3_pressure, o3_avg_kernel, o3_prior, &
o3_nlayer, o3_klev, o3_kend)
   ...
   if(num_omi_o3_obs >= max_omi_o3_obs) then
      write(string1, *) 'Not enough space for omi o3 profile obs.'
      write(string2, *) 'Can only have max_omi_o3_obs (currently ',max_omi_o3_obs,')'
      call error_handler(E_ERR,'set_obs_def_omi_o3_profile',string1,source,revision, &
...
end subroutine set_obs_def_omi_o3_profile

end module obs_def_omi_o3_profile_mod
