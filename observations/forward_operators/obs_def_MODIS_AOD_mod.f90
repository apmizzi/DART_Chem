...
!----------------------------------------------------------------------
!subroutine get_expected_modis_aod(state, ens_size, location, key,  val, istatus)
   ...
   logical                          :: return_now
   character(len=*), parameter :: routine = 'get_expected_modis_aod'
...
!
   if(use_log_aod) then
      write(string1, *) 'APM: MODIS AOD Forward Operator not set up for log_aod = T '
      call error_handler(E_MSG, 'obs_def_MODIS_AOD', string1, source)
      call exit_all(-77)
   endif
!   
...
! Check for missing values
   ...
   any(ss2.lt.0) .or. any(ss3.lt.0) .or. any(ss4.lt.0)) then   
      zstatus(:)=20
      val(:)=missing_r8
      write(string1, *) 'APM: MODIS AOD FO has missing values '
...
! Check expected observation
   if(any(val.lt.0)) then
      zstatus(:)=20
      val(:)=missing_r8
      write(string1, *) 'APM: MODIS AOD expected value is negative '
      call error_handler(E_MSG, 'obs_def_MODIS_AOD', string1, source)
      call track_status(ens_size, zstatus, val, istatus, return_now)
      return
   endif
