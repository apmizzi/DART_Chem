...
! CRIS_O3_TOTAL_COL, QTY_O3
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_cris_o3_total_col_mod, only : get_expected_cris_o3_total_col, &
!                                  read_cris_o3_total_col, &
!                                  write_cris_o3_total_col, &
!                                  interactive_cris_o3_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(CRIS_O3_TOTAL_COL)                                                           
!         call get_expected_cris_o3_total_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(CRIS_O3_TOTAL_COL)
!         call read_cris_o3_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(CRIS_O3_TOTAL_COL)
!         call write_cris_o3_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(CRIS_O3_TOTAL_COL)
!         call interactive_cris_o3_total_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_cris_o3_total_col_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_cris_o3_total_col, &
             read_cris_o3_total_col, &
          interactive_cris_o3_total_col, &
          get_expected_cris_o3_total_col, &
          set_obs_def_cris_o3_total_col

! Storage for the special information required for observations of this type
   integer, parameter    :: max_cris_o3_obs = 10000000
   integer               :: num_cris_o3_obs = 0
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_cris_o3_total_col_mod.f90'
...
! Namelist with default values
   ...
   integer :: nlayer_cris_o3_total_col = -9999
   integer :: nlayer_cris_o3_trop_col = -9999
   integer :: nlayer_cris_o3_profile = -9999
   
   namelist /obs_def_CRIS_O3_nml/ use_log_o3, nlayer_model, &
   nlayer_cris_o3_total_col, nlayer_cris_o3_trop_col, nlayer_cris_o3_profile
...
! Read namelist values
   call find_namelist_in_file("input.nml", "obs_def_CRIS_O3_nml", iunit)
   read(iunit, nml = obs_def_CRIS_O3_nml, iostat = rc)
   call check_namelist_read(iunit, rc, "obs_def_CRIS_O3_nml")

! Record the namelist values
   if (do_nml_file()) write(nmlfileunit, nml=obs_def_CRIS_O3_nml)
   if (do_nml_term()) write(     *     , nml=obs_def_CRIS_O3_nml)

! Check for valid values
   nlayer_cris=nlayer_cris_o3_total_col

   if (nlayer_model < 1) then
      write(string1,*)'obs_def_CRIS_O3_nml:nlayer_model must be > 0, it is ',nlayer_model
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   if (nlayer_cris < 1) then
      write(string1,*)'obs_def_CRIS_O3_nml:nlayer_cris must be > 0, it is ',nlayer_cris
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   allocate(    nlayer(max_cris_o3_obs))
   allocate(    klev(max_cris_o3_obs))
   allocate(    kend(max_cris_o3_obs))
   allocate(  pressure(max_cris_o3_obs,nlayer_cris+1))
   allocate(avg_kernel(max_cris_o3_obs,nlayer_cris))
   allocate(     prior(max_cris_o3_obs,nlayer_cris))
   
end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_cris_o3_total_col(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_cris_o3_obs) then
      write(string1, *)'Not enough space for cris o3 obs.'
      write(string2, *)'Can only have max_cris_o3_obs (currently ',max_cris_o3_obs,')'
      call error_handler(E_ERR,'read_cris_o3_total_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_cris_o3_total_col(key, pressure_1(1:nlayer_1+1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_cris_o3_total_col

!-------------------------------------------------------------------------------

subroutine write_cris_o3_total_col(key, ifile, fform)
...
end subroutine write_cris_o3_total_col

!-------------------------------------------------------------------------------

subroutine interactive_cris_o3_total_col(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_cris_o3_total_col not yet working.'
   call error_handler(E_ERR, 'interactive_cris_o3_total_col', string1, source)
   
   if(num_cris_o3_obs >= max_cris_o3_obs) then
      write(string1, *)'Not enough space for an cris o3 obs.'
      write(string2, *)'Can only have max_cris_o3_obs (currently ',max_cris_o3_obs,')'
      call error_handler(E_ERR, 'interactive_cris_o3_total_col', string1, &
                 source, text2=string2)
   endif
   
! Increment the index
   num_cris_o3_obs = num_cris_o3_obs + 1
   key            = num_cris_o3_obs

! Otherwise, prompt for input for the three required beasts

   write(*, *) 'Creating an interactive_cris_o3_total_col observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_cris_o3_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_cris_o3_total_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_cris_o3_total_col'
   ...
   real(r8), dimension(ens_size) :: o3_mdl_tmp, tmp_mdl_tmp, qmr_mdl_tmp, prs_mdl_tmp
   real(r8), dimension(ens_size) :: prs_sfc,rec_o3_val,rec_tmp_val,rec_qmr_val
...
! Assign vertical grid information (CRIS O3 grid is top to bottom)
...
! pressure at model surface (Pa)
   ...
   do k=1,layer_mdl
      ...
      call interpolate(state_handle, ens_size, loc2, QTY_O3, o3_mdl_tmp, zstatus) ! ppmv 
      zstatus(:)=0
...
!
      interp_new=0
      do imem=1,ens_size
         if(o3_mdl_tmp(imem).eq.missing_r8 .or. tmp_mdl_tmp(imem).eq.missing_r8 .or. &
         qmr_mdl_tmp(imem).eq.missing_r8 .or. prs_mdl_tmp(imem).eq.missing_r8) then
            interp_new=1
         else
            kbnd_1(imem)=k
            o3_mdl_1(imem)=o3_mdl_tmp(imem)
...
! pressure at model top (Pa)
   ...
   do k=layer_mdl,1,-1
      ...
      zstatus(:)=0
      loc2 = set_location(mloc(1), mloc(2), level, VERTISLEVEL)
      call interpolate(state_handle, ens_size, loc2, QTY_O3, o3_mdl_tmp, zstatus) 
      zstatus(:)=0
...
! Get total_cols at CRIS pressure levels
   ...
   allocate(o3_val(ens_size,level_cris))
...
! Check data for missing values      
      do imem=1,ens_size
         ...
         qmr_val(imem,k).eq.missing_r8) then
            ...
            do imemm=1,ens_size
               write(string1, *) &
               'APM: Model total_col values: o3,tmp,qmr',key,imem,k,o3_val(imemm,k), &
...
!
         call get_upper_bdy_o3(lon_obs,lat_obs,prs_cris_top,ncnt, &
...
! Impose ensemble perturbations from level kstart(imem)+1      
         ...
         do k=1,kstart(imem) 
            o3_val(imem,k)=o3_prf_mdl(k)*o3_val(imem,kstart(imem)+1)/ &
            (sum(o3_val(:,kstart(imem)+1))/real(ens_size))
      ...
      do k=1,level_cris   
         ...
         qmr_val(imem,k).lt.0.) then
            ...
            if(k.le.kstart(imem)) then
               do kk=1,level_cris
                  write(string1, *) &
                  'APM: prs, o3, tmp, qmr',key,imem,prs_cris(kk),o3_val(imem,kk), &
...
! (CRIS O3 vertical grid is top to bottom)
...
!         'avgk*thick*o3_conv ', avg_kernel(key,k)*thick(k)*o3_val_conv, &
...
!      
      if(isnan(expct_val(imem))) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: CRIS O3 expected value is NaN '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
!
      if(expct_val(imem).lt.0) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: CRIS O3 expected value is negative '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
end subroutine get_expected_cris_o3_total_col
...
subroutine set_obs_def_cris_o3_total_col(key, o3_pressure, o3_avg_kernel, o3_prior, &
o3_nlayer, o3_klev, o3_kend)
   ...
   if(num_cris_o3_obs >= max_cris_o3_obs) then
      write(string1, *)'Not enough space for cris o3 obs.'
      write(string2, *)'Can only have max_cris_o3_obs (currently ',max_cris_o3_obs,')'
      call error_handler(E_ERR,'set_obs_def_cris_o3_total_col',string1,source,revision, &
...
end subroutine set_obs_def_cris_o3_total_col

end module obs_def_cris_o3_total_col_mod
