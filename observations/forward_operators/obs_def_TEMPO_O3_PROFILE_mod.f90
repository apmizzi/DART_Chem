...
! TEMPO_O3_PROFILE, QTY_O3
! END DART PREPROCESS KIND LIST
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_tempo_o3_profile_mod, only : get_expected_tempo_o3_profile, &
!                                  read_tempo_o3_profile, &
!                                  write_tempo_o3_profile, &
!                                  interactive_tempo_o3_profile
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(TEMPO_O3_PROFILE)                                                           
!         call get_expected_tempo_o3_profile(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(TEMPO_O3_PROFILE)
!         call read_tempo_o3_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(TEMPO_O3_PROFILE)
!         call write_tempo_o3_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(TEMPO_O3_PROFILE)
!         call interactive_tempo_o3_profile(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_tempo_o3_profile_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_tempo_o3_profile, &
             read_tempo_o3_profile, &
          interactive_tempo_o3_profile, &
          get_expected_tempo_o3_profile, &
          set_obs_def_tempo_o3_profile
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_tempo_o3_profile_mod.f90'
...
! Namelist with default values
   ...
   integer :: nlayer_tempo_o3_total_col = -9999
   integer :: nlayer_tempo_o3_trop_col = -9999
   integer :: nlayer_tempo_o3_profile = -9999
   
   namelist /obs_def_TEMPO_O3_nml/ use_log_o3, nlayer_model, &
   nlayer_tempo_o3_total_col, nlayer_tempo_o3_trop_col, nlayer_tempo_o3_profile
...
! Check for valid values
   nlayer_tempo=nlayer_tempo_o3_profile
   ...
   if (nlayer_tempo < 1) then
      write(string1,*)'obs_def_TEMPO_O3_nml:nlayer_tempo must be > 0, it is ',nlayer_tempo
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   allocate(    nlayer(max_tempo_o3_obs))
   allocate(    klev(max_tempo_o3_obs))
   allocate(    kend(max_tempo_o3_obs))
   allocate(  pressure(max_tempo_o3_obs,nlayer_tempo+1))
   allocate(avg_kernel(max_tempo_o3_obs,nlayer_tempo))
   allocate(     prior(max_tempo_o3_obs,nlayer_tempo))
   
end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_tempo_o3_profile(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_tempo_o3_obs) then
      ...
      call error_handler(E_ERR,'read_tempo_o3_profile',string1,source,text2=string2)
   endif
   
   call set_obs_def_tempo_o3_profile(key, pressure_1(1:nlayer_1+1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_tempo_o3_profile

!-------------------------------------------------------------------------------

subroutine write_tempo_o3_profile(key, ifile, fform)
...
end subroutine write_tempo_o3_profile

!-------------------------------------------------------------------------------

subroutine interactive_tempo_o3_profile(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_tempo_o3_profile not yet working.'
   call error_handler(E_ERR, 'interactive_tempo_o3_profile', string1, source)
   
   if(num_tempo_o3_obs >= max_tempo_o3_obs) then
      write(string1, *)'Not enough space for an tempo o3 obs.'
      write(string2, *)'Can only have max_tempo_o3_obs (currently ',max_tempo_o3_obs,')'
      call error_handler(E_ERR, 'interactive_tempo_o3_profile', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_tempo_o3_profile observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_tempo_o3_profile

!-------------------------------------------------------------------------------

subroutine get_expected_tempo_o3_profile(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_tempo_o3_profile'
...
! Assign vertical grid information (TEMPO O3 grid is top to bottom)
...
! Get profiles at TEMPO pressure levels
...
!
         call get_upper_bdy_o3(lon_obs,lat_obs,prs_tempo_top,ncnt, &
      ...
      do k=1,level_tempo   
         ...
         qmr_val(imem,k).lt.0.) then
            ...
            if(k.le.kstart(imem)) then
               do kk=1,level_tempo
                  write(string1, *) &
                  'APM: prs, o3, tmp, qmr',key,imem,prs_tempo(kk),o3_val(imem,kk), &
...
! (TEMPO O3 vertical grid is top to bottom)
...
!      
      if(isnan(expct_val(imem))) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: TEMPO O3 expected value is NaN '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
!
      if(expct_val(imem).lt.0) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: TEMPO O3 expected value is negative '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
end subroutine get_expected_tempo_o3_profile
...
subroutine set_obs_def_tempo_o3_profile(key, o3_pressure, o3_avg_kernel, o3_prior, &
o3_nlayer, o3_klev, o3_kend)
   ...
   if(num_tempo_o3_obs >= max_tempo_o3_obs) then
      write(string1, *)'Not enough space for tempo o3 obs.'
      write(string2, *)'Can only have max_tempo_o3_obs (currently ',max_tempo_o3_obs,')'
      call error_handler(E_ERR,'set_obs_def_tempo_o3_profile',string1,source,revision, &
...
end subroutine set_obs_def_tempo_o3_profile
...
end module obs_def_tempo_o3_profile_mod
