...
! CRIS_CO_PROFILE, QTY_CO
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_cris_co_profile_mod, only : get_expected_cris_co_profile, &
!                                  read_cris_co_profile, &
!                                  write_cris_co_profile, &
!                                  interactive_cris_co_profile
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(CRIS_CO_PROFILE)                                                           
!         call get_expected_cris_co_profile(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(CRIS_CO_PROFILE)
!         call read_cris_co_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(CRIS_CO_PROFILE)
!         call write_cris_co_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(CRIS_CO_PROFILE)
!         call interactive_cris_co_profile(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_cris_co_profile_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_cris_co_profile, &
             read_cris_co_profile, &
          interactive_cris_co_profile, &
          get_expected_cris_co_profile, &
          set_obs_def_cris_co_profile
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_cris_co_profile_mod.f90'
...
! Check for valid values
   nlayer_cris=nlayer_cris_co_profile
   ...
   allocate(  pressure(max_cris_co_obs,nlayer_cris))
...
subroutine read_cris_co_profile(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_cris_co_obs) then
      ...
      call error_handler(E_ERR,'read_cris_co_profile',string1,source,text2=string2)
   endif
   
   call set_obs_def_cris_co_profile(key, pressure_1(1:nlayer_1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_cris_co_profile

!-------------------------------------------------------------------------------

subroutine write_cris_co_profile(key, ifile, fform)
...
end subroutine write_cris_co_profile

!-------------------------------------------------------------------------------

subroutine interactive_cris_co_profile(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_cris_co_profile not yet working.'
   call error_handler(E_ERR, 'interactive_cris_co_profile', string1, source)
   
   if(num_cris_co_obs >= max_cris_co_obs) then
      write(string1, *)'Not enough space for an cris co obs.'
      write(string2, *)'Can only have max_cris_co_obs (currently ',max_cris_co_obs,')'
      call error_handler(E_ERR, 'interactive_cris_co_profile', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_cris_co_profile observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_cris_co_profile

!-------------------------------------------------------------------------------

subroutine get_expected_cris_co_profile(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_cris_co_profile'
...
end subroutine get_expected_cris_co_profile
...
subroutine set_obs_def_cris_co_profile(key, co_pressure, co_avg_kernel, co_prior, &
co_nlayer, co_klev, co_kend)

   integer,                           intent(in)   :: key, co_nlayer, co_klev, co_kend
   real(r8), dimension(co_nlayer),  intent(in)   :: co_pressure
   ...
   if(num_cris_co_obs >= max_cris_co_obs) then
      write(string1, *)'Not enough space for cris co obs.'
      write(string2, *)'Can only have max_cris_co_obs (currently ',max_cris_co_obs,')'
      call error_handler(E_ERR,'set_obs_def_cris_co_profile',string1,source,revision, &
   ...
   klev(key) = co_klev
   kend(key) = co_kend
   pressure(key,1:co_nlayer) = co_pressure(1:co_nlayer)
   avg_kernel(key,1:co_nlayer) = co_avg_kernel(1:co_nlayer)
   prior(key,1:co_nlayer)      = co_prior(1:co_nlayer)
   
end subroutine set_obs_def_cris_co_profile

end module obs_def_cris_co_profile_mod
