...
! TEMPO_O3_TROP_COL, QTY_O3
! END DART PREPROCESS KIND LIST
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_tempo_o3_trop_col_mod, only : get_expected_tempo_o3_trop_col, &
!                                  read_tempo_o3_trop_col, &
!                                  write_tempo_o3_trop_col, &
!                                  interactive_tempo_o3_trop_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(TEMPO_O3_TROP_COL)                                                           
!         call get_expected_tempo_o3_trop_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(TEMPO_O3_TROP_COL)
!         call read_tempo_o3_trop_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(TEMPO_O3_TROP_COL)
!         call write_tempo_o3_trop_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(TEMPO_O3_TROP_COL)
!         call interactive_tempo_o3_trop_col(obs_def%key)
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_tempo_o3_trop_col, &
             read_tempo_o3_trop_col, &
          interactive_tempo_o3_trop_col, &
          get_expected_tempo_o3_trop_col, &
          set_obs_def_tempo_o3_trop_col

! Storage for the special information required for observations of this type
   integer, parameter    :: max_tempo_o3_obs = 10000000
   integer               :: num_tempo_o3_obs = 0
   integer,  allocatable :: trop_index(:)
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_tempo_o3_trop_col_mod.f90'
...
! Record the namelist values
   ...
   nlayer_tempo=nlayer_tempo_o3_trop_col
...
! Check for valid values
   ...
   allocate(    trop_index(max_tempo_o3_obs))
   ...
   allocate(     prior(max_tempo_o3_obs))
   
end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_tempo_o3_trop_col(key, ifile, fform)
...
! Need to know how many layers for this one
   trop_index_1 = read_int_scalar( ifile, fileformat, 'trop_index_1')
   ...
   if(counts1 > max_tempo_o3_obs) then
      ...
      call error_handler(E_ERR,'read_tempo_o3_trop_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_tempo_o3_trop_col(key, pressure_1, avg_kernel_1,prior_1, trop_index_1, nlayer_1)
   
   deallocate(pressure_1, avg_kernel_1)
   
end subroutine read_tempo_o3_trop_col

!-------------------------------------------------------------------------------

subroutine write_tempo_o3_trop_col(key, ifile, fform)
...
! you can come extend the context strings to include the key if desired.
   ...
   call write_int_scalar(ifile,                     trop_index(key), fileformat,'trop_index')
...
end subroutine write_tempo_o3_trop_col

!-------------------------------------------------------------------------------

subroutine interactive_tempo_o3_trop_col(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_tempo_o3_trop_col not yet working.'
   call error_handler(E_ERR, 'interactive_tempo_o3_trop_col', string1, source)
   
   if(num_tempo_o3_obs >= max_tempo_o3_obs) then
      write(string1, *)'Not enough space for an tempo o3 obs.'
      write(string2, *)'Can only have max_tempo_o3_obs (currently ',max_tempo_o3_obs,')'
      call error_handler(E_ERR, 'interactive_tempo_o3_trop_col', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_tempo_o3_trop_col observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_tempo_o3_trop_col

!-------------------------------------------------------------------------------

subroutine get_expected_tempo_o3_trop_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_tempo_o3_trop_col'
   type(location_type) :: loc2
   
   integer :: layer_tempo,level_tempo, kend_tempo
   ...
   do imem=1,ens_size
      do k=1,level_tempo
         if (prs_tempo(k).ge.prs_mdl_n(imem)) then
            kstart(imem)=k-1
!            write(string1, *) 'APM: imem,k-1,prs,mdl_n ',imem,k-1,prs_tempo(k-1), &
!            prs_mdl_n(imem),prs_tempo(k)
!            call error_handler(E_MSG, routine, string1, source)
            exit
         endif
      enddo
...
!
! Save upper BC data in the profiles   
      do k=1,kstart(imem)
...
!      do k=1,level_tempo
!         write(string1, *) 'APM: imem,k,prs,o3,tmp,qmr ',imem,k,prs_tempo(k), &
...
! Calculate the thicknesses
      ...
      thick(:)=0.
      do k=1,layer_tempo
         lnpr_mid=(log(prs_tempo_mem(k))+log(prs_tempo_mem(k+1)))/2.
         up_wt=log(prs_tempo_mem(k+1))-lnpr_mid
         dw_wt=log(lnpr_mid)-log(prs_tempo_mem(k))
         tl_wt=up_wt+dw_wt
...
! Get expected observation
         ...
         if(imem.eq.1) then         
            ...
            k,expct_val(imem),(prs_tempo_mem(k)+prs_tempo_mem(k+1))/2., &
            thick(k), o3_val_conv, avg_kernel(key,k), prior(key)
            call error_handler(E_MSG, routine, string1, source)
         endif
         expct_val(imem)=expct_val(imem) + prior(key) 
      enddo
!
      if(imem.eq.1) then         
         write(string1, *) &
...
end subroutine get_expected_tempo_o3_trop_col
...
subroutine set_obs_def_tempo_o3_trop_col(key, o3_pressure, o3_avg_kernel, o3_prior, o3_trop_index, o3_nlayer)

   integer,                           intent(in)   :: key, o3_trop_index, o3_nlayer
   ...
   if(num_tempo_o3_obs >= max_tempo_o3_obs) then
      write(string1, *)'Not enough space for tempo o3 obs.'
      write(string2, *)'Can only have max_tempo_o3_obs (currently ',max_tempo_o3_obs,')'
      call error_handler(E_ERR,'set_obs_def_tempo_o3_trop_col',string1,source,revision, &
      revdate,text2=string2)
   endif
   
   trop_index(key) = o3_trop_index
...
end subroutine set_obs_def_tempo_o3_trop_col
...
end module obs_def_tempo_o3_trop_col_mod
