...
!   use obs_def_tropomi_no2_mod, only : get_expected_tropomi_no2, &
!                                  read_tropomi_no2, &
!                                  write_tropomi_no2, &
...
integer :: nlayer_tropomi_no2 = -9999
...
if (nlayer_tropomi_no2 < 1) then
   write(string1,*)'obs_def_TROPOMI_NO2_nml:nlayer_tropomi_no2 must be > 0, it is ',nlayer_tropomi_no2
   call error_handler(E_ERR,'initialize_module',string1,source)
endif
...
allocate(  pressure(max_tropomi_no2_obs,nlayer_tropomi_no2+1))
allocate(avg_kernel(max_tropomi_no2_obs,nlayer_tropomi_no2))
...
if(counts1 > max_tropomi_no2_obs) then
   ...
   call error_handler(E_ERR,'read_tropomi_no2',string1,source,text2=string2)
endif
...
write(string1,*)'interactive_tropomi_no2 not yet working.'
call error_handler(E_ERR, 'interactive_tropomi_no2', string1, source)

if(num_tropomi_no2_obs >= max_tropomi_no2_obs) then
   write(string1, *)'Not enough space for an tropomi no2 obs.'
   write(string2, *)'Can only have max_tropomi_no2_obs (currently ',max_tropomi_no2_obs,')'
   call error_handler(E_ERR, 'interactive_tropomi_no2', string1, &
...
subroutine get_expected_tropomi_no2(state_handle, ens_size, location, key, expct_val, istatus)
...
character(len=*), parameter :: routine = 'get_expected_tropomi_no2'
...
do k=1,level_tropomi
   ...
   if(any(no2_istatus /= 0)) then
      write(string1,*) &
      'APM NOTICE: model NO2 obs value on TROPOMI grid has a bad value '
      call error_handler(E_MSG, routine, string1, source)
...
subroutine set_obs_def_tropomi_no2(key, no2_pressure, no2_avg_kernel, no2_amf_trop, no2_trop_indx, no2_nlayer)
...
if(num_tropomi_no2_obs >= max_tropomi_no2_obs) then
   write(string1, *)'Not enough space for tropomi no2 obs.'
   write(string2, *)'Can only have max_tropomi_no2_obs (currently ',max_tropomi_no2_obs,')'
   call error_handler(E_ERR,'set_obs_def_tropomi_no2',string1,source,revision, &
