...
! MLS_HNO3_TOTAL_COL, QTY_HNO3
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_mls_hno3_total_col_mod, only : get_expected_mls_hno3_total_col, &
!                                  read_mls_hno3_total_col, &
!                                  write_mls_hno3_total_col, &
!                                  interactive_mls_hno3_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(MLS_HNO3_TOTAL_COL)                                                           
!         call get_expected_mls_hno3_total_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(MLS_HNO3_TOTAL_COL)
!         call read_mls_hno3_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(MLS_HNO3_TOTAL_COL)
!         call write_mls_hno3_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(MLS_HNO3_TOTAL_COL)
!         call interactive_mls_hno3_total_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_mls_hno3_total_col_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_mls_hno3_total_col, &
             read_mls_hno3_total_col, &
          interactive_mls_hno3_total_col, &
          get_expected_mls_hno3_total_col, &
          set_obs_def_mls_hno3_total_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_mls_hno3_total_col_mod.f90'
...
! Check for valid values
   nlayer_mls=nlayer_mls_hno3_total_col
   ...
   allocate(    prior(max_mls_hno3_obs))
   allocate(  pressure(max_mls_hno3_obs,nlayer_mls+1))
...
subroutine read_mls_hno3_total_col(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_mls_hno3_obs) then
      ...
      call error_handler(E_ERR,'read_mls_hno3_total_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_mls_hno3_total_col(key, pressure_1(1:nlayer_1+1), avg_kernel_1(1:nlayer_1), &
   prior_1, nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1)
   
end subroutine read_mls_hno3_total_col

!-------------------------------------------------------------------------------

subroutine write_mls_hno3_total_col(key, ifile, fform)
...
end subroutine write_mls_hno3_total_col

!-------------------------------------------------------------------------------

subroutine interactive_mls_hno3_total_col(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_mls_hno3_total_col not yet working.'
   call error_handler(E_ERR, 'interactive_mls_hno3_total_col', string1, source)
   
   if(num_mls_hno3_obs >= max_mls_hno3_obs) then
      write(string1, *)'Not enough space for an mls o3 obs.'
      write(string2, *)'Can only have max_mls_hno3_obs (currently ',max_mls_hno3_obs,')'
      call error_handler(E_ERR, 'interactive_mls_hno3_total_col', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_mls_hno3_total_col observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_mls_hno3_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_mls_hno3_total_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_mls_hno3_total_col'
...
! Check data for missing values      
      do imem=1,ens_size
         ...
         qmr_val(imem,k).eq.missing_r8) then
            ...
            do imemm=1,ens_size
               write(string1, *) &
               'APM: Model total_col values: hno3,tmp,qmr',key,imem,k,hno3_val(imemm,k), &
...
end subroutine get_expected_mls_hno3_total_col

!-------------------------------------------------------------------------------

subroutine set_obs_def_mls_hno3_total_col(key, hno3_pressure, hno3_avg_kernel, hno3_prior, &
hno3_nlayer, hno3_klev, hno3_kend)

   integer,                             intent(in)   :: key, hno3_nlayer, hno3_klev, hno3_kend
   real(r8),                            intent(in)   :: hno3_prior
   real(r8), dimension(hno3_nlayer+1),  intent(in)   :: hno3_pressure
   ...
   if(num_mls_hno3_obs >= max_mls_hno3_obs) then
      write(string1, *)'Not enough space for mls hno3 obs.'
      write(string2, *)'Can only have max_mls_hno3_obs (currently ',max_mls_hno3_obs,')'
      call error_handler(E_ERR,'set_obs_def_mls_hno3_total_col',string1,source,revision, &
   ...
   klev(key) = hno3_klev
   kend(key) = hno3_kend
   prior(key)                    = hno3_prior
   pressure(key,1:hno3_nlayer+1) = hno3_pressure(1:hno3_nlayer+1)
   avg_kernel(key,1:hno3_nlayer) = hno3_avg_kernel(1:hno3_nlayer)
   
end subroutine set_obs_def_mls_hno3_total_col

end module obs_def_mls_hno3_total_col_mod
