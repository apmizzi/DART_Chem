...
! TES_NH3_TOTAL_COL, QTY_NH3
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_tes_nh3_total_col_mod, only : get_expected_tes_nh3_total_col, &
!                                  read_tes_nh3_total_col, &
!                                  write_tes_nh3_total_col, &
!                                  interactive_tes_nh3_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(TES_NH3_TOTAL_COL)                                                           
!         call get_expected_tes_nh3_total_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(TES_NH3_TOTAL_COL)
!         call read_tes_nh3_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(TES_NH3_TOTAL_COL)
!         call write_tes_nh3_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(TES_NH3_TOTAL_COL)
!         call interactive_tes_nh3_total_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_tes_nh3_total_col_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_tes_nh3_total_col, &
             read_tes_nh3_total_col, &
          interactive_tes_nh3_total_col, &
          get_expected_tes_nh3_total_col, &
          set_obs_def_tes_nh3_total_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_tes_nh3_total_col_mod.f90'
...
! Check for valid values
   nlayer_tes=nlayer_tes_nh3_total_col
   ...
   allocate(  pressure(max_tes_nh3_obs,nlayer_tes+1))
...
subroutine read_tes_nh3_total_col(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_tes_nh3_obs) then
      ...
      call error_handler(E_ERR,'read_tes_nh3_total_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_tes_nh3_total_col(key, pressure_1(1:nlayer_1+1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_tes_nh3_total_col

!-------------------------------------------------------------------------------

subroutine write_tes_nh3_total_col(key, ifile, fform)
...
end subroutine write_tes_nh3_total_col

!-------------------------------------------------------------------------------

subroutine interactive_tes_nh3_total_col(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_tes_nh3_total_col not yet working.'
   call error_handler(E_ERR, 'interactive_tes_nh3_total_col', string1, source)
   
   if(num_tes_nh3_obs >= max_tes_nh3_obs) then
      write(string1, *)'Not enough space for an tes nh3 obs.'
      write(string2, *)'Can only have max_tes_nh3_obs (currently ',max_tes_nh3_obs,')'
      call error_handler(E_ERR, 'interactive_tes_nh3_total_col', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_tes_nh3_total_col observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_tes_nh3_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_tes_nh3_total_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_tes_nh3_total_col'
...
! Get total_cols at TES pressure levels
   ...
   allocate(nh3_val(ens_size,level_tes))
...
! Check data for missing values      
      do imem=1,ens_size
         ...
         qmr_val(imem,k).eq.missing_r8) then
            ...
            do imemm=1,ens_size
               write(string1, *) &
               'APM: Model total_col values: nh3,tmp,qmr',key,imem,k,nh3_val(imemm,k), &
      ...
      do k=1,level_tes   
         ...
         qmr_val(imem,k).lt.0.) then
            ...
            if(k.le.kstart(imem)) then
               do kk=1,level_tes
                  write(string1, *) &
                  'APM: prs, nh3, tmp, qmr',key,imem,prs_tes(kk),nh3_val(imem,kk), &
...
! (TES NH3 vertical grid is top to bottom)
      prs_tes_mem(:)=prs_tes(:)
...
!
      if(expct_val(imem).lt.0) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: TES NH3 expected value is negative '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
end subroutine get_expected_tes_nh3_total_col
...
subroutine set_obs_def_tes_nh3_total_col(key, nh3_pressure, nh3_avg_kernel, nh3_prior, &
nh3_nlayer, nh3_klev, nh3_kend)
   ...
   if(num_tes_nh3_obs >= max_tes_nh3_obs) then
      write(string1, *)'Not enough space for tes nh3 obs.'
      write(string2, *)'Can only have max_tes_nh3_obs (currently ',max_tes_nh3_obs,')'
      call error_handler(E_ERR,'set_obs_def_tes_nh3_total_col',string1,source,revision, &
...
end subroutine set_obs_def_tes_nh3_total_col
...
end module obs_def_tes_nh3_total_col_mod
