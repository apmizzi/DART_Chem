...
! CRIS_PAN_PROFILE, QTY_PAN
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_cris_pan_profile_mod, only : get_expected_cris_pan_profile, &
!                                  read_cris_pan_profile, &
!                                  write_cris_pan_profile, &
!                                  interactive_cris_pan_profile
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(CRIS_PAN_PROFILE)                                                           
!         call get_expected_cris_pan_profile(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(CRIS_PAN_PROFILE)
!         call read_cris_pan_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(CRIS_PAN_PROFILE)
!         call write_cris_pan_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(CRIS_PAN_PROFILE)
!         call interactive_cris_pan_profile(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_cris_pan_profile_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_cris_pan_profile, &
             read_cris_pan_profile, &
          interactive_cris_pan_profile, &
          get_expected_cris_pan_profile, &
          set_obs_def_cris_pan_profile
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_cris_pan_profile_mod.f90'
...
! Check for valid values
   nlayer_cris=nlayer_cris_pan_profile
   ...
   allocate(     prior(max_cris_pan_obs,nlayer_cris))
   
end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_cris_pan_profile(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_cris_pan_obs) then
      ...
      call error_handler(E_ERR,'read_cris_pan_profile',string1,source,text2=string2)
   endif
   
   call set_obs_def_cris_pan_profile(key, pressure_1(1:nlayer_1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_cris_pan_profile

!-------------------------------------------------------------------------------

subroutine write_cris_pan_profile(key, ifile, fform)
...
end subroutine write_cris_pan_profile

!-------------------------------------------------------------------------------

subroutine interactive_cris_pan_profile(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_cris_pan_profile not yet working.'
   call error_handler(E_ERR, 'interactive_cris_pan_profile', string1, source)
   
   if(num_cris_pan_obs >= max_cris_pan_obs) then
      write(string1, *)'Not enough space for an cris pan obs.'
      write(string2, *)'Can only have max_cris_pan_obs (currently ',max_cris_pan_obs,')'
      call error_handler(E_ERR, 'interactive_cris_pan_profile', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_cris_pan_profile observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_cris_pan_profile

!-------------------------------------------------------------------------------

subroutine get_expected_cris_pan_profile(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_cris_pan_profile'
...
! Assign vertical grid information (CRIS PAN grid is top to bottom)
   ...
   prs_cris(1:level_cris)=pressure(key,1:layer_cris)
...
! Get profiles at CRIS pressure levels
...
! Check data for missing values      
      do imem=1,ens_size
         ...
         qmr_val(imem,k).eq.missing_r8) then
            ...
            do imemm=1,ens_size
               write(string1, *) &
               'APM: Model profile values: pan,tmp,qmr',key,imem,k,pan_val(imemm,k), &
...
end subroutine get_expected_cris_pan_profile
...
subroutine set_obs_def_cris_pan_profile(key, pan_pressure, pan_avg_kernel, pan_prior, &
pan_nlayer, pan_klev, pan_kend)
   ...
   real(r8), dimension(pan_nlayer),    intent(in)   :: pan_prior
   ...
   if(num_cris_pan_obs >= max_cris_pan_obs) then
      write(string1, *)'Not enough space for cris pan obs.'
      write(string2, *)'Can only have max_cris_pan_obs (currently ',max_cris_pan_obs,')'
      call error_handler(E_ERR,'set_obs_def_cris_pan_profile',string1,source,revision, &
   ...
   prior(key,1:pan_nlayer)      = pan_prior(1:pan_nlayer)
   
end subroutine set_obs_def_cris_pan_profile
...
end module obs_def_cris_pan_profile_mod
