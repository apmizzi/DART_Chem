...
! MOPITT_V9_CO_PROFILE, QTY_CO
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_mopitt_v9_co_profile_mod, only : get_expected_mopitt_v9_co_profile, &
!                                  read_mopitt_v9_co_profile, &
!                                  write_mopitt_v9_co_profile, &
!                                  interactive_mopitt_v9_co_profile
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(MOPITT_V9_CO_PROFILE)                                                           
!         call get_expected_mopitt_v9_co_profile(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(MOPITT_V9_CO_PROFILE)
!         call read_mopitt_v9_co_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(MOPITT_V9_CO_PROFILE)
!         call write_mopitt_v9_co_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(MOPITT_V9_CO_PROFILE)
!         call interactive_mopitt_v9_co_profile(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_mopitt_v9_co_profile_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_mopitt_v9_co_profile, &
          read_mopitt_v9_co_profile, &
          interactive_mopitt_v9_co_profile, &
          get_expected_mopitt_v9_co_profile, &
          set_obs_def_mopitt_v9_co_profile
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_mopitt_v9_co_profile_mod.f90'
...
subroutine read_mopitt_v9_co_profile(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_mopitt_co_obs) then
      ...
      call error_handler(E_ERR,'read_mopitt_v9_co_profile',string1,source,text2=string2)
   endif
   
   call set_obs_def_mopitt_v9_co_profile(key, pressure_1, avg_kernel_1, prior_1, kobs_1, nlayer_1, nlevel_1)
   
   deallocate(kobs_1, pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_mopitt_v9_co_profile

!-------------------------------------------------------------------------------

subroutine write_mopitt_v9_co_profile(key, ifile, fform)
...
end subroutine write_mopitt_v9_co_profile

!-------------------------------------------------------------------------------

subroutine interactive_mopitt_v9_co_profile(key)
...
! STOP because routine is not finished.
   write(string1,*) 'interactive_mopitt_v9_co_profile not yet working.'
   call error_handler(E_ERR, 'interactive_mopitt_v9_co_profile', string1, source)
   
   if(num_mopitt_co_obs >= max_mopitt_co_obs) then
      write(string1, *) 'Not enough space for an mopitt co obs.'
      write(string2, *) 'Can only have max_mopitt_co_obs (currently ',max_mopitt_co_obs,')'
      call error_handler(E_ERR, 'interactive_mopitt_v9_co_profile', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_mopitt_v9_co_profile observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_mopitt_v9_co_profile

!-------------------------------------------------------------------------------

subroutine get_expected_mopitt_v9_co_profile(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_mopitt_v9_co_profile'
...
! Upper BC variables   
   ...
   tmp_max  = 600._r8
   del_prs  = 5000._r8
   VMR_conv = 28.9644_r8/47.9982_r8
! 
! WACCM - MMR
! WRFChem - VMR ppmv
!
...
!
      interp_new=0
      do imem=1,ens_size
         if(co_mdl_1(imem).lt.0._r8 .or. tmp_mdl_1(imem).lt.0._r8 .or. &
         qmr_mdl_1(imem).lt.0._r8 .or. prs_mdl_1(imem).lt.0._r8) then
            interp_new=1
            exit
         endif
      enddo
...
!
      interp_new=0
      do imem=1,ens_size
         if(co_mdl_n(imem).lt.0._r8 .or. tmp_mdl_n(imem).lt.0._r8 .or. &
         qmr_mdl_n(imem).lt.0._r8 .or. prs_mdl_n(imem).lt.0._r8) then
            interp_new=1
            exit
         endif
      enddo
...
! Check full profile for negative values
   do imem=1,ens_size
      do k=1,level_mopitt
         if((co_val(imem,k).lt.0._r8 .and. co_val(imem,k).ne.missing_r8) .or. &
         (tmp_val(imem,k).lt.0._r8 .and. tmp_val(imem,k).ne.missing_r8) .or. &
         (qmr_val(imem,k).lt.0._r8 .and. qmr_val(imem,k).ne.missing_r8)) then
!            if(prs_mopitt(k).le.prs_mdl_1(imem) .and. prs_mopitt(k).ge.prs_mdl_2(imem)) then
            if(prs_mopitt(k).le.prs_mdl_1(imem) .and. prs_mopitt(k).ge.(prs_mdl_2(imem)-20000._r8)) then
               if(co_val(imem,k).lt.0._r8) co_val(imem,k)=co_mdl_1(imem)
               if(tmp_val(imem,k).lt.0._r8) tmp_val(imem,k)=tmp_mdl_1(imem)
               if(qmr_val(imem,k).lt.0._r8) qmr_val(imem,k)=qmr_mdl_1(imem)
!            elseif(prs_mopitt(k).le.prs_mdl_nm(imem) .and. prs_mopitt(k).ge.prs_mdl_n(imem)) then
!               if(co_val(imem,k).lt.0._r8) co_val(imem,k)=co_mdl_n(imem)
!               if(tmp_val(imem,k).lt.0._r8) tmp_val(imem,k)=tmp_mdl_n(imem)
!               if(qmr_val(imem,k).lt.0._r8) qmr_val(imem,k)=qmr_mdl_n(imem)
            else
               write(string1, *) &
         ...
         if(co_val(imem,k).lt.0._r8 .or. tmp_val(imem,k).lt.0._r8 .or. &
         qmr_val(imem,k).lt.0._r8) then
            write(string1, *) &
...
! Calculate the expected retrievals   
   ...
   do k=1,level_mopitt
      if(prs_mopitt_mem(k).le.0._r8) then
         write(string1, *) &
         'APM: MOPITT pressure is negative key',key,(prs_mopitt_mem(kk),kk=1,level_mopitt)
         call error_handler(E_ALLMSG, routine, string1, source)
         call exit_all(-77)
      endif
   enddo
...
! MOPITT vertical grid is from bottom to top   
   do imem=1,ens_size
      kstart=-1
      if ((prs_mopitt(level_mopitt)+prs_mopitt(level_mopitt-1))/2._r8.lt.prs_mdl_n(imem)) then
         do k=level_mopitt,1,-1
            if ((prs_mopitt(k)+prs_mopitt(k-1))/2._r8.gt.prs_mdl_n(imem)) then
               kstart=k
               write(string1, *) &
               'APM: FIX TOP imem,kstart,prs_mopitt,prs_mdl ',imem,kstart,prs_mopitt(k),prs_mopitt(k-1), &
               prs_mdl_n(imem)
               call error_handler(E_ALLMSG, routine, string1, source)
               exit
            endif
...
!         
         lnpr_mid=(log(prs_mopitt_mem(k))+log(prs_mopitt_mem(k+1)))/2._r8
         ...
         if(tl_wt .eq.0.) then       
            write(string1, *) &
            'APM: MSG 1 tl_wt is zero',key,tl_wt
            call error_handler(E_ALLMSG, routine, string1, source)
            call exit_all(-77)
         endif

! MOPITT retrieval is VMR (ppbv)
         if(use_log_co) then
...
! Use retrieval prior above regional model top         
         ...
         if(co_val_conv.le.0._r8 .or. prior(key,k).le.0._r8) then
            write(string1, *) &
            'APM: FPE k,kstart,mem,co_val_conv,prior ',k,kstart,imem,co_val_conv,prior(key,k)
            call error_handler(E_ALLMSG, routine, string1, source)
            write(string1, *) &
            'APM: FPE co_val_k,co_val_k+1,prs_k,prs_k+1 ',co_val(imem,k),co_val(imem,k+1), &
            prs_mopitt_mem(k),prs_mopitt_mem(k+1)
            call error_handler(E_ALLMSG, routine, string1, source)
         endif
         
! Get expected observation (MOPITT prior is VMR ppbv)

         prior_term=-1.0_r8*avg_kernel(key,k)
...
!         write(string1, *) &
!         'APM: VERT SUM k,expct_val,avgk_trm,prior_trm ',k,expct_val(imem), &
...
!      write(string1, *) &
!      'APM: EXPCT VAL ',key,imem,expct_val(imem),10.**expct_val(imem) 
!      call error_handler(E_ALLMSG, routine, string1, source)
...
end subroutine get_expected_mopitt_v9_co_profile

!-------------------------------------------------------------------------------

subroutine set_obs_def_mopitt_v9_co_profile(key, co_pressure, co_avg_kernel, co_prior, &
co_kobs, co_nlayer, co_nlevel)
   ...
   if(num_mopitt_co_obs >= max_mopitt_co_obs) then
      write(string1, *) 'Not enough space for mopitt co profile obs.'
      write(string2, *) 'Can only have max_mopitt_co_obs (currently ',max_mopitt_co_obs,')'
      call error_handler(E_ERR,'set_obs_def_mopitt_v9_co_profile',string1,source,revision, &
...
end subroutine set_obs_def_mopitt_v9_co_profile

end module obs_def_mopitt_v9_co_profile_mod
