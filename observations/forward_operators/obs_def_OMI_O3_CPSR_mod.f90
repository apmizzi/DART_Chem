...
!   use obs_def_omi_o3_cpsr_mod, only : get_expected_omi_o3_cpsr, &
!                                  read_omi_o3_cpsr, &
!                                  write_omi_o3_cpsr, &
...
!      case(OMI_O3_CPSR)                                                           
!         call get_expected_omi_o3_cpsr(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(OMI_O3_CPSR)
!         call read_omi_o3_cpsr(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(OMI_O3_CPSR)
!         call write_omi_o3_cpsr(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(OMI_O3_CPSR)
!         call interactive_omi_o3_cpsr(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_omi_o3_cpsr_mod
...
subroutine read_omi_o3_cpsr(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_omi_o3_obs) then
      write(string1, *) 'Not enough space for omi o3 cpsr obs.'
      write(string2, *) 'Can only have max_omi_o3_obs (currently ',max_omi_o3_obs,')'
      call error_handler(E_ERR,'read_omi_o3_cpsr',string1,source,text2=string2)
   endif
   
   call set_obs_def_omi_o3_cpsr(key, pressure_1, avg_kernel_1, prior_1, nlayer_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_omi_o3_cpsr

!-------------------------------------------------------------------------------

subroutine write_omi_o3_cpsr(key, ifile, fform)
...
subroutine interactive_omi_o3_cpsr(key)
...
! STOP because routine is not finished.
   write(string1,*) 'interactive_omi_o3_cpsr not yet working.'
   call error_handler(E_ERR, 'interactive_omi_o3_cpsr', string1, source)
   
   if(num_omi_o3_obs >= max_omi_o3_obs) then
      write(string1, *) 'Not enough space for an omi o3 obs.'
      write(string2, *) 'Can only have max_omi_o3_obs (currently ',max_omi_o3_obs,')'
      call error_handler(E_ERR, 'interactive_omi_o3_cpsr', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_omi_o3_cpsr observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_omi_o3_cpsr

!-------------------------------------------------------------------------------

subroutine get_expected_omi_o3_cpsr(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
...
end subroutine get_expected_omi_o3_cpsr

!-------------------------------------------------------------------------------

subroutine set_obs_def_omi_o3_cpsr(key, o3_pressure, o3_avg_kernel, o3_prior, &
o3_nlayer)
   ...
   if(num_omi_o3_obs >= max_omi_o3_obs) then
      write(string1, *) 'Not enough space for omi o3 cpsr obs.'
      write(string2, *) 'Can only have max_omi_o3_obs (currently ',max_omi_o3_obs,')'
      call error_handler(E_ERR,'set_obs_def_omi_o3_cpsr',string1,source,revision, &
...
end module obs_def_omi_o3_cpsr_mod
