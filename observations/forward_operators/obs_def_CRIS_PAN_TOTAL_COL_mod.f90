...
! CRIS_PAN_TOTAL_COL, QTY_PAN
! END DART PREPROCESS KIND LIST
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_cris_pan_total_col_mod, only : get_expected_cris_pan_total_col, &
!                                  read_cris_pan_total_col, &
!                                  write_cris_pan_total_col, &
!                                  interactive_cris_pan_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(CRIS_PAN_TOTAL_COL)                                                           
!         call get_expected_cris_pan_total_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(CRIS_PAN_TOTAL_COL)
!         call read_cris_pan_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(CRIS_PAN_TOTAL_COL)
!         call write_cris_pan_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(CRIS_PAN_TOTAL_COL)
!         call interactive_cris_pan_total_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_cris_pan_total_col_mod
   ...
   use          obs_kind_mod, only : QTY_PAN, QTY_TEMPERATURE, QTY_SURFACE_PRESSURE, &
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_cris_pan_total_col, &
             read_cris_pan_total_col, &
          interactive_cris_pan_total_col, &
          get_expected_cris_pan_total_col, &
          set_obs_def_cris_pan_total_col

! Storage for the special information required for observations of this type
   integer, parameter    :: max_cris_pan_obs = 10000000
   integer               :: num_cris_pan_obs = 0
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_cris_pan_total_col_mod.f90'
...
! Namelist with default values
   logical :: use_log_pan   = .false.
   integer :: nlayer_model = -9999
   integer :: nlayer_cris = -9999
   integer :: nlayer_cris_pan_total_col = -9999
...
! Read namelist values
   call find_namelist_in_file("input.nml", "obs_def_CRIS_PAN_nml", iunit)
   read(iunit, nml = obs_def_CRIS_PAN_nml, iostat = rc)
   call check_namelist_read(iunit, rc, "obs_def_CRIS_PAN_nml")

! Record the namelist values
   if (do_nml_file()) write(nmlfileunit, nml=obs_def_CRIS_PAN_nml)
   if (do_nml_term()) write(     *     , nml=obs_def_CRIS_PAN_nml)

! Check for valid values
   nlayer_cris=nlayer_cris_pan_total_col

   if (nlayer_model < 1) then
      write(string1,*)'obs_def_CRIS_PAN_nml:nlayer_model must be > 0, it is ',nlayer_model
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   if (nlayer_cris < 1) then
      write(string1,*)'obs_def_CRIS_PAN_nml:nlayer_cris must be > 0, it is ',nlayer_cris
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   allocate(    nlayer(max_cris_pan_obs))
   allocate(    klev(max_cris_pan_obs))
   allocate(    kend(max_cris_pan_obs))
   allocate(    prior(max_cris_pan_obs))
   allocate(  pressure(max_cris_pan_obs,nlayer_cris))
   allocate(avg_kernel(max_cris_pan_obs,nlayer_cris))
   
end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_cris_pan_total_col(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_cris_pan_obs) then
      write(string1, *)'Not enough space for cris pan obs.'
      write(string2, *)'Can only have max_cris_pan_obs (currently ',max_cris_pan_obs,')'
      call error_handler(E_ERR,'read_cris_pan_total_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_cris_pan_total_col(key, pressure_1(1:nlayer_1), avg_kernel_1(1:nlayer_1), &
   prior_1, nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1)
   
end subroutine read_cris_pan_total_col

!-------------------------------------------------------------------------------

subroutine write_cris_pan_total_col(key, ifile, fform)
...
end subroutine write_cris_pan_total_col

!-------------------------------------------------------------------------------

subroutine interactive_cris_pan_total_col(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_cris_pan_total_col not yet working.'
   call error_handler(E_ERR, 'interactive_cris_pan_total_col', string1, source)
   
   if(num_cris_pan_obs >= max_cris_pan_obs) then
      write(string1, *)'Not enough space for an cris pan obs.'
      write(string2, *)'Can only have max_cris_pan_obs (currently ',max_cris_pan_obs,')'
      call error_handler(E_ERR, 'interactive_cris_pan_total_col', string1, &
                 source, text2=string2)
   endif
   
! Increment the index
   num_cris_pan_obs = num_cris_pan_obs + 1
   key            = num_cris_pan_obs

! Otherwise, prompt for input for the three required beasts

   write(*, *) 'Creating an interactive_cris_pan_total_col observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_cris_pan_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_cris_pan_total_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_cris_pan_total_col'
   ...
   real(r8) :: missing,pan_min,tmp_max
   real(r8) :: level,del_prs,prior_term
   real(r8) :: tmp_vir_k, tmp_vir_kp
   real(r8) :: mloc(3),obs_prs
   real(r8) :: pan_val_conv, VMR_conv
   real(r8) :: up_wt,dw_wt,tl_wt,lnpr_mid
   real(r8) :: lon_obs,lat_obs,pi,rad2deg

   real(r8), dimension(ens_size) :: pan_mdl_1, tmp_mdl_1, qmr_mdl_1, prs_mdl_1
   real(r8), dimension(ens_size) :: pan_mdl_n, tmp_mdl_n, qmr_mdl_n, prs_mdl_n
   real(r8), dimension(ens_size) :: pan_mdl_tmp, tmp_mdl_tmp, qmr_mdl_tmp, prs_mdl_tmp
   real(r8), dimension(ens_size) :: prs_sfc,rec_pan_val,rec_tmp_val,rec_qmr_val
   
   real(r8), allocatable, dimension(:)   :: thick, prs_cris, prs_cris_mem
   real(r8), allocatable, dimension(:,:) :: pan_val, tmp_val, qmr_val
   real(r8), allocatable, dimension(:)   :: pan_prf_mdl,tmp_prf_mdl,qmr_prf_mdl
   real(r8), allocatable, dimension(:)   :: prs_cris_top   
   logical  :: return_now,pan_return_now,tmp_return_now,qmr_return_now
...
! Assign vertical grid information (CRIS PAN grid is top to bottom)
   ...
   allocate(prs_cris(layer_cris))
   allocate(prs_cris_mem(level_cris))
   prs_cris(1:layer_cris)=pressure(key,1:layer_cris)
...
! pressure at model surface (Pa)
   ...
   do k=1,layer_mdl
      ...
      call interpolate(state_handle, ens_size, loc2, QTY_PAN, pan_mdl_tmp, zstatus) ! ppmv 
      zstatus(:)=0
...
!
      interp_new=0
      do imem=1,ens_size
         if(pan_mdl_tmp(imem).eq.missing_r8 .or. tmp_mdl_tmp(imem).eq.missing_r8 .or. &
         qmr_mdl_tmp(imem).eq.missing_r8 .or. prs_mdl_tmp(imem).eq.missing_r8) then
            interp_new=1
         else
            kbnd_1(imem)=k
            pan_mdl_1(imem)=pan_mdl_tmp(imem)
...
!   write(string1, *) 'APM: pan lower bound ',key,pan_mdl_1
!   call error_handler(E_MSG, routine, string1, source)
...
! pressure at model top (Pa)
   ...
   do k=layer_mdl,1,-1
      ...
      zstatus(:)=0
      loc2 = set_location(mloc(1), mloc(2), level, VERTISLEVEL)
      call interpolate(state_handle, ens_size, loc2, QTY_PAN, pan_mdl_tmp, zstatus) 
      zstatus(:)=0
...
!
      ...
      do imem=1,ens_size
         ...
         qmr_mdl_tmp(imem).eq.missing_r8 .or. prs_mdl_tmp(imem).eq.missing_r8) then
            ...
            kbnd_n(imem)=k
            pan_mdl_n(imem)=pan_mdl_tmp(imem)
...
!   write(string1, *) 'APM: pan upper bound ',key,pan_mdl_n
!   call error_handler(E_MSG, routine, string1, source)
...
! Get total_cols at CRIS pressure levels

   allocate(pan_val(ens_size,level_cris))
   ...
   do k=1,layer_cris
      ...
      call interpolate(state_handle, ens_size, loc2, QTY_PAN, pan_val(:,k), zstatus)  
      zstatus=0
...
! Correcting for expected failures near the top
         if (prs_cris(k).le.prs_mdl_n(imem)) then
            pan_val(imem,k) = pan_mdl_n(imem)
            tmp_val(imem,k) = tmp_mdl_n(imem)
            qmr_val(imem,k) = qmr_mdl_n(imem)
         endif
      enddo

!      write(string1, *)'APM: pan ',key,k,pan_val(1,k)
!      call error_handler(E_MSG, routine, string1, source)
...
! Check data for missing values      
      do imem=1,ens_size
         if(pan_val(imem,k).eq.missing_r8 .or. tmp_val(imem,k).eq.missing_r8 .or. &
         qmr_val(imem,k).eq.missing_r8) then
            zstatus(:)=20
            expct_val(:)=missing_r8
            write(string1, *) 'APM: Model total_col data has missing values for obs, level ',key,k
            call error_handler(E_ALLMSG, routine, string1, source)
            call track_status(ens_size, zstatus, expct_val, istatus, return_now)
            do imemm=1,ens_size
               write(string1, *) &
               'APM: Model total_col values: pan,tmp,qmr',key,imem,k,pan_val(imemm,k), &
      ...
      enddo
!
! Convert units for pan from ppmv
      pan_val(:,k) = pan_val(:,k) * 1.e-6_r8
   enddo
!
...
! CRIS vertical is from top to bottom   
   ...
   do imem=1,ens_size
      if (prs_cris(1).lt.prs_mdl_n(imem)) then
         ...
         allocate(pan_prf_mdl(ncnt),tmp_prf_mdl(ncnt),qmr_prf_mdl(ncnt))
         do k=1,kstart(imem)
...
!
         call get_upper_bdy_pan(lon_obs,lat_obs,prs_cris_top,ncnt, &
...
! Impose ensemble perturbations from level kstart(imem)+1      
         ...
         do k=1,kstart(imem) 
            pan_val(imem,k)=pan_prf_mdl(k)*pan_val(imem,kstart(imem)+1)/ &
            (sum(pan_val(:,kstart(imem)+1))/real(ens_size))
         ...
         deallocate(pan_prf_mdl,tmp_prf_mdl,qmr_prf_mdl)
      endif             
   enddo
!
! Check full total_col for negative values
   do imem=1,ens_size
      flg=0
      do k=1,level_cris   
         if(pan_val(imem,k).lt.0. .or. tmp_val(imem,k).lt.0. .or. &
         qmr_val(imem,k).lt.0.) then
            flg=1   
            write(string1, *) &
            'APM: Recentered full total_col has negative values for key,imem ',key,imem
            call error_handler(E_ALLMSG, routine, string1, source)
            call track_status(ens_size, zstatus, expct_val, istatus, return_now)
            if(k.le.kstart(imem)) then
               do kk=1,level_cris
                  write(string1, *) &
                  'APM: prs, pan, tmp, qmr',key,imem,prs_cris(kk),pan_val(imem,kk), &
...
! (CRIS PAN vertical grid is top to bottom)
...
! Process the vertical summation
      ...
      do k=1,layer_cris
         if(prior(key).lt.0.) then
            write(string1, *) &
            'APM: CRIS Prior is negative. Level may be below surface. Key,Layer: ',key,k
            call error_handler(E_MSG, routine, string1, source)
            write(string1, *) &
            'APM: Key ',key,' Prior: ',prior(key),' Avgk ',avg_kernel(key,k)
            call error_handler(E_MSG, routine, string1, source)
            cycle
         endif
!         
...
! Convert from VMR to molar density (mol/m^3)
         if(use_log_pan) then
            pan_val_conv = (dw_wt*exp(pan_val(imem,k+1))+up_wt*exp(pan_val(imem,k)))/tl_wt * &
            (dw_wt*prs_cris_mem(k+1)+up_wt*prs_cris_mem(k)) / &
            (Ru*(dw_wt*tmp_val(imem,k+1)+up_wt*tmp_val(imem,k)))
         else
            pan_val_conv = (dw_wt*pan_val(imem,k+1)+up_wt*pan_val(imem,k))/tl_wt * &
            (dw_wt*prs_cris_mem(k+1)+up_wt*prs_cris_mem(k)) / &
            (Ru*(dw_wt*tmp_val(imem,k+1)+up_wt*tmp_val(imem,k)))
         endif
...
! Get expected observation
         ...
         expct_val(imem) = expct_val(imem) + thick(k) * pan_val_conv * &
         avg_kernel(key,k) + prior_term * prior(key)
         
!         write(string1, *) &
!         'APM: Mem ',imem,' Key ',key,' Expct Val Terms: prs ',k, &
!         (prs_cris_mem(k)+prs_cris_mem(k+1))/2.,' expct val ',expct_val(imem), &
!         'avgk*thick*pan_conv ', avg_kernel(key,k)*thick(k)*pan_val_conv, &
!         'prior_term*prior ', prior_term*prior(key)
!         call error_handler(E_MSG, routine, string1, source)
      enddo
...
!      
      if(isnan(expct_val(imem))) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: CRIS PAN expected value is NaN '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
!
      if(expct_val(imem).lt.0) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: CRIS PAN expected value is negative '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
! Clean up and return
   deallocate(pan_val, tmp_val, qmr_val)
   deallocate(thick)
   deallocate(prs_cris, prs_cris_mem)

end subroutine get_expected_cris_pan_total_col

!-------------------------------------------------------------------------------

subroutine get_upper_bdy_pan(lon_obs,lat_obs,prs_obs,nprs_obs, &
pan_prf_mdl,tmp_prf_mdl,qmr_prf_mdl,date_obs,datesec_obs)
...
! mozart
   ...
   real(r8),dimension(nprs_obs),      intent(out)   :: pan_prf_mdl,tmp_prf_mdl,qmr_prf_mdl
   ...
   real,dimension(nz)                               :: prs_glb,ztrp_pan,ztrp_tmp,ztrp_qmr
   real,dimension(nz)                               :: pan_glb_xmym,pan_glb_xpym,pan_glb_xmyp,pan_glb_xpyp
   ...
   real,dimension(nx,ny,nz,ntim)                    :: pan_glb,tmp_glb,qmr_glb
   character(len=120)                               :: data_file
   character(len=*), parameter                      :: routine = 'get_upper_bdy_pan'
!
...
! mozart
!   call get_MOZART_REAL_DATA(data_file,'PAN_VMR_inst',nx,ny,nz,ntim,pan_glb)
! waccm
   call get_MOZART_REAL_DATA(data_file,'PAN',nx,ny,nz,ntim,pan_glb)
...
! Temporal
   do k=1,nz
      pan_glb_xmym(k)=(wt_bck*pan_glb(indx,jndx,k,itim_sav) + &
      wt_fwd*pan_glb(indx,jndx,k,itim_sav+1))/(wt_bck+wt_fwd)
      pan_glb_xpym(k)=(wt_bck*pan_glb(indx+1,jndx,k,itim_sav) + &
      wt_fwd*pan_glb(indx+1,jndx,k,itim_sav+1))/(wt_bck+wt_fwd)
      pan_glb_xmyp(k)=(wt_bck*pan_glb(indx,jndx+1,k,itim_sav) + &
      wt_fwd*pan_glb(indx,jndx+1,k,itim_sav+1))/(wt_bck+wt_fwd)
      pan_glb_xpyp(k)=(wt_bck*pan_glb(indx+1,jndx+1,k,itim_sav) + &
      wt_fwd*pan_glb(indx+1,jndx+1,k,itim_sav+1))/(wt_bck+wt_fwd)
!
...
! Horizontal   
   do k=1,nz
      ztrp_jbck=(bck_xwt*pan_glb_xmym(k) + fwd_xwt*pan_glb_xpym(k))/twtx
      ztrp_jfwd=(bck_xwt*pan_glb_xmyp(k) + fwd_xwt*pan_glb_xpyp(k))/twtx
      ztrp_pan(k)=(bck_ywt*ztrp_jbck + fwd_ywt*ztrp_jfwd)/twty
!      
...
! Vertical   
   do k=1,nprs_obs
      ...
      pan_prf_mdl(k)=(zwt_up*ztrp_pan(kndx) + zwt_dw*ztrp_pan(kndx+1))/twt
   ...
   enddo
 end subroutine get_upper_bdy_pan
...
subroutine set_obs_def_cris_pan_total_col(key, pan_pressure, pan_avg_kernel, pan_prior, &
pan_nlayer, pan_klev, pan_kend)

   integer,                           intent(in)   :: key, pan_nlayer, pan_klev, pan_kend
   real(r8),                          intent(in)   :: pan_prior
   real(r8), dimension(pan_nlayer),   intent(in)   :: pan_pressure
   real(r8), dimension(pan_nlayer),   intent(in)   :: pan_avg_kernel
   ...
   if(num_cris_pan_obs >= max_cris_pan_obs) then
      write(string1, *)'Not enough space for cris pan obs.'
      write(string2, *)'Can only have max_cris_pan_obs (currently ',max_cris_pan_obs,')'
      call error_handler(E_ERR,'set_obs_def_cris_pan_total_col',string1,source,revision, &
      revdate,text2=string2)
   endif
   
   nlayer(key) = pan_nlayer
   klev(key) = pan_klev
   kend(key) = pan_kend
   prior(key) = pan_prior
   pressure(key,1:pan_nlayer) = pan_pressure(1:pan_nlayer)
   avg_kernel(key,1:pan_nlayer) = pan_avg_kernel(1:pan_nlayer)
   
end subroutine set_obs_def_cris_pan_total_col
...
end module obs_def_cris_pan_total_col_mod
