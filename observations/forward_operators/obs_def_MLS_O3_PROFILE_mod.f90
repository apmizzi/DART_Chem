...
!                                  read_mls_o3_profile, &
!                                  write_mls_o3_profile, &
!                                  interactive_mls_o3_profile
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(MLS_O3_PROFILE)                                                           
!         call get_expected_mls_o3_profile(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(MLS_O3_PROFILE)
!         call read_mls_o3_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(MLS_O3_PROFILE)
!         call write_mls_o3_profile(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(MLS_O3_PROFILE)
!         call interactive_mls_o3_profile(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_mls_o3_profile_mod
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_mls_o3_profile_mod.f90'
...
! Check for valid values
   ...
   allocate(  pressure(max_mls_o3_obs,nlayer_mls))
   allocate(avg_kernel(max_mls_o3_obs,nlayer_mls))
   allocate(     prior(max_mls_o3_obs,nlayer_mls))
   
end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_mls_o3_profile(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_mls_o3_obs) then
      ...
      call error_handler(E_ERR,'read_mls_o3_profile',string1,source,text2=string2)
   endif
   
   call set_obs_def_mls_o3_profile(key, pressure_1(1:nlayer_1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_mls_o3_profile

!-------------------------------------------------------------------------------

subroutine write_mls_o3_profile(key, ifile, fform)
...
end subroutine write_mls_o3_profile

!-------------------------------------------------------------------------------

subroutine interactive_mls_o3_profile(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_mls_o3_profile not yet working.'
   call error_handler(E_ERR, 'interactive_mls_o3_profile', string1, source)
   
   if(num_mls_o3_obs >= max_mls_o3_obs) then
      write(string1, *)'Not enough space for an mls o3 obs.'
      write(string2, *)'Can only have max_mls_o3_obs (currently ',max_mls_o3_obs,')'
      call error_handler(E_ERR, 'interactive_mls_o3_profile', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_mls_o3_profile observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_mls_o3_profile

!-------------------------------------------------------------------------------

subroutine get_expected_mls_o3_profile(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_mls_o3_profile'
...
! Assign vertical grid information (MLS O3 grid is top to bottom)
   ...
   prs_mls(1:level_mls)=pressure(key,1:layer_mls)
...
subroutine set_obs_def_mls_o3_profile(key, o3_pressure, o3_avg_kernel, o3_prior, &
o3_nlayer, o3_klev, o3_kend)
   ...
   if(num_mls_o3_obs >= max_mls_o3_obs) then
      write(string1, *)'Not enough space for mls o3 obs.'
      write(string2, *)'Can only have max_mls_o3_obs (currently ',max_mls_o3_obs,')'
      call error_handler(E_ERR,'set_obs_def_mls_o3_profile',string1,source,revision, &
...
end subroutine set_obs_def_mls_o3_profile
...
end module obs_def_mls_o3_profile_mod
