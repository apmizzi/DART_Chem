...
! Assign vertical grid information (MLS O3 grid is bottom to top)
...
!
      interp_new=0
      do imem=1,ens_size
         if(o3_mdl_1(imem).lt.0. .or. tmp_mdl_1(imem).lt.0. .or. &
...
!
      interp_new=0
      do imem=1,ens_size
         if(o3_mdl_n(imem).lt.0. .or. tmp_mdl_n(imem).lt.0. .or. &
...
!               if(qmr_val(imem,k).lt.0.) qmr_val(imem,k)=qmr_mdl_n(imem)
               ...
               write(string1, *) &
               'APM: O3 VAL ',(o3_val(imem,kk),kk=1,layer_mls)
               call error_handler(E_ALLMSG, routine, string1, source)
               write(string1, *) &
...
! TCR2 O3 is 0.0 above 50 hPa         
   do imem=1,ens_size
      do k=1,layer_mls
         o3_val_conv=o3_val(imem,k)
         o3_term=o3_val_conv
         if(prs_mls(k).le.tcr2_top) o3_term=0.
!
...
!
!         write(string1, *) &
!         'APM: EX_VAL,O3_TRM,AVGK,PRIOR,TRM1,TRM2 ',k,klev_mls, &
!         expct_val(imem),o3_term,avg_kernel(key,k),prior(key,k),&
