...
! TROPOMI_CO_TOTAL_COL, QTY_CO
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_tropomi_co_total_col_mod, only : get_expected_tropomi_co_total_col, &
!                                  read_tropomi_co_total_col, &
!                                  write_tropomi_co_total_col, &
!                                  interactive_tropomi_co_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(TROPOMI_CO_TOTAL_COL)                                                           
!         call get_expected_tropomi_co_total_col(state_handle, ens_size, location, obs_def%key, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(TROPOMI_CO_TOTAL_COL)
!         call read_tropomi_co_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(TROPOMI_CO_TOTAL_COL)
!         call write_tropomi_co_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(TROPOMI_CO_TOTAL_COL)
!         call interactive_tropomi_co_total_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_tropomi_co_total_col_mod
...
public :: write_tropomi_co_total_col, &
          read_tropomi_co_total_col, &
          interactive_tropomi_co_total_col, &
          get_expected_tropomi_co_total_col, &
          set_obs_def_tropomi_co_total_col
...
character(len=*), parameter :: source   = 'obs_def_tropomi_co_total_col_mod.f90'
...
integer :: nlayer_tropomi_co_total_col = -9999

namelist /obs_def_TROPOMI_CO_nml/ upper_data_file, use_log_co, nlayer_model, &
nlayer_tropomi_co_total_col, ls_chem_dx, ls_chem_dy, ls_chem_dz, ls_chem_dt, &
upper_data_model     
...
nlayer_tropomi=nlayer_tropomi_co_total_col
...
if (nlayer_tropomi < 1) then
   write(string1,*)'obs_def_TROPOMI_CO_nml:nlayer_tropomi must be > 0, it is ',nlayer_tropomi
   call error_handler(E_ERR,'initialize_module',string1,source)
endif

allocate(    nlayer(max_tropomi_co_obs))
allocate(    kend(max_tropomi_co_obs))
allocate(  pressure(max_tropomi_co_obs,nlayer_tropomi+1))
allocate(avg_kernel(max_tropomi_co_obs,nlayer_tropomi))

end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_tropomi_co_total_col(key, ifile, fform)
...
if(counts1 > max_tropomi_co_obs) then
   write(string1, *)'Not enough space for tropomi co total col obs.'
   write(string2, *)'Can only have max_tropomi_co_obs (currently ',max_tropomi_co_obs,')'
   call error_handler(E_ERR,'read_tropomi_co_total_col',string1,source,text2=string2)
endif

call set_obs_def_tropomi_co_total_col(key, pressure_1, avg_kernel_1, nlayer_1, kend_1)

deallocate(pressure_1, avg_kernel_1)

end subroutine read_tropomi_co_total_col

!-------------------------------------------------------------------------------

subroutine write_tropomi_co_total_col(key, ifile, fform)
...
end subroutine write_tropomi_co_total_col

!-------------------------------------------------------------------------------

subroutine interactive_tropomi_co_total_col(key)
...
write(string1,*)'interactive_tropomi_co_total_col not yet working.'
call error_handler(E_ERR, 'interactive_tropomi_co_total_col', string1, source)

if(num_tropomi_co_obs >= max_tropomi_co_obs) then
   write(string1, *)'Not enough space for an tropomi co total col obs.'
   write(string2, *)'Can only have max_tropomi_co_obs (currently ',max_tropomi_co_obs,')'
   call error_handler(E_ERR, 'interactive_tropomi_co_total_col', string1, &
...
write(*, *) 'Creating an interactive_tropomi_co total_col observation'
write(*, *) 'This featue is not setup '

end subroutine interactive_tropomi_co_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_tropomi_co_total_col(state_handle, ens_size, location, key, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_tropomi_co_total_col'
   ...
   real(r8) :: eps, AvogN, Rd, Ru, Cp, grav, msq2cmsq, base_tmp
   real(r8) :: co_min,tmp_max
...
!   write(string1, *)'APM: co lower bound ',level,co_mdl_1(1)
!   call error_handler(E_MSG, routine, string1, source)
!   write(string1, *)'APM: tmp lower bound ',level,tmp_mdl_1(1)
!   call error_handler(E_MSG, routine, string1, source)
!   write(string1, *)'APM: qmr lower bound ',level,qmr_mdl_1(1)
!   call error_handler(E_MSG, routine, string1, source)
!   write(string1, *)'APM: prs lower bound ',level,prs_mdl_1(1)
!   call error_handler(E_MSG, routine, string1, source)
...
!   write(string1, *)'APM: co upper bound ',level,co_mdl_n(1)
!   call error_handler(E_MSG, routine, string1, source)
!   write(string1, *)'APM: tmp upper bound ',level,tmp_mdl_n(1)
!   call error_handler(E_MSG, routine, string1, source)
!   write(string1, *)'APM: qmr upper bound ',level,qmr_mdl_n(1)
!   call error_handler(E_MSG, routine, string1, source)
!   write(string1, *)'APM: prs upper bound ',level,prs_mdl_n(1)
!   call error_handler(E_MSG, routine, string1, source)
...
! Get expected observation
      ...
      if(expct_val(imem).lt.0) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: TROPOMI CO expected value is negative '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
end subroutine get_expected_tropomi_co_total_col

!-------------------------------------------------------------------------------

subroutine set_obs_def_tropomi_co_total_col(key, co_pressure, co_avg_kernel, co_nlayer, co_kend)
...
if(num_tropomi_co_obs >= max_tropomi_co_obs) then
   write(string1, *)'Not enough space for tropomi co total col obs.'
   write(string2, *)'Can only have max_tropomi_co_obs (currently ',max_tropomi_co_obs,')'
   call error_handler(E_ERR,'set_obs_def_tropomi_co_total_col',string1,source,revision, &
...
end subroutine set_obs_def_tropomi_co_total_col

end module obs_def_tropomi_co_total_col_mod
