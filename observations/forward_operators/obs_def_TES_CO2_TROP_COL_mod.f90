...
! TES_CO2_TROP_COL, QTY_CO2
! END DART PREPROCESS KIND LIST
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_tes_co2_trop_col_mod, only : get_expected_tes_co2_trop_col, &
!                                  read_tes_co2_trop_col, &
!                                  write_tes_co2_trop_col, &
!                                  interactive_tes_co2_trop_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(TES_CO2_TROP_COL)                                                           
!         call get_expected_tes_co2_trop_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(TES_CO2_TROP_COL)
!         call read_tes_co2_trop_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(TES_CO2_TROP_COL)
!         call write_tes_co2_trop_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(TES_CO2_TROP_COL)
!         call interactive_tes_co2_trop_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_tes_co2_trop_col_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_tes_co2_trop_col, &
             read_tes_co2_trop_col, &
          interactive_tes_co2_trop_col, &
          get_expected_tes_co2_trop_col, &
          set_obs_def_tes_co2_trop_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_tes_co2_trop_col_mod.f90'
...
! Check for valid values
   nlayer_tes=nlayer_tes_co2_trop_col
...
subroutine read_tes_co2_trop_col(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_tes_co2_obs) then
      ...
      call error_handler(E_ERR,'read_tes_co2_trop_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_tes_co2_trop_col(key, pressure_1(1:nlayer_1+1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_tes_co2_trop_col

!-------------------------------------------------------------------------------

subroutine write_tes_co2_trop_col(key, ifile, fform)
...
end subroutine write_tes_co2_trop_col

!-------------------------------------------------------------------------------

subroutine interactive_tes_co2_trop_col(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_tes_co2_trop_col not yet working.'
   call error_handler(E_ERR, 'interactive_tes_co2_trop_col', string1, source)
   
   if(num_tes_co2_obs >= max_tes_co2_obs) then
      write(string1, *)'Not enough space for an tes co2 obs.'
      write(string2, *)'Can only have max_tes_co2_obs (currently ',max_tes_co2_obs,')'
      call error_handler(E_ERR, 'interactive_tes_co2_trop_col', string1, &
...
! Otherwise, prompt for input for the three required beasts
   ...
   write(*, *) 'Creating an interactive_tes_co2_trop_col observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_tes_co2_trop_col

!-------------------------------------------------------------------------------

subroutine get_expected_tes_co2_trop_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_tes_co2_trop_col'
...
end subroutine get_expected_tes_co2_trop_col
...
subroutine set_obs_def_tes_co2_trop_col(key, co2_pressure, co2_avg_kernel, co2_prior, &
co2_nlayer, co2_klev, co2_kend)
   ...
   if(num_tes_co2_obs >= max_tes_co2_obs) then
      write(string1, *)'Not enough space for tes co2 obs.'
      write(string2, *)'Can only have max_tes_co2_obs (currently ',max_tes_co2_obs,')'
      call error_handler(E_ERR,'set_obs_def_tes_co2_trop_col',string1,source,revision, &
