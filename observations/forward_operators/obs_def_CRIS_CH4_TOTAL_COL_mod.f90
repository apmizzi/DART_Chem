...
! CRIS_CH4_TOTAL_COL, QTY_CH4
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_cris_ch4_total_col_mod, only : get_expected_cris_ch4_total_col, &
!                                  read_cris_ch4_total_col, &
!                                  write_cris_ch4_total_col, &
!                                  interactive_cris_ch4_total_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(CRIS_CH4_TOTAL_COL)                                                           
!         call get_expected_cris_ch4_total_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(CRIS_CH4_TOTAL_COL)
!         call read_cris_ch4_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(CRIS_CH4_TOTAL_COL)
!         call write_cris_ch4_total_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(CRIS_CH4_TOTAL_COL)
!         call interactive_cris_ch4_total_col(obs_def%key)
! END DART PREPROCESS INTERACTIVE_OBS_DEF
!
! BEGIN DART PREPROCESS MODULE CODE

module obs_def_cris_ch4_total_col_mod
...
! set_time sets time_type from julian day and seconds
   ...
   public :: write_cris_ch4_total_col, &
             read_cris_ch4_total_col, &
          interactive_cris_ch4_total_col, &
          get_expected_cris_ch4_total_col, &
          set_obs_def_cris_ch4_total_col

! Storage for the special information required for observations of this type
   integer, parameter    :: max_cris_ch4_obs = 10000000
   integer               :: num_cris_ch4_obs = 0
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter :: source   = 'obs_def_cris_ch4_total_col_mod.f90'
...
! Namelist with default values
   ...
   integer :: nlayer_cris_ch4_total_col = -9999
   integer :: nlayer_cris_ch4_trop_col = -9999
   integer :: nlayer_cris_ch4_profile = -9999
   
   namelist /obs_def_CRIS_CH4_nml/ use_log_ch4, nlayer_model, &
   nlayer_cris_ch4_total_col, nlayer_cris_ch4_trop_col, nlayer_cris_ch4_profile
...
! Read namelist values
   call find_namelist_in_file("input.nml", "obs_def_CRIS_CH4_nml", iunit)
   read(iunit, nml = obs_def_CRIS_CH4_nml, iostat = rc)
   call check_namelist_read(iunit, rc, "obs_def_CRIS_CH4_nml")

! Record the namelist values
   if (do_nml_file()) write(nmlfileunit, nml=obs_def_CRIS_CH4_nml)
   if (do_nml_term()) write(     *     , nml=obs_def_CRIS_CH4_nml)

! Check for valid values
   nlayer_cris=nlayer_cris_ch4_total_col

   if (nlayer_model < 1) then
      write(string1,*)'obs_def_CRIS_CH4_nml:nlayer_model must be > 0, it is ',nlayer_model
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   if (nlayer_cris < 1) then
      write(string1,*)'obs_def_CRIS_CH4_nml:nlayer_cris must be > 0, it is ',nlayer_cris
      call error_handler(E_ERR,'initialize_module',string1,source)
   endif
   
   allocate(    nlayer(max_cris_ch4_obs))
   allocate(    klev(max_cris_ch4_obs))
   allocate(    kend(max_cris_ch4_obs))
   allocate(  pressure(max_cris_ch4_obs,nlayer_cris+1))
   allocate(avg_kernel(max_cris_ch4_obs,nlayer_cris))
   allocate(     prior(max_cris_ch4_obs,nlayer_cris))
   
end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_cris_ch4_total_col(key, ifile, fform)
...
! Need to know how many layers for this one
   ...
   if(counts1 > max_cris_ch4_obs) then
      write(string1, *)'Not enough space for cris ch4 obs.'
      write(string2, *)'Can only have max_cris_ch4_obs (currently ',max_cris_ch4_obs,')'
      call error_handler(E_ERR,'read_cris_ch4_total_col',string1,source,text2=string2)
   endif
   
   call set_obs_def_cris_ch4_total_col(key, pressure_1(1:nlayer_1+1), avg_kernel_1(1:nlayer_1), &
   prior_1(1:nlayer_1), nlayer_1, klev_1, kend_1)
   
   deallocate(pressure_1, avg_kernel_1, prior_1)
   
end subroutine read_cris_ch4_total_col

!-------------------------------------------------------------------------------

subroutine write_cris_ch4_total_col(key, ifile, fform)
...
end subroutine write_cris_ch4_total_col

!-------------------------------------------------------------------------------

subroutine interactive_cris_ch4_total_col(key)
...
! STOP because routine is not finished.
   write(string1,*)'interactive_cris_ch4_total_col not yet working.'
   call error_handler(E_ERR, 'interactive_cris_ch4_total_col', string1, source)
   
   if(num_cris_ch4_obs >= max_cris_ch4_obs) then
      write(string1, *)'Not enough space for an cris ch4 obs.'
      write(string2, *)'Can only have max_cris_ch4_obs (currently ',max_cris_ch4_obs,')'
      call error_handler(E_ERR, 'interactive_cris_ch4_total_col', string1, &
                 source, text2=string2)
   endif
   
! Increment the index
   num_cris_ch4_obs = num_cris_ch4_obs + 1
   key            = num_cris_ch4_obs

! Otherwise, prompt for input for the three required beasts

   write(*, *) 'Creating an interactive_cris_ch4_total_col observation'
   write(*, *) 'This featue is not setup '

end subroutine interactive_cris_ch4_total_col

!-------------------------------------------------------------------------------

subroutine get_expected_cris_ch4_total_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_cris_ch4_total_col'
...
! Assign vertical grid information (CRIS CH4 grid is top to bottom)
...
! Get total_cols at CRIS pressure levels
   ...
   allocate(ch4_val(ens_size,level_cris))
...
! Check data for missing values      
      do imem=1,ens_size
         ...
         qmr_val(imem,k).eq.missing_r8) then
            ...
            do imemm=1,ens_size
               write(string1, *) &
               'APM: Model total_col values: ch4,tmp,qmr',key,imem,k,ch4_val(imemm,k), &
...
!
         call get_upper_bdy_ch4(lon_obs,lat_obs,prs_cris_top,ncnt, &
      ...
      do k=1,level_cris   
         ...
         qmr_val(imem,k).lt.0.) then
            ...
            if(k.le.kstart(imem)) then
               do kk=1,level_cris
                  write(string1, *) &
                  'APM: prs, ch4, tmp, qmr',key,imem,prs_cris(kk),ch4_val(imem,kk), &
...
! (CRIS CH4 vertical grid is top to bottom)
...
!      
      if(isnan(expct_val(imem))) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: CRIS CH4 expected value is NaN '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
!
      if(expct_val(imem).lt.0) then
         ...
         expct_val(:)=missing_r8
         write(string1, *) &
         'APM NOTICE: CRIS CH4 expected value is negative '
         call error_handler(E_MSG, routine, string1, source)
         call track_status(ens_size, zstatus, expct_val, istatus, return_now)
         return
      endif
...
end subroutine get_expected_cris_ch4_total_col

!-------------------------------------------------------------------------------

subroutine get_upper_bdy_ch4(lon_obs,lat_obs,prs_obs,nprs_obs, &
ch4_prf_mdl,tmp_prf_mdl,qmr_prf_mdl,date_obs,datesec_obs)
...
! mozart
   ...
   real(r8),dimension(nprs_obs),      intent(out)   :: ch4_prf_mdl,tmp_prf_mdl,qmr_prf_mdl
   ...
   real,dimension(nz)                               :: prs_glb,ztrp_ch4,ztrp_tmp,ztrp_qmr
   real,dimension(nz)                               :: ch4_glb_xmym,ch4_glb_xpym,ch4_glb_xmyp,ch4_glb_xpyp
   ...
   real,dimension(nx,ny,nz,ntim)                    :: ch4_glb,tmp_glb,qmr_glb
   character(len=120)                               :: data_file
   character(len=*), parameter                      :: routine = 'get_upper_bdy_ch4'
!
...
! mozart
!   call get_MOZART_REAL_DATA(data_file,'CH4_VMR_inst',nx,ny,nz,ntim,ch4_glb)
! waccm
   call get_MOZART_REAL_DATA(data_file,'CH4',nx,ny,nz,ntim,ch4_glb)
...
! Temporal
   do k=1,nz
      ch4_glb_xmym(k)=(wt_bck*ch4_glb(indx,jndx,k,itim_sav) + &
      wt_fwd*ch4_glb(indx,jndx,k,itim_sav+1))/(wt_bck+wt_fwd)
      ch4_glb_xpym(k)=(wt_bck*ch4_glb(indx+1,jndx,k,itim_sav) + &
      wt_fwd*ch4_glb(indx+1,jndx,k,itim_sav+1))/(wt_bck+wt_fwd)
      ch4_glb_xmyp(k)=(wt_bck*ch4_glb(indx,jndx+1,k,itim_sav) + &
      wt_fwd*ch4_glb(indx,jndx+1,k,itim_sav+1))/(wt_bck+wt_fwd)
      ch4_glb_xpyp(k)=(wt_bck*ch4_glb(indx+1,jndx+1,k,itim_sav) + &
      wt_fwd*ch4_glb(indx+1,jndx+1,k,itim_sav+1))/(wt_bck+wt_fwd)
!
...
! Horizontal   
   do k=1,nz
      ztrp_jbck=(bck_xwt*ch4_glb_xmym(k) + fwd_xwt*ch4_glb_xpym(k))/twtx
      ztrp_jfwd=(bck_xwt*ch4_glb_xmyp(k) + fwd_xwt*ch4_glb_xpyp(k))/twtx
      ztrp_ch4(k)=(bck_ywt*ztrp_jbck + fwd_ywt*ztrp_jfwd)/twty
!      
...
! Vertical   
   do k=1,nprs_obs
      ...
      ch4_prf_mdl(k)=(zwt_up*ztrp_ch4(kndx) + zwt_dw*ztrp_ch4(kndx+1))/twt
   ...
   enddo
 end subroutine get_upper_bdy_ch4
...
subroutine set_obs_def_cris_ch4_total_col(key, ch4_pressure, ch4_avg_kernel, ch4_prior, &
ch4_nlayer, ch4_klev, ch4_kend)
   ...
   if(num_cris_ch4_obs >= max_cris_ch4_obs) then
      write(string1, *)'Not enough space for cris ch4 obs.'
      write(string2, *)'Can only have max_cris_ch4_obs (currently ',max_cris_ch4_obs,')'
      call error_handler(E_ERR,'set_obs_def_cris_ch4_total_col',string1,source,revision, &
...
end subroutine set_obs_def_cris_ch4_total_col
...
end module obs_def_cris_ch4_total_col_mod
