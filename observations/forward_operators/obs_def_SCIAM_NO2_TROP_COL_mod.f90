...
! SCIAM_NO2_TROP_COL, QTY_NO2
! END DART PREPROCESS TYPE DEFINITIONS
!
! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_sciam_no2_trop_col_mod, only : get_expected_sciam_no2_trop_col, &
!                                  read_sciam_no2_trop_col, &
!                                  write_sciam_no2_trop_col, &
!                                  interactive_sciam_no2_trop_col
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!
! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!      case(SCIAM_NO2_TROP_COL)                                                           
!         call get_expected_sciam_no2_trop_col(state_handle, ens_size, location, obs_def%key, obs_time, expected_obs, istatus)  
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!
! BEGIN DART PREPROCESS READ_OBS_DEF
!      case(SCIAM_NO2_TROP_COL)
!         call read_sciam_no2_trop_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF
!
! BEGIN DART PREPROCESS WRITE_OBS_DEF
!      case(SCIAM_NO2_TROP_COL)
!         call write_sciam_no2_trop_col(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF
!
! BEGIN DART PREPROCESS INTERACTIVE_OBS_DEF
!      case(SCIAM_NO2_TROP_COL)
!         call interactive_sciam_no2_trop_col(obs_def%key)
...
public :: write_sciam_no2_trop_col, &
          read_sciam_no2_trop_col, &
          interactive_sciam_no2_trop_col, &
          get_expected_sciam_no2_trop_col, &
          set_obs_def_sciam_no2_trop_col

! Storage for the special information required for observations of this type
integer, parameter    :: max_sciam_no2_obs = 10000000
integer               :: num_sciam_no2_obs = 0
...
character(len=*), parameter :: source   = 'obs_def_sciam_no2_trop_col_mod.f90'
...
integer :: nlayer_sciam = -9999
integer :: nlayer_sciam_no2_total_col = -9999
integer :: nlayer_sciam_no2_trop_col = -9999

namelist /obs_def_SCIAM_NO2_nml/ upper_data_file, use_log_no2, &
nlayer_model, nlayer_sciam_no2_total_col, nlayer_sciam_no2_trop_col, &
...
call find_namelist_in_file("input.nml", "obs_def_SCIAM_NO2_nml", iunit)
read(iunit, nml = obs_def_SCIAM_NO2_nml, iostat = rc)
call check_namelist_read(iunit, rc, "obs_def_SCIAM_NO2_nml")

! Record the namelist values
if (do_nml_file()) write(nmlfileunit, nml=obs_def_SCIAM_NO2_nml)
if (do_nml_term()) write(     *     , nml=obs_def_SCIAM_NO2_nml)
nlayer_sciam=nlayer_sciam_no2_trop_col

! Check for valid values

if (nlayer_model < 1) then
   write(string1,*)'obs_def_SCIAM_NO2_nml:nlayer_model must be > 0, it is ',nlayer_model
   call error_handler(E_ERR,'initialize_module',string1,source)
endif

if (nlayer_sciam < 1) then
   write(string1,*)'obs_def_SCIAM_NO2_nml:nlayer_sciam must be > 0, it is ',nlayer_sciam
   call error_handler(E_ERR,'initialize_module',string1,source)
endif

allocate(    nlayer(max_sciam_no2_obs))
allocate(    trop_indx(max_sciam_no2_obs))
allocate(    amf_trop(max_sciam_no2_obs))
allocate(  pressure(max_sciam_no2_obs,nlayer_sciam+1))
allocate(scat_wts(max_sciam_no2_obs,nlayer_sciam))

end subroutine initialize_module

!-------------------------------------------------------------------------------

subroutine read_sciam_no2_trop_col(key, ifile, fform)

integer,          intent(out)          :: key
integer,          intent(in)           :: ifile
character(len=*), intent(in), optional :: fform

! sciamrary arrays to hold buffer till we decide if we have enough room
...
if(counts1 > max_sciam_no2_obs) then
   write(string1, *)'Not enough space for sciam no2 trop col obs.'
   write(string2, *)'Can only have max_sciam_no2_obs (currently ',max_sciam_no2_obs,')'
   call error_handler(E_ERR,'read_sciam_no2_trop_col',string1,source,text2=string2)
endif

call set_obs_def_sciam_no2_trop_col(key, pressure_1, scat_wts_1, amf_trop_1, trop_indx_1, nlayer_1)

deallocate(pressure_1, scat_wts_1)

end subroutine read_sciam_no2_trop_col

!-------------------------------------------------------------------------------

subroutine write_sciam_no2_trop_col(key, ifile, fform)
...
end subroutine write_sciam_no2_trop_col

!-------------------------------------------------------------------------------

subroutine interactive_sciam_no2_trop_col(key)
...
write(string1,*)'interactive_sciam_no2_trop_col not yet working.'
call error_handler(E_ERR, 'interactive_sciam_no2_trop_col', string1, source)

if(num_sciam_no2_obs >= max_sciam_no2_obs) then
   write(string1, *)'Not enough space for an sciam no2 trop col obs.'
   write(string2, *)'Can only have max_sciam_no2_obs (currently ',max_sciam_no2_obs,')'
   call error_handler(E_ERR, 'interactive_sciam_no2_trop_col', string1, &
              source, text2=string2)
endif

! Increment the index
num_sciam_no2_obs = num_sciam_no2_obs + 1
key            = num_sciam_no2_obs

! Otherwise, prompt for input for the three required beasts

write(*, *) 'Creating an interactive_sciam_no2_trop_col observation'
write(*, *) 'This featue is not setup '

end subroutine interactive_sciam_no2_trop_col

!-------------------------------------------------------------------------------

subroutine get_expected_sciam_no2_trop_col(state_handle, ens_size, location, key, obs_time, expct_val, istatus)
   ...
   character(len=*), parameter :: routine = 'get_expected_sciam_no2_trop_col'
...
! Upper BC variables
   ...
   real(r8), allocatable, dimension(:)   :: prs_sciam_top
...
! Assign vertical grid information
   ...
   layer_sciam = nlayer(key)
   level_sciam = nlayer(key)+1
   kend_sciam  = trop_indx(key)
...
! Get profiles at SCIAM pressure levels

   allocate(no2_val(ens_size,level_sciam))
   allocate(tmp_val(ens_size,level_sciam))
   allocate(qmr_val(ens_size,level_sciam))
...
! Correcting for expected failures near the surface
      do imem=1,ens_size
         if (prs_sciam(k).ge.prs_mdl_1(imem)) then
...
! Correcting for expected failures near the top
         if (prs_sciam(k).le.prs_mdl_n(imem)) then
...
! SCIAMACHY vertical is from bottom to top in Pa
!
   do imem=1,ens_size
!!      do k=1,level_sciam
!!         if (prs_sciam(k).lt.prs_mdl_n(imem)) then
!!            kstart=k
!!            exit
!!         endif
!!      enddo
!!      ncnt=level_sciam-kstart+1
!!      allocate(prs_sciam_top(ncnt))
!!      allocate(no2_prf_mdl(ncnt),tmp_prf_mdl(ncnt),qmr_prf_mdl(ncnt))
!!      do k=kstart,level_sciam
!!         prs_sciam_top(k-kstart+1)=prs_sciam(k)/100.
!!      enddo
!!!
...
!!      ls_chem_dz,ls_chem_dt,lon_obs,lat_obs,prs_sciam_top, &
...
!
!            if(prs_sciam(k).le.prs_mdl_1(imem) .and. prs_sciam(k).ge.prs_mdl_2(imem)) then
            if(prs_sciam(k).le.prs_mdl_1(imem) .and. prs_sciam(k).ge.(prs_mdl_2(imem)-20000.)) then
...
!            elseif(prs_sciam(k).le.prs_mdl_nm(imem) .and. prs_sciam(k).ge.prs_mdl_n(imem)) then
...
!               if(qmr_val(imem,k).lt.0.) qmr_val(imem,k)=qmr_mdl_n(imem)
               ...
               write(string1, *) &
               'APM: NO2 VAL ',(no2_val(imem,kk),kk=1,level_sciam)
               call error_handler(E_ALLMSG, routine, string1, source)
               write(string1, *) &
               'APM: TM VAL ',(tmp_val(imem,kk),kk=1,level_sciam)
               call error_handler(E_ALLMSG, routine, string1, source)
               write(string1, *) &
               'APM: QV VAL ',(qmr_val(imem,kk),kk=1,level_sciam)
               call error_handler(E_ALLMSG, routine, string1, source)
               write(string1, *) &
               'APM: PR VAL ',(prs_sciam(kk),kk=1,level_sciam)
               call error_handler(E_ALLMSG, routine, string1, source)
               write(string1, *) &
...
! SCIAM vertical is from bottom to top
   istatus(:)=0.
   zstatus(:)=0.
   expct_val(:)=0.0
...
! Get expected observation (convert mol/m^2 to molec/cm^2
         ...
         if(prs_sciam(k+1).le.tcr2_top) no2_term=0.
         
         expct_val(imem) = expct_val(imem) + thick(k) * no2_term * &
...
! Clean up and return
   ...
   deallocate(thick)
   deallocate(prs_sciam, prs_sciam_mem)

end subroutine get_expected_sciam_no2_trop_col

!-------------------------------------------------------------------------------

subroutine set_obs_def_sciam_no2_trop_col(key, no2_pressure, no2_scat_wts, no2_amf_trop, no2_trop_indx, no2_nlayer)
...
if(num_sciam_no2_obs >= max_sciam_no2_obs) then
   write(string1, *)'Not enough space for sciam no2 trop col obs.'
   write(string2, *)'Can only have max_sciam_no2_obs (currently ',max_sciam_no2_obs,')'
   call error_handler(E_ERR,'set_obs_def_sciam_no2_trop_col',string1,source,revision, &
...
end subroutine set_obs_def_sciam_no2_trop_col

end module obs_def_sciam_no2_trop_col_mod
