...
!
program tropomi_co_total_col_ascii_to_obs
!
!=============================================
...
!=============================================
   ...
   use obs_def_tropomi_co_total_col_mod, only     : set_obs_def_tropomi_co_total_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'tropomi_co_total_col_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: file_name='tropomi_co_total_col_obs_seq'
...
! Determine bias correction
      ...
      if(does_file_exist) then
         ...
         do iobs=1,nobs
            ...
            if(trim(obs_list(iobs)).eq.'TROPOMI_CO_TOTAL_COL') then
               exit
            endif
         enddo
         if(correction_old.eq.-999.) then
            print *, 'APM: TROPOMI_TOTAL_CO - Error assigning bias correction'
            stop
         endif
         close(iunit)
      endif
...
!       
         ...
         call set_obs_def_tropomi_co_total_col(qc_count, prs_obs_r8, avgk_obs_r8, nlay_obs, kend)
...
!
end program tropomi_co_total_col_ascii_to_obs
!
...
subroutine get_model_profile(prf_locl,prf_full,nz_mdl,prs_obs,prs_mdl, &
   tmp_mdl,qmr_mdl,co_mdl,nlev_obs,v_wgts,kend)
   implicit none
   integer                                :: nz_mdl
   integer                                :: nlev_obs,nlay_obs
   integer                                :: i,j,k,kk,kend
   real                                   :: Ru,Rd,cp,eps,AvogN,msq2cmsq,grav
   real,dimension(nz_mdl)                 :: prs_mdl,tmp_mdl,qmr_mdl,co_mdl
   real,dimension(nz_mdl)                 :: tmp_prf,vtmp_prf,co_prf
   real,dimension(nlev_obs-1)             :: thick,v_wgts,prf_locl,prf_full
   real,dimension(nlev_obs)               :: co_prf_mdl,vtmp_prf_mdl,prs_obs
!
! Constants (mks units)
   Ru=8.316
   Rd=286.9
...
! convert to molar density         
   do k=1,nz_mdl
      co_prf(k)=co_mdl(k)*prs_mdl(k)/Ru/tmp_prf(k)
   enddo
! Vertical interpolation
   co_prf_mdl(:)=-9999.  
   vtmp_prf_mdl(:)=-9999.   
   call interp_to_obs(co_prf_mdl,co_prf,prs_mdl,prs_obs,nz_mdl,nlev_obs,kend)
...
! apply averging kernel
   do k=1,nlay_obs
      kk=nlev_obs-k+1
      prf_full(kk)=(co_prf_mdl(kk)+co_prf_mdl(kk-1))/2.* &
      v_wgts(kk)
      prf_locl(kk)=(co_prf_mdl(kk)+co_prf_mdl(kk-1))/2.* &
      v_wgts(kk)
   enddo
!   print *, 'prf_full  ',prf_full(:)
!   print *, 'co fld   ',co_prf_mdl(:)
...
! Assumes prs_obs top down and prs_mdl is bottom up 
   implicit none
