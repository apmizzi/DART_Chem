...
! Obs value is the tropospheric slant column
      ...
      obs_err_var=(fac*fac_err*slnt_col_amt)**2.
      omi_qc(:)=0
...
!--------------------------------------------------------
!
      call vertical_locate(prs_loc,prs_obs,nlev_obs,scat_wt,nlay_obs)
      level=prs_loc*100.
...
!
      ...
      call set_obs_def_omi_so2(qc_count, prs_obs, scat_wt, nlay_obs)
...
! apply vertical smoother
   ...
   do k=1,nlay
      ...
      elseif(k.eq.nlay) then
         avgk_sm(k)=(wt_end*avgk(k)+wt_ctr*avgk(k+1))/(wt_ctr+wt_end)
         cycle
      else
...
! locate the three-point maximum
   ...
   kstr=1
!   do k=1,nlay
!      if((prs(k)+prs(k+1))/2.lt.prs_trop) then
!         cycle
!      else
!         kstr=k
!         exit
