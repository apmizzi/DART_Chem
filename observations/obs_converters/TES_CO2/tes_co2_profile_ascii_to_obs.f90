...
!
program tes_co2_profile_ascii_to_obs
!
!=============================================
! TES CO2 column obs
!=============================================
   ...
   use obs_def_tes_co2_profile_mod, only     : set_obs_def_tes_co2_profile
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'tes_co2_profile_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: file_name='tes_co2_profile_obs_seq'
...
!
      prs_obs(:)=prs_obs(:)*100.
      prs_obs_r8(:)=prs_obs(:)
      prior_obs_r8(:)=co2_obs_prior(:)
      lon_obs_r8=lon_obs
      lat_obs_r8=lat_obs
!
!--------------------------------------------------------
! Find model CO2 profile corresponding to the observation
! TES CO2 and WRF-Chem vertical grids are bottom to top        
!--------------------------------------------------------
!      reject=0
!      call get_model_profile(prf_locl,prf_full,nz_model, &
!      prs_obs,prs_fld(i_min,j_min,:),tmp_fld(i_min,j_min,:), &
!      qmr_fld(i_min,j_min,:),co2_fld(i_min,j_min,:), &
!      nlev_obs,avgk_obs,prior_obs,kend)
!
! Obs thinning test
      obs_accept=obs_accept+1
      if(obs_accept/obs_co2_reten_freq*obs_co2_reten_freq.eq.obs_accept) then
!      
...
!            obs_err_var=(fac_obs_error*fac_err*sqrt(cov_obs(ilay,ilay)))**2
            obs_err_var=(fac_obs_error*fac_err*co2_obs(ilay))**2
            tes_qc(:)=0
...
!
            ...
            call set_obs_def_tes_co2_profile(qc_count, prs_obs_r8(1:nlay_obs), &
...
!
end program tes_co2_profile_ascii_to_obs
!
...
subroutine get_model_profile(prf_locl,prf_full,nz_mdl,prs_obs,prs_mdl, &
   tmp_mdl,qmr_mdl,co2_mdl,nlev_obs,v_wgts,prior,kend)
   implicit none
   integer                                :: nz_mdl
   integer                                :: nlev_obs
   integer                                :: i,j,k,kend
   real                                   :: Ru,Rd,cp,eps,AvogN,msq2cmsq,grav
   real,dimension(nz_mdl)                 :: prs_mdl,tmp_mdl,qmr_mdl,co2_mdl
   real,dimension(nz_mdl)                 :: tmp_prf,vtmp_prf,co2_prf
   real,dimension(nlev_obs-1)             :: thick,v_wgts,prior,prf_locl,prf_full
   real,dimension(nlev_obs)               :: co2_prf_mdl,vtmp_prf_mdl,prs_obs
!
! Constants (mks units)
   Ru=8.316
   Rd=286.9
...
! convert to molar density         
   do k=1,nz_mdl
      co2_prf(k)=co2_mdl(k)*prs_mdl(k)/Ru/tmp_prf(k)
   enddo
! Vertical interpolation
   co2_prf_mdl(:)=-9999.  
   vtmp_prf_mdl(:)=-9999.   
   call interp_to_obs(co2_prf_mdl,co2_prf,prs_mdl,prs_obs,nz_mdl,nlev_obs,kend)
...
! apply scattering weights
   do k=1,nlev_obs-1
! full term
      prf_full(k)=thick(k) * (co2_prf_mdl(k)+co2_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
!
! no thicknesses      
      prf_locl(k)=(co2_prf_mdl(k)+co2_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
   enddo
!   print *, 'prf_full  ',prf_full(:)
!   print *, 'co2 fld   ',co2_prf_mdl(:)
