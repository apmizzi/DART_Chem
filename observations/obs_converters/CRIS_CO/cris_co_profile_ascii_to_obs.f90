...
!
program cris_co_profile_ascii_to_obs
!
!=============================================
! CRIS CO column obs
!=============================================
   ...
   use obs_def_cris_co_profile_mod, only     : set_obs_def_cris_co_profile
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'cris_co_profile_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: file_name='cris_co_profile_obs_seq'
...
!
   ...
   bin_beg_sec,bin_end_sec,fac_obs_error,use_log_o3,use_log_co,use_log_ch4, &
   use_log_nh3,use_log_pan,lon_min,lon_max,lat_min,lat_max,path_model,file_model,nx_model,ny_model, &
   nz_model,obs_o3_reten_freq,obs_co_reten_freq,obs_ch4_reten_freq, &
...
! Read CRIS CO
   ...
   do while (ios == 0)
      ...
      enddo
      print *, 'CRIS CO: Completed data read'      
!
      prs_obs(:)=prs_obs(:)*100.
      do k=1,nlay_obs
...
! CRIS CO and WRF-Chem vertical grids are bottom to top        
!--------------------------------------------------------
!      reject=0
...
!
            ...
            call set_obs_def_cris_co_profile(qc_count, prs_obs_r8(1:nlay_obs), &
...
!
end program cris_co_profile_ascii_to_obs
!
...
subroutine get_model_profile(prf_locl,prf_full,nz_mdl,prs_obs,prs_mdl, &
   tmp_mdl,qmr_mdl,co_mdl,nlev_obs,v_wgts,prior,kend)
   implicit none
   integer                                :: nz_mdl
   integer                                :: nlev_obs
   integer                                :: i,j,k,kend
...
! apply scattering weights
   do k=1,nlev_obs-1
! full term
      prf_full(k)=thick(k) * (co_prf_mdl(k)+co_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
!
! no thicknesses      
      prf_locl(k)=(co_prf_mdl(k)+co_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
   enddo
