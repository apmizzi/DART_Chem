...
!=============================================
   ...
   use obs_def_omi_no2_mod, only     : set_obs_def_omi_no2
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'omi_no2_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: file_name='omi_no2_obs_seq'
   character*129                   :: data_type,cmd
!
   logical                         :: use_log_o3,use_log_no2,use_log_so2
!
! Species-specific variables
   real*8                          :: amfstrat,amfstrat_clr,amfstrat_cld
   real*8                          :: amftrop,amftrop_clr,amftrop_cld
   real*8                          :: cld_frac,cld_prs,cld_rad_frac
   real*8                          :: col_amt,col_amt_err
   real*8                          :: col_amt_trop,col_amt_trop_err
   real*8                          :: slnt_col_amt,slnt_col_amt_err
   real*8                          :: prs_trop,zenang
   real*8,allocatable,dimension(:) :: scat_wt
!
   namelist /create_omi_obs_nml/filedir,filename,year,month,day,hour, &
   bin_beg,bin_end,fac_obs_error,use_log_o3,use_log_no2,use_log_so2, &
...
! Open OMI NO2 binary file
   ...
   open(fileid,file=TRIM(TRIM(filedir)//TRIM(filename)),                     &
...
! Obs value is the tropospheric slant column
      ...
      obs_err_var=(slnt_col_amt_err)**2.
      omi_qc(:)=0
...
!--------------------------------------------------------
!
      call vertical_locate(prs_loc,prs_obs,nlev_obs,scat_wt,nlay_obs,prs_trop)
      level=prs_loc*100.
...
!
      ...
      call set_obs_def_omi_no2(qc_count, prs_obs, scat_wt, prs_trop, nlay_obs)
...
!
end program omi_no2_ascii_to_obs
!
subroutine vertical_locate(prs_loc,prs,nlev,avgk,nlay,prs_trop)
!
...
! apply vertical smoother
   ...
   do kk=1,nlay
      k=nlay-kk+1
      if(k.eq.nlay) then
         avgk_sm(k)=(wt_ctr*avgk(k+1)+wt_end*avgk(k))/(wt_ctr+wt_end)
         cycle
      elseif(k.eq.1) then
...
! locate the three-point maximum
   ...
   do kk=1,nlay
      k=nlay-kk+1
      if((prs(k+1)+prs(k))/2.ge.prs_trop) then
         kstr=k
         cycle
      else
         exit
