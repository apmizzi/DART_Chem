...
!
program tes_ch4_profile_ascii_to_obs
!
!=============================================
! TES CH4 column obs
!=============================================
   ...
   use obs_def_tes_ch4_profile_mod, only     : set_obs_def_tes_ch4_profile
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'tes_ch4_profile_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: file_name='tes_ch4_profile_obs_seq'
...
! TES CH4 and WRF-Chem vertical grids are bottom to top        
!--------------------------------------------------------
!      reject=0
...
!
            ...
            call set_obs_def_tes_ch4_profile(qc_count, prs_obs_r8(1:nlay_obs), &
...
!
end program tes_ch4_profile_ascii_to_obs
!
...
subroutine get_model_profile(prf_locl,prf_full,nz_mdl,prs_obs,prs_mdl, &
   tmp_mdl,qmr_mdl,ch4_mdl,nlev_obs,v_wgts,prior,kend)
   implicit none
   integer                                :: nz_mdl
   integer                                :: nlev_obs
   integer                                :: i,j,k,kend
   real                                   :: Ru,Rd,cp,eps,AvogN,msq2cmsq,grav
   real,dimension(nz_mdl)                 :: prs_mdl,tmp_mdl,qmr_mdl,ch4_mdl
   real,dimension(nz_mdl)                 :: tmp_prf,vtmp_prf,ch4_prf
   real,dimension(nlev_obs-1)             :: thick,v_wgts,prior,prf_locl,prf_full
   real,dimension(nlev_obs)               :: ch4_prf_mdl,vtmp_prf_mdl,prs_obs
!
! Constants (mks units)
   Ru=8.316
   Rd=286.9
...
! convert to molar density         
   do k=1,nz_mdl
      ch4_prf(k)=ch4_mdl(k)*prs_mdl(k)/Ru/tmp_prf(k)
   enddo
! Vertical interpolation
   ch4_prf_mdl(:)=-9999.  
   vtmp_prf_mdl(:)=-9999.   
   call interp_to_obs(ch4_prf_mdl,ch4_prf,prs_mdl,prs_obs,nz_mdl,nlev_obs,kend)
...
! apply scattering weights
   do k=1,nlev_obs-1
! full term
      prf_full(k)=thick(k) * (ch4_prf_mdl(k)+ch4_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
!
! no thicknesses      
      prf_locl(k)=(ch4_prf_mdl(k)+ch4_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
   enddo
!   print *, 'prf_full  ',prf_full(:)
!   print *, 'ch4 fld   ',ch4_prf_mdl(:)
