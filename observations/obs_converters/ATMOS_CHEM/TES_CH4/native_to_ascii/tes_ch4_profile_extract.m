function tes_ch4_profile_extract (filein,fileout,file_pre,cwyr_mn,cwmn_mn,cwdy_mn,cwhh_mn,cwmm_mn,cwss_mn,cwyr_mx,cwmn_mx,cwdy_mx,cwhh_mx,cwmm_mx,cwss_mx,path_mdl,file_mdl,cnx_mdl,cny_mdl)
%
...
% Process satellite data
   for ifile=1:nfile
      ...
      clear ch4_lay vert_col_total vert_col_trop
      ...
      field='/HDFEOS/SWATHS/CH4NadirSwath';
      zgrid=h5readatt(file_in,field,'VerticalCoordinate');
%
% Read Time
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/UTCTime';
...
% averaging kernel (layer,layer,nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/AveragingKernel';
...
% averaging kernel diagonal (layer,nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/AveragingKernelDiagonal';
      avgk_diag_lay=h5read(file_in,field);
%
% dofs(nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/DegreesOfFreedomForSignal';
      dofs=h5read(file_in,field);
%
% ch4_lay (layer,nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/CH4';
      ch4_lay=h5read(file_in,field);
      units=h5readatt(file_in,field,'Units');
%
% ch4_lay_err (layer,nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/CH4Precision';
      ch4_lay_err=h5read(file_in,field);
      units=h5readatt(file_in,field,'Units');
%
% ch4_lay_prior (layer,nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/Initial';
      ch4_lay_prior=h5read(file_in,field);
      units=h5readatt(file_in,field,'Units');
%
% ch4_total_col (nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/TotalColumnDensity';
      ch4_total_col=h5read(file_in,field);
%
% ch4_total_col_err (nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/TotalColumnDensityError';
      ch4_total_col_err=h5read(file_in,field);
%
% ch4_total_col_prior (nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/TotalColumnDensityInitial';
      ch4_total_col_prior=h5read(file_in,field);
%
% err_cov_mea(layer,layer,nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/MeasurementErrorCovariance';
      err_cov_mea=h5read(file_in,field);
%      
% err_cov_obs(layer,layer,nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/ObservationErrorCovariance';
      err_cov_obs=h5read(file_in,field);
%
% err_cov_total(layer,layer,nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/TotalErrorCovariance';
      err_cov_total=h5read(file_in,field);
%
% total_err (layer,nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/TotalError';
      total_err=h5read(file_in,field);
%
% prs_lay (layer,nobs) Pressure is bottom to top (hPa)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/Pressure';
      prs_lay=h5read(file_in,field);
      units=h5readatt(file_in,field,'Units');
%
% tropopause pressure(nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Data Fields/TropopausePressure';
      trop_pressure=h5read(file_in,field);
%
% lat (nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Geolocation Fields/Latitude';
      lat=h5read(file_in,field);
%
% lon (nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Geolocation Fields/Longitude';
      lon=h5read(file_in,field);
%
% zenang (nobs)
      field='/HDFEOS/SWATHS/CH4NadirSwath/Geolocation Fields/SolarZenithAngle';
...
%
%         if(any(isnan(ch4_lay(:,iobs))) | any(ch4_lay(:,iobs)<=0))
%            continue
%         end
%
%         if(any(isnan(ch4_lay_err(:,iobs))) | any(ch4_lay_err(:,iobs)<=0))
%            continue
%         end
%
%         if(any(isnan(ch4_lay_prior(:,iobs))) | any(ch4_lay_prior(:,iobs)<=0))
%            continue
%         end
%
...
%
         if(isnan(ch4_total_col(iobs)) | ch4_total_col(iobs)<=0.)
            continue
         end
%
         if(isnan(ch4_total_col_err(iobs)) | ch4_total_col_err(iobs)<=0.)
            continue
         end
%
         if(isnan(ch4_total_col_prior(iobs)) | ch4_total_col_prior(iobs)<=0.)
            continue
         end
%
         if(zenang(iobs)>=80.0)
...
% Save data to ascii file
         ...
         fprintf(fid,'\n');
         fprintf(fid,'%14.8g ',ch4_lay(1:layer,iobs));
         fprintf(fid,'\n');
         fprintf(fid,'%14.8g ',ch4_lay_prior(1:layer,iobs));
         fprintf(fid,'\n');
         fprintf(fid,'%14.8g ',ch4_lay_err(1:layer,iobs));
         fprintf(fid,'\n');
         ...
         fprintf(fid,'%14.8g \n',ch4_total_col(iobs));
         fprintf(fid,'%14.8g \n',ch4_total_col_prior(iobs));
         fprintf(fid,'%14.8g \n',ch4_total_col_err(iobs));
      end
   end  
end
