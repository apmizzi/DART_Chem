...
!
program cris_co_cpsr_ascii_to_obs
!
!=============================================
! CRIS CO column obs
!=============================================
  ...
  use apm_upper_bdy_mod, only      : get_upper_bdy_fld, &
   ...
   use obs_def_cris_co_cpsr_mod, only     : set_obs_def_cris_co_cpsr
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'cris_co_cpsr_ascii_to_obs.f90'
...
!
   ...
   integer                         :: obs_accept,obs_co_reten_freq,obs_no2_reten_freq
!
...
!
   ...
   character*129                   :: qc_meta_data='CRIS CO QC index'
   character*129                   :: chr_year,chr_month,chr_day
   character*129                   :: file_name='cris_co_cpsr_obs_seq'
...
!
   ...
   bin_beg_sec,bin_end_sec,fac_obs_error,use_log_co,use_log_co,use_log_no2,use_log_so2, &
   lon_min,lon_max,lat_min,lat_max, &
   path_model,file_model,nx_model,ny_model,nz_model,obs_co_reten_freq,obs_no2_reten_freq
!
! Set constants
   pi=4.*atan(1.)
   rad2deg=360./(2.*pi)
...
!
   do icopy =1, num_copies
      if (icopy == 1) then
         copy_meta_data='CRIS CO observation'
      else
...
! Read CRIS CO
   ...
   do while (ios == 0)
      ...
      allocate(co_cpsr(nlay_obs))
      allocate(prior_cpsr(nlay_obs))
      allocate(avgk_cpsr(nlay_obs,nlay_obs))
      allocate(co_shift(nlay_obs),prior_shift(nlay_obs))
...
!
      ...
      do i=1,nlay_obs
         if(cov_obs(i,i).lt.0.) then
            flg=1
            print *, 'cris_variance is NaN or negative ',cov_obs(i,i)
            exit
         endif
      enddo
      if(flg.eq.1) then
         ...
         deallocate(co_shift,prior_shift)
...
! Calculate CPSRs for this retrieval profile
         call cpsr_calculation(nmodes,nlayer,co_cpsr,avgk_cpsr,prior_cpsr,co_shift,prior_shift, &
         avgk_shift,cov_shift,sum_accept)
!
! Loop through the dominant modes (OMI CO is top to bottom)
         prior_obs_r8(:)=0.
         do imds=1,nmodes
            avgk_obs_r8(1:nlayer)=avgk_cpsr(imds,1:nlayer)
            prior_obs_r8(imds)=prior_cpsr(imds)
!
...
!
            ...
            call set_obs_def_cris_co_cpsr(qc_count, prs_obs_r8(:), avgk_obs_r8(:), prior_obs_r8(:), nlayer, klev, kend)
...
!
end program cris_co_cpsr_ascii_to_obs
!
