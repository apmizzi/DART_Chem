function tempo_o3_profile_extract (filein,fileout,file_pre,cwyr_mn,cwmn_mn,cwdy_mn,cwhh_mn,cwmm_mn,cwss_mn,cwyr_mx,cwmn_mx,cwdy_mx,cwhh_mx,cwmm_mx,cwss_mx,path_mdl,file_mdl,cnx_mdl,cny_mdl)
%
...
%	    if(qa_value(ixtrk,istep)~=0 | zenang(ixtrk,istep)>=80.0)
	    ...
	    icnt=0;
            for k=1:level
               if(isnan(prs_lev(k,ixtrk,istep)))
                  icnt=icnt+1;
               end
            end	 
	    if(icnt>5)
	       'ICNT GREATER THAN FIVE'
               fprintf('APM: ICNT is greater than five. \n');
               continue
            end
  	    layer_trc=layer-icnt;
% o3_lay
            iflg=0;
            for k=1:layer_trc
               if(isnan(o3_lay(k,ixtrk,istep)))
                  iflg=1;
                  break
               end
            end
...
% prior_lay
            iflg=0;
            for k=1:layer_trc
               if(isnan(prior_lay(k,ixtrk,istep)))
                  iflg=1;
                  break
               end
            end
...
% avgk_lay
            ...
            for k=1:layer_trc
               for kk=1:layer_trc
                  if(isnan(avgk_lay(k,k,ixtrk,istep)))
                     iflg=1;
                     break
                  end
               end
...
% noise_corr
            iflg=0;
            for k=1:layer_trc
               if(noise_corr(k,k,ixtrk,istep)<=0 | isnan(noise_corr(k,k,ixtrk,istep)))
                  iflg=1;
                  break
               end
            end
