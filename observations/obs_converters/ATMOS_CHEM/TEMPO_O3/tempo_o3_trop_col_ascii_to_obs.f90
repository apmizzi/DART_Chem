...
!
program tempo_o3_trop_col_ascii_to_obs
!
!=============================================
! TEMPO O3 column obs
!=============================================
  ...
  use apm_upper_bdy_mod, only      : get_upper_bdy_fld, &
   ...
   use obs_def_tempo_o3_trop_col_mod, only     : set_obs_def_tempo_o3_trop_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'tempo_o3_trop_col_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: file_name='tempo_o3_trop_col_obs_seq'
...
! Species-specific variables
   ...
   real*8                            :: prior_trop_r8
   real*8,allocatable,dimension(:)   :: avgk_trop_obs_r8
!
...
! Read TEMPO O3
   ...
   do while (ios == 0)
      ...
      allocate(avgk_trop_obs_r8(nlay_obs))
...
! kernel, and prior term
      ...
      avgk_trop_obs_r8(:)=0.
      do k=1,trop_index
         cov_trop=cov_trop+cov_obs(k,k)
         sum_o3_obs=sum_o3_obs+o3_obs(k)
         do kk=1,nlay_obs
            if(k.eq.kk) then
            ...
            avgk_trop_obs_r8(k)=avgk_trop_obs_r8(k)+avgk_obs(kk,k)
            prior_trop_r8=prior_trop_r8+avgk_term*prior_obs(k)
        enddo
      enddo
!
!--------------------------------------------------------
...
! Obs value is the tropospheric vertical column
         ...
         obs_err_var=(fac_obs_error*fac_err*sqrt(cov_trop))**2
         tempo_qc(:)=0
...
!        
         ...
         call set_obs_def_tempo_o3_trop_col(qc_count, prs_obs_r8(:), avgk_trop_obs_r8(:), prior_trop_r8, trop_index, nlay_obs)
...
!
end program tempo_o3_trop_col_ascii_to_obs
