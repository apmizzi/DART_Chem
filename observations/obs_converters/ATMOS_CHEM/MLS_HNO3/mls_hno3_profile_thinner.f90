...
!
program mls_hno3_profile_thinner
...
! Version controlled file description for error handling, do not edit
   character(len=*), parameter      :: source   = 'mls_hno3_thinning.f90'
...
!
   ...
   integer                          :: i,j,k
!
! MLS HNO3 variable declarations
...
!
      ...
      enddo   
!      read(fileid,*,iostat=ios) &
!         mls_data(i_min,j_min)%hno3_col_obs(icnt)
!      read(fileid,*,iostat=ios) &
!         mls_data(i_min,j_min)%hno3_col_obs_err(icnt)
      read(fileid,*,iostat=ios) data_type, obs_id, &
           i_min, j_min
      mls_data(i_min,j_min)%data_type=data_type
      mls_data(i_min,j_min)%obs_id=obs_id
...
! Write data to file
         if(mls_data(i,j)%icnt.gt.0) then
            ...
            enddo
!            write(fileid,*,iostat=ios) &
!               mls_data(i,j)%hno3_col_obs(icnt)
!            write(fileid,*,iostat=ios) &
!               mls_data(i,j)%hno3_col_obs_err(icnt)
         endif
      enddo
   enddo
!               
! Deallocate structure arrays
   ...
   deallocate (dist_min)
   deallocate (icnt_min)   
end program mls_hno3_profile_thinner
