...
!
program cris_nh3_profile_ascii_to_obs
!
!=============================================
! CRIS NH3 column obs
!=============================================
   ...
   use obs_def_cris_nh3_profile_mod, only     : set_obs_def_cris_nh3_profile
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'cris_nh3_profile_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: qc_meta_data='CRIS NH3 QC index'
   character*129                   :: chr_year,chr_month,chr_day
   character*129                   :: file_name='cris_nh3_profile_obs_seq'
...
!
   do icopy =1, num_copies
      if (icopy == 1) then
         copy_meta_data='CRIS NH3 observation'
      else
...
! Read CRIS NH3
   ...
   do while (ios == 0)
      ...
      enddo
      print *, 'CRIS NH3: Completed data read'      
!
      prs_obs(:)=prs_obs(:)*100.
      do k=1,nlay_obs
...
! CRIS NH3 and WRF-Chem vertical grids are bottom to top        
!--------------------------------------------------------
!      reject=0
...
!
            ...
            call set_obs_def_cris_nh3_profile(qc_count, prs_obs_r8(1:nlay_obs), &
...
!
end program cris_nh3_profile_ascii_to_obs
!
...
subroutine get_model_profile(prf_locl,prf_full,nz_mdl,prs_obs,prs_mdl, &
   tmp_mdl,qmr_mdl,nh3_mdl,nlev_obs,v_wgts,prior,kend)
   implicit none
   integer                                :: nz_mdl
   integer                                :: nlev_obs
   integer                                :: i,j,k,kend
   real                                   :: Ru,Rd,cp,eps,AvogN,msq2cmsq,grav
   real,dimension(nz_mdl)                 :: prs_mdl,tmp_mdl,qmr_mdl,nh3_mdl
   real,dimension(nz_mdl)                 :: tmp_prf,vtmp_prf,nh3_prf
   real,dimension(nlev_obs-1)             :: thick,v_wgts,prior,prf_locl,prf_full
   real,dimension(nlev_obs)               :: nh3_prf_mdl,vtmp_prf_mdl,prs_obs
!
! Constants (mks units)
   Ru=8.316
   Rd=286.9
...
! convert to molar density         
   do k=1,nz_mdl
      nh3_prf(k)=nh3_mdl(k)*prs_mdl(k)/Ru/tmp_prf(k)
   enddo
! Vertical interpolation
   nh3_prf_mdl(:)=-9999.  
   vtmp_prf_mdl(:)=-9999.   
   call interp_to_obs(nh3_prf_mdl,nh3_prf,prs_mdl,prs_obs,nz_mdl,nlev_obs,kend)
...
! apply scattering weights
   do k=1,nlev_obs-1
! full term
      prf_full(k)=thick(k) * (nh3_prf_mdl(k)+nh3_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
!
! no thicknesses      
      prf_locl(k)=(nh3_prf_mdl(k)+nh3_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
   enddo
!   print *, 'prf_full  ',prf_full(:)
!   print *, 'nh3 fld   ',nh3_prf_mdl(:)
