...
!
program omi_so2_total_col_ascii_to_obs
!
!=============================================
! OMI SO2 total column obs
!=============================================
   ...
   use obs_def_omi_so2_total_col_mod, only    : set_obs_def_omi_so2_total_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'omi_so2_total_col_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: file_name='omi_so2_total_col_obs_seq'
...
!
   do icopy =1, num_copies
      if (icopy == 1) then
         copy_meta_data='OMI SO2 observation'
      else
...
!   print *, trim(data_type), obs_id
   do while (ios == 0)
      ...
      read(fileid,*,iostat=ios) col_amt,col_amt_pbl,col_amt_stl
      read(fileid,*,iostat=ios) cld_frac,cld_prs,cld_rad_frac
      read(fileid,*,iostat=ios) rad_cld_frac
      read(fileid,*,iostat=ios) slnt_col_amt
      read(fileid,*,iostat=ios) zenang
...
!      print *, col_amt,col_amt_pbl,col_amt_stl
!      print *, cld_frac,cld_prs,cld_rad_frac
!      print *, rad_cld_frac
!      print *, slnt_col_amt
!      print *, 'zenang ',zenang
!      print *, 'scat_wt ',scat_wt(1:nlay_obs) 
...
!      obs_val(:)=slnt_col_amt
!      obs_err_var=(fac_obs_error*fac_err*slnt_col_amt)**2.
      obs_val(:)=col_amt*AMF
      obs_err_var=(fac_obs_error*fac_err*col_amt*AMF)**2.
      omi_qc(:)=0
!
...
!
      ...
      call set_obs_def_omi_so2_total_col(qc_count, prs_obs_r8, scat_wt_r8, nlay_obs, kend)
...
!
end program omi_so2_total_col_ascii_to_obs
!
...
! (assumes observation vertical grid is top down)  
! 
   implicit none
...
!
subroutine interp_hori_vert(so2_prf,vtmp_prf,vtmp2_scl,psfc_scl,pblh_scl, &
   so2_fld,vtmp_fld,vtmp2_fld,psfc_fld,pblh_fld,x_mdl,y_mdl, &
   ...
   real                                   :: dx_dis,dy_dis
   real                                   :: x_obser,y_obser,x_obs,y_obs,x_obs_temp
   real                                   :: w_q1,w_q2,w_q3,w_q4,wt
   real                                   :: vtmp2_scl,psfc_scl,pblh_scl
   real,dimension(nlev_obs)               :: so2_prf,vtmp_prf,prs_obs
   real,dimension(nz_mdl)                 :: so2_prf_mdl,vtmp_prf_mdl,prs_prf_mdl
   real,dimension(nx_mdl,ny_mdl)          :: x_mdl,y_mdl,vtmp2_fld,psfc_fld,pblh_fld
   real,dimension(nx_mdl,ny_mdl,nz_mdl)   :: so2_fld,vtmp_fld,prs_mdl
...
!
! Use model point closest to the observation 
   so2_prf(:)=-9999.
   so2_prf_mdl(:)=so2_fld(i_min,j_min,:)
   vtmp_prf_mdl(:)=vtmp_fld(i_min,j_min,:)
   prs_prf_mdl(:)=prs_mdl(i_min,j_min,:)
   vtmp2_scl=vtmp2_fld(i_min,j_min)
...
!
   ...
   do k=1,nz_mdl
      if(x_obser.eq.x_mdl(i_min,j_min).and.y_obser.eq.y_mdl(i_min,j_min)) then
         so2_prf_mdl(k)=so2_fld(i_min,j_min,k)
         vtmp_prf_mdl(k)=vtmp_fld(i_min,j_min,k)
         prs_prf_mdl(k)=prs_mdl(i_min,j_min,k)
      else if(quad.eq.1) then
         so2_prf_mdl(k)=(1./w_q1*so2_fld(i_min,j_min,k)+1./w_q2*so2_fld(im,j_min,k)+ &
         1./w_q3*so2_fld(im,jm,k)+1./w_q4*so2_fld(i_min,jm,k))/wt
         vtmp_prf_mdl(k)=(1./w_q1*vtmp_fld(i_min,j_min,k)+1./w_q2*vtmp_fld(im,j_min,k)+ &
         1./w_q3*vtmp_fld(im,jm,k)+1./w_q4*vtmp_fld(i_min,jm,k))/wt
         prs_prf_mdl(k)=(1./w_q1*prs_mdl(i_min,j_min,k)+1./w_q2*prs_mdl(im,j_min,k)+ &
         1./w_q3*prs_mdl(im,jm,k)+1./w_q4*prs_mdl(i_min,jm,k))/wt
      else if(quad.eq.2) then
         so2_prf_mdl(k)=(1./w_q1*so2_fld(ip,j_min,k)+1./w_q2*so2_fld(i_min,j_min,k)+ &
         1./w_q3*so2_fld(i_min,jm,k)+1./w_q4*so2_fld(ip,jm,k))/wt
         vtmp_prf_mdl(k)=(1./w_q1*vtmp_fld(ip,j_min,k)+1./w_q2*vtmp_fld(i_min,j_min,k)+ &
         1./w_q3*vtmp_fld(i_min,jm,k)+1./w_q4*vtmp_fld(ip,jm,k))/wt
         prs_prf_mdl(k)=(1./w_q1*prs_mdl(ip,j_min,k)+1./w_q2*prs_mdl(i_min,j_min,k)+ &
         1./w_q3*prs_mdl(i_min,jm,k)+1./w_q4*prs_mdl(ip,jm,k))/wt
      else if(quad.eq.3) then
         so2_prf_mdl(k)=(1./w_q1*so2_fld(ip,jp,k)+1./w_q2*so2_fld(i_min,jp,k)+ &
         1./w_q3*so2_fld(i_min,j_min,k)+1./w_q4*so2_fld(ip,j_min,k))/wt
         vtmp_prf_mdl(k)=(1./w_q1*vtmp_fld(ip,jp,k)+1./w_q2*vtmp_fld(i_min,jp,k)+ &
         1./w_q3*vtmp_fld(i_min,j_min,k)+1./w_q4*vtmp_fld(ip,j_min,k))/wt
         prs_prf_mdl(k)=(1./w_q1*prs_mdl(ip,jp,k)+1./w_q2*prs_mdl(i_min,jp,k)+ &
         1./w_q3*prs_mdl(i_min,j_min,k)+1./w_q4*prs_mdl(ip,j_min,k))/wt
      else if(quad.eq.4) then
         so2_prf_mdl(k)=(1./w_q1*so2_fld(i_min,jp,k)+1./w_q2*so2_fld(im,jp,k)+ &
         1./w_q3*so2_fld(im,j_min,k)+1./w_q4*so2_fld(i_min,j_min,k))/wt
         vtmp_prf_mdl(k)=(1./w_q1*vtmp_fld(i_min,jp,k)+1./w_q2*vtmp_fld(im,jp,k)+ &
         1./w_q3*vtmp_fld(im,j_min,k)+1./w_q4*vtmp_fld(i_min,j_min,k))/wt
...
! Assumes prs_obs is top down and prs_mdl is bottom up 
   implicit none
   integer                        :: nz_mdl,nlev_obs
   integer                        :: k,kk,ll,kend
   real                           :: wt_dw,wt_up 
   real,dimension(nz_mdl)         :: fld_mdl,prs_mdl
   real,dimension(nlev_obs)       :: prf_mdl,prs_obs
!
   prf_mdl(:)=-9999.
   kend=-9999
   do k=1,nlev_obs-1
