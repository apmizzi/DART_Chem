...
!
program tropomi_so2_pbl_col_thinner
...
! Version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'tropomi_so2_thinner.f90'
...
!
   ...
   integer                         :: i,j
!
! TROPOMI SO2 variable declarations
...
!  
   type observation_data
      ...
      real,allocatable,dimension(:)    :: so2_col
      real,allocatable,dimension(:)    :: so2_col_err
      real,allocatable,dimension(:)    :: so2_slnt_col
      real,allocatable,dimension(:)    :: so2_slnt_col_err
      real,allocatable,dimension(:)    :: so2_1km_col
      real,allocatable,dimension(:)    :: so2_1km_err
      real,allocatable,dimension(:)    :: so2_1km_sys
      real,allocatable,dimension(:)    :: amf_1km
      real,allocatable,dimension(:)    :: amf_1km_err
      real,allocatable,dimension(:)    :: amf_1km_sys
      real,allocatable,dimension(:)    :: amf_1km_clr
      real,allocatable,dimension(:)    :: amf_1km_cld
      real,allocatable,dimension(:)    :: so2_7km_col
      real,allocatable,dimension(:)    :: so2_7km_err
      real,allocatable,dimension(:)    :: so2_7km_sys
      real,allocatable,dimension(:)    :: amf_7km
      real,allocatable,dimension(:)    :: amf_7km_err
      real,allocatable,dimension(:)    :: amf_7km_sys
      real,allocatable,dimension(:)    :: amf_7km_clr
      real,allocatable,dimension(:)    :: amf_7km_cld
...
!
   ...
   do while (ios == 0)
      ...
      if(tropomi_data(i_min,j_min)%icnt.eq.1) then
         ...
         allocate(tropomi_data(i_min,j_min)%so2_col(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%so2_col_err(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%so2_slnt_col(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%so2_slnt_col_err(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%so2_1km_col(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%so2_1km_err(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%so2_1km_sys(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%amf_1km(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%amf_1km_err(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%amf_1km_sys(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%amf_1km_clr(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%amf_1km_cld(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%so2_7km_col(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%so2_7km_err(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%so2_7km_sys(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%amf_7km(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%amf_7km_err(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%amf_7km_sys(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%amf_7km_clr(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%amf_7km_cld(max_num_obs))
      endif
!
      ...
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%prior_obs(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%so2_col(icnt), &
         tropomi_data(i_min,j_min)%so2_col_err(icnt), &
         tropomi_data(i_min,j_min)%so2_slnt_col(icnt), &
         tropomi_data(i_min,j_min)%so2_slnt_col_err(icnt)
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%so2_1km_col(icnt), &
         tropomi_data(i_min,j_min)%so2_1km_err(icnt), &
         tropomi_data(i_min,j_min)%so2_1km_sys(icnt)
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%amf_1km(icnt), &
         tropomi_data(i_min,j_min)%amf_1km_err(icnt), &
         tropomi_data(i_min,j_min)%amf_1km_sys(icnt), &
         tropomi_data(i_min,j_min)%amf_1km_clr(icnt), &
         tropomi_data(i_min,j_min)%amf_1km_cld(icnt)
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%so2_7km_col(icnt), &
         tropomi_data(i_min,j_min)%so2_7km_err(icnt), &
         tropomi_data(i_min,j_min)%so2_7km_sys(icnt)
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%amf_7km(icnt), &
         tropomi_data(i_min,j_min)%amf_7km_err(icnt), &
         tropomi_data(i_min,j_min)%amf_7km_sys(icnt), &
         tropomi_data(i_min,j_min)%amf_7km_clr(icnt), &
         tropomi_data(i_min,j_min)%amf_7km_cld(icnt)
      read(fileid,*,iostat=ios) data_type, obs_id, &
         i_min, j_min
      tropomi_data(i_min,j_min)%data_type=data_type
      tropomi_data(i_min,j_min)%obs_id=obs_id
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.1) then
            call median_code(tropomi_data(i,j)%median_col,tropomi_data(i,j)%so2_1km_col(1:tropomi_data(i,j)%icnt), &
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.20) then
            call mode_code(tropomi_data(i,j)%mode_col,tropomi_data(i,j)%so2_1km_col(1:tropomi_data(i,j)%icnt), &
            tropomi_data(i,j)%icnt)
         endif
      enddo
   enddo
      ...
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.0) then
            ...
            do icnt=1,tropomi_data(i,j)%icnt
               tropomi_data(i,j)%mean_col=tropomi_data(i,j)%mean_col+tropomi_data(i,j)%so2_1km_col(icnt)
               tropomi_data(i,j)%mean_err=tropomi_data(i,j)%mean_err+tropomi_data(i,j)%so2_1km_err(icnt)
            enddo
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.1) then
            ...
            do icnt=1,tropomi_data(i,j)%icnt
               tropomi_data(i,j)%stdv_col=tropomi_data(i,j)%stdv_col+(tropomi_data(i,j)%so2_1km_col(icnt)- &
               tropomi_data(i,j)%mean_col)**2.
               tropomi_data(i,j)%stdv_err=tropomi_data(i,j)%stdv_err+(tropomi_data(i,j)%so2_1km_err(icnt)- &
   ...
   do i=1,nx_model
      do j=1,ny_model
         ...
         if(tropomi_data(i,j)%icnt.eq.1) then
            ...
            do icnt=1,tropomi_data(i,j)%icnt
               dist=abs(tropomi_data(i,j)%mean_col-tropomi_data(i,j)%so2_1km_col(icnt))
...
! Deallocate structure arrays
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.0) then
            ...
            deallocate(tropomi_data(i,j)%prior_obs)
            deallocate(tropomi_data(i_min,j_min)%so2_col)
            deallocate(tropomi_data(i_min,j_min)%so2_col_err)
            deallocate(tropomi_data(i_min,j_min)%so2_slnt_col)
            deallocate(tropomi_data(i_min,j_min)%so2_slnt_col_err)
            deallocate(tropomi_data(i_min,j_min)%so2_1km_col)
            deallocate(tropomi_data(i_min,j_min)%so2_1km_err)
            deallocate(tropomi_data(i_min,j_min)%so2_1km_sys)
            deallocate(tropomi_data(i_min,j_min)%amf_1km)
            deallocate(tropomi_data(i_min,j_min)%amf_1km_err)
            deallocate(tropomi_data(i_min,j_min)%amf_1km_sys)
            deallocate(tropomi_data(i_min,j_min)%amf_1km_clr)
            deallocate(tropomi_data(i_min,j_min)%amf_1km_cld)
            deallocate(tropomi_data(i_min,j_min)%so2_7km_col)
            deallocate(tropomi_data(i_min,j_min)%so2_7km_err)
            deallocate(tropomi_data(i_min,j_min)%so2_7km_sys)
            deallocate(tropomi_data(i_min,j_min)%amf_7km)
            deallocate(tropomi_data(i_min,j_min)%amf_7km_err)
            deallocate(tropomi_data(i_min,j_min)%amf_7km_sys)
            deallocate(tropomi_data(i_min,j_min)%amf_7km_clr)
            deallocate(tropomi_data(i_min,j_min)%amf_7km_cld)
         endif
      enddo
   enddo
   ...
   deallocate (dist_min)
   deallocate (icnt_min)   
end program tropomi_so2_pbl_col_thinner
