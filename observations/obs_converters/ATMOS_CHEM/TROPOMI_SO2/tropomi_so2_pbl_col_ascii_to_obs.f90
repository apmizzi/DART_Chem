...
!
program tropomi_so2_pbl_col_ascii_to_obs
!
!=============================================
! TROPOMI SO2_PBL_COL column obs
!=============================================
   ...
   use obs_def_tropomi_so2_pbl_col_mod, only     : set_obs_def_tropomi_so2_pbl_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'tropomi_so2_pbl_col_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: file_name='tropomi_so2_pbl_col_obs_seq'
...
!
   logical                         :: use_log_co,use_log_o3,use_log_no2,use_log_so2, &
                                      use_log_hcho,use_log_ch4
!
! Species-specific variables
   real                          :: col_amt_obs,col_amt_obs_err,slnt_col_amt,slnt_col_amt_sys   
   real                          :: col_amt_1km,col_amt_1km_err,col_amt_1km_sys
   real                          :: col_amt_7km,col_amt_7km_err,col_amt_7km_sys
   real                          :: amf_1km,amf_1km_err,amf_1km_sys
   real                          :: amf_1km_clr,amf_1km_cld
   real                          :: amf_7km,amf_7km_err,amf_7km_sys
   real                          :: amf_7km_clr,amf_7km_cld
   real                            :: lat_obs,lon_obs
   real*8                          :: amf_1km_r8,lat_obs_r8,lon_obs_r8
...
!
   ...
   use_log_hcho,use_log_ch4,lon_min,lon_max,lat_min,lat_max,path_model,file_model,nx_model, &
   ny_model,nz_model,obs_co_reten_freq,obs_o3_reten_freq,obs_no2_reten_freq,obs_so2_reten_freq, &
   obs_hcho_reten_freq,obs_ch4_reten_freq
!
! Set constants
   pi=4.*atan(1.)
   rad2deg=360./(2.*pi)
...
!
   do icopy =1, num_copies
      if (icopy == 1) then
         copy_meta_data='TROPOMI SO2 observation'
      else
...
! Open TROPOMI SO2 binary file
   fileid=100
   write(6,*)'APM: opening ',TRIM(filedir)//TRIM(filename)
...
! Read TROPOMI SO2
   ...
   do while (ios == 0)
      ...
      slnt_col_amt,slnt_col_amt_sys
      read(fileid,*,iostat=ios) col_amt_1km, &
      col_amt_1km_err,col_amt_1km_sys
      read(fileid,*,iostat=ios) amf_1km, &
      amf_1km_err,amf_1km_sys,amf_1km_clr,amf_1km_cld
      read(fileid,*,iostat=ios) col_amt_7km, &
      col_amt_7km_err,col_amt_7km_sys
      read(fileid,*,iostat=ios) amf_7km, &
      amf_7km_err,amf_7km_sys,amf_7km_clr,amf_7km_cld
      prs_obs(:)=prs_obs(:)*100.
      prs_obs_r8(:)=prs_obs(:)
...
!
      print *, 'APM: assign values ', fac_obs_error, fac_err, col_amt_obs
      print *, 'APM: assign values ', col_amt_obs, col_amt_obs_err
         obs_val(:)=col_amt_obs
         obs_err_var=(fac_obs_error*fac_err*col_amt_obs_err)**2.
!         obs_val(:)=col_amt_obs*trop_sum/(strat_sum+trop_sum)
!         obs_err_var=fac_obs_error*fac_err*(col_amt_obs_err*trop_sum/(strat_sum+trop_sum))**2.
!         print *, 'obs_val ',col_amt_obs*trop_sum/(strat_sum+trop_sum)
!         print *, 'obs_err ',col_amt_obs_err*trop_sum/(strat_sum+trop_sum)
...
!      
         ...
         call set_obs_def_tropomi_so2_pbl_col(qc_count, prs_obs_r8, avgk_obs_r8, &
         prior_obs_r8, amf_1km_r8, nlay_obs, kend)
...
!
end program tropomi_so2_pbl_col_ascii_to_obs
!
...
subroutine get_model_profile(prf_locl,prf_full,nz_mdl,prs_obs,prs_mdl, &
   tmp_mdl,qmr_mdl,so2_mdl,nlev_obs,v_wgts,prior,kend)
   implicit none
   integer                                :: nz_mdl
   integer                                :: nlev_obs
   integer                                :: i,j,k,kend
   ...
   real,dimension(nlev_obs-1)             :: thick,v_wgts,prior,prior_prf_mdl,prf_locl,prf_full
   real,dimension(nlev_obs)               :: so2_prf_mdl,vtmp_prf_mdl,tmp_prf_mdl,prs_obs
!
! Constants (mks units)
   Ru=8.316
   Rd=286.9
...
! Vertical interpolation
   ...
   call interp_to_obs(tmp_prf_mdl,tmp_prf,prs_mdl,prs_obs,nz_mdl,nlev_obs,kend)
   do k=1,nlev_obs-1
      prior_prf_mdl(k)=prior(k)*(prs_obs(k)+prs_obs(k+1))/2./Ru/ &
      ((tmp_prf_mdl(k)+tmp_prf_mdl(k+1))/2.)
   enddo
!   
...
! apply averaging kernel
   do k=1,nlev_obs-1
      prf_full(k)=thick(k)*((so2_prf_mdl(k)+so2_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior_prf_mdl(k))
!
      prf_locl(k)=thick(k)*((so2_prf_mdl(k)+so2_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior_prf_mdl(k))
   enddo
...
subroutine interp_hori_vert(fld1_prf,fld2_prf,fld1_mdl,fld2_mdl,x_mdl,y_mdl, &
   ...
   integer                                :: i,j,k,kend,i_min,j_min
