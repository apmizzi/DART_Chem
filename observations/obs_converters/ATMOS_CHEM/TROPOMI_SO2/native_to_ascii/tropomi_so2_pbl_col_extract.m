function tropomi_so2_pbl_col_extract (filein,fileout,file_pre,cwyr_mn,cwmn_mn,cwdy_mn,cwhh_mn,cwmm_mn,cwss_mn,cwyr_mx,cwmn_mx,cwdy_mx,cwhh_mx,cwmm_mx,cwss_mx,path_mdl,file_mdl,cnx_mdl,cny_mdl)
%
...
% Define TROPOMI vertical pressure grid (hPa) (bottom to top)
         for ipxl=1:pixel
            for ilin=1:scanline
               for ilv=1:layer
                  prs_lay(ilv,ipxl,ilin)=tm5_a(ilv)+tm5_b(ilv)*prs_sfc(ipxl,ilin);
               end
               for ilv=layer:-1:1
                  if(ilv==layer)
                     prs_lev(ilv+1,ipxl,ilin)=.1;
                  end
                  prs_lev(ilv,ipxl,ilin)=prs_lay(ilv,ipxl,ilin)*2.- ...
