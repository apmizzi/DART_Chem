...
!
program cris_ch4_cpsr_ascii_to_obs
!
!=============================================
! CRIS CH4 column obs
!=============================================
  ...
  use utilities_mod, only          : timestamp,                  &
   ...
   use obs_def_cris_ch4_cpsr_mod, only     : set_obs_def_cris_ch4_cpsr
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'cris_ch4_cpsr_ascii_to_obs.f90'
...
!
   ...
   integer                         :: obs_accept,obs_ch4_reten_freq,obs_no2_reten_freq
!
...
!
   ...
   character*129                   :: file_name='cris_ch4_cpsr_obs_seq'
   character*129                   :: data_type,cmd
   character*129                   :: path_model,file_model,file_in
!
   logical                         :: use_log_co,use_log_ch4,use_log_no2,use_log_so2
!
! Species-specific variables
   ...
   real,allocatable,dimension(:)     :: prior_obs,ch4_obs
   real*8,allocatable,dimension(:)   :: avgk_obs_r8
   real*8,allocatable,dimension(:,:) :: cov_obs_r8
   real*8,allocatable,dimension(:)   :: ch4_obs_r8
...
!
   ...
   bin_beg_sec,bin_end_sec,fac_obs_error,use_log_co,use_log_ch4,use_log_no2,use_log_so2, &
   lon_min,lon_max,lat_min,lat_max, &
   path_model,file_model,nx_model,ny_model,nz_model,obs_ch4_reten_freq,obs_no2_reten_freq
!
! Set constants
   pi=4.*atan(1.)
   rad2deg=360./(2.*pi)
...
! Read CRIS CH4
   ...
   do while (ios == 0)
      ...
      allocate(ch4_cpsr(nlay_obs))
      allocate(prior_cpsr(nlay_obs))
      allocate(avgk_cpsr(nlay_obs,nlay_obs))
      allocate(ch4_shift(nlay_obs),prior_shift(nlay_obs))
...
!
      ...
      if(flg.eq.1) then
         ...
         deallocate(ch4_shift,prior_shift)
...
! Calculate CPSRs for this retrieval profile
         call cpsr_calculation(nmodes,nlayer,ch4_cpsr,avgk_cpsr,prior_cpsr,ch4_shift,prior_shift, &
         avgk_shift,cov_shift,sum_accept)
!
! Loop through the dominant modes (OMI CH4 is top to bottom)
         prior_obs_r8(:)=0.         
         do imds=1,nmodes
...
!
            ...
            call set_obs_def_cris_ch4_cpsr(qc_count, prs_obs_r8(:), avgk_obs_r8(:), prior_obs_r8(:), nlayer, klev, kend)
...
!
end program cris_ch4_cpsr_ascii_to_obs
!
