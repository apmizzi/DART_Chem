function cris_pan_profile_extract (filein,fileout,file_pre,cwyr_mn,cwmn_mn,cwdy_mn,cwhh_mn,cwmm_mn,cwss_mn,cwyr_mx,cwmn_mx,cwdy_mx,cwhh_mx,cwmm_mx,cwss_mx,path_mdl,file_mdl,cnx_mdl,cny_mdl)  
%
...
%
% pan_lay (nlay,ntarg) (VMR)
      field='/x';
      pan_lay=ncread(file_in,field);
%
% pan_lay_prior (nlay,ntarg) (VMR)
      field='/observation_ops/xa';
      pan_lay_prior=ncread(file_in,field);
%
...
%      
	 for ilay=1:nlay
            if(isnan(pan_lay(ilay,itarg)))
               fprintf('APM: pan_lay has NaNs \n')
               continue
            end
         end
%
...
% Save data to ascii file
         ...
         fprintf(fid,'\n');
         fprintf(fid,'%14.8g ',pan_lay(1:nlay,itarg));
         fprintf(fid,'\n');
         fprintf(fid,'%14.8g ',pan_lay_prior(1:nlay,itarg));
         fprintf(fid,'\n');
