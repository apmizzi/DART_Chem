...
!
program cris_pan_total_col_ascii_to_obs
!
!=============================================
! CRIS PAN column obs
!=============================================
   ...
   use obs_def_cris_pan_total_col_mod, only     : set_obs_def_cris_pan_total_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'cris_pan_total_col_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: qc_meta_data='CRIS PAN QC index'
   character*129                   :: chr_year,chr_month,chr_day
   character*129                   :: file_name='cris_pan_total_col_obs_seq'
...
! Species-specific variables
   real                              :: pan_trop_col_obs,pan_trop_col_prior,pan_trop_col_err
   real                              :: pan_total_col_obs,pan_total_col_prior,pan_total_col_err
   real                              :: lat_obs,lon_obs,dofs_obs,trop_prs
   real*8                            :: lat_obs_r8,lon_obs_r8
   real,allocatable,dimension(:,:)   :: avgk_obs,cov_obs,cov_total
   real,allocatable,dimension(:)     :: avgk_diag_obs,avgk_total_col,pan_obs,pan_obs_prior,pan_obs_err
   real*8,allocatable,dimension(:)   :: avgk_obs_r8
   real*8,allocatable,dimension(:,:) :: cov_obs_r8
   real*8,allocatable,dimension(:)   :: prior_obs_r8,pan_obs_r8
   ...
   real,allocatable,dimension(:,:,:) :: pan_fld,qmr_fld

   real                              :: trop_index,cov_trop,wt1_sum,wt2_sum
   real                              :: avgk_term, sum_pan_obs
   real*8                            :: prior_col_r8
...
!
   do icopy =1, num_copies
      if (icopy == 1) then
         copy_meta_data='CRIS PAN observation'
      else
...
! Read model data
   ...
   allocate(pan_fld(nx_model,ny_model,nz_model))
   ...
   call get_DART_diag_data(file_in,'pan',pan_fld,nx_model,ny_model,nz_model,1)
   prs_fld(:,:,:)=prs_bas(:,:,:)+prs_prt(:,:,:)
   tmp_fld(:,:,:)=300.+tmp_prt(:,:,:)
   pan_fld(:,:,:)=pan_fld(:,:,:)*1.e-6
!
! Open CRIS PAN binary file
   fileid=100
...
! Read CRIS PAN
   ...
   do while (ios == 0)
      ...
      allocate(avgk_total_col(nlay_obs))
      ...
      read(fileid,*,iostat=ios) pan_total_col_obs
      read(fileid,*,iostat=ios) pan_total_col_prior
      read(fileid,*,iostat=ios) pan_total_col_err
      read(fileid,*,iostat=ios) (avgk_total_col(l),l=1,nlay_obs)
      print *, 'CRIS PAN: Completed data read'      
!
      prs_obs(:)=prs_obs(:)*100.
!      do k=1,nlay_obs
!         if(isnan(prs_obs(k))) then
!           prs_obs(k)=-9999.
!           avgk_total_col(k)=-9999.
!        endif
!      enddo
      prs_obs_r8(:)=prs_obs(:)
      prior_col_r8=pan_total_col_prior
      lon_obs_r8=lon_obs
      lat_obs_r8=lat_obs
!
!--------------------------------------------------------
! Find model PAN total_col corresponding to the observation
! CRIS PAN and WRF-Chem vertical grids are bottom to top        
!--------------------------------------------------------
!      reject=0
!      call get_model_total_col(prf_locl,prf_full,nz_model, &
!      prs_obs,prs_fld(i_min,j_min,:),tmp_fld(i_min,j_min,:), &
!      qmr_fld(i_min,j_min,:),pan_fld(i_min,j_min,:), &
!      nlev_obs,avgk_obs,prior_obs,kend)
!
! Obs thinning test
      obs_accept=obs_accept+1
      if(obs_accept/obs_pan_reten_freq*obs_pan_reten_freq.eq.obs_accept) then
!      
...
! Set data for writing obs_sequence file
         qc_count=qc_count+1
         print *, 'APM: qc_count ',qc_count
!
! Obs value is the tropospheric vertical column
         avgk_obs_r8(1:nlay_obs)=avgk_total_col(1:nlay_obs)         
         obs_val(:)=pan_total_col_obs
!        obs_err_var=(fac_obs_error*fac_err*sqrt(cov_obs(ilay,ilay)))**2
         obs_err_var=(fac_obs_error*fac_err*pan_total_col_obs)**2
         cris_qc(:)=0
...
!
         ...
         call set_obs_def_cris_pan_total_col(qc_count, prs_obs_r8(1:nlay_obs), &
         avgk_obs_r8(1:nlay_obs), prior_col_r8, nlay_obs, klev, kend)
...
!
end program cris_pan_total_col_ascii_to_obs
!
...
!
subroutine get_model_total_col(prf_locl,prf_full,nz_mdl,prs_obs,prs_mdl, &
   tmp_mdl,qmr_mdl,pan_mdl,nlev_obs,v_wgts,prior,kend)
   implicit none
   integer                                :: nz_mdl
   integer                                :: nlev_obs
   integer                                :: i,j,k,kend
   real                                   :: Ru,Rd,cp,eps,AvogN,msq2cmsq,grav
   real,dimension(nz_mdl)                 :: prs_mdl,tmp_mdl,qmr_mdl,pan_mdl
   real,dimension(nz_mdl)                 :: tmp_prf,vtmp_prf,pan_prf
   real,dimension(nlev_obs-1)             :: thick,v_wgts,prior,prf_locl,prf_full
   real,dimension(nlev_obs)               :: pan_prf_mdl,vtmp_prf_mdl,prs_obs
!
! Constants (mks units)
   Ru=8.316
   Rd=286.9
...
! convert to molar density         
   do k=1,nz_mdl
      pan_prf(k)=pan_mdl(k)*prs_mdl(k)/Ru/tmp_prf(k)
   enddo
! Vertical interpolation
   pan_prf_mdl(:)=-9999.  
   vtmp_prf_mdl(:)=-9999.   
   call interp_to_obs(pan_prf_mdl,pan_prf,prs_mdl,prs_obs,nz_mdl,nlev_obs,kend)
...
! apply scattering weights
   do k=1,nlev_obs-1
! full term
      prf_full(k)=thick(k) * (pan_prf_mdl(k)+pan_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
!
! no thicknesses      
      prf_locl(k)=(pan_prf_mdl(k)+pan_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
   enddo
!   print *, 'prf_full  ',prf_full(:)
!   print *, 'pan fld   ',pan_prf_mdl(:)
!   print *, 'avgk_obs ',v_wgts(:)
end subroutine get_model_total_col
!
