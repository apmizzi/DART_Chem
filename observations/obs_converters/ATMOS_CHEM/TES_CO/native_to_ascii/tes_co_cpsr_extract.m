function tes_co_cpsr_extract (filein,fileout,file_pre,cwyr_mn,cwmn_mn,cwdy_mn,cwhh_mn,cwmm_mn,cwss_mn,cwyr_mx,cwmn_mx,cwdy_mx,cwhh_mx,cwmm_mx,cwss_mx,path_mdl,file_mdl,cnx_mdl,cny_mdl)
...
% Process satellite data
   for ifile=1:nfile
      ...
      clear co_lay vert_col_total vert_col_trop
      ...
      field='/HDFEOS/SWATHS/CONadirSwath';
...
% averaging kernel (layer,layer,nobs)
      field='/HDFEOS/SWATHS/CONadirSwath/Data Fields/AveragingKernel';
...
%
% co_total_col (nobs)
      field='/HDFEOS/SWATHS/CONadirSwath/Data Fields/TotalColumnDensity';
      co_total_col=h5read(file_in,field);
%
% co_total_col_err (nobs)
      field='/HDFEOS/SWATHS/CONadirSwath/Data Fields/TotalColumnDensityError';
      co_total_col_err=h5read(file_in,field);
%
% co_total_col_prior (nobs)
      field='/HDFEOS/SWATHS/CONadirSwath/Data Fields/TotalColumnDensityInitial';
      co_total_col_prior=h5read(file_in,field);
%
% total_err (layer,nobs)
      field='/HDFEOS/SWATHS/CONadirSwath/Data Fields/TotalError';
      total_err=h5read(file_in,field);
%
% err_cov_total(layer,layer,nobs) (ln(vmr)^2)
      field='/HDFEOS/SWATHS/CONadirSwath/Data Fields/TotalErrorCovariance';
      err_cov_total=h5read(file_in,field);
      units=h5readatt(file_in,field,'Units');
%
% tropopause pressure(nobs)
      field='/HDFEOS/SWATHS/CONadirSwath/Data Fields/TropopausePressure';
      trop_pressure=h5read(file_in,field);
%
% lat (nobs)
      field='/HDFEOS/SWATHS/CONadirSwath/Geolocation Fields/Latitude';
      lat=h5read(file_in,field);
%
% lon (nobs)
      field='/HDFEOS/SWATHS/CONadirSwath/Geolocation Fields/Longitude';
      lon=h5read(file_in,field);
%
% zenang (nobs)
      field='/HDFEOS/SWATHS/CONadirSwath/Geolocation Fields/SolarZenithAngle';
...
% Save data to ascii file
         ...
         fprintf(fid,'\n');
         fprintf(fid,'%14.8g ',co_lay(1:layer,iobs));
         fprintf(fid,'\n');
         fprintf(fid,'%14.8g ',co_lay_prior(1:layer,iobs));
         fprintf(fid,'\n');
         fprintf(fid,'%14.8g ',co_lay_err(1:layer,iobs));
         fprintf(fid,'\n');
         ...
         fprintf(fid,'%14.8g \n',co_total_col(iobs));
         fprintf(fid,'%14.8g \n',co_total_col_prior(iobs));
         fprintf(fid,'%14.8g \n',co_total_col_err(iobs));
      end
   end  
end
