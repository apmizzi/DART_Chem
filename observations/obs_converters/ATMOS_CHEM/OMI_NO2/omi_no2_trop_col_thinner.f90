...
!  
   type observation_data
      ...
      real,allocatable,dimension(:)    :: amf_strat_clr
      real,allocatable,dimension(:)    :: amf_strat_cld
      real,allocatable,dimension(:)    :: amf_trop
      real,allocatable,dimension(:)    :: amf_trop_clr
      real,allocatable,dimension(:)    :: amf_trop_cld
      ...
      real,allocatable,dimension(:)    :: col_amt_err
...
!
   ...
   do while (ios == 0)
      ...
      if(omi_data(i_min,j_min)%icnt.eq.1) then
         ...
         allocate(omi_data(i_min,j_min)%amf_strat_clr(max_num_obs))
         allocate(omi_data(i_min,j_min)%amf_strat_cld(max_num_obs))
         allocate(omi_data(i_min,j_min)%amf_trop(max_num_obs))
         allocate(omi_data(i_min,j_min)%amf_trop_clr(max_num_obs))
         allocate(omi_data(i_min,j_min)%amf_trop_cld(max_num_obs))
         ...
         allocate(omi_data(i_min,j_min)%col_amt_err(max_num_obs))
...
!
      ...
      if(omi_data(i_min,j_min)%icnt.eq.1) then
         allocate(omi_data(i_min,j_min)%scat_wts(max_num_obs,nlay_obs))
         allocate(omi_data(i_min,j_min)%prs_obs(max_num_obs,nlev_obs))
      endif
      read(fileid,*,iostat=ios) &
         omi_data(i_min,j_min)%amf_strat(icnt), &
         omi_data(i_min,j_min)%amf_strat_clr(icnt), &
         omi_data(i_min,j_min)%amf_strat_cld(icnt)
      read(fileid,*,iostat=ios) &
         omi_data(i_min,j_min)%amf_trop(icnt), &
         omi_data(i_min,j_min)%amf_trop_clr(icnt), &
         omi_data(i_min,j_min)%amf_trop_cld(icnt)
      read(fileid,*,iostat=ios) &
      ...
      read(fileid,*,iostat=ios) &
         ...
         omi_data(i_min,j_min)%col_amt_err(icnt)
      read(fileid,*,iostat=ios) &
      ...
      read(fileid,*,iostat=ios) &
         omi_data(i_min,j_min)%scat_wts(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) &
...
! Write data to file
         if(omi_data(i,j)%icnt.gt.0) then
            ...
            write(fileid,*,iostat=ios) &
               ...
               omi_data(i,j)%amf_strat_clr(icnt), &
               omi_data(i,j)%amf_strat_cld(icnt)
            write(fileid,*,iostat=ios) &
               omi_data(i,j)%amf_trop(icnt), &
               omi_data(i,j)%amf_trop_clr(icnt), &
               omi_data(i,j)%amf_trop_cld(icnt)
            write(fileid,*,iostat=ios) &
            ...
            write(fileid,*,iostat=ios) &
               omi_data(i,j)%scat_wts(icnt,1:nlay_obs)
            write(fileid,*,iostat=ios) &
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(omi_data(i,j)%icnt.gt.0) then
            ...
            deallocate(omi_data(i,j)%amf_strat_clr)
            deallocate(omi_data(i,j)%amf_strat_cld)
            deallocate(omi_data(i,j)%amf_trop)
            deallocate(omi_data(i,j)%amf_trop_clr)
            deallocate(omi_data(i,j)%amf_trop_cld)
            ...
            deallocate(omi_data(i,j)%col_amt_err)
            ...
            deallocate(omi_data(i,j)%scat_wts)
