...
program mopitt_co_profile_ascii_to_obs
...
use obs_def_mopitt_co_profile_mod, only : set_obs_def_mopitt_co_profile
...
character(len=*), parameter :: source   = 'mopitt_co_profile_ascii_to_obs.f90'
...
real,dimension(mop_dim)         :: mop_prs
...
character*129           :: file_name='mopitt_co_profile_obs_seq'
...
! Read MOPITT file 1
  index_qc=0
  open(unit=fileid,file=TRIM(filedir)//TRIM(filen),                     &
     ...
     if(prs_sfc_mop.ge.prs_lay_mop(1)) then
        ...
        do ilv=1,lay_mop-1
           if(prs_sfc_mop.lt.prs_lay_mop(ilv) .and. &
              prs_sfc_mop.ge.prs_lay_mop(ilv+1)) then
              kstr=ilv+1
              exit
           endif
        enddo
...
! Assign MOPITT variables
     ...
     x_p(2:nlvls+1)=log10(prior_prof_lay(kstr:lay_mop))
     avg_k(1:nlvls+1,1:nlvls+1)=avgk_lev(kstr:lev_mop,kstr:lev_mop)
     cov_r(1:nlvls+1,1:nlvls+1)=cov_r_lev(kstr:lev_mop,kstr:lev_mop)
     nlvls=mop_dim-kstr+1
!
...
! Locate index for first pressure level
        prs_idx=0
        do k=1,mop_dim
           if(xg_prs_norm(i,j,k).ne.0) then
              do kk=1,mop_dimm-1
                 if(xg_prs(i,j,k).gt.nprs(kk) .and. kk.eq.1) then
                    prs_idx=1
                    exit   
...
! Get number of vertical levels
        klvls=mop_dim
        do k=1,mop_dim
          if(xg_prs_norm(i,j,k).eq.0) then
             klvls=klvls-1
          endif
        enddo
!
...
! Determine bias correction
           ...
           if(does_file_exist) then
              ...
              if(correction_old.eq.-999.) then
                 print *, 'APM: MOPITT_CO - Error assigning bias correction'
                 stop
              endif
              close(iunit)
           endif
!
           obs_kind = MOPITT_CO_PROFILE
...
!
           ...
           call set_obs_def_mopitt_co_profile(qc_count, co_avgker, co_prior, co_psurf, &
           xg_nlvls(i,j))
...
end program mopitt_co_profile_ascii_to_obs
