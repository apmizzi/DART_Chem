...
!
program sciam_no2_trop_col_ascii_to_obs
!
!=============================================
! SCIAM NO2 column obs
!=============================================
  ...
  use apm_upper_bdy_mod, only      : get_upper_bdy_fld, &
   ...
   use obs_def_sciam_no2_trop_col_mod, only     : set_obs_def_sciam_no2_trop_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'sciam_no2_trop_col_ascii_to_obs.f90'
...
! Species-specific variables
   ...
   real                            :: amf_trop_obs, amf_total_obs
...
!
   namelist /create_sciam_obs_nml/filedir,filename,fileout,bin_beg_sec, &
   bin_end_sec,fac_obs_error,use_log_no2,lon_min,lon_max,lat_min,lat_max, &
   path_model,file_model,nx_model,ny_model,nz_model,obs_no2_reten_freq
!
! Set constants
   pi=4.*atan(1.)
   rad2deg=360./(2.*pi)
...
!
   call find_namelist_in_file("input.nml", "create_sciam_obs_nml", iunit)
   read(iunit, nml = create_sciam_obs_nml, iostat = io)
   call check_namelist_read(iunit, io, "create_sciam_obs_nml")
!
! Record the namelist values used for the run ...
   call error_handler(E_MSG,'init_create_sciam_obs','create_sciam_obs_nml values are',' ',' ',' ')
   write(     *     , nml=create_sciam_obs_nml)
!
...
!
   do icopy =1, num_copies
      if (icopy == 1) then
         copy_meta_data='SCIAM NO2 observation'
      else
...
! Read SCIAM NO2
   ...
   do while (ios == 0)
      ...
      read(fileid,*,iostat=ios) no2_total_col_obs, no2_total_col_obs_err
...
!
end program sciam_no2_trop_col_ascii_to_obs
