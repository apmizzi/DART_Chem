...
!
program cris_pan_profile_ascii_to_obs
!
!=============================================
! CRIS PAN column obs
!=============================================
   ...
   use obs_def_cris_pan_profile_mod, only     : set_obs_def_cris_pan_profile
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'cris_pan_profile_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: file_name='cris_pan_profile_obs_seq'
...
! Species-specific variables
   real                              :: pan_trop_col_obs,pan_trop_col_obs_prior,pan_trop_col_obs_err
   real                              :: pan_total_col_obs,pan_total_col_obs_prior,pan_total_col_obs_err
   real                              :: lat_obs,lon_obs,dofs_obs,trop_prs
   real*8                            :: lat_obs_r8,lon_obs_r8
   real,allocatable,dimension(:,:)   :: avgk_obs,cov_obs,cov_total
   real,allocatable,dimension(:)     :: avgk_diag_obs,pan_obs,pan_obs_prior,pan_obs_err
...
! Read CRIS PAN
   ...
   do while (ios == 0)
      ...
      allocate(pan_obs(nlay_obs))
      allocate(pan_obs_prior(nlay_obs))
      allocate(pan_obs_err(nlay_obs))
      ...
      read(fileid,*,iostat=ios) pan_obs(1:nlay_obs)
      read(fileid,*,iostat=ios) pan_obs_prior(1:nlay_obs)
...
!
      ...
      do k=1,nlay_obs
         if(isnan(prs_obs(k))) then
            ...
            avgk_obs(k,:)=-9999.
            avgk_obs(:,k)=-9999.
            pan_obs_prior(k)=-9999.
         endif
      enddo
      prs_obs_r8(:)=prs_obs(:)
      prior_obs_r8(:)=pan_obs_prior(:)
      lon_obs_r8=lon_obs
      lat_obs_r8=lat_obs
!
!--------------------------------------------------------
! Find model PAN profile corresponding to the observation
...
! Obs value is the tropospheric vertical column
            ...
            obs_val(:)=pan_obs(ilay)
!            obs_err_var=(fac_obs_error*fac_err*sqrt(cov_obs(ilay,ilay)))**2
            obs_err_var=(fac_obs_error*fac_err*pan_obs(ilay))**2
            cris_qc(:)=0
...
!
            ...
            call set_obs_def_cris_pan_profile(qc_count, prs_obs_r8(1:nlay_obs), &
...
!
      ...
      deallocate(prs_obs) 
      deallocate(pan_obs) 
      deallocate(pan_obs_prior)
      deallocate(pan_obs_err)
...
!
end program cris_pan_profile_ascii_to_obs
!
