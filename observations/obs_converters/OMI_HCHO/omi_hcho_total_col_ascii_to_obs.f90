...
!
program omi_hcho_total_col_ascii_to_obs
!
!=============================================
! OMI HCHO total column obs
!=============================================
   ...
   use obs_def_omi_hcho_total_col_mod, only     : set_obs_def_omi_hcho_total_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'omi_hcho_total_col_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: qc_meta_data='OMI HCHO QC index'
   character*129                   :: chr_year,chr_month,chr_day
   character*129                   :: file_name='omi_hcho_total_col_obs_seq'
...
! Species-specific variables
   ...
   real                            :: amftotal,amftotal_clr,amftotal_cld
...
!
   do icopy =1, num_copies
      if (icopy == 1) then
         copy_meta_data='OMI HCHO observation'
      else
...
!
end program omi_hcho_total_col_ascii_to_obs
!
...
subroutine get_model_profile(prf_mdl,nz_mdl,prs_obs,prs_mdl, &
   tmp_mdl,qmr_mdl,hcho_mdl,nlev_obs,v_wgts,kend)
   implicit none
   integer                                :: nz_mdl
   integer                                :: nlev_obs
   integer                                :: i,j,k,kend
   real                                   :: Ru,Rd,cp,eps,AvogN,msq2cmsq,grav
   real,dimension(nz_mdl)                 :: prs_mdl,tmp_mdl,qmr_mdl,hcho_mdl
   real,dimension(nz_mdl)                 :: tmp_prf,vtmp_prf,hcho_prf
   real,dimension(nlev_obs-1)             :: thick,v_wgts,prf_mdl
   real,dimension(nlev_obs)               :: hcho_prf_mdl,vtmp_prf_mdl,prs_obs
!
! Constants (mks units)
   Ru=8.316
   Rd=286.9
...
! convert to molar density         
   do k=1,nz_mdl
      hcho_prf(k)=hcho_mdl(k)*prs_mdl(k)/Ru/tmp_prf(k)
   enddo
! Vertical interpolation
   hcho_prf_mdl(:)=-9999.  
   vtmp_prf_mdl(:)=-9999.   
   call interp_to_obs(hcho_prf_mdl,hcho_prf,prs_mdl,prs_obs,nz_mdl,nlev_obs)
...
! convert to molecules/cm^2 and apply scattering weights
   do k=1,nlev_obs-1
!      prf_mdl(k)=thick(k)*(hcho_prf_mdl(k)+hcho_prf_mdl(k+1))/2.* &
!      AvogN/msq2cmsq * v_wgts(k)
!
      prf_mdl(k)=(hcho_prf_mdl(k)+hcho_prf_mdl(k+1))/2.* &
      AvogN/msq2cmsq * v_wgts(k)
   enddo
!   print *, 'prf_mdl  ',prf_model(:)
!   print *, 'hcho fld   ',hcho_prf_mdl(:)
