...
!
program mls_hno3_total_col_ascii_to_obs
!
!=============================================
! MLS HNO3 column obs
!=============================================
   ...
   use obs_def_mls_hno3_total_col_mod, only     : set_obs_def_mls_hno3_total_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'mls_hno3_total_col_ascii_to_obs.f90'
...
!
   ...
   integer                         :: reject,k,l,kk,kend,klev
   integer                         :: i_min,j_min
   integer                         :: sum_reject,sum_accept,sum_total
   integer                         :: obs_accept,obs_hno3_reten_freq,obs_no2_reten_freq
!
...
!
   ...
   character*129                   :: file_name='mls_hno3_total_col_obs_seq'
   character*129                   :: data_type,cmd
   character*129                   :: path_model,file_model,file_in
!
   logical                         :: use_log_co,use_log_hno3,use_log_no2,use_log_so2
!
! Species-specific variables
   ...
   real,allocatable,dimension(:)     :: prior_obs,hno3_obs
   ...
   real                              :: trop_index,cov_total,cov_trop
   real                              :: avgk_term, sum_hno3_obs
   real*8                            :: prior_total_r8
   real*8,allocatable,dimension(:)   :: avgk_total_obs_r8
!
   namelist /create_mls_obs_nml/filedir,filename,fileout, &
   bin_beg_sec,bin_end_sec,fac_obs_error,use_log_co,use_log_hno3,use_log_no2,use_log_so2, &
   lon_min,lon_max,lat_min,lat_max, &
   path_model,file_model,nx_model,ny_model,nz_model,obs_hno3_reten_freq,obs_no2_reten_freq
!
! Set constants
   pi=4.*atan(1.)
   rad2deg=360./(2.*pi)
...
!
      ...
      lat_obs_r8=lat_obs
!
! Calculate vertically integrated error covariance, averaging
! kernel, and prior term
      cov_total=0.
      sum_hno3_obs=0.
      prior_total_r8=0.
      avgk_total_obs_r8(:)=0.
      do k=1,nlay_obs
         cov_total=cov_total+cov_obs(k,k)
         sum_hno3_obs=sum_hno3_obs+hno3_obs(k)
         do kk=1,nlay_obs
            if(k.eq.kk) then
               avgk_term=(1.-avgk_obs(kk,k))
            else
               avgk_term=-avgk_obs(kk,k)
            endif
            avgk_total_obs_r8(k)=avgk_total_obs_r8(k)+avgk_obs(kk,k)
            prior_total_r8=prior_total_r8+avgk_term*prior_obs(k)
         enddo
      enddo
!
!--------------------------------------------------------
! Find model HNO3 profile corresponding to the observation
! MLS HNO3 and WRF-Chem vertical grids are bottom to top        
!--------------------------------------------------------
!      reject=0
!      call get_model_profile(prf_locl,prf_full,nz_model, &
!      prs_obs,prs_fld(i_min,j_min,:),tmp_fld(i_min,j_min,:), &
!      qmr_fld(i_min,j_min,:),hno3_fld(i_min,j_min,:), &
...
! Obs value is the tropospheric vertical column
         obs_val(:)=col_amt_total_obs
         obs_err_var=(fac_obs_error*fac_err*sqrt(cov_total))**2
         mls_qc(:)=0
...
!        
         ...
         call set_obs_def_mls_hno3_total_col(qc_count, prs_obs_r8(:), avgk_total_obs_r8(:), prior_total_r8, nlay_obs, klev, kend)
...
!
end program mls_hno3_total_col_ascii_to_obs
!
...
subroutine get_model_profile(prf_locl,prf_full,nz_mdl,prs_obs,prs_mdl, &
   tmp_mdl,qmr_mdl,hno3_mdl,nlev_obs,v_wgts,prior,kend)
   implicit none
   integer                                :: nz_mdl
   integer                                :: nlev_obs
   integer                                :: i,j,k,kend
   real                                   :: Ru,Rd,cp,eps,AvogN,msq2cmsq,grav
   real,dimension(nz_mdl)                 :: prs_mdl,tmp_mdl,qmr_mdl,hno3_mdl
   real,dimension(nz_mdl)                 :: tmp_prf,vtmp_prf,hno3_prf
   real,dimension(nlev_obs-1)             :: thick,v_wgts,prior,prf_locl,prf_full
   real,dimension(nlev_obs)               :: hno3_prf_mdl,vtmp_prf_mdl,prs_obs
!
! Constants (mks units)
   Ru=8.316
   Rd=286.9
...
! convert to molar density         
   do k=1,nz_mdl
      hno3_prf(k)=hno3_mdl(k)*prs_mdl(k)/Ru/tmp_prf(k)
   enddo
! Vertical interpolation
   hno3_prf_mdl(:)=-9999.  
   vtmp_prf_mdl(:)=-9999.   
   call interp_to_obs(hno3_prf_mdl,hno3_prf,prs_mdl,prs_obs,nz_mdl,nlev_obs,kend)
...
! apply scattering weights
   do k=1,nlev_obs-1
! full term
      prf_full(k)=thick(k) * (hno3_prf_mdl(k)+hno3_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
!
! no thicknesses      
      prf_locl(k)=(hno3_prf_mdl(k)+hno3_prf_mdl(k+1))/2.* &
      v_wgts(k) + (1.-v_wgts(k))*prior(k)
   enddo
!   print *, 'prf_full  ',prf_full(:)
!   print *, 'hno3 fld   ',hno3_prf_mdl(:)
