...
!
program tropomi_hcho_trop_col_ascii_to_obs
!
!=============================================
...
!=============================================
   ...
   use obs_def_tropomi_hcho_trop_col_mod, only     : set_obs_def_tropomi_hcho_trop_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'tropomi_hcho_trop_col_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: file_name='tropomi_hcho_trop_col_obs_seq'
...
!       
         obs_kind = TROPOMI_HCHO_TROP_COL
...
!      
         ...
         call set_obs_def_tropomi_hcho_trop_col(qc_count, prs_obs_r8, avgk_obs_r8, amf_trop_obs_r8, trop_indx, nlay_obs)
...
!
end program tropomi_hcho_trop_col_ascii_to_obs
!
...
! Vertical interpolation
   hcho_prf_mdl(:)=-9999.  
   vtmp_prf_mdl(:)=-9999.   
   call interp_to_obs(hcho_prf_mdl,hcho_prf,prs_mdl,prs_obs,nz_mdl,nlev_obs,kend)
...
! apply averging kernel
   do k=1,nlev_obs-1
      prf_locl(k)=thick(k)*(hcho_prf_mdl(k)+hcho_prf_mdl(k+1))/2.* &
      v_wgts(k)
      prf_full(k)=thick(k)*(hcho_prf_mdl(k)+hcho_prf_mdl(k+1))/2.* &
      v_wgts(k)
   enddo
