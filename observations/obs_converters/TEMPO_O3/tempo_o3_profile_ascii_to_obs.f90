...
!
program tempo_o3_profile_ascii_to_obs
!
!=============================================
! TEMPO O3 column obs
!=============================================
   ...
   use obs_def_tempo_o3_profile_mod, only     : set_obs_def_tempo_o3_profile
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'tempo_o3_profile_ascii_to_obs.f90'
...
!
   ...
   integer                         :: nlay_obs,nlev_obs,ilv,icnt,nlay_obs_trnc,nlev_obs_trnc
...
!
   ...
   character*129                   :: file_name='tempo_o3_profile_obs_seq'
...
! Species-specific variables
   ...
   real,allocatable,dimension(:)     :: prs_obs_lev,prs_obs_lay
...
! Read TEMPO O3
   ...
   do while (ios == 0)
      ...
      allocate(prs_obs_lev(nlev_obs))
      allocate(prs_obs_lay(nlay_obs))
      ...
      read(fileid,*,iostat=ios) prs_obs_lev(1:nlev_obs)
...
!
      icnt=0      
      do k=1,nlev_obs
         if(isnan(prs_obs_lev(k))) then
            icnt=icnt+1
         endif
      enddo
      prs_obs_lay(:)=-9999.
      do k=1+icnt,nlay_obs
         prs_obs_lay(k)=(prs_obs_lev(k)+prs_obs_lev(k+1))/2.
      enddo
      nlev_obs_trnc=nlev_obs-icnt
      nlay_obs_trnc=nlay_obs-icnt
!
      prs_obs_lev(:)=prs_obs_lev(:)*100.
      prs_obs_lay(:)=prs_obs_lay(:)*100.
      prs_obs_r8(:)=prs_obs_lev(:)
...
! Obs value is the tropospheric vertical column
            avgk_obs_r8(:)=avgk_obs(ilay,:)         
            obs_val(:)=o3_obs(ilay)
!            obs_err_var=(fac_obs_error*fac_err*sqrt(cov_obs(ilay,ilay)))**2
            obs_err_var=(fac_obs_error*fac_err*o3_obs(ilay))**2
            tempo_qc(:)=0
...
!
            obs_kind = TEMPO_O3_PROFILE
! (0 <= lon_obs <= 360); (-90 <= lat_obs <= 90)
            klev=nlay_obs_trnc
            kend=nlay_obs_trnc
            level=prs_obs_lay(ilay+icnt)
...
!
            ...
            call set_obs_def_tempo_o3_profile(qc_count, prs_obs_r8(icnt+1:nlev_obs), &
            avgk_obs_r8(1:nlay_obs_trnc), prior_obs_r8(1:nlay_obs_trnc), nlay_obs_trnc, klev, kend)
...
!
      ...
      deallocate(prs_obs_lev,prs_obs_lay) 
      deallocate(o3_obs) 
...
!
end program tempo_o3_profile_ascii_to_obs
!
