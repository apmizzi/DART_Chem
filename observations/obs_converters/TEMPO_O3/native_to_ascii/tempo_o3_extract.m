...
% dimensions
      ...
      [dimname,xtrack]=netcdf.inqDim(fidd,0);
      [dimname,layer]=netcdf.inqDim(fidd,4);
      [dimname,mirror_step]=netcdf.inqDim(fidd,5);
      level=layer+1;
...
% prs_sfc(xtrack,mirror_step) (hPa)
      field='/geolocation/surface_pressure';
      prs_sfc=ncread(file_in,field);
% TEMPORARILY GET PRESSURE DATA FROM NO2 FILE
      file_in_no2='/projects/mizzi/TEMPO_FILES/TEMPO_DATA/TEMPO_NO2_L2_V01_20130825T143538Z_S004G07.nc';
      field='/support_data/surface_pressure';
      Eta_A=ncreadatt(file_in_no2,field,'Eta_A');  
      Eta_B=ncreadatt(file_in_no2,field,'Eta_B');
% time(mirror_step) (sec)
      field='/geolocation/time';
      time=ncread(file_in,field);
% prs_trop(xtrack,mirror_step) (hPa)
      field='/geolocation/tropopause_pressure';
      prs_trop=ncread(file_in,field);
% col_amt_total(xtrack,mirror_step) (molec/cm2)
      field='/product/column_amount';
      col_amt_total=ncread(file_in,field);
% col_amt_total_err(xtrack,mirror_step) (molec/cm2)
      field='/product/column_uncertainty';
      col_amt_total_err=ncread(file_in,field);
% col_amt_strat(xtrack,mirror_step) (molec/cm2)
      field='/product/stratospheric_column_amount';
      col_amt_strat=ncread(file_in,field);
% col_amt_trop(xtrack,mirror_step) (molec/cm2)
      field='/product/tropospheric_column_amount';
      col_amt_trop=ncread(file_in,field);
% col_amt_pbl(xtrack,mirror_step) (0-2 km column amount, molec/cm2)
      field='/product/pbl_column_amount';
      col_amt_pbl=ncread(file_in,field);
% amf_cld_frac(xtrack,mirror_step)
      field='/support_data/amf_cloud_fraction';
      amf_cld_frac=ncread(file_in,field);
% amf_geom(xtrack,mirror_step)
      field='/support_data/amf_geometric';
      amf_geom=ncread(file_in,field);
% amf_total(xtrack,mirror_step)
      field='/support_data/amf_molecule_specific';
      amf_total=ncread(file_in,field);
% amf_strat(xtrack,mirror_step)
      field='/support_data/amf_molecule_stratospheric';
      amf_strat=ncread(file_in,field);
% amf_trop(xtrack,mirror_step)
      field='/support_data/amf_molecule_tropospheric';
      amf_trop=ncread(file_in,field);
% o3_prof(xtrack,mirror_step,layer) (layer partial column - molec/cm2)
      field='/support_data/gas_profile';
      gas_prof=ncread(file_in,field);
% scat_wts(xtrack,mirror_step,layer)
