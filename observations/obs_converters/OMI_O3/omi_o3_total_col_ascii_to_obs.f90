...
!
program omi_o3_total_col_ascii_to_obs
!
!=============================================
! OMI O3 column obs
!=============================================
   ...
   use obs_def_omi_o3_total_col_mod, only     : set_obs_def_omi_o3_total_col
...
! version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'omi_o3_total_col_ascii_to_obs.f90'
...
!
   ...
   character*129                   :: file_name='omi_o3_total_col_obs_seq'
...
!
      ...
      call set_obs_def_omi_o3_total_col(qc_count, prs_obs_r8, avgk_obs_r8, prior_obs_r8, nlay_obs, kend)
...
!
end program omi_o3_total_col_ascii_to_obs
!
...
! (assumes model and observation vertical grid is bottom up)  
! 
   implicit none
   integer                         :: nlay_obs,nlev_obs
   integer                         :: k,kk,kstr,kend,kmin,kmax
   real                            :: prs_loc
   real                            :: wt_ctr,wt_end
   real                            :: zmin,zmax
   real                            :: prs_trop
...
!
   do i=1,nx_mdl
      do j=1,ny_mdl
         mdl_x=(x_mdl(i,j))/rad2deg
         if(x_mdl(i,j).lt.0.) mdl_x=(360.+x_mdl(i,j))/rad2deg
         mdl_y=y_mdl(i,j)/rad2deg
         obs_x=x_obser/rad2deg
         obs_y=y_obser/rad2deg
         dx_dis=abs(obs_x-mdl_x)*cos((obs_y+mdl_y)/2.)*re
         dy_dis=abs(obs_y-mdl_y)*re
         rad=sqrt(dx_dis*dx_dis + dy_dis*dy_dis)  
         rad_min=min(rad_min,rad)
         if(rad_min.eq.rad) then
            i_min=i
            j_min=j
         endif 
      enddo
   enddo
   if(rad_min.ge.rad_crit) then            
!      print *, 'APM: ERROR in intrp - min dist exceeds threshold ',rad_min, rad_crit
!      print *, 'model ',i_min,j_min,mdl_x*rad2deg,mdl_y*rad2deg
!      print *, 'observation ',x_obser,y_obser
      reject=1
      return
   else
      reject=0
