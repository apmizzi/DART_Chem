...
%
         ...
         for ilay=1:layer
            if(isnan(ch4_lay(ilay,iobs)))
               fprintf('APM: ch4_lay has NaNs \n')
               continue
            end
         end
         if(isnan(ch4_total_col(iobs)))
            fprintf('APM: ch4_total_col is a NaN \n')
            continue
         end
%
% Check domain
...
%
function [fld_interp]=prs_interp_ch4l(fld,i_tmp,j_tmp,i_mdl,j_mdl, ...
   x_tmp,y_tmp,p_tmp,nz_tmp,x_mdl,y_mdl,p_mdl,nz_mdl)
%
% Interpolation weights
   x_wt_m=x_mdl(i_mdl+1)-x_tmp(i_tmp,j_tmp);
