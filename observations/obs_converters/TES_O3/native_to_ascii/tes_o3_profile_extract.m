function tes_o3_profile_extract (filein,fileout,file_pre,cwyr_mn,cwmn_mn,cwdy_mn,cwhh_mn,cwmm_mn,cwss_mn,cwyr_mx,cwmn_mx,cwdy_mx,cwhh_mx,cwmm_mx,cwss_mx,path_mdl,file_mdl,cnx_mdl,cny_mdl)
...
% Process satellite data
   for ifile=1:nfile
      ...
      field='/HDFEOS/SWATHS/O3NadirSwath';
      zgrid=h5readatt(file_in,field,'VerticalCoordinate');
%
% Read Time
      field='/HDFEOS/SWATHS/O3NadirSwath/Data Fields/UTCTime';
...
% averaging kernel diagonal (layer,nobs)
      field='/HDFEOS/SWATHS/O3NadirSwath/Data Fields/AveragingKernelDiagonal';
      avgk_diag_lay=h5read(file_in,field);
%
% dofs(nobs)
      field='/HDFEOS/SWATHS/O3NadirSwath/Data Fields/DegreesOfFreedomForSignal';
...
% o3_lay_err (layer,nobs)
      ...
      o3_lay_err=h5read(file_in,field);
      units=h5readatt(file_in,field,'Units');
%
% o3_trop col (nobs)
      field='/HDFEOS/SWATHS/O3NadirSwath/Data Fields/O3TroposphericColumn';
      o3_trop_col=h5read(file_in,field);
%
% o3_trop_col_err (nobs)
      field='/HDFEOS/SWATHS/O3NadirSwath/Data Fields/O3TroposphericColumnError';
      o3_trop_col_err=h5read(file_in,field);
%
% o3_trop_col_prior (nobs)
      field='/HDFEOS/SWATHS/O3NadirSwath/Data Fields/O3TroposphericColumnInitial';
      o3_trop_col_prior=h5read(file_in,field);
%
% o3_total_col (nobs)
      field='/HDFEOS/SWATHS/O3NadirSwath/Data Fields/TotalColumnDensity';
      o3_total_col=h5read(file_in,field);
%
% o3_total_col_err (nobs)
      field='/HDFEOS/SWATHS/O3NadirSwath/Data Fields/TotalColumnDensityError';
      o3_total_col_err=h5read(file_in,field);
%
% o3_total_col_prior (nobs)
...
%
% lon (nobs)
      field='/HDFEOS/SWATHS/O3NadirSwath/Geolocation Fields/Longitude';
      lon=h5read(file_in,field);
%
% zenang (nobs)
      field='/HDFEOS/SWATHS/O3NadirSwath/Geolocation Fields/SolarZenithAngle';
...
%
         ...
         for ilay=1:layer
            if(isnan(o3_lay(ilay,iobs)))
               fprintf('APM: o3_lay has NaNs \n')
               continue
            end
         end
         if(isnan(o3_trop_col(iobs)))
            fprintf('APM: o3_trop_col is a NaN \n')
            continue
         end
         if(isnan(o3_total_col(iobs)))
            fprintf('APM: o3_total_col is a NaN \n')
            continue
         end
%
% Check domain
...
% Save data to ascii file
         ...
         fprintf(fid,'\n');
         fprintf(fid,'%14.8g ',o3_lay(1:layer,iobs));
         fprintf(fid,'\n');
         fprintf(fid,'%14.8g ',o3_lay_prior(1:layer,iobs));
         fprintf(fid,'\n');
         fprintf(fid,'%14.8g ',o3_lay_err(1:layer,iobs));
         fprintf(fid,'\n');
         ...
         fprintf(fid,'%14.8g \n',o3_trop_col(iobs));
         fprintf(fid,'%14.8g \n',o3_trop_col_prior(iobs));
         fprintf(fid,'%14.8g \n',o3_trop_col_err(iobs));
         fprintf(fid,'%14.8g \n',o3_total_col(iobs));
         fprintf(fid,'%14.8g \n',o3_total_col_prior(iobs));
         fprintf(fid,'%14.8g \n',o3_total_col_err(iobs));
      end
   end  
end
...
%
function [fld_interp]=prs_interp_o3l(fld,i_tmp,j_tmp,i_mdl,j_mdl, ...
   x_tmp,y_tmp,p_tmp,nz_tmp,x_mdl,y_mdl,p_mdl,nz_mdl)
%
% Interpolation weights
   x_wt_m=x_mdl(i_mdl+1)-x_tmp(i_tmp,j_tmp);
